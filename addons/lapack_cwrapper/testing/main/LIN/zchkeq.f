...
*  =======
*
*  ZCHKEQ tests czgeequ, czgbequ, czpoequ, czppequ and czpbequ
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           czgbequ, czgeequ, czpbequ, czpoequ, czppequ
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   40       CONTINUE
*
            CALL czgeequ( M, N, A, NSZ, R, C, RCOND, CCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 1 ) = ONE
            ELSE
...
*
   ...
   90 CONTINUE
      CALL czgeequ( NSZ, NSZ, A, NSZ, R, C, RCOND, CCOND, NORM, INFO )
...
*
  ...
  150             CONTINUE
*
                  CALL czgbequ( M, N, KL, KU, AB, NSZB, R, C, RCOND,
     $                         CCOND, NORM, INFO )
*
                  IF( INFO.NE.0 ) THEN
...
*
  ...
  270    CONTINUE
*
         CALL czpoequ( N, A, NSZ, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 3 ) = ONE
         ELSE
  ...
  290 CONTINUE
      ...
      CALL czpoequ( NSZ, A, NSZ, R, RCOND, NORM, INFO )
      IF( INFO.NE.MAX( NSZ-1, 1 ) )
     $   RESLTS( 3 ) = ONE
...
*
  ...
  310    CONTINUE
*
         CALL czppequ( 'U', N, AP, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 4 ) = ONE
         ELSE
...
*
  ...
  340    CONTINUE
*
         CALL czppequ( 'L', N, AP, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 4 ) = ONE
         ELSE
...
*
  360 CONTINUE
      I = ( NSZ*( NSZ+1 ) ) / 2 - 2
      AP( I ) = -CONE
      CALL czppequ( 'L', NSZ, AP, R, RCOND, NORM, INFO )
      IF( INFO.NE.MAX( NSZ-1, 1 ) )
     $   RESLTS( 4 ) = ONE
...
*
  ...
  390       CONTINUE
*
            CALL czpbequ( 'U', N, KL, AB, NSZB, R, RCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 5 ) = ONE
            ELSE
...
*
  ...
  430       CONTINUE
*
            CALL czpbequ( 'L', N, KL, AB, NSZB, R, RCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 5 ) = ONE
            ELSE
 ...
 9998 FORMAT( ' czgeequ failed test with value ', D10.3, ' exceeding',
     $      ' threshold ', D10.3 )
 9997 FORMAT( ' czgbequ failed test with value ', D10.3, ' exceeding',
     $      ' threshold ', D10.3 )
 9996 FORMAT( ' czpoequ failed test with value ', D10.3, ' exceeding',
     $      ' threshold ', D10.3 )
 9995 FORMAT( ' czppequ failed test with value ', D10.3, ' exceeding',
     $      ' threshold ', D10.3 )
 9994 FORMAT( ' czpbequ failed test with value ', D10.3, ' exceeding',
     $      ' threshold ', D10.3 )
      RETURN
*
*     End of ZCHKEQ
