...
*     .. Local Scalars ..
      ...
      CHARACTER*10 INTSTR
      CHARACTER*72 ALINE
      INTEGER I,IC,J,K,LA,LAFAC,LDA,NB,NM,NMATS,NN,NNB,NNB2,NNS,NRHS,
     +        NTYPES
      DOUBLE PRECISION EPS,S1,S2,THREQ,THRESH
*     ..
*     .. Local Arrays ..
      LOGICAL DOTYPE(MATMAX)
      INTEGER IWORK(25*NMAX),MVAL(MAXIN),NBVAL(MAXIN),NBVAL2(MAXIN),
...
*     .. External Subroutines ..
      EXTERNAL ALAREQ,DCHKEQ,DCHKGB,DCHKGE,DCHKGT,DCHKLQ,DCHKPB,DCHKPO,
     +         DCHKPP,DCHKPT,DCHKQ3,DCHKQL,DCHKQP,DCHKQR,DCHKRQ,DCHKSP,
     +         DCHKSY,DCHKTB,DCHKTP,DCHKTR,DCHKTZ,DDRVGB,DDRVGE,DDRVGT,
     +         DDRVLS,DDRVPB,DDRVPO,DDRVPP,DDRVPT,DDRVSP,DDRVSY
*     ..
*     .. Scalars in Common ..
      LOGICAL LERR,OK
      CHARACTER*7 SRNAMT
...
*
     ...
     +                    A(1,3),LAFAC,A(1,6),B(1,1),B(1,2),B(1,3),
     +                    B(1,4),S,WORK,RWORK,IWORK,NOUT)
          ELSE
              WRITE (NOUT,FMT=9988) PATH
          END IF
*
...
*
          IF (TSTDRV) THEN
              CALL DDRVGT(DOTYPE,NN,NVAL,NRHS,THRESH,TSTERR,A(1,1),
     +                    A(1,2),B(1,1),B(1,2),B(1,3),WORK,RWORK,IWORK,
     +                    NOUT)
          ELSE
              WRITE (NOUT,FMT=9988) PATH
...
*
          IF (TSTDRV) THEN
              CALL DDRVPT(DOTYPE,NN,NVAL,NRHS,THRESH,TSTERR,A(1,1),
     +                    A(1,2),A(1,3),B(1,1),B(1,2),B(1,3),WORK,RWORK,
     +                    NOUT)
          ELSE
              WRITE (NOUT,FMT=9988) PATH
...
*
     ...
     +                    THRESH,TSTERR,NMAX,A(1,1),A(1,2),A(1,3),
     +                    A(1,4),A(1,5),B(1,1),B(1,2),B(1,3),B(1,4),
     +                    WORK,RWORK,IWORK,NOUT)
          ELSE
              WRITE (NOUT,FMT=9989) PATH
          END IF
...
*
          IF (TSTCHK) THEN
              CALL DCHKQP(DOTYPE,NM,MVAL,NN,NVAL,THRESH,TSTERR,A(1,1),
     +                    A(1,2),B(1,1),B(1,2),B(1,3),WORK,IWORK,NOUT)
              CALL DCHKQ3(DOTYPE,NM,MVAL,NN,NVAL,NNB,NBVAL,NXVAL,THRESH,
     +                    A(1,1),A(1,2),B(1,1),B(1,2),B(1,3),WORK,IWORK,
     +                    NOUT)
          ELSE
              WRITE (NOUT,FMT=9989) PATH
          END IF
...
*
     ...
     +                    A(1,2),B(1,1),B(1,2),B(1,3),WORK,NOUT)
          ELSE
              WRITE (NOUT,FMT=9989) PATH
          END IF
*
...
*
     ...
     +                    NXVAL,THRESH,TSTERR,A(1,1),A(1,2),B(1,1),
     +                    B(1,2),B(1,3),RWORK,RWORK(NMAX+1),WORK,IWORK,
     +                    NOUT)
          ELSE
              WRITE (NOUT,FMT=9988) PATH
          END IF
