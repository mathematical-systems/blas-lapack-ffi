...
*  =======
*
*  DDRVPB tests the driver routines cdpbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,I1,I2,IEQUED,IFACT,IKD,IMAT,IN,INFO,IOFF,IUPLO,IW,IZERO,
     +        K,K1,KD,KL,KOFF,KU,LDA,LDAB,MODE,N,NB,NBMIN,NERRS,NFACT,
...
*     .. External Subroutines ..
      EXTERNAL ALADHD,ALAERH,ALASVM,DCOPY,DERRVX,DGET04,cdlacpy,cdlaqsb,
     +         DLARHS,cdlaset,DLATB4,DLATMS,cdpbequ,cdpbsv,cdpbsvX,
     +         DPBT01,DPBT02,DPBT05,cdpbtrf,cdpbtrs,DSWAP,XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX,MIN
*     ..
...
*
                      CALL cdlacpy('Full',KD+1,N,A,LDAB,ASAV,LDAB)
*
                      DO 70 IEQUED = 1,2
                          EQUED = EQUEDS(IEQUED)
                          IF (IEQUED.EQ.1) THEN
...
*                          with the value returned by cdpbsvX (FACT =
*                          'N' reuses the condition number from the
*                          previous iteration with FACT = 'F').
*
                                  CALL cdlacpy('Full',KD+1,N,ASAV,LDAB,
     +                                         AFAC,LDAB)
...
*
                                      CALL cdpbequ(UPLO,N,KD,AFAC,LDAB,
     +                                             S,SCOND,AMAX,INFO)
...
*
                                  ANORM = DLANSB('1',UPLO,N,KD,AFAC,
     +                                    LDAB,RWORK)
*
*                          Factor the matrix A.
*
                                  CALL cdpbtrf(UPLO,N,KD,AFAC,LDAB,INFO)
*
*                          Form the inverse of A.
*
                                  CALL cdlaset('Full',N,N,ZERO,ONE,A,
     +                                         LDA)
                                  SRNAMT = 'cdpbtrs'
                                  CALL cdpbtrs(UPLO,N,KD,N,AFAC,LDAB,A,
     +                                         LDA,INFO)
*
...
*
                              CALL cdlacpy('Full',KD+1,N,ASAV,LDAB,A,
     +                                     LDAB)
*
...
*
                                  CALL cdlacpy('Full',KD+1,N,A,LDAB,
     +                                         AFAC,LDAB)
                                  CALL cdlacpy('Full',N,NRHS,B,LDA,X,
     +                                         LDA)
*
                                  SRNAMT = 'cdpbsv '
                                  CALL cdpbsv(UPLO,N,KD,NRHS,AFAC,LDAB,
     +                                        X,LDA,INFO)
*
*                          Check error code from cdpbsv .
*
                                  IF (INFO.NE.IZERO) THEN
                                      CALL ALAERH(PATH,'cdpbsv ',INFO,
     +                                            IZERO,UPLO,N,N,KD,KD,
...
*                       --- Test cdpbsvX ---
*
                              IF (.NOT.PREFAC) CALL cdlaset('Full',KD+1,
     +                            N,ZERO,ZERO,AFAC,LDAB)
                              CALL cdlaset('Full',N,NRHS,ZERO,ZERO,X,
     +                                     LDA)
...
*                          EQUED='Y'
*
                                  CALL cdlaqsb(UPLO,N,KD,A,LDAB,S,SCOND,
     +                                         AMAX,EQUED)
                              END IF
*
*                       Solve the system and compute the condition
*                       number and error bounds using cdpbsvX.
*
                              SRNAMT = 'cdpbsvX'
                              CALL cdpbsvX(FACT,UPLO,N,KD,NRHS,A,LDAB,
     +                                     AFAC,LDAB,EQUED,S,B,LDA,X,
     +                                     LDA,RCOND,RWORK,
     +                                     RWORK(NRHS+1),WORK,IWORK,
     +                                     INFO)
*
*                       Check the error code from cdpbsvX.
*
                              IF (INFO.NE.IZERO) THEN
                                  CALL ALAERH(PATH,'cdpbsvX',INFO,IZERO,
     +                                        FACT//UPLO,N,N,KD,KD,NRHS,
...
*                          Compute residual of the computed solution.
*
                                  CALL cdlacpy('Full',N,NRHS,BSAV,LDA,
     +                                         WORK,LDA)
...
*
     ...
     +                                        RESULT(4))
                              ...
                              END IF
*
*                       Compare RCOND from cdpbsvX with the computed
*                       value in RCONDC.
*
...
*
 9999 FORMAT (1X,A6,', UPLO=''',A1,''', N =',I5,', KD =',I5,', type ',
     +       I1,', test(',I1,')=',G12.5)
