...
*  =======
*
*  SCHKPT tests cspttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                   cslacpy, SLAPTM, cslarnv, SLATB4, SLATMS, 
     $  csptcon, 
     $                   csptrfs, SPTT01, SPTT02, SPTT05, cspttrf, 
     $  cspttrs, 
     $                   SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
*           Check error code from cspttrf.
*
            IF( INFO.NE.IZERO ) THEN
               CALL ALAERH( PATH, 'cspttrf', INFO, IZERO, ' ', N, N, 
     $  -1, 
...
*
*           Use cspttrs to solve for one column at a time of inv(A),
...
*
   ...
   40          CONTINUE
               X( I ) = ONE
               CALL cspttrs( N, 1, D( N+1 ), E( N+1 ), X, LDA, INFO )
...
*
               ...
               CALL cspttrs( N, NRHS, D( N+1 ), E( N+1 ), X, LDA, INFO 
     $  ) 
*
*           Check error code from cspttrs.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'cspttrs', INFO, 0, ' ', N, N, -1,
...
*
               SRNAMT = 'csptrfs'
               CALL csptrfs( N, NRHS, D, E, D( N+1 ), E( N+1 ), B, LDA,
     $                      X, LDA, RWORK, RWORK( NRHS+1 ), WORK, INFO 
     $  ) 
*
*           Check error code from csptrfs.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'csptrfs', INFO, 0, ' ', N, N, -1,
     $                         -1, NRHS, IMAT, NFAIL, NERRS, NOUT )
*
               CALL SGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                      RESULT( 4 ) )
...
*
   90       CONTINUE
            SRNAMT = 'csptcon'
            CALL csptcon( N, D( N+1 ), E( N+1 ), ANORM, RCOND, RWORK,
     $                   INFO )
*
*           Check error code from csptcon.
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'csptcon', INFO, 0, ' ', N, N, -1, 
     $  -1, 
