...
*  =======
*
*  CDRVGB tests the driver routines ccgbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALADHD, ALAERH, ALASVM, CERRVX, ccgbequ, ccgbsv,
     $                ccgbsvX, CGBT01, CGBT02, CGBT05, ccgbtrf, ccgbtrs,
     $                CGET04, cclacpy, cclaqgb, CLARHS, cclaset, CLATB4,
     $                   CLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                  CALL cclacpy( 'Full', KL+KU+1, N, A, LDA, ASAV, LDA )
*
                  DO 110 IEQUED = 1, 4
                     EQUED = EQUEDS( IEQUED )
                     IF( IEQUED.EQ.1 ) THEN
...
*                          with the value returned by csgesvX (FACT =
*                          'N' reuses the condition number from the
*                          previous iteration with FACT = 'F').
*
                           CALL cclacpy( 'Full', KL+KU+1, N, ASAV, LDA,
     $                                  AFB( KL+1 ), LDAFB )
...
*
                              CALL ccgbequ( N, N, KL, KU, AFB( KL+1 ),
...
*
                                CALL cclaqgb( N, N, KL, KU, AFB( KL+1 ),
     $                                        LDAFB, S, S( N+1 ),
     $                                        ROWCND, COLCND, AMAX,
     $                                        EQUED )
                              END IF
...
*                          Factor the matrix A.
*
                          CALL ccgbtrf( N, N, KL, KU, AFB, LDAFB, IWORK,
     $                                  INFO )
*
...
*
     ...
     $                                  CMPLX( ONE ), WORK, LDB )
                           SRNAMT = 'ccgbtrs'
                           CALL ccgbtrs( 'No transpose', N, KL, KU, N,
...
*
     ...
     $                                  LDB, B, LDB, ISEED, INFO )
                           XTYPE = 'C'
                           CALL cclacpy( 'Full', N, NRHS, B, LDB, BSAV,
     $                                  LDB )
*
...
*
                              CALL cclacpy( 'Full', KL+KU+1, N, A, LDA,
     $                                     AFB( KL+1 ), LDAFB )
                              CALL cclacpy( 'Full', N, NRHS, B, LDB, X,
     $                                     LDB )
*
                              SRNAMT = 'ccgbsv '
                              CALL ccgbsv( N, KL, KU, NRHS, AFB, LDAFB,
     $                                    IWORK, X, LDB, INFO )
*
*                             Check error code from ccgbsv .
*
                              IF( INFO.NE.IZERO )
     $                           CALL ALAERH( PATH, 'ccgbsv ', INFO,
     $                                        IZERO, ' ', N, N, KL, KU,
...
*                                solution.
*
                                 CALL cclacpy( 'Full', N, NRHS, B, LDB,
     $                                        WORK, LDB )
...
*                             EQUED = 'R', 'C', or 'B'.
*
                              CALL cclaqgb( N, N, KL, KU, A, LDA, S,
     $                                     S( N+1 ), ROWCND, COLCND,
     $                                     AMAX, EQUED )
                           END IF
*
*                          Solve the system and compute the condition
*                          number and error bounds using ccgbsvX.
*
                           SRNAMT = 'ccgbsvX'
                          CALL ccgbsvX( FACT, TRANS, N, KL, KU, NRHS, A,
     $                                  LDA, AFB, LDAFB, IWORK, EQUED,
     $                                  S, S( LDB+1 ), B, LDB, X, LDB,
     $                                  RCOND, RWORK, RWORK( NRHS+1 ),
     $                                  WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                          Check the error code from ccgbsvX.
*
                           IF( INFO.NE.IZERO )
     $                        CALL ALAERH( PATH, 'ccgbsvX', INFO, IZERO,
     $                                     FACT // TRANS, N, N, KL, KU,
     $                                     NRHS, IMAT, NFAIL, NERRS,
     $                                     NOUT )
*                          Compare RWORK(2*NRHS+1) from ccgbsvX with the
...
*
                              CALL cclacpy( 'Full', N, NRHS, BSAV, LDB,
     $                                     WORK, LDB )
...
*
     ...
     $                                     RESULT( 4 ) )
                           ...
                           END IF
*
*                          Compare RCOND from ccgbsvX with the computed
*                          value in RCONDC.
*
...
*
   ...
   80                         CONTINUE
     ...
     $                              CALL ALADHD( NOUT, PATH )
                                 IF( PREFAC ) THEN
                                    WRITE( NOUT, FMT = 9995 )'ccgbsvX',
     $                                 FACT, TRANS, N, KL, KU, EQUED,
     $                                 IMAT, 1, RESULT( 1 )
                                 ELSE
                                    WRITE( NOUT, FMT = 9996 )'ccgbsvX',
