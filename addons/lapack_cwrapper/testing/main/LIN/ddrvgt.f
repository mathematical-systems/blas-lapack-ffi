...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IFACT,IMAT,IN,INFO,ITRAN,IX,IZERO,J,K,K1,KL,KOFF,KU,LDA,
     +        M,MODE,N,NERRS,NFAIL,NIMAT,NRUN,NT
      DOUBLE PRECISION AINVNM,ANORM,ANORMI,ANORMO,COND,RCOND,RCONDC,
     +                 RCONDI,RCONDO
*     ..
*     .. Local Arrays ..
      CHARACTER TRANSS(3)
...
*     .. External Subroutines ..
      EXTERNAL ALADHD,ALAERH,ALASVM,DCOPY,DERRVX,DGET04,cdgtsv,cdgtsvX,
     +         DGTT01,DGTT02,DGTT05,cdgttrf,cdgttrs,cdlacpy,cdlagtm,
     +         cdlarnv,cdlaset,DLATB4,DLATMS,DSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX
*     ..
...
*                 Factor the matrix A.
*
                      CALL cdgttrf(N,AF,AF(M+1),AF(N+M+1),AF(N+2*M+1),
     +                             IWORK,INFO)
*
*                 Use cdgttrs to solve for one column at a time of
...
*
   ...
   30                     CONTINUE
                          X(I) = ONE
                          CALL cdgttrs('No transpose',N,1,AF,AF(M+1),
...
*
   ...
   50                     CONTINUE
                          X(I) = ONE
                          CALL cdgttrs('Transpose',N,1,AF,AF(M+1),
     +                                 AF(N+M+1),AF(N+2*M+1),IWORK,X,
     +                                 LDA,INFO)
                          AINVNM = MAX(AINVNM,DASUM(N,X,1))
   60                 CONTINUE
...
*
                          SRNAMT = 'cdgtsv '
                          CALL cdgtsv(N,NRHS,AF,AF(M+1),AF(N+M+1),X,LDA,
     +                                INFO)
*
*                    Check error code from cdgtsv .
*
                          IF (INFO.NE.IZERO) CALL ALAERH(PATH,'cdgtsv ',
     +                        INFO,IZERO,' ',N,N,1,1,NRHS,IMAT,NFAIL,
     +                        NERRS,NOUT)
                          NT = 1
                          IF (IZERO.EQ.0) THEN
*
...
*                 error bounds using cdgtsvX.
*
                      SRNAMT = 'cdgtsvX'
                      CALL cdgtsvX(FACT,TRANS,N,NRHS,A,A(M+1),A(N+M+1),
     +                             AF,AF(M+1),AF(N+M+1),AF(N+2*M+1),
     +                             IWORK,B,LDA,X,LDA,RCOND,RWORK,
     +                             RWORK(NRHS+1),WORK,IWORK(N+1),INFO)
*
*                 Check the error code from cdgtsvX.
*
                      IF (INFO.NE.IZERO) CALL ALAERH(PATH,'cdgtsvX',
     +                                        INFO,IZERO,FACT//TRANS,N,
...
*
                      DO 100 K = K1,NT
                          IF (RESULT(K).GE.THRESH) THEN
                              IF (NFAIL.EQ.0 .AND.
     +                            NERRS.EQ.0) CALL ALADHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9998) 'cdgtsvX',FACT,
...
*
     ...
     +                        0) CALL ALADHD(NOUT,PATH)
                          WRITE (NOUT,FMT=9998) 'cdgtsvX',FACT,TRANS,N,
     +                      IMAT,K,RESULT(K)
                          NFAIL = NFAIL + 1
                      END IF
                      NRUN = NRUN + NT - K1 + 2
...
*
 9999 FORMAT (1X,A6,', N =',I5,', type ',I2,', test ',I2,', ratio = ',
     +       G12.5)
