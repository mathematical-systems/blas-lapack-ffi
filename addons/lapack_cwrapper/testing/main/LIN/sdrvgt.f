...
*     .. External Subroutines ..
     ...
     $                   csgtsv, csgtsvX, SGTT01, SGTT02, SGTT05, 
     $  csgttrf, 
     $                   csgttrs, cslacpy, cslagtm, cslarnv, cslaset, 
     $  SLATB4, 
     $                   SLATMS, SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*                 Factor the matrix A.
*
                  CALL csgttrf( N, AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, INFO )
*
*                 Use csgttrs to solve for one column at a time of
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL csgttrs( 'No transpose', N, 1, AF, AF( M+1 ),
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL csgttrs( 'Transpose', N, 1, AF, AF( M+1 ),
     $                            AF( N+M+1 ), AF( N+2*M+1 ), IWORK, X,
     $                            LDA, INFO )
                     AINVNM = MAX( AINVNM, SASUM( N, X, 1 ) )
   60             CONTINUE
...
*
                  IX = 1
                  DO 70 J = 1, NRHS
                     CALL cslarnv( 2, ISEED, N, XACT( IX ) )
                     IX = IX + LDA
   70             CONTINUE
*
*                 Set the right hand side.
*
                  CALL cslagtm( TRANS, N, NRHS, ONE, A, A( M+1 ),
...
*
                     SRNAMT = 'csgtsv '
                     CALL csgtsv( N, NRHS, AF, AF( M+1 ), AF( N+M+1 ), 
     $  X, 
     $                           LDA, INFO )
*
*                    Check error code from csgtsv .
*
                     IF( INFO.NE.IZERO )
     $                  CALL ALAERH( PATH, 'csgtsv ', INFO, IZERO, ' ',
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'csgtsv ', N, IMAT,
     $                        K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
   80                CONTINUE
...
*                 error bounds using csgtsvX.
*
                  SRNAMT = 'csgtsvX'
                  CALL csgtsvX( FACT, TRANS, N, NRHS, A, A( M+1 ),
     $                         A( N+M+1 ), AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, B, LDA, X, LDA,
     $                         RCOND, RWORK, RWORK( NRHS+1 ), WORK,
     $                         IWORK( N+1 ), INFO )
*
*                 Check the error code from csgtsvX.
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'csgtsvX', INFO, IZERO,
...
*
                  DO 100 K = K1, NT
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'csgtsvX', FACT, 
     $  TRANS, 
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
...
*
                  ...
                  IF( RESULT( 6 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALADHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9998 )'csgtsvX', FACT, TRANS, 
     $  N, 
     $                  IMAT, K, RESULT( K )
                     NFAIL = NFAIL + 1
                  END IF
