...
*  =======
*
*  CDRVHE tests the driver routines cchesv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALADHD, ALAERH, ALASVM, CERRVX, CGET04, cchesv,
     $               cchesvX, CHET01, cchetrf, cchetri, cclacpy, CLAIPD,
     $                  CLARHS, cclaset, CLATB4, CLATMS, CPOT02, CPOT05,
     $                   XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL cchetrf( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL cclacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                     CALL cchetri( UPLO, N, AINV, LDA, IWORK, WORK,
     $                            INFO )
...
*
*                    Factor the matrix and solve the system using cchesv.
*
                     SRNAMT = 'cchesv '
                     CALL cchesv( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from cchesv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'cchesv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'cchesv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using cchesvX.
*
                  SRNAMT = 'cchesvX'
                  CALL cchesvX( FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from cchesvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'cchesvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from cchesvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'cchesvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
