...
*  =======
*
*  ZCHKTP tests cztptri, -TRS, -RFS, and -CON, and czlatps
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $               czlacpy, ZLARHS, czlatps, ZLATTP, cztpcon, cztprfs,
     $                  ZTPT01, ZTPT02, ZTPT03, ZTPT05, ZTPT06, cztptri,
     $                   cztptrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
     ...
     $            CALL ZCOPY( LAP, AP, 1, AINVP, 1 )
               SRNAMT = 'cztptri'
               CALL cztptri( UPLO, DIAG, N, AINVP, INFO )
*
*              Check error code from cztptri.
*
               IF( INFO.NE.0 )
     $           CALL ALAERH( PATH, 'cztptri', INFO, 0, UPLO // DIAG, N,
...
*
                     SRNAMT = 'cztptrs'
                     CALL cztptrs( UPLO, TRANS, DIAG, N, NRHS, AP, X,
     $                            LDA, INFO )
*
*                 Check error code from cztptrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cztptrs', INFO, 0,
...
*
                     SRNAMT = 'cztprfs'
                     CALL cztprfs( UPLO, TRANS, DIAG, N, NRHS, AP, B,
     $                            LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                            WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                 Check error code from cztprfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cztprfs', INFO, 0,
     $                               UPLO // TRANS // DIAG, N, N, -1,
...
*
               DO 50 ITRAN = 1, 2
                  ...
                  END IF
                  SRNAMT = 'cztpcon'
                  CALL cztpcon( NORM, UPLO, DIAG, N, AP, RCOND, WORK,
     $                         RWORK, INFO )
*
*                 Check error code from cztpcon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cztpcon', INFO, 0,
...
*
                  IF( RESULT( 7 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9997 ) 'cztpcon', NORM, UPLO,
   ...
   70    CONTINUE
*
*        Use pathological test matrices to test czlatps.
*
...
*
                  ...
                  CALL czlatps( UPLO, TRANS, DIAG, 'N', N, AP, B, SCALE,
     $                         RWORK, INFO )
*
*                 Check error code from czlatps.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czlatps', INFO, 0,
...
*
                  ...
                  CALL czlatps( UPLO, TRANS, DIAG, 'Y', N, AP, B( N+1 ),
     $                         SCALE, RWORK, INFO )
*
*                 Check error code from czlatps.
*
...
*
                  IF( RESULT( 8 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9996 )'czlatps', UPLO, TRANS,
