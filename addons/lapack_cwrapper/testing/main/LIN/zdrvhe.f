...
*  =======
*
*  ZDRVHE tests the driver routines czhesv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $               czhesv, czhesvX, ZHET01, czhetrf, czhetri, czlacpy,
     $                  ZLAIPD, ZLARHS, czlaset, ZLATB4, ZLATMS, ZPOT02,
     $                   ZPOT05
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL czhetrf( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL czlacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                     CALL czhetri( UPLO, N, AINV, LDA, IWORK, WORK,
     $                            INFO )
...
*
*                    Factor the matrix and solve the system using czhesv.
*
                     SRNAMT = 'czhesv '
                     CALL czhesv( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from czhesv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'czhesv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'czhesv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using czhesvX.
*
                  SRNAMT = 'czhesvX'
                  CALL czhesvX( FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from czhesvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'czhesvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from czhesvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'czhesvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
