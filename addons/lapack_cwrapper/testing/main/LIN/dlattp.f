...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IY,J,JC,JCNEXT,JCOUNT,JJ,JL,JR,JX,KL,KU,MODE
      DOUBLE PRECISION ANORM,BIGNUM,BNORM,BSCAL,C,CNDNUM,PLUS1,PLUS2,RA,
     +                 RB,REXP,S,SFAC,SMLNUM,STAR1,STEMP,T,TEXP,TLEFT,
...
*     .. External Subroutines ..
      EXTERNAL cdlabad,cdlarnv,DLATB4,DLATMS,DROT,DROTG,DSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
              ...
              DO 100 J = 2,N
                  A(JC+1) = Y
                  IF (J.GT.2) A(JC+J-1) = WORK(J-2)
                  IF (J.GT.3) A(JC+J-2) = WORK(N+J-3)
                  JC = JC + J
  100         CONTINUE
              JC = JC - N
              A(JC+1) = Z
...
*              Multiply by [ c -s;  s  c] on the right.
*
                  IF (N.GT.J+1) CALL DROT(N-J-1,A(JCNEXT+1),1,A(JC+2),1,
     +                                    C,-S)
*
*              Multiply by [-c  s; -s -c] on the left.
*
...
*
  ...
  180         CONTINUE
          ELSE
              JC = 1
              DO 190 J = 1,N
                  CALL cdlarnv(2,ISEED,N-J+1,A(JC))
                  A(JC) = SIGN(TWO,A(JC))
                  JC = JC + N - J + 1
  190         CONTINUE
          END IF
...
*
  ...
  200         CONTINUE
          ...
          ELSE
              JC = 1
              DO 210 J = 1,N
                  CALL cdlarnv(2,ISEED,N-J,A(JC+1))
...
*
  ...
  390         CONTINUE
          ELSE
              JC = 1
              DO 400 J = 1,N
                  IF (J.LT.N) CALL cdlarnv(2,ISEED,N-J,A(JC+1))
                  A(JC) = ZERO
                  JC = JC + N - J + 1
  400         CONTINUE
          END IF
