...
*  =======
*
*  SDRVSP tests the driver routines csspsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $  SPPT02, 
     $                   SPPT05, csspsv, csspsvX, SSPT01, cssptrf, 
     $  cssptri 
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
*                    Factor the matrix and solve the system using csspsv.
*
                     SRNAMT = 'csspsv '
                     CALL csspsv( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                           INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from csspsv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'csspsv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'csspsv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using csspsvX.
*
                  SRNAMT = 'csspsvX'
                  CALL csspsvX( FACT, UPLO, N, NRHS, A, AFAC, IWORK, B,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from csspsvX.
*
                  IF( INFO.NE.K ) THEN
                     CALL ALAERH( PATH, 'csspsvX', INFO, K, FACT // 
     $  UPLO, 
...
*                       residual.
*
                        CALL SSPT01( UPLO, N, A, AFAC, IWORK, AINV, 
     $  LDA, 
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from csspsvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'csspsvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
