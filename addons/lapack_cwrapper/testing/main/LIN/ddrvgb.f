...
*     .. Array Arguments ..
      LOGICAL DOTYPE(*)
      INTEGER IWORK(*),NVAL(*)
      DOUBLE PRECISION A(*),AFB(*),ASAV(*),B(*),BSAV(*),RWORK(*),S(*),
     +                 WORK(*),X(*),XACT(*)
*     ..
*
*  Purpose
*  =======
*
*  DDRVGB tests the driver routines cdgbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,I1,I2,IEQUED,IFACT,IKL,IKU,IMAT,IN,INFO,IOFF,ITRAN,
     +        IZERO,J,K,K1,KL,KU,LDA,LDAFB,LDB,MODE,N,NB,NBMIN,NERRS,
     +        NFACT,NFAIL,NIMAT,NKL,NKU,NRUN,NT
      DOUBLE PRECISION AINVNM,AMAX,ANORM,ANORMI,ANORMO,ANRMPV,CNDNUM,
...
*     .. External Subroutines ..
      EXTERNAL ALADHD,ALAERH,ALASVM,DERRVX,cdgbequ,cdgbsv,cdgbsvX,
     +         DGBT01,DGBT02,DGBT05,cdgbtrf,cdgbtrs,DGET04,cdlacpy,
     +         cdlaqgb,DLARHS,cdlaset,DLATB4,DLATMS,XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                  ...
                  IF (LDA*N.GT.LA .OR. LDAFB*N.GT.LAFB) THEN
                      IF (NFAIL.EQ.0 .AND. NERRS.EQ.0) CALL ALADHD(NOUT,
     +                    PATH)
                      IF (LDA*N.GT.LA) THEN
...
*
                      CALL cdlacpy('Full',KL+KU+1,N,A,LDA,ASAV,LDA)
*
                      DO 110 IEQUED = 1,4
                          EQUED = EQUEDS(IEQUED)
                          IF (IEQUED.EQ.1) THEN
...
*                          with the value returned by cdgesvX (FACT =
*                          'N' reuses the condition number from the
*                          previous iteration with FACT = 'F').
*
                                  CALL cdlacpy('Full',KL+KU+1,N,ASAV,
     +                                         LDA,AFB(KL+1),LDAFB)
...
*
                                      CALL cdgbequ(N,N,KL,KU,AFB(KL+1),
     +                                             LDAFB,S,S(N+1),
     +                                             ROWCND,COLCND,AMAX,
     +                                             INFO)
...
*                          Factor the matrix A.
*
                                  CALL cdgbtrf(N,N,KL,KU,AFB,LDAFB,
     +                                         IWORK,INFO)
*
*                          Form the inverse of A.
*
                                  CALL cdlaset('Full',N,N,ZERO,ONE,WORK,
     +                                         LDB)
                                  SRNAMT = 'cdgbtrs'
                                  CALL cdgbtrs('No transpose',N,KL,KU,N,
...
*
     ...
     +                                        LDB,B,LDB,ISEED,INFO)
                                  XTYPE = 'C'
                                  CALL cdlacpy('Full',N,NRHS,B,LDB,BSAV,
     +                                         LDB)
*
...
*
                                      CALL cdlacpy('Full',KL+KU+1,N,A,
     +                                             LDA,AFB(KL+1),LDAFB)
                                      CALL cdlacpy('Full',N,NRHS,B,LDB,
     +                                             X,LDB)
*
                                      SRNAMT = 'cdgbsv '
                                      CALL cdgbsv(N,KL,KU,NRHS,AFB,
     +                                            LDAFB,IWORK,X,LDB,
     +                                            INFO)
*
*                             Check error code from cdgbsv .
*
                                      IF (INFO.NE.IZERO)
     +                                    CALL ALAERH(PATH,'cdgbsv ',
     +                                    INFO,IZERO,' ',N,N,KL,KU,NRHS,
...
*                                solution.
*
                                          CALL cdlacpy('Full',N,NRHS,B,
     +                                                 LDB,WORK,LDB)
                                          CALL DGBT02('No transpose',N,
     +                                                N,KL,KU,NRHS,A,
     +                                                LDA,X,LDB,WORK,
     +                                                LDB,RESULT(2))
*
...
*                          --- Test cdgbsvX ---
*
                                  IF (.NOT.PREFAC) CALL cdlaset('Full',
     +                                2*KL+KU+1,N,ZERO,ZERO,AFB,LDAFB)
                                  CALL cdlaset('Full',N,NRHS,ZERO,ZERO,
     +                                         X,LDB)
...
*                             EQUED = 'R', 'C', or 'B'.
*
                                      CALL cdlaqgb(N,N,KL,KU,A,LDA,S,
     +                                             S(N+1),ROWCND,COLCND,
     +                                             AMAX,EQUED)
                                  END IF
*
*                          Solve the system and compute the condition
*                          number and error bounds using cdgbsvX.
*
                                  SRNAMT = 'cdgbsvX'
                                  CALL cdgbsvX(FACT,TRANS,N,KL,KU,NRHS,
     +                                         A,LDA,AFB,LDAFB,IWORK,
     +                                         EQUED,S,S(N+1),B,LDB,X,
     +                                         LDB,RCOND,RWORK,
     +                                         RWORK(NRHS+1),WORK,
     +                                         IWORK(N+1),INFO)
*
*                          Check the error code from cdgbsvX.
*
                                  IF (INFO.NE.IZERO) CALL ALAERH(PATH,
     +                                'cdgbsvX',INFO,IZERO,FACT//TRANS,
     +                                N,N,KL,KU,NRHS,IMAT,NFAIL,NERRS,
     +                                NOUT)
*
C*PL*ERROR* Comment line too long
*                          Compare WORK(1) from cdgbsvX with the computed
...
*
   ...
   70                                 CONTINUE
     ...
     +                                         LDAFB,WORK)
                                      ...
                                      END IF
                                  ELSE
                                      RPVGRW = DLANTB('M','U','N',N,
     +                                         KL+KU,AFB,LDAFB,WORK)
                                      IF (RPVGRW.EQ.ZERO) THEN
                                          RPVGRW = ONE
                                      ELSE
...
*
     ...
     +                                            RESULT(4))
                                  ...
                                  END IF
*
*                          Compare RCOND from cdgbsvX with the computed
*                          value in RCONDC.
*
...
*
 ...
 9998 FORMAT (' *** In DDRVGB, LAFB=',I5,' is too small for N=',I5,
     ...
     +       I5)
 9997 FORMAT (1X,A6,', N=',I5,', KL=',I5,', KU=',I5,', type ',I1,
     +       ', test(',I1,')=',G12.5)
 9996 FORMAT (1X,A6,'( ''',A1,''',''',A1,''',',I5,',',I5,',',I5,
     +       ',...), type ',I1,', test(',I1,')=',G12.5)
 9995 FORMAT (1X,A6,'( ''',A1,''',''',A1,''',',I5,',',I5,',',I5,
