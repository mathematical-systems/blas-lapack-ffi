...
*  =======
*
*  CCHKPP tests ccpptrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 cclacpy, CLAIPD, CLARHS, CLATB4, CLATMS, ccppcon,
     $                 ccpprfs, CPPT01, CPPT02, CPPT03, CPPT05, ccpptrf,
     $                   ccpptri, ccpptrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
*              Check error code from ccpptrf.
*
               IF( INFO.NE.IZERO ) THEN
                  CALL ALAERH( PATH, 'ccpptrf', INFO, IZERO, UPLO, N, N,
...
*
               CALL CCOPY( NPP, AFAC, 1, AINV, 1 )
               SRNAMT = 'ccpptri'
               CALL ccpptri( UPLO, N, AINV, INFO )
*
*              Check error code from ccpptri.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'ccpptri', INFO, 0, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
...
*
                  SRNAMT = 'ccpptrs'
                  CALL ccpptrs( UPLO, N, NRHS, AFAC, X, LDA, INFO )
*
*              Check error code from ccpptrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccpptrs', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'ccpprfs'
                  CALL ccpprfs( UPLO, N, NRHS, A, AFAC, B, LDA, X, LDA,
     $                         RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from ccpprfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccpprfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
               ...
               SRNAMT = 'ccppcon'
               CALL ccppcon( UPLO, N, AFAC, ANORM, RCOND, WORK, RWORK,
     $                      INFO )
*
*              Check error code from ccppcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'ccppcon', INFO, 0, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
