...
*  =======
*
*  CDRVSP tests the driver routines ccspsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 cclacpy, CLARHS, cclaset, CLATB4, CLATMS, CLATSP,
     $                 CPPT05, ccspsv, ccspsvX, CSPT01, CSPT02, ccsptrf,
     $                   ccsptri, XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL ccsptrf( UPLO, N, AFAC, IWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL CCOPY( NPP, AFAC, 1, AINV, 1 )
                     CALL ccsptri( UPLO, N, AINV, IWORK, WORK, INFO )
...
*
*                    Factor the matrix and solve the system using ccspsv.
*
                     SRNAMT = 'ccspsv '
                     CALL ccspsv( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                           INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from ccspsv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'ccspsv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'ccspsv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using ccspsvX.
*
                  SRNAMT = 'ccspsvX'
                  CALL ccspsvX( FACT, UPLO, N, NRHS, A, AFAC, IWORK, B,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from ccspsvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'ccspsvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from ccspsvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'ccspsvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
