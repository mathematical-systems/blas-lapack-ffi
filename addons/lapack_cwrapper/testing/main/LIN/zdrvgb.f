...
*  =======
*
*  ZDRVGB tests the driver routines czgbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALADHD, ALAERH, ALASVM, XLAENV, ZERRVX, czgbequ,
     $                 czgbsv, czgbsvX, ZGBT01, ZGBT02, ZGBT05, czgbtrf,
     $               czgbtrs, ZGET04, czlacpy, czlaqgb, ZLARHS, czlaset,
     $                   ZLATB4, ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*
                  CALL czlacpy( 'Full', KL+KU+1, N, A, LDA, ASAV, LDA )
*
                  DO 110 IEQUED = 1, 4
                     EQUED = EQUEDS( IEQUED )
                     IF( IEQUED.EQ.1 ) THEN
...
*
                           CALL czlacpy( 'Full', KL+KU+1, N, ASAV, LDA,
     $                                  AFB( KL+1 ), LDAFB )
...
*
                              CALL czgbequ( N, N, KL, KU, AFB( KL+1 ),
...
*
                                CALL czlaqgb( N, N, KL, KU, AFB( KL+1 ),
     $                                        LDAFB, S, S( N+1 ),
     $                                        ROWCND, COLCND, AMAX,
     $                                        EQUED )
                              END IF
...
*                          Factor the matrix A.
*
                          CALL czgbtrf( N, N, KL, KU, AFB, LDAFB, IWORK,
     $                                  INFO )
*
...
*
     ...
     $                                  DCMPLX( ONE ), WORK, LDB )
                           SRNAMT = 'czgbtrs'
                           CALL czgbtrs( 'No transpose', N, KL, KU, N,
...
*
     ...
     $                                  LDB, B, LDB, ISEED, INFO )
                           XTYPE = 'C'
                           CALL czlacpy( 'Full', N, NRHS, B, LDB, BSAV,
     $                                  LDB )
*
...
*
                              CALL czlacpy( 'Full', KL+KU+1, N, A, LDA,
     $                                     AFB( KL+1 ), LDAFB )
                              CALL czlacpy( 'Full', N, NRHS, B, LDB, X,
     $                                     LDB )
*
                              SRNAMT = 'czgbsv '
                              CALL czgbsv( N, KL, KU, NRHS, AFB, LDAFB,
     $                                    IWORK, X, LDB, INFO )
*
*                             Check error code from czgbsv .
*
                              IF( INFO.NE.IZERO )
     $                           CALL ALAERH( PATH, 'czgbsv ', INFO,
     $                                        IZERO, ' ', N, N, KL, KU,
...
*                                solution.
*
                                 CALL czlacpy( 'Full', N, NRHS, B, LDB,
     $                                        WORK, LDB )
...
*                             EQUED = 'R', 'C', or 'B'.
*
                              CALL czlaqgb( N, N, KL, KU, A, LDA, S,
     $                                     S( N+1 ), ROWCND, COLCND,
     $                                     AMAX, EQUED )
                           END IF
*
*                          Solve the system and compute the condition
*                          number and error bounds using czgbsvX.
*
                           SRNAMT = 'czgbsvX'
                          CALL czgbsvX( FACT, TRANS, N, KL, KU, NRHS, A,
     $                                  LDA, AFB, LDAFB, IWORK, EQUED,
     $                                  S, S( LDB+1 ), B, LDB, X, LDB,
     $                                  RCOND, RWORK, RWORK( NRHS+1 ),
     $                                  WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                          Check the error code from czgbsvX.
*
                           IF( INFO.NE.IZERO )
     $                        CALL ALAERH( PATH, 'czgbsvX', INFO, IZERO,
     $                                     FACT // TRANS, N, N, KL, KU,
     $                                     NRHS, IMAT, NFAIL, NERRS,
     $                                     NOUT )
*                          Compare RWORK(2*NRHS+1) from czgbsvX with the
...
*
                              CALL czlacpy( 'Full', N, NRHS, BSAV, LDB,
     $                                     WORK, LDB )
...
*
     ...
     $                                     RESULT( 4 ) )
                           ...
                           END IF
*
*                          Compare RCOND from czgbsvX with the computed
*                          value in RCONDC.
*
...
*
   ...
   80                         CONTINUE
     ...
     $                              CALL ALADHD( NOUT, PATH )
                                 IF( PREFAC ) THEN
                                    WRITE( NOUT, FMT = 9995 )'czgbsvX',
     $                                 FACT, TRANS, N, KL, KU, EQUED,
     $                                 IMAT, 1, RESULT( 1 )
                                 ELSE
                                    WRITE( NOUT, FMT = 9996 )'czgbsvX',
