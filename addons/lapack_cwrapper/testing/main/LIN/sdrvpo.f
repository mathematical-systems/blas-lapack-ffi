...
*  =======
*
*  SDRVPO tests the driver routines csposv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALADHD, ALAERH, ALASVM, SERRVX, SGET04, 
     $  cslacpy, 
     $                   cslaqsy, SLARHS, cslaset, SLATB4, SLATMS, 
     $  cspoequ, 
     $                   csposv, csposvX, SPOT01, SPOT02, SPOT05, 
     $  cspotrf, 
     $                   cspotri, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
               CALL cslacpy( UPLO, N, N, A, LDA, ASAV, LDA )
*
               DO 100 IEQUED = 1, 2
                  EQUED = EQUEDS( IEQUED )
                  IF( IEQUED.EQ.1 ) THEN
...
*                       the value returned by csposvX (FACT = 'N' reuses
*                       the condition number from the previous iteration
*                       with FACT = 'F').
*
                        CALL cslacpy( UPLO, N, N, ASAV, LDA, AFAC, LDA 
     $  ) 
...
*
                           CALL cspoequ( N, AFAC, LDA, S, SCOND, AMAX,
     $                                  INFO )
...
*
                              CALL cslaqsy( UPLO, N, AFAC, LDA, S, 
     $  SCOND, 
     $                                     AMAX, EQUED )
                           END IF
                        END IF
...
*                       Factor the matrix A.
*
                        CALL cspotrf( UPLO, N, AFAC, LDA, INFO )
*
*                       Form the inverse of A.
*
                        CALL cslacpy( UPLO, N, N, AFAC, LDA, A, LDA )
                        CALL cspotri( UPLO, N, A, LDA, INFO )
*
...
*
                     CALL cslacpy( UPLO, N, N, ASAV, LDA, A, LDA )
*
...
*
                        CALL cslacpy( UPLO, N, N, A, LDA, AFAC, LDA )
                        CALL cslacpy( 'Full', N, NRHS, B, LDA, X, LDA )
*
                        SRNAMT = 'csposv '
                        CALL csposv( UPLO, N, NRHS, AFAC, LDA, X, LDA,
     $                              INFO )
*
*                       Check error code from csposv .
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'csposv ', INFO, IZERO,
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              WRITE( NOUT, FMT = 9999 )'csposv ', UPLO,
     $                           N, IMAT, K, RESULT( K )
                              NFAIL = NFAIL + 1
                           END IF
   60                   CONTINUE
...
*
                     IF( .NOT.PREFAC )
     $                  CALL cslaset( UPLO, N, N, ZERO, ZERO, AFAC, LDA 
     $  ) 
...
*                       EQUED='Y'.
*
                        CALL cslaqsy( UPLO, N, A, LDA, S, SCOND, AMAX,
     $                               EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using csposvX.
*
                     SRNAMT = 'csposvX'
                     CALL csposvX( FACT, UPLO, N, NRHS, A, LDA, AFAC,
     $                            LDA, EQUED, S, B, LDA, X, LDA, RCOND,
     $                            RWORK, RWORK( NRHS+1 ), WORK, IWORK,
     $                            INFO )
*
*                    Check the error code from csposvX.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'csposvX', INFO, IZERO,
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from csposvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'csposvX', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'csposvX', FACT,
