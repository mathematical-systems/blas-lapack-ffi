...
*  =======
*
*  SCHKPO tests cspotrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, SERRPO, SGET04, 
     $  cslacpy, 
     $                   SLARHS, SLATB4, SLATMS, cspocon, csporfs, 
     $  SPOT01, 
     $                   SPOT02, SPOT03, SPOT05, cspotrf, cspotri, 
     $  cspotrs, 
     $                   XLAENV
*     ..
*     .. Scalars in Common ..
...
*
*                 Check error code from cspotrf.
*
                  IF( INFO.NE.IZERO ) THEN
                     CALL ALAERH( PATH, 'cspotrf', INFO, IZERO, UPLO, 
     $  N, 
...
*
                  CALL cslacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                  SRNAMT = 'cspotri'
                  CALL cspotri( UPLO, N, AINV, LDA, INFO )
*
*                 Check error code from cspotri.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cspotri', INFO, 0, UPLO, N, N,
...
*
                     SRNAMT = 'cspotrs'
                     CALL cspotrs( UPLO, N, NRHS, AFAC, LDA, X, LDA,
     $                            INFO )
*
*                 Check error code from cspotrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cspotrs', INFO, 0, UPLO, N,
...
*
                     SRNAMT = 'csporfs'
                     CALL csporfs( UPLO, N, NRHS, A, LDA, AFAC, LDA, B,
     $                            LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                            WORK, IWORK, INFO )
*
*                 Check error code from csporfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'csporfs', INFO, 0, UPLO, N,
     $                               N, -1, -1, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL SGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
                  ...
                  SRNAMT = 'cspocon'
                  CALL cspocon( UPLO, N, AFAC, LDA, ANORM, RCOND, WORK,
     $                         IWORK, INFO )
*
*                 Check error code from cspocon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cspocon', INFO, 0, UPLO, N, N,
     $                            -1, -1, -1, IMAT, NFAIL, NERRS, NOUT 
     $  ) 
*
