...
*     .. External Subroutines ..
     ...
     $                  ZGET04, czgtsv, czgtsvX, ZGTT01, ZGTT02, ZGTT05,
     $             czgttrf, czgttrs, czlacpy, czlagtm, czlarnv, czlaset,
     $                   ZLATB4, ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*                 Factor the matrix A.
*
                  CALL czgttrf( N, AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, INFO )
*
*                 Use czgttrs to solve for one column at a time of
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL czgttrs( 'No transpose', N, 1, AF, AF( M+1 ),
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL czgttrs( 'Conjugate transpose', N, 1, AF,
...
*
                  IX = 1
                  DO 70 J = 1, NRHS
                     CALL czlarnv( 2, ISEED, N, XACT( IX ) )
                     IX = IX + LDA
   70             CONTINUE
*
*                 Set the right hand side.
*
                  CALL czlagtm( TRANS, N, NRHS, ONE, A, A( M+1 ),
...
*
                     SRNAMT = 'czgtsv '
                    CALL czgtsv( N, NRHS, AF, AF( M+1 ), AF( N+M+1 ), X,
     $                           LDA, INFO )
*
*                    Check error code from czgtsv .
*
                     IF( INFO.NE.IZERO )
     $                  CALL ALAERH( PATH, 'czgtsv ', INFO, IZERO, ' ',
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'czgtsv ', N, IMAT,
     $                        K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
   80                CONTINUE
...
*                 error bounds using czgtsvX.
*
                  SRNAMT = 'czgtsvX'
                  CALL czgtsvX( FACT, TRANS, N, NRHS, A, A( M+1 ),
     $                         A( N+M+1 ), AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, B, LDA, X, LDA,
     $                         RCOND, RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*                 Check the error code from czgtsvX.
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'czgtsvX', INFO, IZERO,
...
*                    Check residual of computed solution.
*
                     CALL czlacpy( 'Full', N, NRHS, B, LDA, WORK, LDA )
                     CALL ZGTT02( TRANS, N, NRHS, A, A( M+1 ),
...
*
                  DO 100 K = K1, NT
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'czgtsvX', FACT, TRANS,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  100             CONTINUE
...
*
                  ...
                  IF( RESULT( 6 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALADHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9998 )'czgtsvX', FACT, TRANS, N,
     $                  IMAT, K, RESULT( K )
                     NFAIL = NFAIL + 1
                  END IF
                  NRUN = NRUN + NT - K1 + 2
