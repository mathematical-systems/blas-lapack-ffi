...
*          cdgbtrf.  U is stored as an upper triangular band matrix with
*          KL+KU superdiagonals in rows 1 to KL+KU+1, and the
*          multipliers used during the factorization are stored in rows
*          KL+KU+2 to 2*KL+KU+1.  See cdgbtrf for further details.
*
...
*          The pivot indices from cdgbtrf.
*
...
*
      ...
      DO 10 J = 1,N
          I1 = MAX(KD+1-J,1)
          I2 = MIN(KD+M-J,KL+KD)
          IF (I2.GE.I1) ANORM = MAX(ANORM,DASUM(I2-I1+1,A(I1,J),1))
   10 CONTINUE
*
...
*
              JUA = MIN(JU,KU)
              IF (JUA+JL+1.GT.0) CALL DAXPY(JUA+JL+1,-ONE,A(KU+1-JUA,J),
     +                                1,WORK(JU+1-JUA),1)
*
