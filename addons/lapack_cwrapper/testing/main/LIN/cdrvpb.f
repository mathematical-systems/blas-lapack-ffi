...
*  =======
*
*  CDRVPB tests the driver routines ccpbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                cclacpy, CLAIPD, cclaqhb, CLARHS, cclaset, CLATB4,
     $                 CLATMS, ccpbequ, ccpbsv, ccpbsvX, CPBT01, CPBT02,
     $                   CPBT05, ccpbtrf, ccpbtrs, CSWAP, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                  CALL cclacpy( 'Full', KD+1, N, A, LDAB, ASAV, LDAB )
*
                  DO 70 IEQUED = 1, 2
                     EQUED = EQUEDS( IEQUED )
                     IF( IEQUED.EQ.1 ) THEN
...
*                          with the value returned by ccpbsvX (FACT =
*                          'N' reuses the condition number from the
*                          previous iteration with FACT = 'F').
*
                           CALL cclacpy( 'Full', KD+1, N, ASAV, LDAB,
     $                                  AFAC, LDAB )
...
*
                              CALL ccpbequ( UPLO, N, KD, AFAC, LDAB, S,
     $                                     SCOND, AMAX, INFO )
...
*
                                 CALL cclaqhb( UPLO, N, KD, AFAC, LDAB,
     $                                        S, SCOND, AMAX, EQUED )
                              END IF
                           END IF
*
...
*                          Factor the matrix A.
*
                           CALL ccpbtrf( UPLO, N, KD, AFAC, LDAB, INFO )
*
...
*
     ...
     $                                  CMPLX( ONE ), A, LDA )
                           SRNAMT = 'ccpbtrs'
                           CALL ccpbtrs( UPLO, N, KD, N, AFAC, LDAB, A,
     $                                  LDA, INFO )
*
...
*
                        CALL cclacpy( 'Full', KD+1, N, ASAV, LDAB, A,
     $                               LDAB )
*
...
*
     ...
     $                               LDA, ISEED, INFO )
                        XTYPE = 'C'
                        CALL cclacpy( 'Full', N, NRHS, B, LDA, BSAV,
     $                               LDA )
*
                        IF( NOFACT ) THEN
*
...
*
                           CALL cclacpy( 'Full', KD+1, N, A, LDAB, AFAC,
     $                                  LDAB )
                           CALL cclacpy( 'Full', N, NRHS, B, LDA, X,
     $                                  LDA )
*
                           SRNAMT = 'ccpbsv '
                          CALL ccpbsv( UPLO, N, KD, NRHS, AFAC, LDAB, X,
     $                                 LDA, INFO )
*
*                          Check error code from ccpbsv .
*
                           IF( INFO.NE.IZERO ) THEN
                              CALL ALAERH( PATH, 'ccpbsv ', INFO, IZERO,
     $                                     UPLO, N, N, KD, KD, NRHS,
...
*
                        IF( .NOT.PREFAC )
     $                     CALL cclaset( 'Full', KD+1, N, CMPLX( ZERO ),
...
*                          EQUED='Y'
*
                           CALL cclaqhb( UPLO, N, KD, A, LDAB, S, SCOND,
     $                                  AMAX, EQUED )
                        END IF
*
*                       Solve the system and compute the condition
*                       number and error bounds using ccpbsvX.
*
                        SRNAMT = 'ccpbsvX'
                        CALL ccpbsvX( FACT, UPLO, N, KD, NRHS, A, LDAB,
     $                               AFAC, LDAB, EQUED, S, B, LDA, X,
     $                               LDA, RCOND, RWORK, RWORK( NRHS+1 ),
     $                               WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                       Check the error code from ccpbsvX.
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'ccpbsvX', INFO, IZERO,
     $                                  FACT // UPLO, N, N, KD, KD,
...
*                          Compute residual of the computed solution.
*
                           CALL cclacpy( 'Full', N, NRHS, BSAV, LDA,
     $                                  WORK, LDA )
...
*
     ...
     $                                  RESULT( 4 ) )
                        ...
                        END IF
*
*                       Compare RCOND from ccpbsvX with the computed
*                       value in RCONDC.
*
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9997 )'ccpbsvX',
     $                              FACT, UPLO, N, KD, EQUED, IMAT, K,
     $                              RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9998 )'ccpbsvX',
