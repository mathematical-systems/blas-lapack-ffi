...
*     .. External Subroutines ..
      EXTERNAL           ALADHD, ALAERH, ALASVM, DCOPY, cdlarnv, DSCAL,
     $                   ZCOPY, ZDSCAL, ZERRVX, ZGET04, czlacpy, ZLAPTM,
     $                czlarnv, czlaset, ZLATB4, ZLATMS, czptsv, czptsvX,
     $                   ZPTT01, ZPTT02, ZPTT05, czpttrf, czpttrs
*     ..
*     .. Intrinsic Functions ..
...
*
*                 Use czpttrs to solve for one column at a time of
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL czpttrs( 'Lower', N, 1, D( N+1 ), E( N+1 ), X,
     $                            LDA, INFO )
...
*
                  SRNAMT = 'czptsv '
                  CALL czptsv( N, NRHS, D( N+1 ), E( N+1 ), X, LDA,
     $                        INFO )
*
*                 Check error code from czptsv .
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'czptsv ', INFO, IZERO, ' ', N,
...
*
     ...
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 )'czptsv ', N, IMAT, K,
     $                     RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
   70             CONTINUE
...
*              error bounds using czptsvX.
*
               SRNAMT = 'czptsvX'
               CALL czptsvX( FACT, N, NRHS, D, E, D( N+1 ), E( N+1 ), B,
     $                      LDA, X, LDA, RCOND, RWORK, RWORK( NRHS+1 ),
     $                      WORK, RWORK( 2*NRHS+1 ), INFO )
*
*              Check the error code from czptsvX.
*
               IF( INFO.NE.IZERO )
     $            CALL ALAERH( PATH, 'czptsvX', INFO, IZERO, FACT, N, N,
...
*
               DO 90 K = K1, 6
                  IF( RESULT( K ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALADHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9998 )'czptsvX', FACT, N, IMAT,
     $                  K, RESULT( K )
                     NFAIL = NFAIL + 1
                  END IF
   90          CONTINUE
