...
*  =======
*
*  CQLT03 tests ccunmql, which computes Q*C, Q'*C, C*Q or C*Q'.
*
*  CQLT03 compares the results of a call to ccunmql with the results of
*  forming Q explicitly by a call to ccungql and then performing matrix
...
*          returned by ccgeqlf. See ccgeqlf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL        CGEMM, cclacpy, cclarnv, cclaset, ccungql, ccunmql
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
*     ..
...
*
     ...
     $   CALL cclacpy( 'Full', M-K, K, AF( 1, N-K+1 ), LDA,
     $                Q( 1, M-K+1 ), LDA )
      IF( K.GT.1 )
     $   CALL cclacpy( 'Upper', K-1, K-1, AF( M-K+1, N-K+2 ), LDA,
     $                Q( M-K+1, M-K+2 ), LDA )
*
*     Generate the m-by-m matrix Q
*
      SRNAMT = 'ccungql'
      CALL ccungql( M, M, K, Q, LDA, TAU( MINMN-K+1 ), WORK, LWORK,
     $             INFO )
*
      DO 30 ISIDE = 1, 2
         IF( ISIDE.EQ.1 ) THEN
...
*
            SRNAMT = 'ccunmql'
            IF( K.GT.0 )
     $         CALL ccunmql( SIDE, TRANS, MC, NC, K, AF( 1, N-K+1 ),
