...
*  =======
*
*  ZQRT01 tests czgeqrf, which computes the QR factorization of an m-by-n
*  matrix A, and partially tests czungqr which forms the m-by-m
*  orthogonal matrix Q.
*
...
*          Details of the QR factorization of A, as returned by czgeqrf.
*          See czgeqrf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL          ZGEMM, czgeqrf, ZHERK, czlacpy, czlaset, czungqr
*     ..
*     .. Intrinsic Functions ..
...
*
      SRNAMT = 'czgeqrf'
      CALL czgeqrf( M, N, AF, LDA, TAU, WORK, LWORK, INFO )
*
*     Copy details of Q
*
      CALL czlaset( 'Full', M, M, ROGUE, ROGUE, Q, LDA )
      CALL czlacpy( 'Lower', M-1, N, AF( 2, 1 ), LDA, Q( 2, 1 ), LDA )
*
*     Generate the m-by-m matrix Q
*
      SRNAMT = 'czungqr'
      CALL czungqr( M, M, MINMN, Q, LDA, TAU, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL czlaset( 'Full', M, N, DCMPLX( ZERO ), DCMPLX( ZERO ), R,
     $             LDA )
      CALL czlacpy( 'Upper', M, N, AF, LDA, R, LDA )
*
*     Compute R - Q'*A
*
...
*     Compute I - Q'*Q
*
      CALL czlaset('Full', M, M, DCMPLX( ZERO ), DCMPLX( ONE ), R, LDA )
