...
*  =======
*
*  DRQT02 tests cdorgrq, which generates an m-by-n matrix Q with
...
*     .. External Subroutines ..
      EXTERNAL DGEMM,cdlacpy,cdlaset,cdorgrq,DSYRK
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC DBLE,MAX
*     ..
...
*
      CALL cdlaset('Full',M,N,ROGUE,ROGUE,Q,LDA)
      IF (K.LT.N) CALL cdlacpy('Full',K,N-K,AF(M-K+1,1),LDA,Q(M-K+1,1),
     +                         LDA)
...
*
      SRNAMT = 'cdorgrq'
      CALL cdorgrq(M,N,K,Q,LDA,TAU(M-K+1),WORK,LWORK,INFO)
*
*     Copy R(m-k+1:m,n-m+1:n)
*
      CALL cdlaset('Full',K,M,ZERO,ZERO,R(M-K+1,N-M+1),LDA)
      CALL cdlacpy('Upper',K,K,AF(M-K+1,N-K+1),LDA,R(M-K+1,N-K+1),LDA)
*
