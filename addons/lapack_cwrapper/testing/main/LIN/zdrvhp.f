...
*  =======
*
*  ZDRVHP tests the driver routines czhpsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                ZGET04, czhpsv, czhpsvX, ZHPT01, czhptrf, czhptri,
     $                 czlacpy, ZLAIPD, ZLARHS, czlaset, ZLATB4, ZLATMS,
     $                   ZPPT02, ZPPT05
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL czhptrf( UPLO, N, AFAC, IWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL ZCOPY( NPP, AFAC, 1, AINV, 1 )
                     CALL czhptri( UPLO, N, AINV, IWORK, WORK, INFO )
...
*
*                    Factor the matrix and solve the system using czhpsv.
*
                     SRNAMT = 'czhpsv '
                     CALL czhpsv( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                           INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from czhpsv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'czhpsv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'czhpsv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using czhpsvX.
*
                  SRNAMT = 'czhpsvX'
                  CALL czhpsvX( FACT, UPLO, N, NRHS, A, AFAC, IWORK, B,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from czhpsvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'czhpsvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from czhpsvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'czhpsvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
