...
*  =======
*
*  CCHKPB tests ccpbtrf, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 cclacpy, CLAIPD, CLARHS, cclaset, CLATB4, CLATMS,
     $                ccpbcon, ccpbrfs, CPBT01, CPBT02, CPBT05, ccpbtrf,
     $                   ccpbtrs, CSWAP, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*                    matrix.
*
                    CALL cclacpy( 'Full', KD+1, N, A, LDAB, AFAC, LDAB )
                     SRNAMT = 'ccpbtrf'
                     CALL ccpbtrf( UPLO, N, KD, AFAC, LDAB, INFO )
*
*                    Check error code from ccpbtrf.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'ccpbtrf', INFO, IZERO, UPLO,
...
*                    residual.
*
                     CALL cclacpy( 'Full', KD+1, N, AFAC, LDAB, AINV,
     $                            LDAB )
...
*
     ...
     $                            CMPLX( ONE ), AINV, LDA )
                     SRNAMT = 'ccpbtrs'
                    CALL ccpbtrs( UPLO, N, KD, N, AFAC, LDAB, AINV, LDA,
     $                            INFO )
*
...
*
                        SRNAMT = 'ccpbtrs'
                        CALL ccpbtrs( UPLO, N, KD, NRHS, AFAC, LDAB, X,
     $                               LDA, INFO )
*
*                    Check error code from ccpbtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'ccpbtrs', INFO, 0, UPLO,
...
*
                        SRNAMT = 'ccpbrfs'
                        CALL ccpbrfs( UPLO, N, KD, NRHS, A, LDAB, AFAC,
     $                               LDAB, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                    Check error code from ccpbrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'ccpbrfs', INFO, 0, UPLO,
     $                                  N, N, KD, KD, NRHS, IMAT, NFAIL,
     $                                  NERRS, NOUT )
*
                        CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
                     SRNAMT = 'ccpbcon'
                    CALL ccpbcon( UPLO, N, KD, AFAC, LDAB, ANORM, RCOND,
     $                            WORK, RWORK, INFO )
*
*                    Check error code from ccpbcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'ccpbcon', INFO, 0, UPLO, N,
     $                               N, KD, KD, -1, IMAT, NFAIL, NERRS,
     $                               NOUT )
*
