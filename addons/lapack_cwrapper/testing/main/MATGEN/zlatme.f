...
*     .. External Subroutines ..
      EXTERNAL           DLATM1, cxerbla, ZCOPY, ZDSCAL, ZGEMV, ZGERC,
     $               czlacgv, czlarfG, ZLARGE, czlarnv, czlaset, ZLATM1,
     $                   ZSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( IUPPER.NE.0 ) THEN
         DO 40 JC = 2, N
            CALL czlarnv( IDIST, ISEED, JC-1, A( 1, JC ) )
   40    CONTINUE
      END IF
*
...
*
            ...
            XNORMS = WORK( 1 )
            CALL czlarfG( IROWS, XNORMS, WORK( 2 ), 1, TAU )
            TAU = DCONJG( TAU )
            WORK( 1 ) = CONE
...
*
            A( JCR, IC ) = XNORMS
            CALL czlaset( 'Full', IROWS-1, 1, CZERO, CZERO,
     $                   A( JCR+1, IC ), LDA )
*
...
*
            ...
            XNORMS = WORK( 1 )
            CALL czlarfG( ICOLS, XNORMS, WORK( 2 ), 1, TAU )
            TAU = DCONJG( TAU )
            WORK( 1 ) = CONE
            CALL czlacgv( ICOLS-1, WORK( 2 ), 1 )
            ALPHA = ZLARND( 5, ISEED )
*
...
*
            A( IR, JCR ) = XNORMS
            CALL czlaset( 'Full', 1, ICOLS-1, CZERO, CZERO,
     $                   A( IR, JCR+1 ), LDA )
*
