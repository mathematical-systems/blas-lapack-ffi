...
*     .. External Subroutines ..
      EXTERNAL           DLATM1, DSCAL, cxerbla, ZLAGGE, ZLAGHE, ZLAGSY,
     $                   ZLAROT, czlartg, czlaset
*     ..
*     .. Intrinsic Functions ..
...
*
                     ...
                     DO 50 JCH = JR - JKL, 1, -JKL - JKU
                        IF( IR.LT.M ) THEN
                           CALL czlartg( A( IR+1-ISKEW*( IC+1 )+IOFFST,
     ...
     $                               ILDA, CTEMP, EXTRA )
                        IF( ILTEMP ) THEN
                          CALL czlartg( A( IROW+1-ISKEW*( IC+1 )+IOFFST,
...
*
     ...
     $                               ILDA, CTEMP, EXTRA )
                        IF( ILTEMP ) THEN
                          CALL czlartg( A( IR+1-ISKEW*( ICOL+1 )+IOFFST,
...
*
                     ...
                     DO 110 JCH = JC + JKL, IENDCH, JKL + JKU
                        ILEXTR = IC.GT.0
                        IF( ILEXTR ) THEN
                           CALL czlartg( A( JCH-ISKEW*IC+IOFFST, IC ),
     $                                  EXTRA, REALC, S, DUMMY )
                           DUMMY = ZLARND( 5, ISEED )
                           C = REALC*DUMMY
                           S = S*DUMMY
     ...
     $                               ILDA, EXTRA, CTEMP )
                        IF( ILTEMP ) THEN
                           CALL czlartg( A( JCH-ISKEW*ICOL+IOFFST,
...
*
                     ...
                     DO 140 JCH = JR + JKU, IENDCH, JKL + JKU
                        ILEXTR = IR.GT.0
                        IF( ILEXTR ) THEN
                           CALL czlartg( A( IR-ISKEW*JCH+IOFFST, JCH ),
     $                                  EXTRA, REALC, S, DUMMY )
                           DUMMY = ZLARND( 5, ISEED )
                           C = REALC*DUMMY
                           S = S*DUMMY
     ...
     $                               JCH ), ILDA, EXTRA, CTEMP )
                        IF( ILTEMP ) THEN
                          CALL czlartg( A( IROW-ISKEW*JCH+IOFFST, JCH ),
     $                                  CTEMP, REALC, S, DUMMY )
                           DUMMY = ZLARND( 5, ISEED )
                           C = REALC*DUMMY
                           S = S*DUMMY
...
*
                     ICOL = JC
                     DO 180 JCH = JC - K, 1, -K
                        CALL czlartg( A( JCH+1-ISKEW*( ICOL+1 )+IOFFG,
...
*
                     ...
                     DO 270 JCH = JC + K, N - 1, K
                        CALL czlartg( A( JCH-ISKEW*ICOL+IOFFG, ICOL ),
     $                               EXTRA, REALC, S, DUMMY )
                        DUMMY = ZLARND( 5, ISEED )
                        C = REALC*DUMMY
                        S = S*DUMMY
