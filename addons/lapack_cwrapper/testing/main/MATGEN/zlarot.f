...
*           output of czrotG or to most versions of czrot, both C and S
...
*
      IF( NL.LT.NT ) THEN
         CALL cxerbla( 'ZLAROT', 4 )
         RETURN
      END IF
      IF( LDA.LE.0 .OR. ( .NOT.LROWS .AND. LDA.LT.NL-NT ) ) THEN
         CALL cxerbla( 'ZLAROT', 8 )
         RETURN
      END IF
*
*     Rotate
*
*     czrot( NL-NT, A(IX),IINC, A(IY),IINC, C, S ) with complex C, S
*
      DO 10 J = 0, NL - NT - 1
   ...
   10 CONTINUE
*
*     czrot( NT, XT,1, YT,1, C, S ) with complex C, S
*
      DO 20 J = 1, NT
         TEMPX = C*XT( J ) + S*YT( J )
