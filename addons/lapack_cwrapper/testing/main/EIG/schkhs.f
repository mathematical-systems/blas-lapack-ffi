...
*
*             csgehrd factors A as  U H U' , where ' means transpose,
*             H is hessenberg, and U is an orthogonal matrix.
*
*             csorghr generates the orthogonal matrix U.
*
*             csormhr multiplies a matrix by the orthogonal matrix U.
*
*             cshseqr factors H as  Z T Z' , where Z is orthogonal and
*             T is "quasi-triangular", and the eigenvalue vector W.
*
*             cstrevc computes the left and right eigenvector matrices
*             L and R for T.
*
*             cshsein computes the left and right eigenvector matrices
...
*           The upper hessenberg matrix computed by csgehrd.  On exit,
*           H contains the Hessenberg form of the matrix in A.
*           Modified.
*
*  T1     - REAL array, dimension (LDA,max(NN))
*           The Schur (="quasi-triangular") matrix computed by cshseqr
*           if Z is computed.  On exit, T1 contains the Schur form of
*           the matrix in A.
*           Modified.
*
*  T2     - REAL array, dimension (LDA,max(NN))
*           The Schur matrix computed by cshseqr when Z is not computed.
...
*           The orthogonal matrix computed by csgehrd.
*           Modified.
*
*  Z      - REAL array, dimension (LDU,max(NN))
*           The orthogonal matrix computed by cshseqr.
*           Modified.
*
...
*           but those computed when cshseqr only computes the
...
*           Details of the orthogonal matrix computed by csgehrd.
*           Modified.
*
*  TAU    - REAL array, dimension(max(NN))
*           Further details of the orthogonal matrix computed by csgehrd.
*           Modified.
*
...
*           If 1, then cshseqr could not find all the shifts.
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, csgehrd, SGEMM, SGET10, SGET22, cshsein,
     $               cshseqr, SHST01, cslabad, cslacpy, SLAFTS, cslaset,
     $                 SLASUM, SLATME, SLATMR, SLATMS, csorghr, csormhr,
     $                   cstrevc, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
   70       CONTINUE
*
            CALL cslaset( 'A', 3, 3, 4.0, 12.0, G, 3 )
            IINFO = 0
            COND = ULPINV
*
*           Special Matrices
...
*
  100       CONTINUE
*
*           Call csgehrd to compute H and U, do tests.
*
...
*
            ...
            IHI = N
*
            CALL csgehrd( N, ILO, IHI, H, LDA, WORK, WORK( N+1 ),
     $                   NWORK-N, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'csgehrd', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
*
  ...
  120       CONTINUE
            ...
            CALL csorghr( N, ILO, IHI, U, LDU, WORK, WORK( N+1 ),
     $                   NWORK-N, IINFO )
            NTEST = 2
*
     ...
     $                   NWORK, RESULT( 1 ) )
*
*           Call cshseqr to compute T1, T2 and Z, do tests.
*
*           Eigenvalues only (WR3,WI3)
*
            CALL cslacpy( ' ', N, N, H, LDA, T2, LDA )
            NTEST = 3
            RESULT( 3 ) = ULPINV
*
            CALL cshseqr( 'E', 'N', N, ILO, IHI, T2, LDA, WR3, WI3, UZ,
     $                   LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cshseqr(E)', IINFO, N, JTYPE,
     $            IOLDSD
...
*
            CALL cslacpy( ' ', N, N, H, LDA, T2, LDA )
*
            CALL cshseqr( 'S', 'N', N, ILO, IHI, T2, LDA, WR1, WI1, UZ,
     $                   LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 .AND. IINFO.LE.N+2 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cshseqr(S)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
...
*           (UZ)
*
            CALL cslacpy( ' ', N, N, H, LDA, T1, LDA )
            CALL cslacpy( ' ', N, N, U, LDU, UZ, LDA )
*
            CALL cshseqr( 'S', 'V', N, ILO, IHI, T1, LDA, WR1, WI1, UZ,
     $                   LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 .AND. IINFO.LE.N+2 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cshseqr(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
...
*
  ...
  140       CONTINUE
     ...
     $         GO TO 140
*
           CALL cstrevc( 'Right', 'All', SELECT, N, T1, LDA, DUMMA, LDU,
     $                   EVECTR, LDU, N, IN, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cstrevc(R,A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
...
*
     ...
     $                   WI1, WORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cstrevc',
...
*
            CALL cstrevc( 'Right', 'Some', SELECT, N, T1, LDA, DUMMA,
     $                   LDU, EVECTL, LDU, N, IN, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cstrevc(R,S)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
*
  ...
  180       CONTINUE
            IF( .NOT.MATCH )
     $         WRITE( NOUNIT, FMT = 9997 )'Right', 'cstrevc', N, JTYPE,
     $         IOLDSD
*
...
*
            NTEST = 10
            RESULT( 10 ) = ULPINV
           CALL cstrevc( 'Left', 'All', SELECT, N, T1, LDA, EVECTL, LDU,
     $                   DUMMA, LDU, N, IN, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cstrevc(L,A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
...
*
     ...
     $                   WR1, WI1, WORK, DUMMA( 3 ) )
            ...
            IF( DUMMA( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cstrevc', DUMMA( 4 ),
     $            N, JTYPE, IOLDSD
            END IF
*
...
*
            CALL cstrevc( 'Left', 'Some', SELECT, N, T1, LDA, EVECTR,
     $                   LDU, DUMMA, LDU, N, IN, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cstrevc(L,S)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 250
            END IF
*
  ...
  220       CONTINUE
            IF( .NOT.MATCH )
     $         WRITE( NOUNIT, FMT = 9997 )'Left', 'cstrevc', N, JTYPE,
     $         IOLDSD
*
*           Call cshsein for Right eigenvectors of H, do test 11
*
            NTEST = 11
            RESULT( 11 ) = ULPINV
            DO 230 J = 1, N
  ...
  230       CONTINUE
*
            CALL cshsein( 'Right', 'Qr', 'Ninitv', SELECT, N, H, LDA,
     $                   WR3, WI3, DUMMA, LDU, EVECTX, LDU, N1, IN,
     $                   WORK, IWORK, IWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cshsein(R)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 250
...
*
     ...
     $            RESULT( 11 ) = DUMMA( 1 )*ANINV
               IF( DUMMA( 2 ).GT.THRESH ) THEN
                  WRITE( NOUNIT, FMT = 9998 )'Right', 'cshsein',
     $               DUMMA( 2 ), N, JTYPE, IOLDSD
               END IF
            END IF
*
*           Call cshsein for Left eigenvectors of H, do test 12
*
            NTEST = 12
            RESULT( 12 ) = ULPINV
            DO 240 J = 1, N
  ...
  240       CONTINUE
*
           CALL cshsein( 'Left', 'Qr', 'Ninitv', SELECT, N, H, LDA, WR3,
     $                   WI3, EVECTY, LDU, DUMMA, LDU, N1, IN, WORK,
     $                   IWORK, IWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cshsein(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 250
...
*
     ...
     $            RESULT( 12 ) = DUMMA( 3 )*ANINV
               IF( DUMMA( 4 ).GT.THRESH ) THEN
                  WRITE( NOUNIT, FMT = 9998 )'Left', 'cshsein',
     $               DUMMA( 4 ), N, JTYPE, IOLDSD
               END IF
            END IF
*
*           Call csormhr for Right eigenvectors of A, do test 13
*
            NTEST = 13
            RESULT( 13 ) = ULPINV
*
            CALL csormhr( 'Left', 'No transpose', N, N, ILO, IHI, UU,
     $                   LDU, TAU, EVECTX, LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csormhr(R)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 250
...
*
     ...
     $            RESULT( 13 ) = DUMMA( 1 )*ANINV
            END IF
*
*           Call csormhr for Left eigenvectors of A, do test 14
*
            NTEST = 14
            RESULT( 14 ) = ULPINV
*
     ...
     $                   LDU, TAU, EVECTY, LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csormhr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 250
