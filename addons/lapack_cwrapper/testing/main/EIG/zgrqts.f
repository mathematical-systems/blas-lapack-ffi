...
*  =======
*
*  ZGRQTS tests czggrqf, which computes the GRQ factorization of an
...
*          by czggrqf, see ccggrqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL         ZGEMM, czggrqf, ZHERK, czlacpy, czlaset, czungqr,
     $                   czungrq
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      CALL czlacpy( 'Full', P, N, B, LDB, BF, LDB )
*
...
*
      CALL czggrqf( M, P, N, AF, LDA, TAUA, BF, LDB, TAUB, WORK, LWORK,
     $             INFO )
*
...
*
     ...
     $                   Q( 2, 1 ), LDA )
      END IF
      CALL czungrq( N, N, MIN( M, N ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL czlaset( 'Full', P, P, CROGUE, CROGUE, Z, LDB )
      IF( P.GT.1 )
     $  CALL czlacpy( 'Lower', P-1, N, BF( 2, 1 ), LDB, Z( 2, 1 ), LDB )
      CALL czungqr( P, P, MIN( P, N ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL czlaset( 'Full', M, N, CZERO, CZERO, R, LDA )
      IF( M.LE.N ) THEN
        CALL czlacpy( 'Upper', M, M, AF( 1, N-M+1 ), LDA, R( 1, N-M+1 ),
     $                LDA )
      ELSE
         CALL czlacpy( 'Full', M-N, N, AF, LDA, R, LDA )
        CALL czlacpy( 'Upper', N, N, AF( M-N+1, 1 ), LDA, R( M-N+1, 1 ),
     $                LDA )
      END IF
*
*     Copy T
*
      CALL czlaset( 'Full', P, N, CZERO, CZERO, T, LDB )
      CALL czlacpy( 'Upper', P, N, BF, LDB, T, LDB )
*
*     Compute R - A*Q'
*
