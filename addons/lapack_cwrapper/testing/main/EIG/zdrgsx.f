...
*
*  czgges factors A and B as Q*S*Z'  and Q*T*Z' , where ' means conjugate
...
*  matrix pairs are used to test the routine czggesX.
*
...
*        EPS*norm(A, B).
*
*  (9)   If INFO = N+3 is returned by czggesX, the reordering "failed"
...
*
*  SVD (routine czgesvD) is used for computing the true value of DIF_u
...
*          read-in test matrices are used to test cdggesX.
*
*  NCMAX   (input) INTEGER
...
*          Copy of A, modified by czggesX.
*
*  BI      (workspace) COMPLEX*16 array, dimension (LDA, NSIZE)
*          Copy of B, modified by czggesX.
*
*  Z       (workspace) COMPLEX*16 array, dimension (LDA, NSIZE)
*          Z holds the left Schur vectors computed by czggesX.
*
*  Q       (workspace) COMPLEX*16 array, dimension (LDA, NSIZE)
*          Q holds the right Schur vectors computed by czggesX.
*
...
*     .. External Subroutines ..
      EXTERNAL       ALASVM, cdlabad, xerbla, czgesvD, ZGET51, czggesX,
     $                   czlacpy, ZLAKF2, czlaset, ZLATM5
*     ..
*     .. Scalars in Common ..
      LOGICAL            FS
...
*
         MAXWRK = NSIZE*( 1+ILAENV( 1, 'czgeqrf', ' ', NSIZE, 1, NSIZE,
     $            0 ) )
         MAXWRK = MAX( MAXWRK, NSIZE*( 1+ILAENV( 1, 'czungqr', ' ',
     $            NSIZE, 1, NSIZE, -1 ) ) )
*
...
*
     ...
     $            ( 1+ILAENV( 1, 'czgebrd', ' ', NSIZE*NSIZE / 2,
     $            NSIZE*NSIZE / 2, -1, -1 ) ) )
...
*     Loop over different functions (IFUNC) of czggesX, types (PRTYPE)
...
*
                  ...
                  K = 0
*
                 CALL czlaset( 'Full', MPLUSN, MPLUSN, CZERO, CZERO, AI,
     $                         LDA )
                 CALL czlaset( 'Full', MPLUSN, MPLUSN, CZERO, CZERO, BI,
     $                         LDA )
*
...
*
                  ...
                  END IF
*
                 CALL czlacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, A, LDA )
                 CALL czlacpy( 'Full', MPLUSN, MPLUSN, BI, LDA, B, LDA )
*
                 CALL czggesX( 'V', 'V', 'S', ZLCTSX, SENSE, MPLUSN, AI,
...
*
                  IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
                     RESULT( 1 ) = ULPINV
                     WRITE( NOUT, FMT = 9999 )'czggesX', LINFO, MPLUSN,
     $                  PRTYPE
                     INFO = LINFO
                     GO TO 30
                  END IF
...
*
                  CALL czlacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK,
     $                         MPLUSN )
                  CALL czlacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
...
*
     ...
     $                            BI( MM+1, MM+1 ), C, LDC )
*
                     CALL czgesvD( 'N', 'N', MN2, MN2, C, LDC, S, WORK,
...
*     m-by-m (1,1)-blocks with n-by-n (2,2)-blocks.
*
      CALL czggesX('V', 'V', 'S', ZLCTSX, 'B', MPLUSN, AI, LDA, BI, LDA,
     $             MM, ALPHA, BETA, Q, LDA, Z, LDA, PL, DIFEST, WORK,
     $             LWORK, RWORK, IWORK, LIWORK, BWORK, LINFO )
*
      IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
         RESULT( 1 ) = ULPINV
         WRITE( NOUT, FMT = 9998 )'czggesX', LINFO, MPLUSN, NPTKNT
         GO TO 130
      END IF
*
*     Compute the norm(A, B)
...
*
      CALL czlacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK, MPLUSN )
      CALL czlacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
     $             WORK( MPLUSN*MPLUSN+1 ), MPLUSN )
