...
*
*  cdggesX factors A and B as Q S Z' and Q T Z', where ' means
...
*  matrix pairs are used to test the routine cdggesX.
*
...
*        EPS*norm(A, B).
*
*  (9)   If INFO = N+3 is returned by cdggesX, the reordering "failed"
...
*
*  SVD (routine cdgesvD) is used for computing the true value of DIF_u
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, cdgesvD, DGET51, DGET53, cdggesX, cdlabad,
     $                   cdlacpy, DLAKF2, cdlaset, DLATM5, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*     Loop over different functions (IFUNC) of cdggesX, types (PRTYPE)
...
*
                  ...
                  K = 0
*
                  CALL cdlaset( 'Full', MPLUSN, MPLUSN, ZERO, ZERO, AI,
     $                         LDA )
                  CALL cdlaset( 'Full', MPLUSN, MPLUSN, ZERO, ZERO, BI,
     $                         LDA )
*
...
*
                  ...
                  END IF
*
                 CALL cdlacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, A, LDA )
                 CALL cdlacpy( 'Full', MPLUSN, MPLUSN, BI, LDA, B, LDA )
*
                 CALL cdggesX( 'V', 'V', 'S', DLCTSX, SENSE, MPLUSN, AI,
...
*
                  IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
                     RESULT( 1 ) = ULPINV
                     WRITE( NOUT, FMT = 9999 )'cdggesX', LINFO, MPLUSN,
     $                  PRTYPE
                     INFO = LINFO
                     GO TO 30
                  END IF
...
*
                  CALL cdlacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK,
     $                         MPLUSN )
                  CALL cdlacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
...
*
     ...
     $                            BI( MM+1, MM+1 ), C, LDC )
*
                     CALL cdgesvD( 'N', 'N', MN2, MN2, C, LDC, S, WORK,
...
*     m-by-m (1,1)-blocks with n-by-n (2,2)-blocks.
*
      CALL cdggesX('V', 'V', 'S', DLCTSX, 'B', MPLUSN, AI, LDA, BI, LDA,
     $             MM, ALPHAR, ALPHAI, BETA, Q, LDA, Z, LDA, PL, DIFEST,
     $             WORK, LWORK, IWORK, LIWORK, BWORK, LINFO )
*
      IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
         RESULT( 1 ) = ULPINV
         WRITE( NOUT, FMT = 9998 )'cdggesX', LINFO, MPLUSN, NPTKNT
         GO TO 130
      END IF
*
*     Compute the norm(A, B)
...
*
      CALL cdlacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK, MPLUSN )
      CALL cdlacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
     $             WORK( MPLUSN*MPLUSN+1 ), MPLUSN )
