...
*  =======
*
*  ZCHKGK tests czggbak, a routine for backward balancing  of
*  a matrix pair (A, B).
*
*  Arguments
*  =========
...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, czggbak, czggbal, czlacpy
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL czlacpy( 'FULL', N, N, A, LDA, AF, LDA )
      CALL czlacpy( 'FULL', N, N, B, LDB, BF, LDB )
*
      CALL czggbal( 'B', N, A, LDA, B, LDB, ILO, IHI, LSCALE, RSCALE,
     $             RWORK, INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 1 ) = KNT
      END IF
*
      CALL czlacpy( 'FULL', N, M, VL, LDVL, VLF, LDVL )
      CALL czlacpy( 'FULL', N, M, VR, LDVR, VRF, LDVR )
*
      CALL czggbak( 'B', 'L', N, ILO, IHI, LSCALE, RSCALE, M, VL, LDVL,
     $             INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 2 ) = KNT
      END IF
*
      CALL czggbak( 'B', 'R', N, ILO, IHI, LSCALE, RSCALE, M, VR, LDVR,
     $             INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 3 ) = KNT
...
*
      WRITE( NOUT, FMT = 9999 )
 9999 FORMAT( 1X, '.. test output of czggbak .. ' )
*
      WRITE( NOUT, FMT = 9998 )RMAX
 9998 FORMAT( ' value of largest test error                  =', D12.3 )
      WRITE( NOUT, FMT = 9997 )LMAX( 1 )
 9997 FORMAT( ' example number where czggbal info is not 0    =', I4 )
      WRITE( NOUT, FMT = 9996 )LMAX( 2 )
 9996 FORMAT( ' example number where czggbak(L) info is not 0 =', I4 )
      WRITE( NOUT, FMT = 9995 )LMAX( 3 )
 9995 FORMAT( ' example number where czggbak(R) info is not 0 =', I4 )
