...
*
*  cdgges factors A and B as Q S Z'  and Q T Z' , where ' means
...
*          The (left) orthogonal matrix computed by cdgges.
*
*  LDQ     (input) INTEGER
...
*          The generalized eigenvalues of (A,B) computed by cdgges.
...
*     .. External Subroutines ..
      EXTERNAL         ALASVM, DGET51, DGET53, DGET54, cdgges, cdlabad,
     $               cdlacpy, cdlarfG, cdlaset, DLATM4, cdorm2r, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
            DO 150 ISORT = 0, 1
               ...
               END IF
*
*              Call cdgges to compute H, T, Q, Z, alpha, and beta.
*
               CALL cdlacpy( 'Full', N, N, A, LDA, S, LDA )
               CALL cdlacpy( 'Full', N, N, B, LDA, T, LDA )
               NTEST = 1 + RSUB + ISORT
               RESULT( 1+RSUB+ISORT ) = ULPINV
               CALL cdgges( 'V', 'V', SORT, DLCTES, N, S, LDA, T, LDA,
     $                     SDIM, ALPHAR, ALPHAI, BETA, Q, LDQ, Z, LDQ,
     $                     WORK, LWORK, BWORK, IINFO )
               IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                  RESULT( 1+RSUB+ISORT ) = ULPINV
                  WRITE( NOUNIT, FMT = 9999 )'cdgges', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 160
               END IF
