...
*
*  csgebrd reduces a real general m by n matrix A to upper or lower
*  bidiagonal form B by an orthogonal transformation:  Q' * A * P = B
*  (or A = Q * B * P').  The matrix B is upper bidiagonal if m >= n
*  and lower bidiagonal if m < n.
*
*  csorgbr generates the orthogonal matrices Q and P' from csgebrd.
*  Note that Q and P are not necessarily square.
*
*  csbdsqr computes the singular value decomposition of the bidiagonal
...
*  In addition, csbdsqr has an option to apply the left orthogonal matrix
*  U to a matrix X, useful in least squares applications.
*
*  csbdsdc computes the singular value decomposition of the bidiagonal
...
*  (3)   | I - PT PT' | / ( N ulp )
*
*  Test csbdsqr on bidiagonal matrix B
*
...
*
*  Test csbdsdc on bidiagonal matrix B
*
...
*       (a) csgebrd is not called to reduce it to bidiagonal form.
...
*          and Z, used in testing csbdsqr.  If NRHS = 0, then the
...
*          -27: LWORK too small.
*          If  SLATMR, SLATMS, csgebrd, csorgbr, or csbdsqr,
...
*     .. External Subroutines ..
      EXTERNAL         ALASUM, csbdsdc, csbdsqr, SBDT01, SBDT02, SBDT03,
     $                  SCOPY, csgebrd, SGEMM, cslabad, cslacpy, SLAHD2,
     $                 cslaset, SLATMR, SLATMS, csorgbr, SORT01, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  100       CONTINUE
*
*           Call csgebrd and csorgbr to compute B, Q, and P, do tests.
*
            IF( .NOT.BIDIAG ) THEN
*
...
*              B := Q' * A * P.
*
               CALL cslacpy( ' ', M, N, A, LDA, Q, LDQ )
              CALL csgebrd( M, N, Q, LDQ, BD, BE, WORK, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from csgebrd.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'csgebrd', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
*
               CALL cslacpy( ' ', M, N, Q, LDQ, PT, LDPT )
               IF( M.GE.N ) THEN
                  UPLO = 'U'
               ELSE
                  UPLO = 'L'
...
*
               ...
               IF( NRHS.LE.0 )
     $            MQ = MNMIN
               CALL csorgbr( 'Q', M, MQ, N, Q, LDQ, WORK,
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from csorgbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'csorgbr(Q)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*              Generate P'
*
              CALL csorgbr( 'P', MNMIN, N, M, PT, LDPT, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from csorgbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'csorgbr(P)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
     ...
     $                      RESULT( 3 ) )
            END IF
*
*           Use csbdsqr to form the SVD of the bidiagonal matrix B:
*           B := U * S1 * VT, and compute Z = U' * Y.
*
            CALL SCOPY( MNMIN, BD, 1, S1, 1 )
            IF( MNMIN.GT.0 )
     $         CALL SCOPY( MNMIN-1, BE, 1, WORK, 1 )
            CALL cslacpy( ' ', M, NRHS, Y, LDX, Z, LDX )
            CALL cslaset( 'Full', MNMIN, MNMIN, ZERO, ONE, U, LDPT )
            CALL cslaset( 'Full', MNMIN, MNMIN, ZERO, ONE, VT, LDPT )
*
            CALL csbdsqr( UPLO, MNMIN, MNMIN, MNMIN, NRHS, S1, WORK, VT,
     $                   LDPT, U, LDPT, Z, LDX, WORK( MNMIN+1 ), IINFO )
*
*           Check error code from csbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'csbdsqr(vects)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
*           Use csbdsqr to compute only the singular values of the
...
*
     ...
     $         CALL SCOPY( MNMIN-1, BE, 1, WORK, 1 )
*
            CALL csbdsqr( UPLO, MNMIN, 0, 0, 0, S2, WORK, VT, LDPT, U,
     $                   LDPT, Z, LDX, WORK( MNMIN+1 ), IINFO )
*
*           Check error code from csbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'csbdsqr(values)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  110       CONTINUE
     ...
     $            RESULT( 8 ) = ULPINV
            END IF
*
*           Test 9:  Compare csbdsqr with and without singular vectors
*
            TEMP2 = ZERO
*
            DO 120 J = 1, MNMIN
...
*
  140       CONTINUE
            RESULT( 10 ) = TEMP1
*
*           Use csbdsqr to form the decomposition A := (QU) S (VT PT)
...
*
     ...
     $            CALL SCOPY( MNMIN-1, BE, 1, WORK, 1 )
*
              CALL csbdsqr( UPLO, MNMIN, N, M, NRHS, S2, WORK, PT, LDPT,
...
*
     ...
     $                      RESULT( 14 ) )
            END IF
*
*           Use csbdsdc to form the SVD of the bidiagonal matrix B:
*           B := U * S1 * VT
*
            CALL SCOPY( MNMIN, BD, 1, S1, 1 )
            IF( MNMIN.GT.0 )
     $         CALL SCOPY( MNMIN-1, BE, 1, WORK, 1 )
            ...
            CALL cslaset( 'Full', MNMIN, MNMIN, ZERO, ONE, VT, LDPT )
*
            CALL csbdsdc( UPLO, 'I', MNMIN, S1, WORK, U, LDPT, VT, LDPT,
     $                   DUM, IDUM, WORK( MNMIN+1 ), IWORK, IINFO )
*
*           Check error code from csbdsdc.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'csbdsdc(vects)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
*           Use csbdsdc to compute only the singular values of the
...
*
     ...
     $         CALL SCOPY( MNMIN-1, BE, 1, WORK, 1 )
*
            CALL csbdsdc( UPLO, 'N', MNMIN, S2, WORK, DUM, 1, DUM, 1,
     $                   DUM, IDUM, WORK( MNMIN+1 ), IWORK, IINFO )
*
*           Check error code from csbdsdc.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'csbdsdc(values)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  150       CONTINUE
     ...
     $            RESULT( 18 ) = ULPINV
            END IF
*
*           Test 19:  Compare csbdsqr with and without singular vectors
*
            TEMP2 = ZERO
*
            DO 160 J = 1, MNMIN
