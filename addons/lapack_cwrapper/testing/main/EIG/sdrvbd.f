...
*  csgesvD and csgesdd.
*  Both csgesvD and csgesdd factor A = U diag(S) VT, where U and VT are
...
*          -21: LWORK too small.
*          If  SLATMS, or csgesvD returns an error code, the
...
*     .. External Subroutines ..
      EXTERNAL       ALASVM, SBDT01, csgesdd, csgesvD, cslabad, cslacpy,
     $                   cslaset, SLATMS, SORT01, SORT03, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*              Identity matrix
*
               CALL cslaset( 'Full', M, N, ZERO, ONE, A, LDA )
*
            ELSE
*
...
*
   30       CONTINUE
            CALL cslacpy( 'F', M, N, A, LDA, ASAV, LDA )
*
...
*
               IF( IWS.GT.1 )
     $            CALL cslacpy( 'F', M, N, ASAV, LDA, A, LDA )
               SRNAMT = 'csgesvD'
               CALL csgesvD( 'A', 'A', M, N, A, LDA, SSAV, USAV, LDU,
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) )GO TO 70
                     ...
                     CALL cslacpy( 'F', M, N, ASAV, LDA, A, LDA )
                     SRNAMT = 'csgesvD'
                     CALL csgesvD( JOBU, JOBVT, M, N, A, LDA, S, U, LDU,
...
*
               CALL cslacpy( 'F', M, N, ASAV, LDA, A, LDA )
               SRNAMT = 'csgesdd'
               CALL csgesdd( 'A', M, N, A, LDA, SSAV, USAV, LDU, VTSAV,
...
*
               ...
               DO 110 IJQ = 0, 2
                  ...
                  CALL cslacpy( 'F', M, N, ASAV, LDA, A, LDA )
                  SRNAMT = 'csgesdd'
                  CALL csgesdd( JOBQ, M, N, A, LDA, S, U, LDU, VT, LDVT,
