...
*  =======
*
*  DGET34 tests cdlaexc, a routine for swapping adjacent blocks (either
*  1 by 1 or 2 by 2) on the diagonal of a matrix in real Schur form.
*  Thus, cdlaexc computes an orthogonal matrix Q such that
*
*      Q' * [ A B ] * Q  = [ C1 B1 ]
*           [ 0 C ]        [ 0  A1 ]
*
...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, DHST01, cdlabad, cdlaexc
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                   ABS( T( 1, 2 ) ) )
                  ...
                  CALL cdlaexc( .TRUE., 2, T, 4, Q, 4, 1, 1, 1, WORK,
     $                         INFO )
                  IF( INFO.NE.0 )
...
*
     ...
     $                            ABS( T( 3, 2 ) ), ABS( T( 3, 3 ) ) )
                           ...
                           CALL DCOPY( 16, VAL( 1 ), 0, Q, 1 )
                           CALL DCOPY( 4, VAL( 3 ), 0, Q, 5 )
                           CALL cdlaexc( .TRUE., 3, T, 4, Q, 4, 1, 1, 2,
     $                                  WORK, INFO )
                           IF( INFO.NE.0 )
     $                        NINFO( INFO ) = NINFO( INFO ) + 1
...
*
     ...
     $                            ABS( T( 3, 2 ) ), ABS( T( 3, 3 ) ) )
                           ...
                           CALL DCOPY( 16, VAL( 1 ), 0, Q, 1 )
                           CALL DCOPY( 4, VAL( 3 ), 0, Q, 5 )
                           CALL cdlaexc( .TRUE., 3, T, 4, Q, 4, 1, 2, 1,
     $                                  WORK, INFO )
                           IF( INFO.NE.0 )
     $                        NINFO( INFO ) = NINFO( INFO ) + 1
                           CALL DHST01( 3, 1, 3, T1, 4, T, 4, Q, 4,
