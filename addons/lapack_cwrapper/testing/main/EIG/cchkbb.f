...
*
*  ccgbbrd factors a general band matrix A as  Q B P* , where * means
*  conjugate transpose, B is upper bidiagonal, and Q and P are unitary;
*  ccgbbrd can also overwrite a given matrix C with Q* C .
*
...
*          Used to hold the unitary matrix Q computed by ccgbbrd.
*
*  LDQ     (input) INTEGER
...
*          Used to hold the unitary matrix P computed by ccgbbrd.
*
*  LDP     (input) INTEGER
...
*          Used to hold the matrix C updated by ccgbbrd.
*
*  LDC     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL        CBDT01, CBDT02, ccgbbrd, cclacpy, cclaset, CLATMR,
...
*
               ...
               CALL cclaset( 'Full', LDAB, N, CZERO, CZERO, AB, LDAB )
               IINFO = 0
               COND = ULPINV
*
...
*              Copy C
*
               CALL cclacpy( 'Full', M, NRHS, C, LDC, CC, LDC )
*
*              Call ccgbbrd to compute B, Q and P, and to update C.
*
               CALL ccgbbrd( 'B', M, N, NRHS, KL, KU, AB, LDAB, BD, BE,
     $                      Q, LDQ, P, LDP, CC, LDC, WORK, RWORK,
     $                      IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'ccgbbrd', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
