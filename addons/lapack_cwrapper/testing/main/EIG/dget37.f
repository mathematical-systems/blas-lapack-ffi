...
*  =======
*
*  DGET37 tests cdtrsna, a routine for estimating condition numbers of
...
*          RMAX(1) = largest ratio comparing different calls to cdtrsna
...
*          If cdtrsna returns INFO nonzero on example i, LMAX(3)=i
*
...
*          NINFO(3) = No. of times cdtrsna returned INFO nonzero
*
*  KNT     (output) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL         DCOPY, cdgehrd, cdhseqr, cdlabad, cdlacpy, DSCAL,
     $                   cdtrevc, cdtrsna
*     ..
*     .. Intrinsic Functions ..
...
*        Compute Schur form
*
         CALL cdhseqr( 'S', 'N', N, 1, N, T, LDT, WR, WI, DUM, 1, WORK,
     $                LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 2 ) = KNT
            NINFO( 2 ) = NINFO( 2 ) + 1
...
*
         CALL cdtrevc( 'Both', 'All', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, N, M, WORK, INFO )
*
*        Compute condition numbers
*
         CALL cdtrsna( 'Both', 'All', SELECT, N, T, LDT, LE, LDT, RE,
...
*
         ...
         CALL cdtrsna( 'Eigcond', 'All', SELECT, N, T, LDT, LE, LDT, RE,
...
*
         CALL DCOPY( N, DUM, 0, STMP, 1 )
         CALL DCOPY( N, DUM, 0, SEPTMP, 1 )
         CALL cdtrsna( 'Veccond', 'All', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, STMP, SEPTMP, N, M, WORK, N, IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
  ...
  150    CONTINUE
         CALL DCOPY( N, DUM, 0, STMP, 1 )
         CALL DCOPY( N, DUM, 0, SEPTMP, 1 )
         CALL cdtrsna( 'Bothcond', 'Some', SELECT, N, T, LDT, LE, LDT,
...
*
         CALL DCOPY( N, DUM, 0, STMP, 1 )
         CALL DCOPY( N, DUM, 0, SEPTMP, 1 )
        CALL cdtrsna( 'Eigcond', 'Some', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, STMP, SEPTMP, N, M, WORK, N, IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
         CALL DCOPY( N, DUM, 0, STMP, 1 )
         CALL DCOPY( N, DUM, 0, SEPTMP, 1 )
        CALL cdtrsna( 'Veccond', 'Some', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, STMP, SEPTMP, N, M, WORK, N, IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
