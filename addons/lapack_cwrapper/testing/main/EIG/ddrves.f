...
*          Another copy of the test matrix A, modified by cdgees.
*
*  HT      (workspace) DOUBLE PRECISION array, dimension (LDA, max(NN))
*          Yet another copy of the test matrix A, modified by cdgees.
*
...
*          but those computed when cdgees only computes a partial
...
*          -20: NWORK too small.
*          If  DLATMR, SLATMS, SLATME or cdgees returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL        cdgees, DHST01, cdlabad, cdlacpy, cdlaset, DLASUM,
...
*
                  ...
                  CALL cdgees( 'V', SORT, DSLECT, N, H, LDA, SDIM, WR,
     $                        WI, VS, LDVS, WORK, NNWORK, BWORK, IINFO )
                  IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                     RESULT( 1+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'cdgees1', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 220
                  END IF
...
*
                  ...
                  CALL cdgees( 'N', SORT, DSLECT, N, HT, LDA, SDIM, WRT,
     $                        WIT, VS, LDVS, WORK, NNWORK, BWORK,
     $                        IINFO )
                  IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                     RESULT( 5+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'cdgees2', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 220
                  END IF
