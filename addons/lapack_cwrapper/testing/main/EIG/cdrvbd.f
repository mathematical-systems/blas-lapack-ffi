...
*  =======
*
*  CDRVBD checks the singular value decomposition (SVD) driver ccgesvD
*  and ccgesdd.
*  ccgesvD and ccgesdd factors A = U diag(S) VT, where U and VT are
...
*          -21: LWORK too small.
*          If  CLATMS, or ccgesvD returns an error code, the
...
*     .. External Subroutines ..
      EXTERNAL       ALASVM, CBDT01, ccgesdd, ccgesvD, cclacpy, cclaset,
...
*              Identity matrix
*
               CALL cclaset( 'Full', M, N, CZERO, CONE, A, LDA )
               DO 40 I = 1, MIN( M, N )
                  S( I ) = ONE
   40          CONTINUE
*
...
*
   50       CONTINUE
            CALL cclacpy( 'F', M, N, A, LDA, ASAV, LDA )
*
...
*
               IF( IWSPC.GT.1 )
     $            CALL cclacpy( 'F', M, N, ASAV, LDA, A, LDA )
               CALL ccgesvD( 'A', 'A', M, N, A, LDA, SSAV, USAV, LDU,
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) )GO TO 90
                     ...
                     JOBVT = CJOB( IJVT+1 )
                     CALL cclacpy( 'F', M, N, ASAV, LDA, A, LDA )
                     CALL ccgesvD( JOBU, JOBVT, M, N, A, LDA, S, U, LDU,
...
*
               CALL cclacpy( 'F', M, N, ASAV, LDA, A, LDA )
               CALL ccgesdd( 'A', M, N, A, LDA, SSAV, USAV, LDU, VTSAV,
...
*
               ...
               DO 130 IJQ = 0, 2
                  JOBQ = CJOB( IJQ+1 )
                  CALL cclacpy( 'F', M, N, ASAV, LDA, A, LDA )
                  CALL ccgesdd( JOBQ, M, N, A, LDA, S, U, LDU, VT, LDVT,
