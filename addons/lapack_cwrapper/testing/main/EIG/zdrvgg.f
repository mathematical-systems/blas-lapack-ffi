...
*  routines.
*                                T          T        T
*  czgegs factors A and B as Q S Z  and Q T Z , where   means
...
*      det( m(j) A - B ) = 0
*
*  czgegv computes the generalized eigenvalues (alpha(1),beta(1)), ...,
...
*          The upper triangular matrix computed from A by czgegs.
*
*  T       (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by czgegs.
*
*  S2      (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          The matrix computed from A by czgegv.  This will be the
...
*          The matrix computed from B by czgegv.  This will be the
...
*          The (left) unitary matrix computed by czgegs.
*
*  LDQ     (input) INTEGER
...
*          The generalized eigenvalues of (A,B) computed by czgegv.
...
*          sum of the blocksize and number-of-shifts for czhgeqz, and
*          NB is the greatest of the blocksizes for czgeqrf, czunmqr,
*          and czungqr.  (The blocksizes and the number-of-shifts are
...
*     .. External Subroutines ..
      EXTERNAL         ALASVM, cdlabad, xerbla, czgegs, czgegv, ZGET51,
     $                ZGET52, czlacpy, czlarfG, czlaset, ZLATM4, czunm2r
*     ..
*     .. Intrinsic Functions ..
...
*
      NB = MAX( 1, ILAENV( 1, 'czgeqrf', ' ', NMAX, NMAX, -1, -1 ),
     $     ILAENV( 1, 'czunmqr', 'LC', NMAX, NMAX, NMAX, -1 ),
     $     ILAENV( 1, 'czungqr', ' ', NMAX, NMAX, NMAX, -1 ) )
      NBZ = ILAENV( 1, 'czhgeqz', 'SII', NMAX, 1, NMAX, 0 )
      NS = ILAENV( 4, 'czhgeqz', 'SII', NMAX, 1, NMAX, 0 )
      I1 = NBZ + NS
...
*
   ...
   40                CONTINUE
                    CALL czlarfG( N+1-JC, Q( JC, JC ), Q( JC+1, JC ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, DBLE( Q( JC, JC ) ) )
                     Q( JC, JC ) = CONE
                    CALL czlarfG( N+1-JC, Z( JC, JC ), Z( JC+1, JC ), 1,
     $                            WORK( N+JC ) )
...
*
   ...
   60                CONTINUE
   70             CONTINUE
                  CALL czunm2r( 'L', 'N', N, N, N-1, Q, LDQ, WORK, A,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL czunm2r( 'R', 'C', N, N, N-1, Z, LDQ, WORK( N+1 ),
     $                         A, LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL czunm2r( 'L', 'N', N, N, N-1, Q, LDQ, WORK, B,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL czunm2r( 'R', 'C', N, N, N-1, Z, LDQ, WORK( N+1 ),
...
*
  110       CONTINUE
*
*           Call czgegs to compute H, T, Q, Z, alpha, and beta.
*
            CALL czlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T, LDA )
            NTEST = 1
            RESULT( 1 ) = ULPINV
*
            CALL czgegs( 'V', 'V', N, S, LDA, T, LDA, ALPHA1, BETA1, Q,
     $                  LDQ, Z, LDQ, WORK, LWORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czgegs', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 130
            END IF
...
*
  ...
  120       CONTINUE
            RESULT( 5 ) = TEMP1
*
*           Call czgegv to compute S2, T2, VL, and VR, do tests.
*
*           Eigenvalues and Eigenvectors
*
            CALL czlacpy( ' ', N, N, A, LDA, S2, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T2, LDA )
            NTEST = 6
            RESULT( 6 ) = ULPINV
*
            CALL czgegv( 'V', 'V', N, S2, LDA, T2, LDA, ALPHA2, BETA2,
     $                  VL, LDQ, VR, LDQ, WORK, LWORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czgegv', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 130
            END IF
...
*
     ...
     $                   BETA2, WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRSHN ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'czgegv', DUMMA( 2 ),
     $            N, JTYPE, IOLDSD
            END IF
*
     ...
     $                   BETA2, WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'czgegv', DUMMA( 2 ),
     $            N, JTYPE, IOLDSD
            END IF
*
