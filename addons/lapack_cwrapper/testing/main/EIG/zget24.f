...
*        computed by czgeesX and RCDEIN (the precomputed true value)
...
*        number computed by czgeesX and RCDVIN (the precomputed true
...
*          Another copy of the test matrix A, modified by czgeesX.
*
*  HT      (workspace) COMPLEX*16 array, dimension (LDA, N)
*          Yet another copy of the test matrix A, modified by czgeesX.
*
...
*          but those computed when czgeesX only computes a partial
...
*
*  LWORK   (input) INTEGER
*          The number of entries in WORK to be passed to czgeesX. This
...
*          If >0, czgeesX returned an error code, the absolute
...
*     .. External Subroutines ..
      EXTERNAL           xerbla, ZCOPY, czgeesX, ZGEMM, czlacpy, ZUNT01
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL czlacpy( 'F', N, N, A, LDA, H, LDA )
         CALL czgeesX( 'V', SORT, ZSLECT, 'N', N, H, LDA, SDIM, W, VS,
     $                LDVS, RCONDE, RCONDV, WORK, LWORK, RWORK, BWORK,
     $                IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 1+RSUB ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX1', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX1', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            RETURN
...
*
         CALL czlacpy( ' ', N, N, A, LDA, VS1, LDVS )
*
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'N', SORT, ZSLECT, 'N', N, HT, LDA, SDIM, WT, VS,
     $                LDVS, RCONDE, RCONDV, WORK, LWORK, RWORK, BWORK,
     $                IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 5+RSUB ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX2', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX2', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         ...
         RESULT( 15 ) = ZERO
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'V', SORT, ZSLECT, 'B', N, HT, LDA, SDIM1, WT,
     ...
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            ...
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX3', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX3', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'N', SORT, ZSLECT, 'B', N, HT, LDA, SDIM1, WT,
     ...
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            ...
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX4', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX4', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'V', SORT, ZSLECT, 'E', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 14 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX5', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX5', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'N', SORT, ZSLECT, 'E', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 14 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX6', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX6', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'V', SORT, ZSLECT, 'V', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX7', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX7', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL czgeesX( 'N', SORT, ZSLECT, 'V', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'czgeesX8', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'czgeesX8', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL czlacpy( 'F', N, N, A, LDA, HT, LDA )
        CALL czgeesX( 'N', 'S', ZSLECT, 'B', N, HT, LDA, SDIM1, WT, VS1,
     $                LDVS, RCONDE, RCONDV, WORK, LWORK, RWORK, BWORK,
     $                IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 16 ) = ULPINV
            RESULT( 17 ) = ULPINV
            WRITE( NOUNIT, FMT = 9999 )'czgeesX9', IINFO, N, ISEED( 1 )
            INFO = ABS( IINFO )
            GO TO 270
         END IF
*
