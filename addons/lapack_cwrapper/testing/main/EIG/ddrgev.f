...
*  routine cdggev.
*
*  cdggev computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*                                 dimension (LDA, max(NN))
*          The Schur form matrix computed from A by cdgges.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) DOUBLE PRECISION array,
*                                 dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by cdgges.
*
*  Q       (workspace) DOUBLE PRECISION array,
*                                 dimension (LDQ, max(NN))
*          The (left) eigenvectors matrix computed by cdggev.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by cdgges.
*
...
*          The generalized eigenvalues of (A,B) computed by cdggev.
...
*          eigenvalues of A and B, but those computed when cdggev only
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, DGET52, cdggev, cdlabad, cdlacpy, cdlarfG,
     $                   cdlaset, DLATM4, cdorm2r, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( INFO.EQ.0 .AND. LWORK.GE.1 ) THEN
        ...
        MAXWRK = 7*NMAX + NMAX*ILAENV( 1, 'cdgeqrf', ' ', NMAX, 1, NMAX,
     $            0 )
...
*
  ...
  110       CONTINUE
*
*           Call cdggev to compute eigenvalues and eigenvectors.
*
            CALL cdlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cdlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL cdggev( 'V', 'V', N, S, LDA, T, LDA, ALPHAR, ALPHAI,
     $                  BETA, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'cdggev1', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cdggev1',
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cdggev1',
...
*
            CALL cdlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cdlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL cdggev( 'N', 'N', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'cdggev2', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL cdlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cdlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL cdggev( 'V', 'N', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, QE, LDQE, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'cdggev3', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL cdlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cdlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL cdggev( 'N', 'V', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, QE, LDQE, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'cdggev4', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
