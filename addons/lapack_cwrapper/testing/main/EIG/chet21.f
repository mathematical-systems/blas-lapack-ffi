...
*     .. External Subroutines ..
      EXTERNAL           CGEMM, CHER, CHER2, cclacpy, cclarfY, cclaset,
     $                   ccunm2l, ccunm2r
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL cclaset( 'Full', N, N, CZERO, CZERO, WORK, N )
         CALL cclacpy( CUPLO, N, N, A, LDA, WORK, N )
*
         DO 10 J = 1, N
...
*
               VSAVE = V( J+1, J )
               V( J+1, J ) = ONE
               CALL cclarfY( 'L', N-J, V( J+1, J ), 1, TAU( J ),
...
*
               VSAVE = V( J, J+1 )
               V( J, J+1 ) = ONE
               CALL cclarfY( 'U', J, V( 1, J+1 ), 1, TAU( J ), WORK, N,
     $                      WORK( N**2+1 ) )
               V( J, J+1 ) = VSAVE
               WORK( ( N+1 )*J+1 ) = D( J+1 )
   60       CONTINUE
...
*
         IF( N.LT.2 )
     $      RETURN
         CALL cclacpy( ' ', N, N, U, LDU, WORK, N )
         IF( LOWER ) THEN
            CALL ccunm2r( 'R', 'C', N, N-1, N-1, V( 2, 1 ), LDV, TAU,
     $                   WORK( N+1 ), N, WORK( N**2+1 ), IINFO )
         ELSE
            CALL ccunm2l( 'R', 'C', N, N-1, N-1, V( 1, 2 ), LDV, TAU,
