...
*  =======
*
*  CZTGSYL solves the generalized Sylvester equation:
*
*              A * R - L * B = scale * C            (1)
*              D * R - L * E = scale * F
*
...
*  and (B,E), using CZLACON.
*
*  If IJOB >= 1, CZTGSYL computes a Frobenius norm-based estimate of
...
*              (CZGECON on sub-systems is used).
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZGEMM, CZLACPY, CZLASET, ZSCAL, 
     $  CZTGSY2 
*     ..
*     .. Intrinsic Functions ..
...
*
      MB = ILAENV( 2, 'CZTGSYL', TRANS, M, N, -1, -1 )
      NB = ILAENV( 5, 'CZTGSYL', TRANS, M, N, -1, -1 )
*
      ISOLVE = 1
      IFUNC = 0
      IF( NOTRAN ) THEN
         IF( IJOB.GE.3 ) THEN
            IFUNC = IJOB - 2
            CALL CZLASET( 'F', M, N, CZERO, CZERO, C, LDC )
            CALL CZLASET( 'F', M, N, CZERO, CZERO, F, LDF )
...
*
            ...
            DSUM = ONE
            PQ = M*N
            CALL CZTGSY2( TRANS, IFUNC, M, N, A, LDA, B, LDB, C, LDC, 
     $  D, 
     ...
     $                   INFO )
            ...
            IF( ISOLVE.EQ.2 .AND. IROUND.EQ.1 ) THEN
               ...
               END IF
               SCALE2 = SCALE
               CALL CZLACPY( 'F', M, N, C, LDC, WORK, M )
               CALL CZLACPY( 'F', M, N, F, LDF, WORK( M*N+1 ), M )
               CALL CZLASET( 'F', M, N, CZERO, CZERO, C, LDC )
               CALL CZLASET( 'F', M, N, CZERO, CZERO, F, LDF )
            ELSE IF( ISOLVE.EQ.2 .AND. IROUND.EQ.2 ) THEN
               CALL CZLACPY( 'F', M, N, WORK, M, C, LDC )
               CALL CZLACPY( 'F', M, N, WORK( M*N+1 ), M, F, LDF )
               SCALE = SCALE2
            END IF
   30    CONTINUE
*
...
*
            ...
            DO 130 J = P + 2, Q
               ...
               DO 120 I = P, 1, -1
                  ...
                  IE = IWORK( I+1 ) - 1
                  MB = IE - IS + 1
                  CALL CZTGSY2( TRANS, IFUNC, MB, NB, A( IS, IS ), LDA,
