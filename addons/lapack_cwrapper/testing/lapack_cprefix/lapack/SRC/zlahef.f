...
*  =======
*
*  CZLAHEF computes a partial factorization of a complex Hermitian
...
*
*  CZLAHEF is an auxiliary routine called by CZHETRF. It uses blocked code
...
*     .. External Subroutines ..
      EXTERNAL           ZCOPY, ZDSCAL, ZGEMM, ZGEMV, CZLACGV, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                     W( IMAX+1, KW-1 ), 1 )
               CALL CZLACGV( K-IMAX, W( IMAX+1, KW-1 ), 1 )
               IF( K.LT.N ) THEN
...
*
     ...
     $                     LDA )
               CALL CZLACGV( KK-1-KP, A( KP, KP+1 ), LDA )
...
*
     ...
     $                     LDA )
               CALL CZLACGV( KP-KK-1, A( KP, KK+1 ), LDA )
               IF( KP.LT.N )
...
*
               CALL CZLACGV( N-K, W( K+1, K ), 1 )
               CALL CZLACGV( N-K-1, W( K+2, K+1 ), 1 )
            END IF
         END IF
*
*        Store details of the interchanges in IPIV
