...
*  =======
*
*  CCHSEIN uses inverse iteration to find specified right and/or left
...
*          = 'Q': the eigenvalues were found using CCHSEQR; thus, if
*                 H has zero subdiagonal elements, and so is
*                 block-triangular, then the j-th eigenvalue can be
*                 assumed to be an eigenvalue of the block containing
*                 the j-th row/column.  This property allows CCHSEIN to
*                 perform inverse iteration on just one diagonal block.
*          = 'N': no assumptions are made on the correspondence
*                 between eigenvalues and diagonal blocks.  In this
*                 case, CCHSEIN must always perform inverse iteration
...
*
               CALL CCLAEIN( .FALSE., NOINIT, N-KL+1, H( KL, KL ), LDH,
     $                      WK, VL( KL, KS ), WORK, LDWORK, RWORK, 
     $  EPS3, 
     $                      SMLNUM, IINFO )
               IF( IINFO.GT.0 ) THEN
                  INFO = INFO + 1
...
*
               CALL CCLAEIN( .TRUE., NOINIT, KR, H, LDH, WK, VR( 1, KS 
     $  ), 
