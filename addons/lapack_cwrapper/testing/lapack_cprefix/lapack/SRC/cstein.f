...
*  =======
*
*  CCSTEIN computes the eigenvectors of a real symmetric tridiagonal
...
*  array, which may be passed to CCUNMTR or CCUPMTR for back
...
*          W from CSSTEBZ with ORDER = 'B' is expected here. )
*
...
*          ( The output array ISPLIT from CSSTEBZ is expected here. )
*
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, CSLAGTF, CSLAGTS, CSLARNV, SSCAL, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
*     Initialize seed for random number generator CSLARNV.
*
      DO 40 I = 1, 4
         ISEED( I ) = 1
   40 CONTINUE
...
*
            CALL CSLARNV( 2, ISEED, BLKSIZ, WORK( INDRV1+1 ) )
*
...
*
            TOL = ZERO
            CALL CSLAGTF( BLKSIZ, WORK( INDRV4+1 ), XJ, WORK( INDRV2+2 
     $  ), 
...
*
            CALL CSLAGTS( -1, BLKSIZ, WORK( INDRV4+1 ), WORK( INDRV2+2 
     $  ), 
