...
*  =======
*
*  CDGBBRD reduces a real general m-by-n band matrix A to upper
...
*     .. External Subroutines ..
      EXTERNAL           CDLARGV, CDLARTG, CDLARTV, CDLASET, DROT, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      IF( WANTQ )
     $   CALL CDLASET( 'Full', M, M, ZERO, ONE, Q, LDQ )
      IF( WANTPT )
     $   CALL CDLASET( 'Full', N, N, ZERO, ONE, PT, LDPT )
*
...
*
               IF( NR.GT.0 )
     $            CALL CDLARGV( NR, AB( KLU1, J1-KLM-1 ), INCA,
...
*
               DO 10 L = 1, KB
                  ...
                  END IF
                  IF( NRT.GT.0 )
     $               CALL CDLARTV( NRT, AB( KLU1-L, J1-KLM+L-1 ), INCA,
...
*
                     CALL CDLARTG( AB( KU+ML-1, I ), AB( KU+ML, I ),
...
*
               IF( NR.GT.0 )
     $            CALL CDLARGV( NR, AB( 1, J1+KUN-1 ), INCA,
...
*
               DO 50 L = 1, KB
                  ...
                  END IF
                  IF( NRT.GT.0 )
     $               CALL CDLARTV( NRT, AB( L+1, J1+KUN-1 ), INCA,
     $                            AB( L, J1+KUN ), INCA,
...
*
                     CALL CDLARTG( AB( KU-MU+3, I+MU-2 ),
     $                            AB( KU-MU+2, I+MU-1 ),
...
*
         DO 100 I = 1, MIN( M-1, N )
            CALL CDLARTG( AB( 1, I ), AB( 2, I ), RC, RS, RA )
            D( I ) = RA
            IF( I.LT.N ) THEN
               E( I ) = RS*AB( 1, I+1 )
...
*
            RB = AB( KU, M+1 )
            DO 110 I = M, 1, -1
               CALL CDLARTG( AB( KU+1, I ), RB, RC, RS, RA )
               D( I ) = RA
               IF( I.GT.1 ) THEN
                  RB = -RS*AB( KU, I )
                  E( I-1 ) = RC*AB( KU, I )
