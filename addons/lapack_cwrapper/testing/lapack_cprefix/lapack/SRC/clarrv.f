...
*  =======
*
*  CCLARRV computes the eigenvectors of the tridiagonal matrix
...
*          W from CSLARRE is expected here ).
...
*          > 0:  if INFO = 1, internal error in CSLARRB (this cannot be
*                             triggered since CSLARRB does not set INFO)
*                if INFO = 2, internal error in CCSTEIN
*
*  Further Details
*  ===============
*
...
*     .. Local Scalars ..
      LOGICAL            MGSCLS
      INTEGER            I, IBEGIN, IEND, IINDC1, IINDC2, IINDR, 
     $  IINDWK, 
     $                   IINFO, IM, IN, INDERR, INDGAP, INDIN1, 
     $  INDIN2, 
     $                   INDLD, INDLLD, INDWRK, ITER, ITMP1, ITMP2, J, 
     $                   JBLK, K, KTOT, LSBDPT, MAXITR, NCLUS, NDEPTH, 
     $                   NDONE, NEWCLS, NEWFRS, NEWFTT, NEWLST, 
     $  NEWSIZ, 
     $                   NSPLIT, OLDCLS, OLDFST, OLDIEN, OLDLST, 
     $  OLDNCL, 
     $                   P, Q, TEMP( 1 )
...
*     .. External Subroutines ..
      EXTERNAL           CAXPY, CCLAR1V, CCLASET, CSSCAL, CCSTEIN, 
     $  SCOPY, 
     $                   CSLARRB, CSLARRF
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   50          CONTINUE
               IF( NDEPTH.GT.0 ) THEN
                  IINFO = 0
                  CALL CSLARRB( IN, D( IBEGIN ), L( IBEGIN ),
...
*
     ...
     $                                          NEWFTT+1 ) )
   55                   CONTINUE
                        CALL CSLARRF( IN, D( IBEGIN ), L( IBEGIN ),
     $                               WORK( INDLD+1 ), WORK( INDLLD+1 ),
     $                               NEWFRS, NEWLST, WORK,
     ...
     $                               INFO )
                        ...
                        ELSE
                           ...
                           ELSE
*                           
*                             Call CCSTEIN to process this tight cluster.
*                             This happens only if MINRGP <= MGSTOL
*                             and CSLARRF returns INFO = 1. The latter
...
*
   ...
   90                   CONTINUE
                        LAMBDA = WORK( K )
                        CALL CCLAR1V( IN, 1, IN, LAMBDA, D( IBEGIN ),
