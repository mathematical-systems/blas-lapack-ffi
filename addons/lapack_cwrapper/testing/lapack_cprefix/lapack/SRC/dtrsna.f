...
*  =======
*
*  CDTRSNA estimates reciprocal condition numbers for specified
...
*     .. External Subroutines ..
      EXTERNAL           CDLACN2, CDLACPY, CDLAQTR, CDTREXC, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
            CALL CDLACPY( 'Full', N, N, T, LDT, WORK, LDWORK )
            IFST = K
            ILST = 1
            CALL CDTREXC( 'No Q', N, WORK, LDWORK, DUMMY, 1, IFST, 
     $  ILST, 
     $                   WORK( 1, N+1 ), IERR )
*
...
*
               ...
               KASE = 0
   50          CONTINUE
               CALL CDLACN2( NN, WORK( 1, N+2 ), WORK( 1, N+4 ), IWORK,
     $                      EST, KASE, ISAVE )
               IF( KASE.NE.0 ) THEN
                  IF( KASE.EQ.1 ) THEN
                     IF( N2.EQ.1 ) THEN
...
*
                        CALL CDLAQTR( .TRUE., .TRUE., N-1, WORK( 2, 2 
     $  ), 
...
*
                        CALL CDLAQTR( .TRUE., .FALSE., N-1, WORK( 2, 2 
     $  ), 
...
*                       Real eigenvalue: solve C*x = scale*c.
*
                        CALL CDLAQTR( .FALSE., .TRUE., N-1, WORK( 2, 2 
     $  ), 
     $                               LDWORK, DUMMY, DUMM, SCALE,
     $                               WORK( 1, N+4 ), WORK( 1, N+6 ),
     $                               IERR )
