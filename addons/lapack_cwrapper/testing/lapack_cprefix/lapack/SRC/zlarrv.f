...
*  =======
*
*  CZLARRV computes the eigenvectors of the tridiagonal matrix
...
*          W from CDLARRE is expected here ).
...
*          > 0:  if INFO = 1, internal error in CDLARRB (this cannot be
*                             triggered since CDLARRB does not set INFO)
*                if INFO = 2, internal error in CZSTEIN
*
*  Further Details
*  ===============
*
...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, CDLARRB, CDLARRF, ZAXPY, ZDSCAL, 
     $  CZLAR1V, 
     $                   CZLASET, CZSTEIN
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   60          CONTINUE
               IF( NDEPTH.GT.0 ) THEN
                  IINFO = 0
                  CALL CDLARRB( IN, D( IBEGIN ), L( IBEGIN ),
...
*
     ...
     $                                          NEWFTT+1 ) )
   70                   CONTINUE
                        CALL CDLARRF( IN, D( IBEGIN ), L( IBEGIN ),
     $                               WORK( INDLD+1 ), WORK( INDLLD+1 ),
     $                               NEWFRS, NEWLST, WORK,
     ...
     $                               INFO )
                        ...
                        ELSE
                           ...
                           ELSE
*
*                             Call CZSTEIN to process this tight cluster.
*                             This happens only if MINRGP <= MGSTOL
*                             and CDLARRF returns INFO = 1. The latter
...
*
  ...
  110                   CONTINUE
                        LAMBDA = WORK( K )
                        CALL CZLAR1V( IN, 1, IN, LAMBDA, D( IBEGIN ),
