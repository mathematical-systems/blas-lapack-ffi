...
*  =======
*
*  CSBDSQR computes the singular values and, optionally, the right and/or
...
*  form:  A = U*B*VT, as computed by CSGEBRD, then
* 
*     A = (U*Q) * S * (P**T*VT)
* 
...
*     .. External Subroutines ..
      EXTERNAL           SLARTG, SLAS2, SLASQ1, CSLASR, CSLASV2, 
     $  SROT, 
     $                   SSCAL, SSWAP, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( NRU.GT.0 )
     $      CALL CSLASR( 'R', 'V', 'F', NRU, N, WORK( 1 ), WORK( N ), 
     $  U, 
     $                  LDU )
         IF( NCC.GT.0 )
     $      CALL CSLASR( 'L', 'V', 'F', N, NCC, WORK( 1 ), WORK( N ), 
     $  C, 
     $                  LDC )
      END IF
*
...
*
            IF( NCVT.GT.0 )
     $         CALL CSLASR( 'L', 'V', 'F', M-LL+1, NCVT, WORK( 1 ),
     $                     WORK( N ), VT( LL, 1 ), LDVT )
            IF( NRU.GT.0 )
     $         CALL CSLASR( 'R', 'V', 'F', NRU, M-LL+1, WORK( NM12+1 ),
     $                     WORK( NM13+1 ), U( 1, LL ), LDU )
            IF( NCC.GT.0 )
     $         CALL CSLASR( 'L', 'V', 'F', M-LL+1, NCC, WORK( NM12+1 ),
...
*
            IF( NCVT.GT.0 )
     $         CALL CSLASR( 'L', 'V', 'B', M-LL+1, NCVT, WORK( NM12+1 
     $  ), 
     $                     WORK( NM13+1 ), VT( LL, 1 ), LDVT )
            IF( NRU.GT.0 )
     $         CALL CSLASR( 'R', 'V', 'B', NRU, M-LL+1, WORK( 1 ),
     $                     WORK( N ), U( 1, LL ), LDU )
            IF( NCC.GT.0 )
     $         CALL CSLASR( 'L', 'V', 'B', M-LL+1, NCC, WORK( 1 ),
     $                     WORK( N ), C( LL, 1 ), LDC )
*
*           Test convergence
*
