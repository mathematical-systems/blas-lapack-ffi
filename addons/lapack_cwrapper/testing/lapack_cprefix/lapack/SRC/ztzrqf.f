...
*  =======
*
*  This routine is deprecated and has been replaced by routine CZTZRZF.
*
*  CZTZRQF reduces the M-by-N ( M<=N ) complex upper trapezoidal matrix A
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZAXPY, ZCOPY, ZGEMV, ZGERC, CZLACGV,
     $                   CZLARFG
*     ..
...
*
            A( K, K ) = DCONJG( A( K, K ) )
            CALL CZLACGV( N-M, A( K, M1 ), LDA )
            ALPHA = A( K, K )
            CALL CZLARFG( N-M+1, ALPHA, A( K, M1 ), LDA, TAU( K ) )
            A( K, K ) = ALPHA
            TAU( K ) = DCONJG( TAU( K ) )
*
