...
*  =======
*
*  CSLAQPS computes a step of QR factorization with column pivoting
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, SGEMV, CSLARFG, SSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( RK.LT.M ) THEN
            CALL CSLARFG( M-RK+1, A( RK, K ), A( RK+1, K ), 1, TAU( K ) 
     $  ) 
         ELSE
            CALL CSLARFG( 1, A( RK, K ), A( RK, K ), 1, TAU( K ) )
         END IF
*
         AKK = A( RK, K )
         A( RK, K ) = ONE
