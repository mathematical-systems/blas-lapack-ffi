...
*  =======
*
* CZSTEGR computes selected eigenvalues and, optionally, eigenvectors
...
*
*  Note 1 : Currently CZSTEGR is only set up to find ALL the n
*  eigenvalues and eigenvectors of T in O(n^2) time
*  Note 2 : Currently the routine CZSTEIN is called when an appropriate
*  sigma_i cannot be chosen in step (c) above. CZSTEIN invokes modified
*  Gram-Schmidt when eigenvalues are close.
*  Note 3 : CZSTEGR works only on machines which follow ieee-754
*  floating-point standard in their handling of infinities and NaNs.
*  Normal execution of CZSTEGR may create NaNs and infinities and hence
...
*     .. Local Scalars ..
      ...
      INTEGER            I, IBEGIN, IEND, IINDBL, IINDWK, IINFO, 
     $  IINSPL, 
     $                   INDGRS, INDWOF, INDWRK, ITMP, J, JJ, LIWMIN,
     $                   LWMIN, NSPLIT
      DOUBLE PRECISION   BIGNUM, EPS, RMAX, RMIN, SAFMIN, SCALE, 
     $  SMLNUM, 
     $                   THRESH, TMP, TNRM, TOL
*     ..
*     .. External Functions ..
...
*     .. External Subroutines ..
      EXTERNAL           CDLARRE, DSCAL, XERBLA, CZLARRV, CZLASET, 
     $  ZSWAP 
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $      INFO = -7
      ...
      IF( INFO.EQ.0 ) THEN
         ...
         END IF
      END IF
*     The following change should be made in CCDSTEVX also, otherwise
...
*
      ...
      IINDWK = 2*N + 1
*
      CALL CZLASET( 'Full', N, N, CZERO, CZERO, Z, LDZ )
*
...
*
      THRESH = EPS*TNRM
      CALL CDLARRE( N, D, E, THRESH, NSPLIT, IWORK( IINSPL ), M, W,
...
*
   ...
   20    CONTINUE
*
         CALL CZLARRV( N, D, E, IWORK( IINSPL ), M, W, IWORK( IINDBL ),
