 ...
 * lapack_dtrevc.c
 *
 *This program is a C interface to dtrevc.
 *
...
{
#ifdef F77_INT
	F77_INT F77_n=n, F77_ldt=ldt, F77_ldvl=ldvl, F77_ldvr=ldvr, F77_mm=mm;
	int i_local;
	F77_INT F77_m[1];
	F77_m[0]=(F77_INT) m[0];
	F77_INT F77_info[1];
...
;
	...
	f77_dtrevc(&F77_side, &F77_howmny, F77_select, &F77_n, t, &F77_ldt, vl, &F77_ldvl, vr, &F77_ldvr, &F77_mm, F77_m, work, F77_info);
...
#ifdef F77_LOG
	for(j_local=0;j_local<n*1;j_local++) {
		select[j_local]=(long int) F77_select[j_local];
	}
