...
void fdtgevc_(char* side, char* howmny, long int* select, int* n, double* s, int* lds, double* p, int* ldp, double* vl, int* ldvl, double* vr, int* ldvr, int* mm, int* m, double* work, int* info)
{
	...
	lapack_dtgevc(sideC, howmnyC, select, *n, s, *lds, p, *ldp, vl, *ldvl, vr, *ldvr, *mm, m, work, info);
}
