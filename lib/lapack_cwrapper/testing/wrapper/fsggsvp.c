...
void fsggsvp_(char* jobu, char* jobv, char* jobq, int* m, int* p, int* n, float* a, int* lda, float* b, int* ldb, float* tola, float* tolb, int* k, int* l, float* u, int* ldu, float* v, int* ldv, float* q, int* ldq, int* iwork, float* tau, float* work, int* info)
{
	...
	lapack_sggsvp(jobuC, jobvC, jobqC, *m, *p, *n, a, *lda, b, *ldb, *tola, *tolb, k, l, u, *ldu, v, *ldv, q, *ldq, iwork, tau, work, info);
}
