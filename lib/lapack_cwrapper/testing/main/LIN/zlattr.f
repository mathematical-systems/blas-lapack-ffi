...
*
         IF( UPPER ) THEN
            DO 140 J = 1, N
               CALL czlarnv( 4, ISEED, J-1, A( 1, J ) )
               A( J, J ) = ZLARND( 5, ISEED )*TWO
  140       CONTINUE
         ELSE
            DO 150 J = 1, N
               IF( J.LT.N )
     $            CALL czlarnv( 4, ISEED, N-J, A( J+1, J ) )
               A( J, J ) = ZLARND( 5, ISEED )*TWO
  150       CONTINUE
         END IF
*
...
*        1/3/91:  czlatrs no longer can handle this case
*
         TLEFT = BIGNUM / MAX( ONE, DBLE( N-1 ) )
         TSCAL = BIGNUM*( DBLE( N-1 ) / MAX( ONE, DBLE( N ) ) )
         IF( UPPER ) THEN
            DO 390 J = 1, N
               CALL czlarnv( 5, ISEED, J, A( 1, J ) )
  ...
  390       CONTINUE
         ELSE
            DO 410 J = 1, N
               CALL czlarnv( 5, ISEED, N-J+1, A( J, J ) )
