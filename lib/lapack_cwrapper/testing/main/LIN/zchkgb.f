...
*  =======
*
*  ZCHKGB tests czgbtrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                czgbcon, czgbrfs, ZGBT01, ZGBT02, ZGBT05, czgbtrf,
     $                czgbtrs, ZGET04, czlacpy, ZLARHS, czlaset, ZLATB4,
     $                   ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                                  AFAC( KL+1 ), LDAFAC )
                        SRNAMT = 'czgbtrf'
                        CALL czgbtrf( M, N, KL, KU, AFAC, LDAFAC, IWORK,
     $                               INFO )
*
*                       Check error code from czgbtrf.
*
                        IF( INFO.NE.IZERO )
     $                     CALL ALAERH( PATH, 'czgbtrf', INFO, IZERO,
     $                                  ' ', M, N, KL, KU, NB, IMAT,
     $                                  NFAIL, NERRS, NOUT )
                        TRFCON = .FALSE.
*
...
*
                              SRNAMT = 'czgbtrs'
                             CALL czgbtrs( TRANS, N, KL, KU, NRHS, AFAC,
     $                                     LDAFAC, IWORK, X, LDB, INFO )
*
*                             Check error code from czgbtrs.
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'czgbtrs', INFO, 0,
     $                                        TRANS, N, N, KL, KU, -1,
...
*
                              SRNAMT = 'czgbrfs'
                              CALL czgbrfs( TRANS, N, KL, KU, NRHS, A,
     $                                     LDA, AFAC, LDAFAC, IWORK, B,
     $                                     LDB, X, LDB, RWORK,
     $                                     RWORK( NRHS+1 ), WORK,
     $                                     RWORK( 2*NRHS+1 ), INFO )
*
*                             Check error code from czgbrfs.
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'czgbrfs', INFO, 0,
...
*
   90                   CONTINUE
                        DO 100 ITRAN = 1, 2
                           ...
                           END IF
                           SRNAMT = 'czgbcon'
                           CALL czgbcon( NORM, N, KL, KU, AFAC, LDAFAC,
     $                                  IWORK, ANORM, RCOND, WORK,
     $                                  RWORK, INFO )
*
*                             Check error code from czgbcon.
*
                           IF( INFO.NE.0 )
     $                        CALL ALAERH( PATH, 'czgbcon', INFO, 0,
