...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, cslabad, cslarnv, SLATB4, SLATMS, SROT,
     $                   SROTG, SSCAL, SSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( UPPER ) THEN
            DO 140 J = 1, N
               CALL cslarnv( 2, ISEED, J, A( 1, J ) )
               A( J, J ) = SIGN( TWO, A( J, J ) )
  140       CONTINUE
         ELSE
            DO 150 J = 1, N
               CALL cslarnv( 2, ISEED, N-J+1, A( J, J ) )
               A( J, J ) = SIGN( TWO, A( J, J ) )
  150       CONTINUE
         END IF
*
...
*
         IF( UPPER ) THEN
            DO 360 J = 1, N
               CALL cslarnv( 2, ISEED, J-1, A( 1, J ) )
               A( J, J ) = ZERO
  360       CONTINUE
         ELSE
            DO 370 J = 1, N
               IF( J.LT.N )
     $            CALL cslarnv( 2, ISEED, N-J, A( J+1, J ) )
               A( J, J ) = ZERO
  370       CONTINUE
         END IF
*
...
*        1/3/91:  cslatrs no longer can handle this case
*
