     ...
     $                   NMAX, AB, AINV, B, X, XACT, WORK, RWORK, 
     $  IWORK, 
     $                   NOUT )
*
...
*  =======
*
*  SCHKTB tests cstbtrs, -RFS, and -CON, and cslatbs.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                   cslacpy, SLARHS, cslaset, cslatbs, SLATTB, 
     $  cstbcon, 
     $                   cstbrfs, STBSV, STBT02, STBT03, STBT05, 
     $  STBT06, 
     $                   cstbtrs
*     ..
*     .. Scalars in Common ..
...
*
                  CALL cslaset( 'Full', N, N, ZERO, ONE, AINV, LDA )
...
*
                        SRNAMT = 'SLARHS'
                        CALL SLARHS( PATH, XTYPE, UPLO, TRANS, N, N, 
     $  KD, 
...
*
                        SRNAMT = 'cstbtrs'
                        CALL cstbtrs( UPLO, TRANS, DIAG, N, KD, NRHS, 
     $  AB, 
     $                               LDAB, X, LDA, INFO )
*
*                    Check error code from cstbtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cstbtrs', INFO, 0,
     $                                  UPLO // TRANS // DIAG, N, N, 
     $  KD, 
     $                                  KD, NRHS, IMAT, NFAIL, NERRS,
     $                                  NOUT )
*
                        CALL STBT02( UPLO, TRANS, DIAG, N, KD, NRHS, 
     $  AB, 
...
*
                        SRNAMT = 'cstbrfs'
                        CALL cstbrfs( UPLO, TRANS, DIAG, N, KD, NRHS, 
     $  AB, 
     $                               LDAB, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK, IWORK,
     $                               INFO )
*
*                    Check error code from cstbrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cstbrfs', INFO, 0,
     $                                  UPLO // TRANS // DIAG, N, N, 
     $  KD, 
     $                                  KD, NRHS, IMAT, NFAIL, NERRS,
     $                                  NOUT )
*
     ...
     $  RCONDC, 
     $                               RESULT( 3 ) )
                        CALL STBT05( UPLO, TRANS, DIAG, N, KD, NRHS, 
     $  AB, 
...
*
                  DO 70 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'cstbcon'
                     CALL cstbcon( NORM, UPLO, DIAG, N, KD, AB, LDAB,
     $                            RCOND, WORK, IWORK, INFO )
*
*                    Check error code from cstbcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cstbcon', INFO, 0,
     $                               NORM // UPLO // DIAG, N, N, KD, 
     $  KD, 
...
*
                     IF( RESULT( 6 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 ) 'cstbcon', NORM, 
     $  UPLO, 
   ...
   90       CONTINUE
*
*           Use pathological test matrices to test cslatbs.
*
...
*
                     SRNAMT = 'SLATTB'
                     CALL SLATTB( IMAT, UPLO, TRANS, DIAG, ISEED, N, 
     $  KD, 
     $                            AB, LDAB, X, WORK, INFO )
*
*+    TEST 7
...
*
                     ...
                     CALL cslatbs( UPLO, TRANS, DIAG, 'N', N, KD, AB,
     $                            LDAB, B, SCALE, RWORK, INFO )
*
*                    Check error code from cslatbs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cslatbs', INFO, 0,
     $                               UPLO // TRANS // DIAG // 'N', N, 
     $  N, 
     $                               KD, KD, -1, IMAT, NFAIL, NERRS,
     $                               NOUT )
*
                     CALL STBT03( UPLO, TRANS, DIAG, N, KD, 1, AB, 
     $  LDAB, 
...
*
                     CALL SCOPY( N, X, 1, B, 1 )
                     CALL cslatbs( UPLO, TRANS, DIAG, 'Y', N, KD, AB,
     $                            LDAB, B, SCALE, RWORK, INFO )
*
*                    Check error code from cslatbs.
*
...
*
                     IF( RESULT( 7 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9997 )'cslatbs', UPLO, 
     $  TRANS, 
