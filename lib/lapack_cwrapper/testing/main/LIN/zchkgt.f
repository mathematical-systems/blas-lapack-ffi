...
*  =======
*
*  ZCHKGT tests czgttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 ZGET04, czgtcon, czgtrfs, ZGTT01, ZGTT02, ZGTT05,
     $              czgttrf, czgttrs, czlacpy, czlagtm, czlarnv, ZLATB4,
     $                   ZLATMS
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
...
*
            ...
            SRNAMT = 'czgttrf'
            CALL czgttrf( N, AF, AF( M+1 ), AF( N+M+1 ), AF( N+2*M+1 ),
     $                   IWORK, INFO )
*
*           Check error code from czgttrf.
*
            IF( INFO.NE.IZERO )
     $         CALL ALAERH( PATH, 'czgttrf', INFO, IZERO, ' ', N, N, 1,
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL czgttrs( TRANS, N, 1, AF, AF( M+1 ),
...
*
               SRNAMT = 'czgtcon'
               CALL czgtcon( NORM, N, AF, AF( M+1 ), AF( N+M+1 ),
     $                      AF( N+2*M+1 ), IWORK, ANORM, RCOND, WORK,
     $                      INFO )
*
*              Check error code from czgtcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'czgtcon', INFO, 0, NORM, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
...
*
                  ...
                  SRNAMT = 'czgttrs'
                  CALL czgttrs( TRANS, N, NRHS, AF, AF( M+1 ),
     $                         AF( N+M+1 ), AF( N+2*M+1 ), IWORK, X,
     $                         LDA, INFO )
*
*              Check error code from czgttrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czgttrs', INFO, 0, TRANS, N, N,
...
*
                  SRNAMT = 'czgtrfs'
                  CALL czgtrfs( TRANS, N, NRHS, A, A( M+1 ), A( N+M+1 ),
     $                         AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, B, LDA, X, LDA,
     $                         RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from czgtrfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czgtrfs', INFO, 0, TRANS, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
