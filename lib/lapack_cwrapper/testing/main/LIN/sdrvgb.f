...
*  =======
*
*  SDRVGB tests the driver routines csgbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALADHD, ALAERH, ALASVM, SERRVX, csgbequ, 
     $  csgbsv, 
     $                   csgbsvX, SGBT01, SGBT02, SGBT05, csgbtrf, 
     $  csgbtrs, 
     $                   SGET04, cslacpy, cslaqgb, SLARHS, cslaset, 
     $  SLATB4, 
     $                   SLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                  CALL cslacpy( 'Full', KL+KU+1, N, A, LDA, ASAV, LDA )
*
                  DO 110 IEQUED = 1, 4
                     EQUED = EQUEDS( IEQUED )
                     IF( IEQUED.EQ.1 ) THEN
...
*
                           CALL cslacpy( 'Full', KL+KU+1, N, ASAV, LDA,
     $                                  AFB( KL+1 ), LDAFB )
...
*
                              CALL csgbequ( N, N, KL, KU, AFB( KL+1 ),
...
*
                                 CALL cslaqgb( N, N, KL, KU, AFB( KL+1 
     $  ), 
     $                                        LDAFB, S, S( N+1 ),
     $                                        ROWCND, COLCND, AMAX,
     $                                        EQUED )
...
*                          Factor the matrix A.
*
                           CALL csgbtrf( N, N, KL, KU, AFB, LDAFB, 
     $  IWORK, 
     $                                  INFO )
*
*                          Form the inverse of A.
*
                           CALL cslaset( 'Full', N, N, ZERO, ONE, WORK,
     $                                  LDB )
                           SRNAMT = 'csgbtrs'
                           CALL csgbtrs( 'No transpose', N, KL, KU, N,
...
*
     ...
     $                                  LDB, B, LDB, ISEED, INFO )
                           XTYPE = 'C'
                           CALL cslacpy( 'Full', N, NRHS, B, LDB, BSAV,
     $                                  LDB )
*
...
*
                              CALL cslacpy( 'Full', KL+KU+1, N, A, LDA,
     $                                     AFB( KL+1 ), LDAFB )
                              CALL cslacpy( 'Full', N, NRHS, B, LDB, X,
     $                                     LDB )
*
                              SRNAMT = 'csgbsv '
                              CALL csgbsv( N, KL, KU, NRHS, AFB, LDAFB,
     $                                    IWORK, X, LDB, INFO )
*
*                             Check error code from csgbsv .
*
                              IF( INFO.NE.IZERO )
     $                           CALL ALAERH( PATH, 'csgbsv ', INFO,
     $                                        IZERO, ' ', N, N, KL, KU,
...
*                                solution.
*
                                 CALL cslacpy( 'Full', N, NRHS, B, LDB,
     $                                        WORK, LDB )
...
*
                           IF( .NOT.PREFAC )
     $                        CALL cslaset( 'Full', 2*KL+KU+1, N, ZERO,
     $                                     ZERO, AFB, LDAFB )
                           CALL cslaset( 'Full', N, NRHS, ZERO, ZERO, 
     $  X, 
     $                                  LDB )
...
*                             EQUED = 'R', 'C', or 'B'.
*
                              CALL cslaqgb( N, N, KL, KU, A, LDA, S,
     $                                     S( N+1 ), ROWCND, COLCND,
     $                                     AMAX, EQUED )
                           END IF
*
*                          Solve the system and compute the condition
*                          number and error bounds using csgbsvX.
*
                           SRNAMT = 'csgbsvX'
                           CALL csgbsvX( FACT, TRANS, N, KL, KU, NRHS, 
     $  A, 
     ...
     $                                  WORK, IWORK( N+1 ), INFO )
*
*                          Check the error code from csgbsvX.
*
                           IF( INFO.NE.IZERO )
     $                        CALL ALAERH( PATH, 'csgbsvX', INFO, 
     $  IZERO, 
     $                                     FACT // TRANS, N, N, KL, KU,
     $                                     NRHS, IMAT, NFAIL, NERRS,
     $                                     NOUT )
*
*                          Compare WORK(1) from csgbsvX with the computed
...
*
                              CALL cslacpy( 'Full', N, NRHS, BSAV, LDB,
     $                                     WORK, LDB )
...
*
     ...
     $                                     RESULT( 4 ) )
                           ...
                           END IF
*
*                          Compare RCOND from csgbsvX with the computed
*                          value in RCONDC.
*
...
*
   ...
   80                         CONTINUE
     ...
     $                              CALL ALADHD( NOUT, PATH )
                                 IF( PREFAC ) THEN
                                    WRITE( NOUT, FMT = 9995 )'csgbsvX',
     $                                 FACT, TRANS, N, KL, KU, EQUED,
     $                                 IMAT, 1, RESULT( 1 )
                                 ELSE
                                    WRITE( NOUT, FMT = 9996 )'csgbsvX',
