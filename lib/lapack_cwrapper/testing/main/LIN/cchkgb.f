...
*  =======
*
*  CCHKGB tests ccgbtrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, CCOPY, CERRGE, ccgbcon,
     $                ccgbrfs, CGBT01, CGBT02, CGBT05, ccgbtrf, ccgbtrs,
     $                 CGET04, cclacpy, CLARHS, cclaset, CLATB4, CLATMS,
     $                   XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                                  AFAC( KL+1 ), LDAFAC )
                        SRNAMT = 'ccgbtrf'
                        CALL ccgbtrf( M, N, KL, KU, AFAC, LDAFAC, IWORK,
     $                               INFO )
*
*                       Check error code from ccgbtrf.
*
                        IF( INFO.NE.IZERO )
     $                     CALL ALAERH( PATH, 'ccgbtrf', INFO, IZERO,
     $                                  ' ', M, N, KL, KU, NB, IMAT,
     $                                  NFAIL, NERRS, NOUT )
                        TRFCON = .FALSE.
*
...
*
                              SRNAMT = 'ccgbtrs'
                             CALL ccgbtrs( TRANS, N, KL, KU, NRHS, AFAC,
     $                                     LDAFAC, IWORK, X, LDB, INFO )
*
*                             Check error code from ccgbtrs.
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'ccgbtrs', INFO, 0,
     $                                        TRANS, N, N, KL, KU, -1,
...
*
                              SRNAMT = 'ccgbrfs'
                              CALL ccgbrfs( TRANS, N, KL, KU, NRHS, A,
     $                                     LDA, AFAC, LDAFAC, IWORK, B,
     $                                     LDB, X, LDB, RWORK,
     $                                     RWORK( NRHS+1 ), WORK,
     $                                     RWORK( 2*NRHS+1 ), INFO )
*
*                             Check error code from ccgbrfs.
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'ccgbrfs', INFO, 0,
...
*
   90                   CONTINUE
                        DO 100 ITRAN = 1, 2
                           ...
                           END IF
                           SRNAMT = 'ccgbcon'
                           CALL ccgbcon( NORM, N, KL, KU, AFAC, LDAFAC,
     $                                  IWORK, ANORM, RCOND, WORK,
     $                                  RWORK, INFO )
*
*                             Check error code from ccgbcon.
*
                           IF( INFO.NE.0 )
     $                        CALL ALAERH( PATH, 'ccgbcon', INFO, 0,
