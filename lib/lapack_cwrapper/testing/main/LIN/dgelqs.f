...
*     .. External Subroutines ..
      EXTERNAL cdlaset,cdormlq,DTRSM,xerbla
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX
*     ..
...
*
      ...
      ELSE IF (LDB.LT.MAX(1,N)) THEN
          INFO = -8
      ELSE IF (LWORK.LT.1 .OR. LWORK.LT.NRHS .AND. M.GT.0 .AND.
     +         N.GT.0) THEN
          INFO = -10
      END IF
      IF (INFO.NE.0) THEN
...
*     Set B(m+1:n,:) to zero
*
      IF (M.LT.N) CALL cdlaset('Full',N-M,NRHS,ZERO,ZERO,B(M+1,1),LDB)
*
*     B := Q' * B
*
      CALL cdormlq('Left','Transpose',N,NRHS,M,A,LDA,TAU,B,LDB,WORK,
     +             LWORK,INFO)
*
      RETURN
*
