...
*  =======
*
*  ZRQT03 tests czunmrq, which computes Q*C, Q'*C, C*Q or C*Q'.
*
*  ZRQT03 compares the results of a call to czunmrq with the results of
*  forming Q explicitly by a call to czungrq and then performing matrix
...
*          returned by czgerqf. See ccgerqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL        ZGEMM, czlacpy, czlarnv, czlaset, czungrq, czunmrq
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
*     ..
...
*
     ...
     $   CALL czlacpy( 'Full', K, N-K, AF( M-K+1, 1 ), LDA,
     $                Q( N-K+1, 1 ), LDA )
      IF( K.GT.1 )
     $   CALL czlacpy( 'Lower', K-1, K-1, AF( M-K+2, N-K+1 ), LDA,
     $                Q( N-K+2, N-K+1 ), LDA )
*
*     Generate the n-by-n matrix Q
*
      SRNAMT = 'czungrq'
      CALL czungrq( N, N, K, Q, LDA, TAU( MINMN-K+1 ), WORK, LWORK,
     $             INFO )
*
      DO 30 ISIDE = 1, 2
         IF( ISIDE.EQ.1 ) THEN
...
*
            SRNAMT = 'czunmrq'
            IF( K.GT.0 )
     $        CALL czunmrq( SIDE, TRANS, MC, NC, K, AF( M-K+1, 1 ), LDA,
