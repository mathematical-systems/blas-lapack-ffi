...
*     .. External Subroutines ..
      EXTERNAL DGEMM,DLAORD,cdlarf,cdlarnv,DLAROR,cdlascl,cdlaset,DSCAL,
     +         xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
          CALL cdlarnv(2,ISEED,M,WORK)
          CALL DSCAL(M,ONE/DNRM2(M,WORK,1),WORK,1)
          CALL cdlaset('Full',M,RANK,ZERO,ONE,A,LDA)
          CALL cdlarf('Left',M,RANK,WORK,1,TWO,A,LDA,WORK(M+1))
*
*        workspace used: m+mn
*
*        Generate consistent rhs in the range space of A
*
          CALL cdlarnv(2,ISEED,RANK*NRHS,WORK)
          CALL DGEMM('No transpose','No transpose',M,NRHS,RANK,ONE,A,
     +               LDA,WORK,RANK,ZERO,B,LDB)
*
...
*
   ...
   40     CONTINUE
          IF (RANK.LT.N) CALL cdlaset('Full',M,N-RANK,ZERO,ZERO,
     +                                A(1,RANK+1),LDA)
...
*
   ...
   50     CONTINUE
          ...
          CALL cdlaset('Full',M,NRHS,ZERO,ZERO,B,LDB)
*
      END IF
*
*     Scale the matrix
...
*              matrix scaled up
*
                  CALL cdlascl('General',0,0,NORMA,BIGNUM,M,N,A,LDA,
     +                         INFO)
                  CALL cdlascl('General',0,0,NORMA,BIGNUM,MN,1,S,MN,
     +                         INFO)
                  CALL cdlascl('General',0,0,NORMA,BIGNUM,M,NRHS,B,LDB,
     +                         INFO)
              ELSE IF (SCALE.EQ.3) THEN
*
*              matrix scaled down
*
                  CALL cdlascl('General',0,0,NORMA,SMLNUM,M,N,A,LDA,
     +                         INFO)
                  CALL cdlascl('General',0,0,NORMA,SMLNUM,MN,1,S,MN,
     +                         INFO)
                  CALL cdlascl('General',0,0,NORMA,SMLNUM,M,NRHS,B,LDB,
     +                         INFO)
              ELSE
