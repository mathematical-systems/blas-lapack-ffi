...
*  =======
*
*  ZCHKTR tests cztrtri, -TRS, -RFS, and -CON, and czlatrs
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                ZGET04, czlacpy, ZLARHS, czlatrs, ZLATTR, cztrcon,
     $                  cztrrfs, ZTRT01, ZTRT02, ZTRT03, ZTRT05, ZTRT06,
     $                   cztrtri, cztrtrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                  CALL czlacpy( UPLO, N, N, A, LDA, AINV, LDA )
                  SRNAMT = 'cztrtri'
                  CALL cztrtri( UPLO, DIAG, N, AINV, LDA, INFO )
*
*                 Check error code from cztrtri.
*
                  IF( INFO.NE.0 )
     $              CALL ALAERH( PATH, 'cztrtri', INFO, 0, UPLO // DIAG,
...
*
                        SRNAMT = 'cztrtrs'
                       CALL cztrtrs( UPLO, TRANS, DIAG, N, NRHS, A, LDA,
     $                               X, LDA, INFO )
*
*                       Check error code from cztrtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cztrtrs', INFO, 0,
...
*
                        SRNAMT = 'cztrrfs'
                       CALL cztrrfs( UPLO, TRANS, DIAG, N, NRHS, A, LDA,
     $                               B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                       Check error code from cztrrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cztrrfs', INFO, 0,
     $                                  UPLO // TRANS // DIAG, N, N, -1,
     $                                  -1, NRHS, IMAT, NFAIL, NERRS,
     $                                  NOUT )
*
...
*
                  DO 50 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'cztrcon'
                     CALL cztrcon( NORM, UPLO, DIAG, N, A, LDA, RCOND,
     $                            WORK, RWORK, INFO )
*
*                       Check error code from cztrcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cztrcon', INFO, 0,
...
*
   ...
   80    CONTINUE
*
*        Use pathological test matrices to test czlatrs.
*
...
*
                  ...
                  CALL czlatrs( UPLO, TRANS, DIAG, 'N', N, A, LDA, B,
     $                         SCALE, RWORK, INFO )
*
*                 Check error code from czlatrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czlatrs', INFO, 0,
...
*
                  ...
                  CALL czlatrs( UPLO, TRANS, DIAG, 'Y', N, A, LDA,
...
*
                  IF( RESULT( 8 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9996 )'czlatrs', UPLO, TRANS,
