...
*  =======
*
*  DDRVPO tests the driver routines cdposv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IEQUED,IFACT,IMAT,IN,INFO,IOFF,IUPLO,IZERO,K,K1,KL,KU,
     +        LDA,MODE,N,NB,NBMIN,NERRS,NFACT,NFAIL,NIMAT,NRUN,NT
...
*     .. External Subroutines ..
      EXTERNAL ALADHD,ALAERH,ALASVM,DERRVX,DGET04,cdlacpy,cdlaqsy,
     +         DLARHS,cdlaset,DLATB4,DLATMS,cdpoequ,cdposv,cdposvX,
     +         DPOT01,DPOT02,DPOT05,cdpotrf,cdpotri,XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX
*     ..
...
*
                  CALL cdlacpy(UPLO,N,N,A,LDA,ASAV,LDA)
*
                  DO 100 IEQUED = 1,2
                      EQUED = EQUEDS(IEQUED)
                      IF (IEQUED.EQ.1) THEN
...
*                       the value returned by cdposvX (FACT = 'N' reuses
*                       the condition number from the previous iteration
*                       with FACT = 'F').
*
                              CALL cdlacpy(UPLO,N,N,ASAV,LDA,AFAC,LDA)
                              IF (EQUIL .OR. IEQUED.GT.1) THEN
*
*                          Compute row and column scale factors to
*                          equilibrate the matrix A.
*
                                  CALL cdpoequ(N,AFAC,LDA,S,SCOND,AMAX,
     +                                         INFO)
...
*
                                      CALL cdlaqsy(UPLO,N,AFAC,LDA,S,
     +                                             SCOND,AMAX,EQUED)
                                  END IF
                              END IF
*
...
*                       Factor the matrix A.
*
                              CALL cdpotrf(UPLO,N,AFAC,LDA,INFO)
*
*                       Form the inverse of A.
*
                              CALL cdlacpy(UPLO,N,N,AFAC,LDA,A,LDA)
                              CALL cdpotri(UPLO,N,A,LDA,INFO)
*
...
*
                          CALL cdlacpy(UPLO,N,N,ASAV,LDA,A,LDA)
*
...
*
                              CALL cdlacpy(UPLO,N,N,A,LDA,AFAC,LDA)
                              CALL cdlacpy('Full',N,NRHS,B,LDA,X,LDA)
*
                              SRNAMT = 'cdposv '
                              CALL cdposv(UPLO,N,NRHS,AFAC,LDA,X,LDA,
     +                                    INFO)
*
*                       Check error code from cdposv .
*
                              IF (INFO.NE.IZERO) THEN
                                  CALL ALAERH(PATH,'cdposv ',INFO,IZERO,
...
*
                              CALL cdlacpy('Full',N,NRHS,B,LDA,WORK,LDA)
...
*                    --- Test cdposvX ---
*
                          IF (.NOT.PREFAC) CALL cdlaset(UPLO,N,N,ZERO,
     +                                          ZERO,AFAC,LDA)
...
*                       EQUED='Y'.
*
                              CALL cdlaqsy(UPLO,N,A,LDA,S,SCOND,AMAX,
     +                                     EQUED)
                          END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using cdposvX.
*
                          SRNAMT = 'cdposvX'
                          CALL cdposvX(FACT,UPLO,N,NRHS,A,LDA,AFAC,LDA,
     +                                 EQUED,S,B,LDA,X,LDA,RCOND,RWORK,
     +                                 RWORK(NRHS+1),WORK,IWORK,INFO)
*
*                    Check the error code from cdposvX.
*
                          IF (INFO.NE.IZERO) THEN
                              CALL ALAERH(PATH,'cdposvX',INFO,IZERO,
...
*
     ...
     +                                    RWORK(NRHS+1),RESULT(4))
                          ...
                          END IF
*
*                    Compare RCOND from cdposvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     +                                NERRS.EQ.0) CALL ALADHD(NOUT,PATH)
                                  IF (PREFAC) THEN
                                      WRITE (NOUT,FMT=9997) 'cdposvX',
     +                                  FACT,UPLO,N,EQUED,IMAT,K,
     +                                  RESULT(K)
                                  ELSE
                                      WRITE (NOUT,FMT=9998) 'cdposvX',
     +                                  FACT,UPLO,N,IMAT,K,RESULT(K)
                                  END IF
                                  NFAIL = NFAIL + 1
                              END IF
   80                     CONTINUE
...
*
 9999 FORMAT (1X,A6,', UPLO=''',A1,''', N =',I5,', type ',I1,', test(',
     +       I1,')=',G12.5)
