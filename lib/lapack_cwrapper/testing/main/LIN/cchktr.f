...
*  =======
*
*  CCHKTR tests cctrtri, -TRS, -RFS, and -CON, and cclatrs
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $              cclacpy, CLARHS, cclatrs, CLATTR, cctrcon, cctrrfs,
     $                  CTRT01, CTRT02, CTRT03, CTRT05, CTRT06, cctrtri,
     $                   cctrtrs, XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                  CALL cclacpy( UPLO, N, N, A, LDA, AINV, LDA )
                  SRNAMT = 'cctrtri'
                  CALL cctrtri( UPLO, DIAG, N, AINV, LDA, INFO )
*
*                 Check error code from cctrtri.
*
                  IF( INFO.NE.0 )
     $              CALL ALAERH( PATH, 'cctrtri', INFO, 0, UPLO // DIAG,
...
*
                        SRNAMT = 'cctrtrs'
                       CALL cctrtrs( UPLO, TRANS, DIAG, N, NRHS, A, LDA,
     $                               X, LDA, INFO )
*
*                       Check error code from cctrtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cctrtrs', INFO, 0,
...
*
                        SRNAMT = 'cctrrfs'
                       CALL cctrrfs( UPLO, TRANS, DIAG, N, NRHS, A, LDA,
     $                               B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                       Check error code from cctrrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cctrrfs', INFO, 0,
     $                                  UPLO // TRANS // DIAG, N, N, -1,
     $                                  -1, NRHS, IMAT, NFAIL, NERRS,
     $                                  NOUT )
*
...
*
                  DO 50 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'cctrcon'
                     CALL cctrcon( NORM, UPLO, DIAG, N, A, LDA, RCOND,
     $                            WORK, RWORK, INFO )
*
*                       Check error code from cctrcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cctrcon', INFO, 0,
...
*
   ...
   80    CONTINUE
*
*        Use pathological test matrices to test cclatrs.
*
...
*
                  ...
                  CALL cclatrs( UPLO, TRANS, DIAG, 'N', N, A, LDA, B,
     $                         SCALE, RWORK, INFO )
*
*                 Check error code from cclatrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cclatrs', INFO, 0,
...
*
                  ...
                  CALL cclatrs( UPLO, TRANS, DIAG, 'Y', N, A, LDA,
...
*
                  IF( RESULT( 8 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9996 )'cclatrs', UPLO, TRANS,
