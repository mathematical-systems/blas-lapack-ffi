...
*  =======
*
*  SCHKGE tests csgetrf, -TRI, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALAERH, ALAHD, ALASUM, SERRGE, csgecon, csgerfs,
     $                  SGET01, SGET02, SGET03, SGET04, SGET07, csgetrf,
     $               csgetri, csgetrs, cslacpy, SLARHS, cslaset, SLATB4,
     $                   SLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
   ...
   20                CONTINUE
                  ELSE
                     CALL cslaset( 'Full', M, N-IZERO+1, ZERO, ZERO,
     $                            A( IOFF+1 ), LDA )
                  END IF
               ELSE
                  IZERO = 0
...
*
                  CALL cslacpy( 'Full', M, N, A, LDA, AFAC, LDA )
                  SRNAMT = 'csgetrf'
                  CALL csgetrf( M, N, AFAC, LDA, IWORK, INFO )
*
*                 Check error code from csgetrf.
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'csgetrf', INFO, IZERO, ' ', M,
...
*
                  CALL cslacpy( 'Full', M, N, AFAC, LDA, AINV, LDA )
...
*
                  IF( M.EQ.N .AND. INFO.EQ.0 ) THEN
                     CALL cslacpy( 'Full', N, N, AFAC, LDA, AINV, LDA )
                     SRNAMT = 'csgetri'
                     NRHS = NSVAL( 1 )
                     LWORK = NMAX*MAX( 3, NRHS )
                     CALL csgetri( N, AINV, LDA, IWORK, WORK, LWORK,
     $                            INFO )
*
*                    Check error code from csgetri.
*
                     IF( INFO.NE.0 )
     $                 CALL ALAERH( PATH, 'csgetri', INFO, 0, ' ', N, N,
...
*
                        ...
                        SRNAMT = 'csgetrs'
                        CALL csgetrs( TRANS, N, NRHS, AFAC, LDA, IWORK,
     $                               X, LDA, INFO )
*
*                       Check error code from csgetrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'csgetrs', INFO, 0, TRANS,
...
*
                        SRNAMT = 'csgerfs'
                        CALL csgerfs( TRANS, N, NRHS, A, LDA, AFAC, LDA,
     $                               IWORK, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               IWORK( N+1 ), INFO )
*
*                       Check error code from csgerfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'csgerfs', INFO, 0, TRANS,
     $                                  N, N, -1, -1, NRHS, IMAT, NFAIL,
     $                                  NERRS, NOUT )
*
                        CALL SGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
   70             CONTINUE
                  DO 80 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'csgecon'
                     CALL csgecon( NORM, N, AFAC, LDA, ANORM, RCOND,
     $                            WORK, IWORK( N+1 ), INFO )
*
*                       Check error code from csgecon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'csgecon', INFO, 0, NORM, N,
