...
*     .. External Subroutines ..
      EXTERNAL           ALAHD, ALASUM, DLAORD, ICOPY, XLAENV, czgeqp3,
     $                   czlacpy, czlaset, ZLATMS
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
   ...
   20          CONTINUE
               IF( IMODE.EQ.1 ) THEN
                  CALL czlaset( 'Full', M, N, CZERO, CZERO, COPYA, LDA )
                  DO 30 I = 1, MNMIN
                     COPYS( I ) = ZERO
   30             CONTINUE
               ELSE
...
*                 vector IWORK.
*
                  CALL czlacpy( 'All', M, N, COPYA, LDA, A, LDA )
...
*
                  SRNAMT = 'czgeqp3'
                  CALL czgeqp3( M, N, A, LDA, IWORK( N+1 ), TAU, WORK,
     $                         LW, RWORK, INFO )
*
...
*
     ...
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 )'czgeqp3', M, N, NB,
     $                     IMODE, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
   50             CONTINUE
