...
*  =======
*
*  SDRVGE tests the driver routines csgesv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALADHD, ALAERH, ALASVM, SERRVX, csgeequ, 
     $  csgesv, 
     $                   csgesvX, SGET01, SGET02, SGET04, SGET07, 
     $  csgetrf, 
     $                   csgetri, cslacpy, cslaqge, SLARHS, cslaset, 
     $  SLATB4, 
     $                   SLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   20             CONTINUE
               ELSE
                  CALL cslaset( 'Full', N, N-IZERO+1, ZERO, ZERO,
     $                         A( IOFF+1 ), LDA )
               END IF
            ELSE
               IZERO = 0
...
*
            CALL cslacpy( 'Full', N, N, A, LDA, ASAV, LDA )
*
            DO 70 IEQUED = 1, 4
               EQUED = EQUEDS( IEQUED )
               IF( IEQUED.EQ.1 ) THEN
...
*                    the value returned by csgesvX (FACT = 'N' reuses
*                    the condition number from the previous iteration
*                    with FACT = 'F').
*
                     CALL cslacpy( 'Full', N, N, ASAV, LDA, AFAC, LDA )
                     IF( EQUIL .OR. IEQUED.GT.1 ) THEN
*
*                       Compute row and column scale factors to
*                       equilibrate the matrix A.
*
                        CALL csgeequ( N, N, AFAC, LDA, S, S( N+1 ),
...
*
                           CALL cslaqge( N, N, AFAC, LDA, S, S( N+1 ),
...
*                    Factor the matrix A.
*
                     CALL csgetrf( N, N, AFAC, LDA, IWORK, INFO )
*
*                    Form the inverse of A.
*
                     CALL cslacpy( 'Full', N, N, AFAC, LDA, A, LDA )
                     LWORK = NMAX*MAX( 3, NRHS )
                     CALL csgetri( N, A, LDA, IWORK, WORK, LWORK, INFO 
     $  ) 
*
...
*
                     CALL cslacpy( 'Full', N, N, ASAV, LDA, A, LDA )
*
...
*
     ...
     $                            ISEED, INFO )
                     XTYPE = 'C'
                     CALL cslacpy( 'Full', N, NRHS, B, LDA, BSAV, LDA )
*
...
*                       solve the system.
*
                        CALL cslacpy( 'Full', N, N, A, LDA, AFAC, LDA )
                        CALL cslacpy( 'Full', N, NRHS, B, LDA, X, LDA )
*
                        SRNAMT = 'csgesv '
                        CALL csgesv( N, NRHS, AFAC, LDA, IWORK, X, LDA,
     $                              INFO )
*
*                       Check error code from csgesv .
*
                        IF( INFO.NE.IZERO )
     $                     CALL ALAERH( PATH, 'csgesv ', INFO, IZERO,
     $                                  ' ', N, N, -1, -1, NRHS, IMAT,
     $                                  NFAIL, NERRS, NOUT )
*
...
*
                           CALL cslacpy( 'Full', N, NRHS, B, LDA, WORK,
     $                                  LDA )
...
*
                     IF( .NOT.PREFAC )
     $                  CALL cslaset( 'Full', N, N, ZERO, ZERO, AFAC,
     $                               LDA )
                     CALL cslaset( 'Full', N, NRHS, ZERO, ZERO, X, LDA 
     $  ) 
...
*                       EQUED = 'R', 'C', or 'B'.
*
                        CALL cslaqge( N, N, A, LDA, S, S( N+1 ), 
     $  ROWCND, 
     $                               COLCND, AMAX, EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using csgesvX.
*
                     SRNAMT = 'csgesvX'
                     CALL csgesvX( FACT, TRANS, N, NRHS, A, LDA, AFAC,
     $                            LDA, IWORK, EQUED, S, S( N+1 ), B,
     $                            LDA, X, LDA, RCOND, RWORK,
     $                            RWORK( NRHS+1 ), WORK, IWORK( N+1 ),
     $                            INFO )
*
*                    Check the error code from csgesvX.
*
                     IF( INFO.NE.IZERO )
     $                  CALL ALAERH( PATH, 'csgesvX', INFO, IZERO,
     $                               FACT // TRANS, N, N, -1, -1, NRHS,
     $                               IMAT, NFAIL, NERRS, NOUT )
*
*                    Compare WORK(1) from csgesvX with the computed
...
*                       Compute residual of the computed solution.
*
                        CALL cslacpy( 'Full', N, NRHS, BSAV, LDA, WORK,
     $                               LDA )
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from csgesvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9997 )'csgesvX',
     $                              FACT, TRANS, N, EQUED, IMAT, K,
     $                              RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9998 )'csgesvX',
   ...
   40                   CONTINUE
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'csgesvX', FACT,
     $                           TRANS, N, EQUED, IMAT, 1, RESULT( 1 )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'csgesvX', FACT,
...
*
 ...
 9997 FORMAT( 1X, A6, ', FACT=''', A1, ''', TRANS=''', A1, ''', N=', 
     $  I5, 
