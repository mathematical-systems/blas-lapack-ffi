...
*  =======
*
*  SCHKSY tests cssytrf, -TRI, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, SERRSY, SGET04, 
     $  cslacpy, 
     $                   SLARHS, SLATB4, SLATMS, SPOT02, SPOT03, 
     $  SPOT05, 
     $                   cssycon, cssyrfs, SSYT01, cssytrf, cssytri, 
     $  cssytrs, 
     $                   XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                  ...
                  LWORK = MAX( 2, NB )*LDA
                  SRNAMT = 'cssytrf'
                  CALL cssytrf( UPLO, N, AFAC, LDA, IWORK, AINV, LWORK,
     $                         INFO )
*
...
*
  ...
  100                CONTINUE
                  ...
                  END IF
*
*                 Check error code from cssytrf.
*
                  IF( INFO.NE.K )
     $               CALL ALAERH( PATH, 'cssytrf', INFO, K, UPLO, N, N,
...
*
     ...
     $                            INFO )
*
*                 Check error code from cssytri.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cssytri', INFO, -1, UPLO, 
     $  N, 
     $                               N, -1, -1, -1, IMAT, NFAIL, NERRS,
     $                               NOUT )
*
                     CALL SPOT03( UPLO, N, A, LDA, AINV, LDA, WORK, 
     $  LDA, 
...
*
                     SRNAMT = 'cssytrs'
                     CALL cssytrs( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                            LDA, INFO )
*
*                 Check error code from cssytrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cssytrs', INFO, 0, UPLO, N,
...
*
                     SRNAMT = 'cssyrfs'
                     CALL cssyrfs( UPLO, N, NRHS, A, LDA, AFAC, LDA,
     $                            IWORK, B, LDA, X, LDA, RWORK,
     $                            RWORK( NRHS+1 ), WORK, IWORK( N+1 ),
     $                            INFO )
*
*                 Check error code from cssyrfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cssyrfs', INFO, 0, UPLO, N,
     $                               N, -1, -1, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL SGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
  140             CONTINUE
                  ...
                  SRNAMT = 'cssycon'
                  CALL cssycon( UPLO, N, AFAC, LDA, IWORK, ANORM, 
     $  RCOND, 
     $                         WORK, IWORK( N+1 ), INFO )
*
*                 Check error code from cssycon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cssycon', INFO, 0, UPLO, N, N,
