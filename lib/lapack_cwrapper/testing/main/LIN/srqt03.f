...
*  =======
*
*  SRQT03 tests csormrq, which computes Q*C, Q'*C, C*Q or C*Q'.
*
*  SRQT03 compares the results of a call to csormrq with the results of
*  forming Q explicitly by a call to csorgrq and then performing matrix
...
*          returned by csgerqf. See csgerqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, cslacpy, cslarnv, cslaset, csorgrq, 
     $  csormrq 
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
...
*
     ...
     $   CALL cslacpy( 'Full', K, N-K, AF( M-K+1, 1 ), LDA,
     $                Q( N-K+1, 1 ), LDA )
      IF( K.GT.1 )
     $   CALL cslacpy( 'Lower', K-1, K-1, AF( M-K+2, N-K+1 ), LDA,
     $                Q( N-K+2, N-K+1 ), LDA )
*
*     Generate the n-by-n matrix Q
*
      SRNAMT = 'csorgrq'
      CALL csorgrq( N, N, K, Q, LDA, TAU( MINMN-K+1 ), WORK, LWORK,
     $             INFO )
*
      DO 30 ISIDE = 1, 2
         IF( ISIDE.EQ.1 ) THEN
...
*
            SRNAMT = 'csormrq'
            IF( K.GT.0 )
     $         CALL csormrq( SIDE, TRANS, MC, NC, K, AF( M-K+1, 1 ), 
     $  LDA, 
