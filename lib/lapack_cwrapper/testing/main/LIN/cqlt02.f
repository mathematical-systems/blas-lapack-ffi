...
*  =======
*
*  CQLT02 tests ccungql, which generates an m-by-n matrix Q with
...
*     .. External Subroutines ..
      EXTERNAL           CGEMM, CHERK, cclacpy, cclaset, ccungql
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL cclaset( 'Full', M, N, ROGUE, ROGUE, Q, LDA )
      IF( K.LT.M )
...
*
      SRNAMT = 'ccungql'
      CALL ccungql( M, N, K, Q, LDA, TAU( N-K+1 ), WORK, LWORK, INFO )
*
*     Copy L(m-n+1:m,n-k+1:n)
*
      CALL cclaset( 'Full', N, K, CMPLX( ZERO ), CMPLX( ZERO ),
     $             L( M-N+1, N-K+1 ), LDA )
      CALL cclacpy( 'Lower', K, K, AF( M-K+1, N-K+1 ), LDA,
     $             L( M-K+1, N-K+1 ), LDA )
*
