...
*  =======
*
*  CCHKHP tests cchptrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $              cchpcon, cchprfs, CHPT01, cchptrf, cchptri, cchptrs,
     $                  cclacpy, CLAIPD, CLARHS, CLATB4, CLATMS, CPPT02,
     $                   CPPT03, CPPT05
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
  ...
  100             CONTINUE
               ...
               END IF
*
*              Check error code from cchptrf.
*
               IF( INFO.NE.K )
     $            CALL ALAERH( PATH, 'cchptrf', INFO, K, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
               IF( INFO.NE.0 ) THEN
                  TRFCON = .TRUE.
               ELSE
...
*
*              Check error code from cchptri.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cchptri', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'cchptrs'
                  CALL cchptrs( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                         INFO )
*
*              Check error code from cchptrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cchptrs', INFO, 0, UPLO, N, N,
...
*
                 SRNAMT = 'cchprfs'
                 CALL cchprfs( UPLO, N, NRHS, A, AFAC, IWORK, B, LDA, X,
     $                         LDA, RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from cchprfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cchprfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
  140          CONTINUE
               ...
               SRNAMT = 'cchpcon'
               CALL cchpcon( UPLO, N, AFAC, IWORK, ANORM, RCOND, WORK,
     $                      INFO )
*
*              Check error code from cchpcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'cchpcon', INFO, 0, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
