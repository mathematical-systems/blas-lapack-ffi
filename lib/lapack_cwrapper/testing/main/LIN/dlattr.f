...
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION ANORM,BIGNUM,BNORM,BSCAL,C,CNDNUM,PLUS1,PLUS2,RA,
     +                 RB,REXP,S,SFAC,SMLNUM,STAR1,TEXP,TLEFT,TSCAL,ULP,
     +                 UNFL,X,Y,Z
*     ..
*     .. External Functions ..
      LOGICAL LSAME
...
*     .. External Subroutines ..
      EXTERNAL DCOPY,cdlabad,cdlarnv,DLATB4,DLATMS,DROT,DROTG,DSCAL,
     +         DSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
                  IF (N.GT.J+1) CALL DROT(N-J-1,A(J,J+2),LDA,A(J+1,J+2),
     +                                    LDA,C,S)
*
...
*              Multiply by [ c -s;  s  c] on the right.
*
                  IF (N.GT.J+1) CALL DROT(N-J-1,A(J+2,J+1),1,A(J+2,J),1,
     +                                    C,-S)
*
*              Multiply by [-c  s; -s -c] on the left.
*
                  IF (J.GT.1) CALL DROT(J-1,A(J,1),LDA,A(J+1,1),LDA,-C,
     +                                  S)
*
*              Negate A(J+1,J).
*
...
*
          IF (UPPER) THEN
              DO 140 J = 1,N
                  CALL cdlarnv(2,ISEED,J,A(1,J))
                  A(J,J) = SIGN(TWO,A(J,J))
  140         CONTINUE
          ELSE
              DO 150 J = 1,N
                  CALL cdlarnv(2,ISEED,N-J+1,A(J,J))
                  A(J,J) = SIGN(TWO,A(J,J))
  150         CONTINUE
          END IF
*
...
*
          IF (UPPER) THEN
              DO 360 J = 1,N
                  CALL cdlarnv(2,ISEED,J-1,A(1,J))
                  A(J,J) = ZERO
  360         CONTINUE
          ELSE
              DO 370 J = 1,N
                  IF (J.LT.N) CALL cdlarnv(2,ISEED,N-J,A(J+1,J))
                  A(J,J) = ZERO
  370         CONTINUE
          END IF
*
...
*        1/3/91:  cdlatrs no longer can handle this case
*
