...
*  =======
*
*  ZCHKHP tests czhptrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $              czhpcon, czhprfs, ZHPT01, czhptrf, czhptri, czhptrs,
     $                  czlacpy, ZLAIPD, ZLARHS, ZLATB4, ZLATMS, ZPPT02,
     $                   ZPPT03, ZPPT05
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
  ...
  100             CONTINUE
               ...
               END IF
*
*              Check error code from czhptrf.
*
               IF( INFO.NE.K )
     $            CALL ALAERH( PATH, 'czhptrf', INFO, K, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
               IF( INFO.NE.0 ) THEN
                  TRFCON = .TRUE.
               ELSE
...
*
*              Check error code from czhptri.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czhptri', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'czhptrs'
                  CALL czhptrs( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                         INFO )
*
*              Check error code from czhptrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czhptrs', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'czhprfs'
                 CALL czhprfs( UPLO, N, NRHS, A, AFAC, IWORK, B, LDA, X,
     $                         LDA, RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from czhprfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czhprfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
  140          CONTINUE
               ...
               SRNAMT = 'czhpcon'
               CALL czhpcon( UPLO, N, AFAC, IWORK, ANORM, RCOND, WORK,
     $                      INFO )
*
*              Check error code from czhpcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'czhpcon', INFO, 0, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
