...
*  =======
*
*  CDRVLS tests the least squares driver routines ccgels, ccgelsX, ccgelsS,
*  ccgelsY and ccgelsD.
*
*  Arguments
*  =========
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASVM, CERRLS, ccgels, ccgelsD,
     $               ccgelsS, ccgelsX, ccgelsY, CGEMM, cclacpy, cclarnv,
     $                   CQRT13, CQRT15, CQRT16, CSSCAL, SAXPY, cslasrT,
     $                   XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                              IF( NCOLS.GT.0 ) THEN
                                 CALL cclarnv( 2, ISEED, NCOLS*NRHS,
     $                                        WORK )
     ...
     $                                    WORK, LDWORK, CZERO, B, LDB )
                              CALL cclacpy( 'Full', NROWS, NRHS, B, LDB,
     $                                     COPYB, LDB )
*
*                             Solve LS or overdetermined system
*
                              IF( M.GT.0 .AND. N.GT.0 ) THEN
                                 CALL cclacpy( 'Full', M, N, COPYA, LDA,
     $                                        A, LDA )
                                 CALL cclacpy( 'Full', NROWS, NRHS,
     $                                        COPYB, LDB, B, LDB )
                              END IF
                              SRNAMT = 'ccgels '
                              CALL ccgels( TRANS, M, N, NRHS, A, LDA, B,
     $                                    LDB, WORK, LWORK, INFO )
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'ccgels ', INFO, 0,
     $                                        TRANS, M, N, NRHS, -1, NB,
     $                                        ITYPE, NFAIL, NERRS,
     $                                        NOUT )
*
...
*                    Test ccgelsX
*
*                    ccgelsX:  Compute the minimum-norm solution X
*                    to min( norm( A * X - B ) )
*                    using a complete orthogonal factorization.
*
                     CALL cclacpy( 'Full', M, N, COPYA, LDA, A, LDA )
                     CALL cclacpy( 'Full', M, NRHS, COPYB, LDB, B, LDB )
*
                     SRNAMT = 'ccgelsX'
                     CALL ccgelsX( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                            RCOND, CRANK, WORK, RWORK, INFO )
*
                     IF( INFO.NE.0 )
     $                 CALL ALAERH( PATH, 'ccgelsX', INFO, 0, ' ', M, N,
...
*                             workspace:  M*NRHS + M
*
                     CALL cclacpy( 'Full', M, NRHS, COPYB, LDB, WORK,
     $                            LDWORK )
...
*                       Test ccgelsY
*
*                       ccgelsY:  Compute the minimum-norm solution
*                       X to min( norm( A * X - B ) )
*                       using the rank-revealing orthogonal
*                       factorization.
*
                        CALL cclacpy( 'Full', M, N, COPYA, LDA, A, LDA )
                        CALL cclacpy( 'Full', M, NRHS, COPYB, LDB, B,
     $                               LDB )
*
...
*
                        SRNAMT = 'ccgelsY'
                        CALL ccgelsY( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                               RCOND, CRANK, WORK, LWLSY, RWORK,
     $                               INFO )
                        IF( INFO.NE.0 )
     $                    CALL ALAERH( PATH, 'ccgelsY', INFO, 0, ' ', M,
...
*                       Test ccgelsS
*
*                       ccgelsS:  Compute the minimum-norm solution
*                       X to min( norm( A * X - B ) )
*                       using the SVD.
*
                        CALL cclacpy( 'Full', M, N, COPYA, LDA, A, LDA )
     ...
     $                               LDB )
                        SRNAMT = 'ccgelsS'
                        CALL ccgelsS( M, N, NRHS, A, LDA, B, LDB, S,
     $                               RCOND, CRANK, WORK, LWORK, RWORK,
     $                               INFO )
*
                        IF( INFO.NE.0 )
     $                    CALL ALAERH( PATH, 'ccgelsS', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
...
*                       Test ccgelsD
*
*                       ccgelsD:  Compute the minimum-norm solution X
...
*
                        SRNAMT = 'ccgelsD'
                        CALL ccgelsD( M, N, NRHS, A, LDA, B, LDB, S,
     $                               RCOND, CRANK, WORK, LWORK, RWORK,
     $                               IWORK, INFO )
                        IF( INFO.NE.0 )
     $                    CALL ALAERH( PATH, 'ccgelsD', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
