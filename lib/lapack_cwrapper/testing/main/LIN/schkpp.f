...
*  =======
*
*  SCHKPP tests cspptrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3        PATH
      INTEGER            I, IMAT, IN, INFO, IOFF, IRHS, IUPLO, IZERO, 
     $  K, 
...
*     .. External Subroutines ..
     ...
     $                   cslacpy, SLARHS, SLATB4, SLATMS, csppcon, 
     $  cspprfs, 
     $                   SPPT01, SPPT02, SPPT03, SPPT05, cspptrf, 
     $  cspptri, 
     $                   cspptrs
*     ..
*     .. Scalars in Common ..
...
*
*              Check error code from cspptrf.
*
               IF( INFO.NE.IZERO ) THEN
                  CALL ALAERH( PATH, 'cspptrf', INFO, IZERO, UPLO, N, 
     $  N, 
...
*
               CALL SCOPY( NPP, AFAC, 1, AINV, 1 )
               SRNAMT = 'cspptri'
               CALL cspptri( UPLO, N, AINV, INFO )
*
*              Check error code from cspptri.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'cspptri', INFO, 0, UPLO, N, N, 
     $  -1, 
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
...
*
                  SRNAMT = 'cspptrs'
                  CALL cspptrs( UPLO, N, NRHS, AFAC, X, LDA, INFO )
*
*              Check error code from cspptrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cspptrs', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'cspprfs'
                  CALL cspprfs( UPLO, N, NRHS, A, AFAC, B, LDA, X, LDA,
     $                         RWORK, RWORK( NRHS+1 ), WORK, IWORK,
     $                         INFO )
*
*              Check error code from cspprfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cspprfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL SGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
               ...
               SRNAMT = 'csppcon'
               CALL csppcon( UPLO, N, AFAC, ANORM, RCOND, WORK, IWORK,
     $                      INFO )
*
*              Check error code from csppcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'csppcon', INFO, 0, UPLO, N, N, 
     $  -1, 
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
