...
*  =======
*
*  SDRVLS tests the least squares driver routines csgels, csgelsS, csgelsX,
*  csgelsY and csgelsD.
*
*  Arguments
*  =========
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASVM, SAXPY, SERRLS, csgels,
     $                   csgelsD, csgelsS, csgelsX, csgelsY, SGEMM, 
     $  cslacpy, 
     $                   cslarnv, cslasrT, SQRT13, SQRT15, SQRT16, 
     $  SSCAL, 
     $                   XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
                              IF( NCOLS.GT.0 ) THEN
                                 CALL cslarnv( 2, ISEED, NCOLS*NRHS,
     $                                        WORK )
     ...
     $                                    WORK, LDWORK, ZERO, B, LDB )
                              CALL cslacpy( 'Full', NROWS, NRHS, B, 
     $  LDB, 
     $                                     COPYB, LDB )
*
...
*
                              IF( M.GT.0 .AND. N.GT.0 ) THEN
                                 CALL cslacpy( 'Full', M, N, COPYA, 
     $  LDA, 
     $                                        A, LDA )
                                 CALL cslacpy( 'Full', NROWS, NRHS,
     $                                        COPYB, LDB, B, LDB )
                              END IF
                              SRNAMT = 'csgels '
                              CALL csgels( TRANS, M, N, NRHS, A, LDA, 
     $  B, 
     $                                    LDB, WORK, LWORK, INFO )
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'csgels ', INFO, 0,
     $                                        TRANS, M, N, NRHS, -1, 
     $  NB, 
     $                                        ITYPE, NFAIL, NERRS,
     $                                        NOUT )
...
*                    Test csgelsX
*
*                    csgelsX:  Compute the minimum-norm solution X
*                    to min( norm( A * X - B ) ) using a complete
*                    orthogonal factorization.
*
                     CALL cslacpy( 'Full', M, N, COPYA, LDA, A, LDA )
                     CALL cslacpy( 'Full', M, NRHS, COPYB, LDB, B, LDB 
     $  ) 
*
                     SRNAMT = 'csgelsX'
                     CALL csgelsX( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                            RCOND, CRANK, WORK, INFO )
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'csgelsX', INFO, 0, ' ', M, 
     $  N, 
...
*                             workspace:  M*NRHS + M
*
                     CALL cslacpy( 'Full', M, NRHS, COPYB, LDB, WORK,
     $                            LDWORK )
...
*                       Test csgelsY
*
*                       csgelsY:  Compute the minimum-norm solution X
*                       to min( norm( A * X - B ) )
...
*
                        CALL cslacpy( 'Full', M, N, COPYA, LDA, A, LDA 
     $  ) 
                        CALL cslacpy( 'Full', M, NRHS, COPYB, LDB, B,
     $                               LDB )
*
                        SRNAMT = 'csgelsY'
                        CALL csgelsY( M, N, NRHS, A, LDA, B, LDB, 
     $  IWORK, 
     $                               RCOND, CRANK, WORK, LWLSY, INFO )
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'csgelsY', INFO, 0, ' ', 
     $  M, 
...
*                                workspace:  M*NRHS + M
*
                        CALL cslacpy( 'Full', M, NRHS, COPYB, LDB, 
     $  WORK, 
     $                               LDWORK )
                        CALL SQRT16( 'No transpose', M, N, NRHS, COPYA,
     $                               LDA, B, LDB, WORK, LDWORK,
...
*                       Test csgelsS
*
*                       csgelsS:  Compute the minimum-norm solution X
*                       to min( norm( A * X - B ) )
*                       using the SVD.
*
                        CALL cslacpy( 'Full', M, N, COPYA, LDA, A, LDA 
     ...
     $                               LDB )
                        SRNAMT = 'csgelsS'
                        CALL csgelsS( M, N, NRHS, A, LDA, B, LDB, S,
     $                               RCOND, CRANK, WORK, LWORK, INFO )
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'csgelsS', INFO, 0, ' ', 
     $  M, 
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
...
*                       Test csgelsD
*
*                       csgelsD:  Compute the minimum-norm solution X
...
*
                        SRNAMT = 'csgelsD'
                        CALL csgelsD( M, N, NRHS, A, LDA, B, LDB, S,
     $                               RCOND, CRANK, WORK, LWORK, IWORK,
     $                               INFO )
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'csgelsD', INFO, 0, ' ', 
     $  M, 
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
