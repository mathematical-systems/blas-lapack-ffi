...
*  =======
*
*  ZDRVLS tests the least squares driver routines czgels, ccgelsX, ccgelsS,
*  czgelsY and ccgelsD.
*
*  Arguments
*  =========
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASVM, DAXPY, cdlasrT, XLAENV,
     $                ZDSCAL, ZERRLS, czgels, czgelsD, czgelsS, czgelsX,
     $                 czgelsY, ZGEMM, czlacpy, czlarnv, ZQRT13, ZQRT15,
     $                   ZQRT16
*     ..
*     .. Intrinsic Functions ..
...
*
                              IF( NCOLS.GT.0 ) THEN
                                 CALL czlarnv( 2, ISEED, NCOLS*NRHS,
     $                                        WORK )
     ...
     $                                    WORK, LDWORK, CZERO, B, LDB )
                              CALL czlacpy( 'Full', NROWS, NRHS, B, LDB,
     $                                     COPYB, LDB )
*
*                             Solve LS or overdetermined system
*
                              IF( M.GT.0 .AND. N.GT.0 ) THEN
                                 CALL czlacpy( 'Full', M, N, COPYA, LDA,
     $                                        A, LDA )
                                 CALL czlacpy( 'Full', NROWS, NRHS,
     $                                        COPYB, LDB, B, LDB )
                              END IF
                              SRNAMT = 'czgels '
                              CALL czgels( TRANS, M, N, NRHS, A, LDA, B,
     $                                    LDB, WORK, LWORK, INFO )
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'czgels ', INFO, 0,
     $                                        TRANS, M, N, NRHS, -1, NB,
     $                                        ITYPE, NFAIL, NERRS,
     $                                        NOUT )
*
...
*                    Test czgelsX
*
*                    czgelsX:  Compute the minimum-norm solution X
*                    to min( norm( A * X - B ) )
*                    using a complete orthogonal factorization.
*
                     CALL czlacpy( 'Full', M, N, COPYA, LDA, A, LDA )
                     CALL czlacpy( 'Full', M, NRHS, COPYB, LDB, B, LDB )
*
                     SRNAMT = 'czgelsX'
                     CALL czgelsX( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                            RCOND, CRANK, WORK, RWORK, INFO )
*
                     IF( INFO.NE.0 )
     $                 CALL ALAERH( PATH, 'czgelsX', INFO, 0, ' ', M, N,
...
*                             workspace:  M*NRHS + M
*
                     CALL czlacpy( 'Full', M, NRHS, COPYB, LDB, WORK,
     $                            LDWORK )
...
*                       Test czgelsY
*
*                       czgelsY:  Compute the minimum-norm solution
*                       X to min( norm( A * X - B ) )
*                       using the rank-revealing orthogonal
*                       factorization.
*
                        CALL czlacpy( 'Full', M, N, COPYA, LDA, A, LDA )
                        CALL czlacpy( 'Full', M, NRHS, COPYB, LDB, B,
     $                               LDB )
*
...
*
                        SRNAMT = 'czgelsY'
                        CALL czgelsY( M, N, NRHS, A, LDA, B, LDB, IWORK,
     $                               RCOND, CRANK, WORK, LWLSY, RWORK,
     $                               INFO )
                        IF( INFO.NE.0 )
     $                    CALL ALAERH( PATH, 'czgelsY', INFO, 0, ' ', M,
...
*                       Test czgelsS
*
*                       czgelsS:  Compute the minimum-norm solution
*                       X to min( norm( A * X - B ) )
*                       using the SVD.
*
                        CALL czlacpy( 'Full', M, N, COPYA, LDA, A, LDA )
     ...
     $                               LDB )
                        SRNAMT = 'czgelsS'
                        CALL czgelsS( M, N, NRHS, A, LDA, B, LDB, S,
     $                               RCOND, CRANK, WORK, LWORK, RWORK,
     $                               INFO )
*
                        IF( INFO.NE.0 )
     $                    CALL ALAERH( PATH, 'czgelsS', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
...
*                       Test czgelsD
*
*                       czgelsD:  Compute the minimum-norm solution X
...
*
                        SRNAMT = 'czgelsD'
                        CALL czgelsD( M, N, NRHS, A, LDA, B, LDB, S,
     $                               RCOND, CRANK, WORK, LWORK, RWORK,
     $                               IWORK, INFO )
                        IF( INFO.NE.0 )
     $                    CALL ALAERH( PATH, 'czgelsD', INFO, 0, ' ', M,
     $                                  N, NRHS, -1, NB, ITYPE, NFAIL,
     $                                  NERRS, NOUT )
*
