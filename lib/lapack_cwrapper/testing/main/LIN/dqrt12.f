...
*     .. External Subroutines ..
      EXTERNAL DAXPY,dbdsqr,dgebd2,dlabad,dlascl,dlaset,xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     +                 WORK(M*N+4*MN+1),INFO)
          CALL dbdsqr('Upper',MN,0,0,0,WORK(M*N+1),WORK(M*N+MN+1),
     +                 DUMMY,MN,DUMMY,1,DUMMY,MN,WORK(M*N+2*MN+1),INFO)
*
          IF (ISCL.EQ.1) THEN
              IF (ANRM.GT.BIGNUM) THEN
                  CALL dlascl('G',0,0,BIGNUM,ANRM,MN,1,WORK(M*N+1),MN,
     +                         INFO)
              END IF
              IF (ANRM.LT.SMLNUM) THEN
                  CALL dlascl('G',0,0,SMLNUM,ANRM,MN,1,WORK(M*N+1),MN,
     +                         INFO)
              END IF
          END IF
*
