...
*     .. Array Arguments ..
      ...
      DOUBLE PRECISION A(*),AFAC(*),B(*),RWORK(*),WORK(*),X(*),XACT(*)
*     ..
*
*  Purpose
*  =======
*
*  DCHKGB tests cdgbtrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,I1,I2,IKL,IKU,IM,IMAT,IN,INB,INFO,IOFF,IRHS,ITRAN,IZERO,
     +        J,K,KL,KOFF,KU,LDA,LDAFAC,LDB,M,MODE,N,NB,NERRS,NFAIL,
     +        NIMAT,NKL,NKU,NRHS,NRUN
      DOUBLE PRECISION AINVNM,ANORM,ANORMI,ANORMO,CNDNUM,RCOND,RCONDC,
     +                 RCONDI,RCONDO
*     ..
*     .. Local Arrays ..
...
*     .. External Subroutines ..
      EXTERNAL ALAERH,ALAHD,ALASUM,DCOPY,DERRGE,cdgbcon,cdgbrfs,DGBT01,
     +         DGBT02,DGBT05,cdgbtrf,cdgbtrs,DGET04,cdlacpy,DLARHS,
     +         cdlaset,DLATB4,DLATMS,XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX,MIN
*     ..
...
*
                              IF (INFO.NE.0) THEN
                                  CALL ALAERH(PATH,'DLATMS',INFO,0,' ',
     +                                        M,N,KL,KU,-1,IMAT,NFAIL,
     +                                        NERRS,NOUT)
                                  GO TO 120
                              END IF
...
*
                              IF (M.GT.0 .AND. N.GT.
     +                            0) CALL cdlacpy('Full',KL+KU+1,N,A,
     +                            LDA,AFAC(KL+1),LDAFAC)
                              SRNAMT = 'cdgbtrf'
                              CALL cdgbtrf(M,N,KL,KU,AFAC,LDAFAC,IWORK,
     +                                     INFO)
*
*                       Check error code from cdgbtrf.
*
                              IF (INFO.NE.IZERO) CALL ALAERH(PATH,
     +                            'cdgbtrf',INFO,IZERO,' ',M,N,KL,KU,NB,
...
*
                                      SRNAMT = 'cdgbtrs'
                                      CALL cdgbtrs(TRANS,N,KL,KU,NRHS,
     +                                             AFAC,LDAFAC,IWORK,X,
     +                                             LDB,INFO)
*
*                             Check error code from cdgbtrs.
*
                                      IF (INFO.NE.0) CALL ALAERH(PATH,
     +                                    'cdgbtrs',INFO,0,TRANS,N,N,KL,
     +                                    KU,-1,IMAT,NFAIL,NERRS,NOUT)
*
                                      CALL cdlacpy('Full',N,NRHS,B,LDB,
     +                                             WORK,LDB)
                                      CALL DGBT02(TRANS,M,N,KL,KU,NRHS,
     +                                            A,LDA,X,LDB,WORK,LDB,
     +                                            RESULT(2))
*
*+    TEST 3:
...
*
                                      SRNAMT = 'cdgbrfs'
                                      CALL cdgbrfs(TRANS,N,KL,KU,NRHS,A,
     +                                             LDA,AFAC,LDAFAC,
     +                                             IWORK,B,LDB,X,LDB,
     +                                             RWORK,RWORK(NRHS+1),
     +                                             WORK,IWORK(N+1),INFO)
*
*                             Check error code from cdgbrfs.
*
                                      IF (INFO.NE.0) CALL ALAERH(PATH,
     +                                    'cdgbrfs',INFO,0,TRANS,N,N,KL,
...
*
   90                         CONTINUE
                              DO 100 ITRAN = 1,2
                                  ...
                                  END IF
                                  SRNAMT = 'cdgbcon'
                                  CALL cdgbcon(NORM,N,KL,KU,AFAC,LDAFAC,
     +                                         IWORK,ANORM,RCOND,WORK,
     +                                         IWORK(N+1),INFO)
*
*                             Check error code from cdgbcon.
*
                                  IF (INFO.NE.0) CALL ALAERH(PATH,
     +                                'cdgbcon',INFO,0,NORM,N,N,KL,KU,
...
*
 9999 FORMAT (' *** In DCHKGB, LA=',I5,' is too small for M=',I5,', N=',
 ...
 9995 FORMAT (' NORM =''',A1,''', N=',I5,', KL=',I5,', KU=',I5,',',10X,
     +       ' type ',I1,', test(',I1,')=',G12.5)
*
      RETURN
*
