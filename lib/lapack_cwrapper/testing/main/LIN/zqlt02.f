...
*  =======
*
*  ZQLT02 tests czungql, which generates an m-by-n matrix Q with
...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, ZHERK, czlacpy, czlaset, czungql
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL czlaset( 'Full', M, N, ROGUE, ROGUE, Q, LDA )
      IF( K.LT.M )
...
*
      SRNAMT = 'czungql'
      CALL czungql( M, N, K, Q, LDA, TAU( N-K+1 ), WORK, LWORK, INFO )
*
*     Copy L(m-n+1:m,n-k+1:n)
*
      CALL czlaset( 'Full', N, K, DCMPLX( ZERO ), DCMPLX( ZERO ),
     $             L( M-N+1, N-K+1 ), LDA )
      CALL czlacpy( 'Lower', K, K, AF( M-K+1, N-K+1 ), LDA,
     $             L( M-K+1, N-K+1 ), LDA )
*
