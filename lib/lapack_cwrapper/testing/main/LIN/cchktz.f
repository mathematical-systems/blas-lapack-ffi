...
*  =======
*
*  CCHKTZ tests cctzrqf and cctzrzf.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL         ALAHD, ALASUM, CERRTZ, ccgeqr2, cclacpy, cclaset,
     $                   CLATMS, cctzrqf, cctzrzf, SLAORD
*     ..
*     .. Intrinsic Functions ..
...
*
                  IF( MODE.EQ.0 ) THEN
                     CALL cclaset( 'Full', M, N, CMPLX( ZERO ),
     $                            CMPLX( ZERO ), A, LDA )
                     DO 20 I = 1, MNMIN
                        COPYS( I ) = ZERO
   20                CONTINUE
     ...
     $                            LDA, WORK, INFO )
                     CALL ccgeqr2( M, N, A, LDA, WORK, WORK( MNMIN+1 ),
     $                            INFO )
                     CALL cclaset( 'Lower', M-1, N, CMPLX( ZERO ),
     $                            CMPLX( ZERO ), A( 2 ), LDA )
...
*
                  CALL cclacpy( 'All', M, N, A, LDA, COPYA, LDA )
*
*                 Call cctzrqf to reduce the upper trapezoidal matrix to
*                 upper triangular form.
*
                  SRNAMT = 'cctzrqf'
                  CALL cctzrqf( M, N, A, LDA, TAU, INFO )
*
...
*
                  CALL cclacpy( 'All', M, N, A, LDA, COPYA, LDA )
*
*                 Call cctzrzf to reduce the upper trapezoidal matrix to
*                 upper triangular form.
*
                  SRNAMT = 'cctzrzf'
                  CALL cctzrzf( M, N, A, LDA, TAU, WORK, LWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
