...
*     .. External Subroutines ..
      EXTERNAL cdgelq2,cdgeqr2,cdlacpy,cdlascl,xerbla
*     ..
*     .. Intrinsic Functions ..
...
*     Copy and scale A
*
      CALL cdlacpy('All',M,N,A,LDA,WORK,LDWORK)
      ANRM = DLANGE('M',M,N,WORK,LDWORK,RWORK)
      IF (ANRM.NE.ZERO) CALL cdlascl('G',0,0,ANRM,ONE,M,N,WORK,LDWORK,
     +                               INFO)
*
...
*
          CALL cdlacpy('All',M,NRHS,X,LDX,WORK(N*LDWORK+1),LDWORK)
          XNRM = DLANGE('M',M,NRHS,WORK(N*LDWORK+1),LDWORK,RWORK)
          IF (XNRM.NE.ZERO) CALL cdlascl('G',0,0,XNRM,ONE,M,NRHS,
     +                           WORK(N*LDWORK+1),LDWORK,INFO)
          ANRM = DLANGE('One-norm',M,N+NRHS,WORK,LDWORK,RWORK)
*
*        Compute QR factorization of X
*
          CALL cdgeqr2(M,N+NRHS,WORK,LDWORK,WORK(LDWORK* (N+NRHS)+1),
...
*
          ...
          IF (XNRM.NE.ZERO) CALL cdlascl('G',0,0,XNRM,ONE,NRHS,N,
     +                           WORK(M+1),LDWORK,INFO)
*
*        Compute LQ factorization of work
*
          CALL cdgelq2(LDWORK,N,WORK,LDWORK,WORK(LDWORK*N+1),
