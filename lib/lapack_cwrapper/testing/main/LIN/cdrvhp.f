...
*  =======
*
*  CDRVHP tests the driver routines cchpsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $               cchpsv, cchpsvX, CHPT01, cchptrf, cchptri, cclacpy,
     $                  CLAIPD, CLARHS, cclaset, CLATB4, CLATMS, CPPT02,
     $                   CPPT05, XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL cchptrf( UPLO, N, AFAC, IWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL CCOPY( NPP, AFAC, 1, AINV, 1 )
                     CALL cchptri( UPLO, N, AINV, IWORK, WORK, INFO )
...
*
*                    Factor the matrix and solve the system using cchpsv.
*
                     SRNAMT = 'cchpsv '
                     CALL cchpsv( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                           INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from cchpsv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'cchpsv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'cchpsv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using cchpsvX.
*
                  SRNAMT = 'cchpsvX'
                  CALL cchpsvX( FACT, UPLO, N, NRHS, A, AFAC, IWORK, B,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from cchpsvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'cchpsvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from cchpsvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'cchpsvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
