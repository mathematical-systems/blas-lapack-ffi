...
*     .. Local Scalars ..
      ...
      DOUBLE PRECISION ANORM,BIGNUM,BNORM,BSCAL,CNDNUM,PLUS1,PLUS2,REXP,
     +                 SFAC,SMLNUM,STAR1,TEXP,TLEFT,TNORM,TSCAL,ULP,UNFL
*     ..
*     .. External Functions ..
      LOGICAL LSAME
      INTEGER IDAMAX
...
*     .. External Subroutines ..
      EXTERNAL DCOPY,cdlabad,cdlarnv,DLATB4,DLATMS,DSCAL,DSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
          IF (KD.EQ.1) THEN
              IF (UPPER) THEN
                  ...
                  LENJ = (N-3)/2
                  CALL cdlarnv(2,ISEED,LENJ,WORK)
                  DO 90 J = 1,LENJ
...
*
          IF (UPPER) THEN
              DO 120 J = 1,N
                  LENJ = MIN(J,KD+1)
                  CALL cdlarnv(2,ISEED,LENJ,AB(KD+2-LENJ,J))
                  AB(KD+1,J) = SIGN(TWO,AB(KD+1,J))
  120         CONTINUE
          ELSE
              DO 130 J = 1,N
                  LENJ = MIN(N-J+1,KD+1)
                  IF (LENJ.GT.0) CALL cdlarnv(2,ISEED,LENJ,AB(1,J))
...
*
  ...
  140         CONTINUE
          ...
          ELSE
              DO 150 J = 1,N
                  LENJ = MIN(N-J+1,KD+1)
                  CALL cdlarnv(2,ISEED,LENJ,AB(1,J))
...
*
          IF (UPPER) THEN
              DO 370 J = 1,N
                  LENJ = MIN(J-1,KD)
                  CALL cdlarnv(2,ISEED,LENJ,AB(KD+1-LENJ,J))
                  AB(KD+1,J) = DBLE(J)
  370         CONTINUE
          ELSE
              DO 380 J = 1,N
                  LENJ = MIN(N-J,KD)
                  IF (LENJ.GT.0) CALL cdlarnv(2,ISEED,LENJ,AB(2,J))
                  AB(1,J) = DBLE(J)
  380         CONTINUE
          END IF
*
      ...
      IF (.NOT.LSAME(TRANS,'N')) THEN
          IF (UPPER) THEN
              DO 430 J = 1,N/2
                  ...
                  CALL DSWAP(LENJ,AB(KD+1,J),LDAB-1,AB(KD+2-LENJ,N-J+1),
     +                       -1)
  430         CONTINUE
          ELSE
              DO 440 J = 1,N/2
