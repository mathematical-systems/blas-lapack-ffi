...
*     .. External Subroutines ..
      EXTERNAL           cclarnv, CLATB4, CLATMS, CROT, CROTG, CSSCAL,
     $                   cslabad, cslarnv
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( UPPER ) THEN
            JC = 1
            DO 180 J = 1, N
               CALL cclarnv( 4, ISEED, J-1, AP( JC ) )
  ...
  180       CONTINUE
         ELSE
            ...
            DO 190 J = 1, N
               IF( J.LT.N )
     $            CALL cclarnv( 4, ISEED, N-J, AP( JC+1 ) )
...
*
  ...
  200       CONTINUE
         ...
         ELSE
            JC = 1
            DO 210 J = 1, N
               CALL cclarnv( 2, ISEED, N-J, AP( JC+1 ) )
...
*
  ...
  340       CONTINUE
         ELSE
            JC = 1
            DO 350 J = 1, N
               CALL cclarnv( 4, ISEED, N-J+1, AP( JC ) )
               IF( J.NE.IY ) THEN
                  AP( JC ) = CLARND( 5, ISEED )*TWO
               ELSE
                  AP( JC ) = ZERO
...
*        1/3/91:  cclatps no longer can handle this case
*
         ...
         IF( UPPER ) THEN
            JC = 1
            DO 420 J = 1, N
               CALL cclarnv( 5, ISEED, J, AP( JC ) )
               CALL cslarnv( 1, ISEED, J, RWORK )
               DO 410 I = 1, J
  ...
  420       CONTINUE
         ELSE
            JC = 1
            DO 440 J = 1, N
               CALL cclarnv( 5, ISEED, N-J+1, AP( JC ) )
               CALL cslarnv( 1, ISEED, N-J+1, RWORK )
               DO 430 I = J, N
                  AP( JC+I-J ) = AP( JC+I-J )*
