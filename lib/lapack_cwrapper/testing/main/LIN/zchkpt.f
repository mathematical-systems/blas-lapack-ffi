...
*  =======
*
*  ZCHKPT tests czpttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, DCOPY, cdlarnv, DSCAL,
     $                   ZCOPY, ZDSCAL, ZERRGT, ZGET04, czlacpy, ZLAPTM,
     $                czlarnv, ZLATB4, ZLATMS, czptcon, czptrfs, ZPTT01,
     $                   ZPTT02, ZPTT05, czpttrf, czpttrs
*     ..
*     .. Intrinsic Functions ..
...
*
            CALL czpttrf( N, D( N+1 ), E( N+1 ), INFO )
*
*           Check error code from czpttrf.
*
            IF( INFO.NE.IZERO ) THEN
               CALL ALAERH( PATH, 'czpttrf', INFO, IZERO, ' ', N, N, -1,
...
*
*           Use czpttrs to solve for one column at a time of inv(A),
...
*
   ...
   40          CONTINUE
               X( I ) = ONE
               CALL czpttrs( 'Lower', N, 1, D( N+1 ), E( N+1 ), X, LDA,
     $                      INFO )
...
*
                  ...
                  CALL czpttrs( UPLO, N, NRHS, D( N+1 ), E( N+1 ), X,
     $                         LDA, INFO )
*
*              Check error code from czpttrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czpttrs', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'czptrfs'
                  CALL czptrfs( UPLO, N, NRHS, D, E, D( N+1 ), E( N+1 ),
     $                         B, LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                         WORK, RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from czptrfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czptrfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
  100       CONTINUE
            SRNAMT = 'czptcon'
            CALL czptcon( N, D( N+1 ), E( N+1 ), ANORM, RCOND, RWORK,
     $                   INFO )
*
*           Check error code from czptcon.
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'czptcon', INFO, 0, ' ', N, N, -1, -1,
