...
*  =======
*
*  CCHKGE tests ccgetrf, -TRI, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALAERH, ALAHD, ALASUM, CERRGE, ccgecon, ccgerfs,
     $                  CGET01, CGET02, CGET03, CGET04, CGET07, ccgetrf,
     $               ccgetri, ccgetrs, cclacpy, CLARHS, cclaset, CLATB4,
     $                   CLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   20                CONTINUE
                  ELSE
                     CALL cclaset( 'Full', M, N-IZERO+1, CMPLX( ZERO ),
...
*
                  CALL cclacpy( 'Full', M, N, A, LDA, AFAC, LDA )
                  SRNAMT = 'ccgetrf'
                  CALL ccgetrf( M, N, AFAC, LDA, IWORK, INFO )
*
*                 Check error code from ccgetrf.
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'ccgetrf', INFO, IZERO, ' ', M,
...
*
                  CALL cclacpy( 'Full', M, N, AFAC, LDA, AINV, LDA )
...
*
                  IF( M.EQ.N .AND. INFO.EQ.0 ) THEN
                     CALL cclacpy( 'Full', N, N, AFAC, LDA, AINV, LDA )
                     SRNAMT = 'ccgetri'
                     NRHS = NSVAL( 1 )
                     LWORK = NMAX*MAX( 3, NRHS )
                     CALL ccgetri( N, AINV, LDA, IWORK, WORK, LWORK,
     $                            INFO )
*
*                    Check error code from ccgetri.
*
                     IF( INFO.NE.0 )
     $                 CALL ALAERH( PATH, 'ccgetri', INFO, 0, ' ', N, N,
...
*
                        ...
                        SRNAMT = 'ccgetrs'
                        CALL ccgetrs( TRANS, N, NRHS, AFAC, LDA, IWORK,
     $                               X, LDA, INFO )
*
*                       Check error code from ccgetrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'ccgetrs', INFO, 0, TRANS,
...
*
                        SRNAMT = 'ccgerfs'
                        CALL ccgerfs( TRANS, N, NRHS, A, LDA, AFAC, LDA,
     $                               IWORK, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                       Check error code from ccgerfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'ccgerfs', INFO, 0, TRANS,
     $                                  N, N, -1, -1, NRHS, IMAT, NFAIL,
     $                                  NERRS, NOUT )
*
                        CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
   70             CONTINUE
                  DO 80 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'ccgecon'
                     CALL ccgecon( NORM, N, AFAC, LDA, ANORM, RCOND,
     $                            WORK, RWORK, INFO )
*
*                       Check error code from ccgecon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'ccgecon', INFO, 0, NORM, N,
