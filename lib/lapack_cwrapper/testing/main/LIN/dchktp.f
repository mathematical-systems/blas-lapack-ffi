...
*  =======
*
*  DCHKTP tests cdtptri, -TRS, -RFS, and -CON, and cdlatps
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IDIAG,IMAT,IN,INFO,IRHS,ITRAN,IUPLO,K,LAP,LDA,N,NERRS,
     +        NFAIL,NRHS,NRUN
...
*     .. External Subroutines ..
     ...
     +         cdlatps,DLATTP,cdtpcon,cdtprfs,DTPT01,DTPT02,DTPT03,
     +         DTPT05,DTPT06,cdtptri,cdtptrs
*     ..
*     .. Scalars in Common ..
      LOGICAL LERR,OK
      CHARACTER*7 SRNAMT
...
*
                  ...
                  SRNAMT = 'cdtptri'
                  CALL cdtptri(UPLO,DIAG,N,AINVP,INFO)
*
*              Check error code from cdtptri.
*
                  IF (INFO.NE.0) CALL ALAERH(PATH,'cdtptri',INFO,0,
     +                                UPLO//DIAG,N,N,-1,-1,-1,IMAT,
     +                                NFAIL,NERRS,NOUT)
*
...
*
                  IF (RESULT(1).GE.THRESH) THEN
                      IF (NFAIL.EQ.0 .AND. NERRS.EQ.0) CALL ALAHD(NOUT,
     +                    PATH)
...
*
                          SRNAMT = 'cdtptrs'
                          CALL cdtptrs(UPLO,TRANS,DIAG,N,NRHS,AP,X,LDA,
     +                                 INFO)
*
*                 Check error code from cdtptrs.
*
                          IF (INFO.NE.0) CALL ALAERH(PATH,'cdtptrs',
     +                                        INFO,0,UPLO//TRANS//DIAG,
...
*
                          SRNAMT = 'cdtprfs'
                          CALL cdtprfs(UPLO,TRANS,DIAG,N,NRHS,AP,B,LDA,
     +                                 X,LDA,RWORK,RWORK(NRHS+1),WORK,
     +                                 IWORK,INFO)
*
*                 Check error code from cdtprfs.
*
                          IF (INFO.NE.0) CALL ALAERH(PATH,'cdtprfs',
     +                                        INFO,0,UPLO//TRANS//DIAG,
...
*
                      SRNAMT = 'cdtpcon'
                      CALL cdtpcon(NORM,UPLO,DIAG,N,AP,RCOND,WORK,IWORK,
     +                             INFO)
*
*                 Check error code from cdtpcon.
*
                      IF (INFO.NE.0) CALL ALAERH(PATH,'cdtpcon',INFO,0,
     +                                    NORM//UPLO//DIAG,N,N,-1,-1,-1,
...
*
     ...
     +                        0) CALL ALAHD(NOUT,PATH)
                          WRITE (NOUT,FMT=9997) 'cdtpcon',NORM,UPLO,
   ...
   70     CONTINUE
*
*        Use pathological test matrices to test cdlatps.
*
...
*
                      ...
                      CALL cdlatps(UPLO,TRANS,DIAG,'N',N,AP,B,SCALE,
     +                             RWORK,INFO)
*
*                 Check error code from cdlatps.
*
                      IF (INFO.NE.0) CALL ALAERH(PATH,'cdlatps',INFO,0,
     +                                    UPLO//TRANS//DIAG//'N',N,N,-1,
     +                                    -1,-1,IMAT,NFAIL,NERRS,NOUT)
*
                      CALL DTPT03(UPLO,TRANS,DIAG,N,1,AP,SCALE,RWORK,
...
*
                      ...
                      CALL cdlatps(UPLO,TRANS,DIAG,'Y',N,AP,B(N+1),
     +                             SCALE,RWORK,INFO)
*
*                 Check error code from cdlatps.
*
...
*
                      IF (RESULT(8).GE.THRESH) THEN
                          IF (NFAIL.EQ.0 .AND. NERRS.EQ.
     +                        0) CALL ALAHD(NOUT,PATH)
                          WRITE (NOUT,FMT=9996) 'cdlatps',UPLO,TRANS,
