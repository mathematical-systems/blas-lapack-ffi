...
*  =======
*
*  CCHKPT tests ccpttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                cclacpy, CLAPTM, cclarnv, CLATB4, CLATMS, ccptcon,
     $                ccptrfs, CPTT01, CPTT02, CPTT05, ccpttrf, ccpttrs,
     $                   CSSCAL, SCOPY, cslarnv, SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
                  CALL cslarnv( 2, ISEED, N, D )
                  CALL cclarnv( 2, ISEED, N-1, E )
*
...
*
            CALL ccpttrf( N, D( N+1 ), E( N+1 ), INFO )
*
*           Check error code from ccpttrf.
*
            IF( INFO.NE.IZERO ) THEN
               CALL ALAERH( PATH, 'ccpttrf', INFO, IZERO, ' ', N, N, -1,
...
*
*           Use ccpttrs to solve for one column at a time of inv(A),
...
*
   ...
   40          CONTINUE
               X( I ) = ONE
               CALL ccpttrs( 'Lower', N, 1, D( N+1 ), E( N+1 ), X, LDA,
     $                      INFO )
...
*
                  ...
                  CALL ccpttrs( UPLO, N, NRHS, D( N+1 ), E( N+1 ), X,
     $                         LDA, INFO )
*
*              Check error code from ccpttrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccpttrs', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'ccptrfs'
                  CALL ccptrfs( UPLO, N, NRHS, D, E, D( N+1 ), E( N+1 ),
     $                         B, LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                         WORK, RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from ccptrfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccptrfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
  100       CONTINUE
            SRNAMT = 'ccptcon'
            CALL ccptcon( N, D( N+1 ), E( N+1 ), ANORM, RCOND, RWORK,
     $                   INFO )
*
*           Check error code from ccptcon.
*
            IF( INFO.NE.0 )
     $         CALL ALAERH( PATH, 'ccptcon', INFO, 0, ' ', N, N, -1, -1,
