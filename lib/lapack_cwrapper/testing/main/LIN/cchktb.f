...
*  =======
*
*  CCHKTB tests cctbtrs, -RFS, and -CON, and cclatbs.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $               cclacpy, CLARHS, cclaset, cclatbs, CLATTB, cctbcon,
     $                   cctbrfs, CTBSV, CTBT02, CTBT03, CTBT05, CTBT06,
     $                   cctbtrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                        SRNAMT = 'cctbtrs'
                       CALL cctbtrs( UPLO, TRANS, DIAG, N, KD, NRHS, AB,
     $                               LDAB, X, LDA, INFO )
*
*                    Check error code from cctbtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cctbtrs', INFO, 0,
...
*
                        SRNAMT = 'cctbrfs'
                       CALL cctbrfs( UPLO, TRANS, DIAG, N, KD, NRHS, AB,
     $                               LDAB, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                    Check error code from cctbrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cctbrfs', INFO, 0,
     $                                  UPLO // TRANS // DIAG, N, N, KD,
     $                                  KD, NRHS, IMAT, NFAIL, NERRS,
     $                                  NOUT )
*
...
*
                  DO 70 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'cctbcon'
                     CALL cctbcon( NORM, UPLO, DIAG, N, KD, AB, LDAB,
     $                            RCOND, WORK, RWORK, INFO )
*
*                    Check error code from cctbcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cctbcon', INFO, 0,
...
*
                     IF( RESULT( 6 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 ) 'cctbcon', NORM, UPLO,
   ...
   90       CONTINUE
*
*           Use pathological test matrices to test cclatbs.
*
...
*
                     ...
                     CALL cclatbs( UPLO, TRANS, DIAG, 'N', N, KD, AB,
     $                            LDAB, B, SCALE, RWORK, INFO )
*
*                    Check error code from cclatbs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cclatbs', INFO, 0,
...
*
                     CALL CCOPY( N, X, 1, B, 1 )
                     CALL cclatbs( UPLO, TRANS, DIAG, 'Y', N, KD, AB,
     $                            LDAB, B, SCALE, RWORK, INFO )
*
*                    Check error code from cclatbs.
*
...
*
                     IF( RESULT( 7 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9997 )'cclatbs', UPLO, TRANS,
