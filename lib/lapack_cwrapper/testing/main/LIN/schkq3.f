...
*     .. External Subroutines ..
      EXTERNAL           ALAHD, ALASUM, ICOPY, csgeqp3, cslacpy, SLAORD,
     $                   cslaset, SLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*
   ...
   20          CONTINUE
               IF( IMODE.EQ.1 ) THEN
                  CALL cslaset( 'Full', M, N, ZERO, ZERO, COPYA, LDA )
                  DO 30 I = 1, MNMIN
                     COPYS( I ) = ZERO
   30             CONTINUE
               ELSE
...
*                 vector IWORK.
*
                  CALL cslacpy( 'All', M, N, COPYA, LDA, A, LDA )
...
*
                  SRNAMT = 'csgeqp3'
                  CALL csgeqp3( M, N, A, LDA, IWORK( N+1 ), TAU, WORK,
     $                         LW, INFO )
*
...
*
     ...
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 )'csgeqp3', M, N, NB,
     $                     IMODE, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
   50             CONTINUE
