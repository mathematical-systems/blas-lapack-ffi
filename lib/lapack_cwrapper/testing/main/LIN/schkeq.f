...
*  =======
*
*  SCHKEQ tests csgeequ, csgbequ, cspoequ, csppequ and cspbequ
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           csgbequ, csgeequ, cspbequ, cspoequ, csppequ
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   40       CONTINUE
*
            CALL csgeequ( M, N, A, NSZ, R, C, RCOND, CCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 1 ) = ONE
            ELSE
...
*
   ...
   90 CONTINUE
      CALL csgeequ( NSZ, NSZ, A, NSZ, R, C, RCOND, CCOND, NORM, INFO )
...
*
  ...
  150             CONTINUE
*
                  CALL csgbequ( M, N, KL, KU, AB, NSZB, R, C, RCOND,
     $                         CCOND, NORM, INFO )
*
                  IF( INFO.NE.0 ) THEN
...
*
  ...
  270    CONTINUE
*
         CALL cspoequ( N, A, NSZ, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 3 ) = ONE
         ELSE
  ...
  290 CONTINUE
      ...
      CALL cspoequ( NSZ, A, NSZ, R, RCOND, NORM, INFO )
      IF( INFO.NE.MAX( NSZ-1, 1 ) )
     $   RESLTS( 3 ) = ONE
...
*
  ...
  310    CONTINUE
*
         CALL csppequ( 'U', N, AP, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 4 ) = ONE
         ELSE
...
*
  ...
  340    CONTINUE
*
         CALL csppequ( 'L', N, AP, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 4 ) = ONE
         ELSE
...
*
  360 CONTINUE
      I = ( NSZ*( NSZ+1 ) ) / 2 - 2
      AP( I ) = -ONE
      CALL csppequ( 'L', NSZ, AP, R, RCOND, NORM, INFO )
      IF( INFO.NE.MAX( NSZ-1, 1 ) )
     $   RESLTS( 4 ) = ONE
...
*
  ...
  390       CONTINUE
*
            CALL cspbequ( 'U', N, KL, AB, NSZB, R, RCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 5 ) = ONE
            ELSE
...
*
  ...
  430       CONTINUE
*
            CALL cspbequ( 'L', N, KL, AB, NSZB, R, RCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 5 ) = ONE
            ELSE
 ...
 9998 FORMAT( ' csgeequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9997 FORMAT( ' csgbequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9996 FORMAT( ' cspoequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9995 FORMAT( ' csppequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9994 FORMAT( ' cspbequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
      RETURN
*
*     End of SCHKEQ
