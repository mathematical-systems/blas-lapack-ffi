...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, cclarnv, CLATB4, CLATMS, CROT, CROTG,
     $                   CSSCAL, CSWAP, cslabad, cslarnv
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( UPPER ) THEN
            DO 140 J = 1, N
               CALL cclarnv( 4, ISEED, J-1, A( 1, J ) )
               A( J, J ) = CLARND( 5, ISEED )*TWO
  140       CONTINUE
         ELSE
            DO 150 J = 1, N
               IF( J.LT.N )
     $            CALL cclarnv( 4, ISEED, N-J, A( J+1, J ) )
               A( J, J ) = CLARND( 5, ISEED )*TWO
  150       CONTINUE
         END IF
*
...
*        1/3/91:  cclatrs no longer can handle this case
*
         TLEFT = BIGNUM / MAX( ONE, REAL( N-1 ) )
         TSCAL = BIGNUM*( REAL( N-1 ) / MAX( ONE, REAL( N ) ) )
         IF( UPPER ) THEN
            DO 390 J = 1, N
               CALL cclarnv( 5, ISEED, J, A( 1, J ) )
  ...
  390       CONTINUE
         ELSE
            DO 410 J = 1, N
               CALL cclarnv( 5, ISEED, N-J+1, A( J, J ) )
