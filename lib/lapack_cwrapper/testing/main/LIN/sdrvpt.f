...
*     .. External Subroutines ..
     ...
     $                   cslacpy, SLAPTM, cslarnv, cslaset, SLATB4, 
     $  SLATMS, 
     $                   csptsv, csptsvX, SPTT01, SPTT02, SPTT05, 
     $  cspttrf, 
     $                   cspttrs, SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*                 Factor the matrix A.
*
                  CALL cspttrf( N, D( N+1 ), E( N+1 ), INFO )
*
*                 Use cspttrs to solve for one column at a time of
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL cspttrs( N, 1, D( N+1 ), E( N+1 ), X, LDA,
     $                            INFO )
...
*
                  SRNAMT = 'csptsv '
                  CALL csptsv( N, NRHS, D( N+1 ), E( N+1 ), X, LDA,
     $                        INFO )
*
*                 Check error code from csptsv .
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'csptsv ', INFO, IZERO, ' ', N,
...
*
     ...
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 )'csptsv ', N, IMAT, K,
     $                     RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
   70             CONTINUE
...
*              error bounds using csptsvX.
*
               SRNAMT = 'csptsvX'
               CALL csptsvX( FACT, N, NRHS, D, E, D( N+1 ), E( N+1 ), 
     $  B, 
     $                      LDA, X, LDA, RCOND, RWORK, RWORK( NRHS+1 ),
     $                      WORK, INFO )
*
*              Check the error code from csptsvX.
*
               IF( INFO.NE.IZERO )
     $            CALL ALAERH( PATH, 'csptsvX', INFO, IZERO, FACT, N, 
     $  N, 
...
*
                  CALL SPTT05( N, NRHS, D, E, B, LDA, X, LDA, XACT, 
     $  LDA, 
...
*
               DO 90 K = K1, 6
                  IF( RESULT( K ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALADHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9998 )'csptsvX', FACT, N, IMAT,
     $                  K, RESULT( K )
                     NFAIL = NFAIL + 1
                  END IF
   90          CONTINUE
