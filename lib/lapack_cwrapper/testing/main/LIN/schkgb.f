...
*  =======
*
*  SCHKGB tests csgbtrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, SCOPY, SERRGE, csgbcon,
     $                csgbrfs, SGBT01, SGBT02, SGBT05, csgbtrf, csgbtrs,
     $                 SGET04, cslacpy, SLARHS, cslaset, SLATB4, SLATMS,
     $                   XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
     ...
     $                                  AFAC( KL+1 ), LDAFAC )
                        SRNAMT = 'csgbtrf'
                        CALL csgbtrf( M, N, KL, KU, AFAC, LDAFAC, IWORK,
     $                               INFO )
*
*                       Check error code from csgbtrf.
*
                        IF( INFO.NE.IZERO )
     $                     CALL ALAERH( PATH, 'csgbtrf', INFO, IZERO,
     $                                  ' ', M, N, KL, KU, NB, IMAT,
     $                                  NFAIL, NERRS, NOUT )
                        TRFCON = .FALSE.
*
...
*
                              SRNAMT = 'csgbtrs'
                             CALL csgbtrs( TRANS, N, KL, KU, NRHS, AFAC,
     $                                     LDAFAC, IWORK, X, LDB, INFO )
*
*                             Check error code from csgbtrs.
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'csgbtrs', INFO, 0,
     $                                        TRANS, N, N, KL, KU, -1,
...
*
                              SRNAMT = 'csgbrfs'
                              CALL csgbrfs( TRANS, N, KL, KU, NRHS, A,
     $                                     LDA, AFAC, LDAFAC, IWORK, B,
     $                                     LDB, X, LDB, RWORK,
     $                                     RWORK( NRHS+1 ), WORK,
     $                                     IWORK( N+1 ), INFO )
*
*                             Check error code from csgbrfs.
*
                              IF( INFO.NE.0 )
     $                           CALL ALAERH( PATH, 'csgbrfs', INFO, 0,
...
*
   90                   CONTINUE
                        DO 100 ITRAN = 1, 2
                           ...
                           END IF
                           SRNAMT = 'csgbcon'
                           CALL csgbcon( NORM, N, KL, KU, AFAC, LDAFAC,
     $                                  IWORK, ANORM, RCOND, WORK,
     $                                  IWORK( N+1 ), INFO )
*
*                             Check error code from csgbcon.
*
                           IF( INFO.NE.0 )
     $                        CALL ALAERH( PATH, 'csgbcon', INFO, 0,
