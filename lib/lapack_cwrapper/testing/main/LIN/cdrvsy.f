...
*  =======
*
*  CDRVSY tests the driver routines ccsysv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                  CLARHS, cclaset, CLATB4, CLATMS, CLATSY, CPOT05,
     $                ccsysv, ccsysvX, CSYT01, CSYT02, ccsytrf, ccsytri,
     $                   XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL ccsytrf( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL cclacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                     CALL ccsytri( UPLO, N, AINV, LDA, IWORK, WORK,
     $                            INFO )
...
*
*                    Factor the matrix and solve the system using ccsysv.
*
                     SRNAMT = 'ccsysv '
                     CALL ccsysv( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from ccsysv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'ccsysv ', INFO, K, UPLO, N,
...
*
                     CALL cclacpy( 'Full', N, NRHS, B, LDA, WORK, LDA )
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'ccsysv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using ccsysvX.
*
                  SRNAMT = 'ccsysvX'
                  CALL ccsysvX( FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from ccsysvX.
*
                  IF( INFO.NE.K ) THEN
                   CALL ALAERH( PATH, 'ccsysvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from ccsysvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'ccsysvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
