...
*  =======
*
*  SQLT03 tests csormql, which computes Q*C, Q'*C, C*Q or C*Q'.
*
*  SQLT03 compares the results of a call to csormql with the results of
*  forming Q explicitly by a call to csorgql and then performing matrix
...
*          returned by csgeqlf. See csgeqlf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, cslacpy, cslarnv, cslaset, csorgql, 
     $  csormql 
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
...
*
      SRNAMT = 'csorgql'
      CALL csorgql( M, M, K, Q, LDA, TAU( MINMN-K+1 ), WORK, LWORK,
     $             INFO )
*
      DO 30 ISIDE = 1, 2
         IF( ISIDE.EQ.1 ) THEN
...
*
            SRNAMT = 'csormql'
            IF( K.GT.0 )
     $         CALL csormql( SIDE, TRANS, MC, NC, K, AF( 1, N-K+1 ), 
     $  LDA, 
