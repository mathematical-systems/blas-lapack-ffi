...
*  =======
*
*  DDRVPP tests the driver routines cdppsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
     ...
     +        LDA,MODE,N,NERRS,NFACT,NFAIL,NIMAT,NPP,NRUN,NT
...
*     .. External Subroutines ..
      EXTERNAL ALADHD,ALAERH,ALASVM,DCOPY,DERRVX,DGET04,cdlacpy,cdlaqsp,
     +         DLARHS,cdlaset,DLATB4,DLATMS,cdppequ,cdppsv,cdppsvX,
     +         DPPT01,DPPT02,DPPT05,cdpptrf,cdpptri
*     ..
*     .. Scalars in Common ..
      LOGICAL LERR,OK
      CHARACTER*7 SRNAMT
...
*     .. Data statements ..
      ...
      DATA UPLOS/'U','L'/,FACTS/'F','N','E'/,PACKS/'C','R'/,EQUEDS/'N',
     +     'Y'/
*     ..
...
*                       the value returned by cdppsvX (FACT = 'N' reuses
...
*
                                  CALL cdppequ(UPLO,N,AFAC,S,SCOND,AMAX,
     +                                         INFO)
...
*
                                      CALL cdlaqsp(UPLO,N,AFAC,S,SCOND,
     +                                             AMAX,EQUED)
                                  END IF
                              END IF
*
...
*                       Factor the matrix A.
*
                              CALL cdpptrf(UPLO,N,AFAC,INFO)
*
*                       Form the inverse of A.
*
                              CALL DCOPY(NPP,AFAC,1,A,1)
                              CALL cdpptri(UPLO,N,A,INFO)
*
...
*
                              SRNAMT = 'cdppsv '
                              CALL cdppsv(UPLO,N,NRHS,AFAC,X,LDA,INFO)
*
*                       Check error code from cdppsv .
*
                              IF (INFO.NE.IZERO) THEN
                                  CALL ALAERH(PATH,'cdppsv ',INFO,IZERO,
...
*
                          IF (.NOT.PREFAC .AND.
     +                        NPP.GT.0) CALL cdlaset('Full',NPP,1,ZERO,
     +                        ZERO,AFAC,NPP)
...
*                       EQUED='Y'.
*
                              CALL cdlaqsp(UPLO,N,A,S,SCOND,AMAX,EQUED)
                          END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using cdppsvX.
*
                          SRNAMT = 'cdppsvX'
                          CALL cdppsvX(FACT,UPLO,N,NRHS,A,AFAC,EQUED,S,
     +                                 B,LDA,X,LDA,RCOND,RWORK,
     +                                 RWORK(NRHS+1),WORK,IWORK,INFO)
*
*                    Check the error code from cdppsvX.
*
                          IF (INFO.NE.IZERO) THEN
                              CALL ALAERH(PATH,'cdppsvX',INFO,IZERO,
...
*
     ...
     +                                    RESULT(4))
                          ...
                          END IF
*
*                    Compare RCOND from cdppsvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     +                                NERRS.EQ.0) CALL ALADHD(NOUT,PATH)
                                  IF (PREFAC) THEN
                                      WRITE (NOUT,FMT=9997) 'cdppsvX',
     +                                  FACT,UPLO,N,EQUED,IMAT,K,
     +                                  RESULT(K)
                                  ELSE
                                      WRITE (NOUT,FMT=9998) 'cdppsvX',
