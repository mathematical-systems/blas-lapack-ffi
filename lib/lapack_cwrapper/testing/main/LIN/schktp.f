...
*  =======
*
*  SCHKTP tests cstptri, -TRS, -RFS, and -CON, and cslatps
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                   cslacpy, SLARHS, cslatps, SLATTP, cstpcon, 
     $  cstprfs, 
     $                   STPT01, STPT02, STPT03, STPT05, STPT06, 
     $  cstptri, 
     $                   cstptrs
*     ..
*     .. Scalars in Common ..
...
*
     ...
     $            CALL SCOPY( LAP, AP, 1, AINVP, 1 )
               SRNAMT = 'cstptri'
               CALL cstptri( UPLO, DIAG, N, AINVP, INFO )
*
*              Check error code from cstptri.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'cstptri', INFO, 0, UPLO // DIAG, 
     $  N, 
...
*
                     SRNAMT = 'cstptrs'
                     CALL cstptrs( UPLO, TRANS, DIAG, N, NRHS, AP, X,
     $                            LDA, INFO )
*
*                 Check error code from cstptrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cstptrs', INFO, 0,
...
*
                     SRNAMT = 'cstprfs'
                     CALL cstprfs( UPLO, TRANS, DIAG, N, NRHS, AP, B,
     $                            LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                            WORK, IWORK, INFO )
*
*                 Check error code from cstprfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cstprfs', INFO, 0,
     $                               UPLO // TRANS // DIAG, N, N, -1,
...
*
                  SRNAMT = 'cstpcon'
                  CALL cstpcon( NORM, UPLO, DIAG, N, AP, RCOND, WORK,
     $                         IWORK, INFO )
*
*                 Check error code from cstpcon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cstpcon', INFO, 0,
...
*
                  IF( RESULT( 7 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9997 ) 'cstpcon', NORM, UPLO,
   ...
   70    CONTINUE
*
*        Use pathological test matrices to test cslatps.
*
...
*
                  SRNAMT = 'SLATTP'
                  CALL SLATTP( IMAT, UPLO, TRANS, DIAG, ISEED, N, AP, 
     $  X, 
     $                         WORK, INFO )
*
*+    TEST 8
...
*
                  ...
                  CALL cslatps( UPLO, TRANS, DIAG, 'N', N, AP, B, 
     $  SCALE, 
     $                         RWORK, INFO )
*
*                 Check error code from cslatps.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cslatps', INFO, 0,
...
*
                  ...
                  CALL cslatps( UPLO, TRANS, DIAG, 'Y', N, AP, B( N+1 
     $  ), 
     $                         SCALE, RWORK, INFO )
*
*                 Check error code from cslatps.
...
*
                  IF( RESULT( 8 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9996 )'cslatps', UPLO, TRANS,
