...
*  =======
*
*  ZDRVPP tests the driver routines czppsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                czlacpy, ZLAIPD, czlaqhp, ZLARHS, czlaset, ZLATB4,
     $                 ZLATMS, czppequ, czppsv, czppsvX, ZPPT01, ZPPT02,
     $                   ZPPT05, czpptrf, czpptri
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
      CHARACTER*7        SRNAMT
...
*                       the value returned by czppsvX (FACT = 'N' reuses
...
*
                           CALL czppequ( UPLO, N, AFAC, S, SCOND, AMAX,
     $                                  INFO )
...
*
                              CALL czlaqhp( UPLO, N, AFAC, S, SCOND,
     $                                     AMAX, EQUED )
                           END IF
                        END IF
*
...
*                       Factor the matrix A.
*
                        CALL czpptrf( UPLO, N, AFAC, INFO )
*
*                       Form the inverse of A.
*
                        CALL ZCOPY( NPP, AFAC, 1, A, 1 )
                        CALL czpptri( UPLO, N, A, INFO )
*
...
*
                        SRNAMT = 'czppsv '
                        CALL czppsv( UPLO, N, NRHS, AFAC, X, LDA, INFO )
*
*                       Check error code from czppsv .
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'czppsv ', INFO, IZERO,
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              WRITE( NOUT, FMT = 9999 )'czppsv ', UPLO,
     $                           N, IMAT, K, RESULT( K )
                              NFAIL = NFAIL + 1
                           END IF
   60                   CONTINUE
...
*
     ...
     $                  CALL czlaset( 'Full', NPP, 1, DCMPLX( ZERO ),
...
*                       EQUED='Y'.
*
                       CALL czlaqhp( UPLO, N, A, S, SCOND, AMAX, EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using czppsvX.
*
                     SRNAMT = 'czppsvX'
                     CALL czppsvX( FACT, UPLO, N, NRHS, A, AFAC, EQUED,
     $                            S, B, LDA, X, LDA, RCOND, RWORK,
     $                            RWORK( NRHS+1 ), WORK,
     $                            RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from czppsvX.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'czppsvX', INFO, IZERO,
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from czppsvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'czppsvX', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'czppsvX', FACT,
