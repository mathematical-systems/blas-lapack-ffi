...
*  =======
*
*  SCHKGT tests csgttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
     ...
     $                   K, KL, KOFF, KU, LDA, M, MODE, N, NERRS, 
     $  NFAIL, 
     $                   NIMAT, NRHS, NRUN
...
*     .. External Subroutines ..
     ...
     $                   csgtcon, csgtrfs, SGTT01, SGTT02, SGTT05, 
     $  csgttrf, 
     $                   csgttrs, cslacpy, cslagtm, cslarnv, SLATB4, 
     $  SLATMS, 
     $                   SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
            ...
            SRNAMT = 'csgttrf'
            CALL csgttrf( N, AF, AF( M+1 ), AF( N+M+1 ), AF( N+2*M+1 ),
     $                   IWORK, INFO )
*
*           Check error code from csgttrf.
*
            IF( INFO.NE.IZERO )
     $         CALL ALAERH( PATH, 'csgttrf', INFO, IZERO, ' ', N, N, 1,
...
*
               IF( .NOT.TRFCON ) THEN
*
*                 Use csgttrs to solve for one column at a time of inv(A)
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL csgttrs( TRANS, N, 1, AF, AF( M+1 ),
...
*
               SRNAMT = 'csgtcon'
               CALL csgtcon( NORM, N, AF, AF( M+1 ), AF( N+M+1 ),
     $                      AF( N+2*M+1 ), IWORK, ANORM, RCOND, WORK,
     $                      IWORK( N+1 ), INFO )
*
*              Check error code from csgtcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'csgtcon', INFO, 0, NORM, N, N, 
     $  -1, 
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
...
*
                  ...
                  SRNAMT = 'csgttrs'
                  CALL csgttrs( TRANS, N, NRHS, AF, AF( M+1 ),
     $                         AF( N+M+1 ), AF( N+2*M+1 ), IWORK, X,
     $                         LDA, INFO )
*
*                 Check error code from csgttrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'csgttrs', INFO, 0, TRANS, N, 
     $  N, 
...
*
                  SRNAMT = 'csgtrfs'
                  CALL csgtrfs( TRANS, N, NRHS, A, A( M+1 ), A( N+M+1 
     $  ), 
     $                         AF, AF( M+1 ), AF( N+M+1 ),
     ...
     $                         IWORK( N+1 ), INFO )
*
*                 Check error code from csgtrfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'csgtrfs', INFO, 0, TRANS, N, 
     $  N, 
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
