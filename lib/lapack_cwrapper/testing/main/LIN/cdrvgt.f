...
*     .. External Subroutines ..
     ...
     $                 ccgtsv, ccgtsvX, CGTT01, CGTT02, CGTT05, ccgttrf,
     $              ccgttrs, cclacpy, cclagtm, cclarnv, cclaset, CLATB4,
     $                   CLATMS, CSSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          CMPLX, MAX
...
*                 Factor the matrix A.
*
                  CALL ccgttrf( N, AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, INFO )
*
*                 Use ccgttrs to solve for one column at a time of
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL ccgttrs( 'No transpose', N, 1, AF, AF( M+1 ),
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL ccgttrs( 'Conjugate transpose', N, 1, AF,
...
*
                  IX = 1
                  DO 70 J = 1, NRHS
                     CALL cclarnv( 2, ISEED, N, XACT( IX ) )
                     IX = IX + LDA
   70             CONTINUE
*
*                 Set the right hand side.
*
                  CALL cclagtm( TRANS, N, NRHS, ONE, A, A( M+1 ),
...
*
                     SRNAMT = 'ccgtsv '
                    CALL ccgtsv( N, NRHS, AF, AF( M+1 ), AF( N+M+1 ), X,
     $                           LDA, INFO )
*
*                    Check error code from ccgtsv .
*
                     IF( INFO.NE.IZERO )
     $                  CALL ALAERH( PATH, 'ccgtsv ', INFO, IZERO, ' ',
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'ccgtsv ', N, IMAT,
     $                        K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
   80                CONTINUE
...
*                 error bounds using ccgtsvX.
*
                  SRNAMT = 'ccgtsvX'
                  CALL ccgtsvX( FACT, TRANS, N, NRHS, A, A( M+1 ),
     $                         A( N+M+1 ), AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, B, LDA, X, LDA,
     $                         RCOND, RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*                 Check the error code from ccgtsvX.
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'ccgtsvX', INFO, IZERO,
...
*
                  DO 100 K = K1, NT
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'ccgtsvX', FACT, TRANS,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  100             CONTINUE
...
*
                  ...
                  IF( RESULT( 6 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALADHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9998 )'ccgtsvX', FACT, TRANS, N,
     $                  IMAT, K, RESULT( K )
                     NFAIL = NFAIL + 1
                  END IF
                  NRUN = NRUN + NT - K1 + 2
