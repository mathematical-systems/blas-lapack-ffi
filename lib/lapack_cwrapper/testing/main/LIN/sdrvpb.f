...
*  =======
*
*  SDRVPB tests the driver routines cspbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                   cslacpy, cslaqsb, SLARHS, cslaset, SLATB4, 
     $  SLATMS, 
     $                   cspbequ, cspbsv, cspbsvX, SPBT01, SPBT02, 
     $  SPBT05, 
     $                   cspbtrf, cspbtrs, SSWAP, XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
*     ..
...
*                    matrix with SLATMS.
*
                     CALL SLATB4( PATH, IMAT, N, N, TYPE, KL, KU, 
     $  ANORM, 
     $                            MODE, CNDNUM, DIST )
*
                     SRNAMT = 'SLATMS'
...
*
                  CALL cslacpy( 'Full', KD+1, N, A, LDAB, ASAV, LDAB )
*
                  DO 70 IEQUED = 1, 2
                     EQUED = EQUEDS( IEQUED )
                     IF( IEQUED.EQ.1 ) THEN
...
*                          with the value returned by cspbsvX (FACT =
*                          'N' reuses the condition number from the
*                          previous iteration with FACT = 'F').
*
                           CALL cslacpy( 'Full', KD+1, N, ASAV, LDAB,
     $                                  AFAC, LDAB )
...
*
                              CALL cspbequ( UPLO, N, KD, AFAC, LDAB, S,
     $                                     SCOND, AMAX, INFO )
...
*
                                 CALL cslaqsb( UPLO, N, KD, AFAC, LDAB,
     $                                        S, SCOND, AMAX, EQUED )
                              END IF
                           END IF
*
...
*
                           ANORM = SLANSB( '1', UPLO, N, KD, AFAC, 
     $  LDAB, 
     $                             RWORK )
*
*                          Factor the matrix A.
*
                           CALL cspbtrf( UPLO, N, KD, AFAC, LDAB, INFO 
     $  ) 
*
*                          Form the inverse of A.
*
                           CALL cslaset( 'Full', N, N, ZERO, ONE, A,
     $                                  LDA )
                           SRNAMT = 'cspbtrs'
                           CALL cspbtrs( UPLO, N, KD, N, AFAC, LDAB, A,
     $                                  LDA, INFO )
*
...
*
                        CALL cslacpy( 'Full', KD+1, N, ASAV, LDAB, A,
     $                               LDAB )
*
...
*
     ...
     $                               LDA, ISEED, INFO )
                        XTYPE = 'C'
                        CALL cslacpy( 'Full', N, NRHS, B, LDA, BSAV,
     $                               LDA )
*
                        IF( NOFACT ) THEN
*
...
*
                           CALL cslacpy( 'Full', KD+1, N, A, LDAB, 
     $  AFAC, 
     $                                  LDAB )
                           CALL cslacpy( 'Full', N, NRHS, B, LDA, X,
     $                                  LDA )
*
                           SRNAMT = 'cspbsv '
                           CALL cspbsv( UPLO, N, KD, NRHS, AFAC, LDAB, 
     $  X, 
     $                                 LDA, INFO )
*
*                          Check error code from cspbsv .
*
                           IF( INFO.NE.IZERO ) THEN
                              CALL ALAERH( PATH, 'cspbsv ', INFO, 
     $  IZERO, 
     $                                     UPLO, N, N, KD, KD, NRHS,
...
*
                        IF( .NOT.PREFAC )
     $                     CALL cslaset( 'Full', KD+1, N, ZERO, ZERO,
     $                                  AFAC, LDAB )
                        CALL cslaset( 'Full', N, NRHS, ZERO, ZERO, X,
     $                               LDA )
...
*                          EQUED='Y'
*
                           CALL cslaqsb( UPLO, N, KD, A, LDAB, S, 
     $  SCOND, 
     $                                  AMAX, EQUED )
                        END IF
*
*                       Solve the system and compute the condition
*                       number and error bounds using cspbsvX.
*
                        SRNAMT = 'cspbsvX'
                        CALL cspbsvX( FACT, UPLO, N, KD, NRHS, A, LDAB,
     $                               AFAC, LDAB, EQUED, S, B, LDA, X,
     $                               LDA, RCOND, RWORK, RWORK( NRHS+1 
     $  ), 
     $                               WORK, IWORK, INFO )
*
*                       Check the error code from cspbsvX.
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'cspbsvX', INFO, IZERO,
     $                                  FACT // UPLO, N, N, KD, KD,
...
*                          Compute residual of the computed solution.
*
                           CALL cslacpy( 'Full', N, NRHS, BSAV, LDA,
     $                                  WORK, LDA )
...
*
     ...
     $                                  RESULT( 4 ) )
                        ...
                        END IF
*
*                       Compare RCOND from cspbsvX with the computed
*                       value in RCONDC.
*
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9997 )'cspbsvX',
     $                              FACT, UPLO, N, KD, EQUED, IMAT, K,
     $                              RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9998 )'cspbsvX',
