...
*     .. External Subroutines ..
     ...
     $                cclacpy, CLAPTM, cclarnv, cclaset, CLATB4, CLATMS,
     $                 ccptsv, ccptsvX, CPTT01, CPTT02, CPTT05, ccpttrf,
     $                   ccpttrs, CSSCAL, SCOPY, cslarnv, SSCAL
*     ..
*     .. Intrinsic Functions ..
...
*
*                 Use ccpttrs to solve for one column at a time of
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL ccpttrs( 'Lower', N, 1, D( N+1 ), E( N+1 ), X,
     $                            LDA, INFO )
...
*
                  SRNAMT = 'ccptsv '
                  CALL ccptsv( N, NRHS, D( N+1 ), E( N+1 ), X, LDA,
     $                        INFO )
*
*                 Check error code from ccptsv .
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'ccptsv ', INFO, IZERO, ' ', N,
...
*
     ...
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9999 )'ccptsv ', N, IMAT, K,
     $                     RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
   70             CONTINUE
...
*              error bounds using ccptsvX.
*
               SRNAMT = 'ccptsvX'
               CALL ccptsvX( FACT, N, NRHS, D, E, D( N+1 ), E( N+1 ), B,
     $                      LDA, X, LDA, RCOND, RWORK, RWORK( NRHS+1 ),
     $                      WORK, RWORK( 2*NRHS+1 ), INFO )
*
*              Check the error code from ccptsvX.
*
               IF( INFO.NE.IZERO )
     $            CALL ALAERH( PATH, 'ccptsvX', INFO, IZERO, FACT, N, N,
...
*
               DO 90 K = K1, 6
                  IF( RESULT( K ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALADHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9998 )'ccptsvX', FACT, N, IMAT,
     $                  K, RESULT( K )
                     NFAIL = NFAIL + 1
                  END IF
   90          CONTINUE
