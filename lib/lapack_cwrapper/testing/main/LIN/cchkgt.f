...
*  =======
*
*  CCHKGT tests ccgttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                ccgtcon, ccgtrfs, CGTT01, CGTT02, CGTT05, ccgttrf,
     $               ccgttrs, cclacpy, cclagtm, cclarnv, CLATB4, CLATMS,
     $                   CSSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          CMPLX, MAX
...
*
            ...
            SRNAMT = 'ccgttrf'
            CALL ccgttrf( N, AF, AF( M+1 ), AF( N+M+1 ), AF( N+2*M+1 ),
     $                   IWORK, INFO )
*
*           Check error code from ccgttrf.
*
            IF( INFO.NE.IZERO )
     $         CALL ALAERH( PATH, 'ccgttrf', INFO, IZERO, ' ', N, N, 1,
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL ccgttrs( TRANS, N, 1, AF, AF( M+1 ),
...
*
               SRNAMT = 'ccgtcon'
               CALL ccgtcon( NORM, N, AF, AF( M+1 ), AF( N+M+1 ),
     $                      AF( N+2*M+1 ), IWORK, ANORM, RCOND, WORK,
     $                      INFO )
*
*              Check error code from ccgtcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'ccgtcon', INFO, 0, NORM, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
...
*
                  CALL cclagtm( TRANS, N, NRHS, ONE, A,
...
*
                  ...
                  SRNAMT = 'ccgttrs'
                  CALL ccgttrs( TRANS, N, NRHS, AF, AF( M+1 ),
     $                         AF( N+M+1 ), AF( N+2*M+1 ), IWORK, X,
     $                         LDA, INFO )
*
*              Check error code from ccgttrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccgttrs', INFO, 0, TRANS, N, N,
...
*
                  SRNAMT = 'ccgtrfs'
                  CALL ccgtrfs( TRANS, N, NRHS, A, A( M+1 ), A( N+M+1 ),
     $                         AF, AF( M+1 ), AF( N+M+1 ),
     $                         AF( N+2*M+1 ), IWORK, B, LDA, X, LDA,
     $                         RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from ccgtrfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccgtrfs', INFO, 0, TRANS, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
