...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IA,IFACT,IMAT,IN,INFO,IX,IZERO,J,K,K1,KL,KU,LDA,MODE,N,
...
*     .. External Subroutines ..
      EXTERNAL ALADHD,ALAERH,ALASVM,DCOPY,DERRVX,DGET04,cdlacpy,DLAPTM,
     +         cdlarnv,cdlaset,DLATB4,DLATMS,cdptsv,cdptsvX,DPTT01,
     +         DPTT02,DPTT05,cdpttrf,cdpttrs,DSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC ABS,MAX
*     ..
...
*
                      CALL cdlarnv(2,ISEED,N,D)
                      CALL cdlarnv(2,ISEED,N-1,E)
*
...
*                 Factor the matrix A.
*
                      CALL cdpttrf(N,D(N+1),E(N+1),INFO)
*
*                 Use cdpttrs to solve for one column at a time of
...
*
   ...
   50                     CONTINUE
                          X(I) = ONE
                          CALL cdpttrs(N,1,D(N+1),E(N+1),X,LDA,INFO)
...
*
                      SRNAMT = 'cdptsv '
                      CALL cdptsv(N,NRHS,D(N+1),E(N+1),X,LDA,INFO)
*
*                 Check error code from cdptsv .
*
                      IF (INFO.NE.IZERO) CALL ALAERH(PATH,'cdptsv ',
     +                                        INFO,IZERO,' ',N,N,1,1,
...
*
     ...
     +                            NERRS.EQ.0) CALL ALADHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9999) 'cdptsv ',N,IMAT,K,
     +                          RESULT(K)
                              NFAIL = NFAIL + 1
                          END IF
   70                 CONTINUE
...
*              error bounds using cdptsvX.
*
                  SRNAMT = 'cdptsvX'
                  CALL cdptsvX(FACT,N,NRHS,D,E,D(N+1),E(N+1),B,LDA,X,
     +                         LDA,RCOND,RWORK,RWORK(NRHS+1),WORK,INFO)
*
*              Check the error code from cdptsvX.
*
                  IF (INFO.NE.IZERO) CALL ALAERH(PATH,'cdptsvX',INFO,
     +                                    IZERO,FACT,N,N,1,1,NRHS,IMAT,
     +                                    NFAIL,NERRS,NOUT)
                  IF (IZERO.EQ.0) THEN
                      IF (IFACT.EQ.2) THEN
*
...
*
     ...
     +                        0) CALL ALADHD(NOUT,PATH)
                          WRITE (NOUT,FMT=9998) 'cdptsvX',FACT,N,IMAT,K,
     +                      RESULT(K)
                          NFAIL = NFAIL + 1
                      END IF
   90             CONTINUE
...
*
 9999 FORMAT (1X,A6,', N =',I5,', type ',I2,', test ',I2,', ratio = ',
     +       G12.5)
 9998 FORMAT (1X,A6,', FACT=''',A1,''', N =',I5,', type ',I2,', test ',
     +       I2,', ratio = ',G12.5)
      RETURN
*
*     End of DDRVPT
