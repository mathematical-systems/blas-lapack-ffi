...
*  =======
*
*  SDRVSY tests the driver routines cssysv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $  cslacpy, 
     $                   SLARHS, cslaset, SLATB4, SLATMS, SPOT02, 
     $  SPOT05, 
     $                   cssysv, cssysvX, SSYT01, cssytrf, cssytri, 
     $  XLAENV 
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL cssytrf( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL cslacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                     CALL cssytri( UPLO, N, AINV, LDA, IWORK, WORK,
     $                            INFO )
...
*
*                    Factor the matrix and solve the system using cssysv.
*
                     SRNAMT = 'cssysv '
                     CALL cssysv( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from cssysv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'cssysv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'cssysv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using cssysvX.
*
                  SRNAMT = 'cssysvX'
                  CALL cssysvX( FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from cssysvX.
*
                  IF( INFO.NE.K ) THEN
                     CALL ALAERH( PATH, 'cssysvX', INFO, K, FACT // 
     $  UPLO, 
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from cssysvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'cssysvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
...
*
 9999 FORMAT( 1X, A6, ', UPLO=''', A1, ''', N =', I5, ', type ', I2,
     $      ', test ', I2, ', ratio =', G12.5 )
 9998 FORMAT( 1X, A6, ', FACT=''', A1, ''', UPLO=''', A1, ''', N =', 
     $  I5, 
