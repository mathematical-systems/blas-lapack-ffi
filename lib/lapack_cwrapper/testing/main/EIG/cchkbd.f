...
*
*  ccgebrd reduces a complex general m by n matrix A to real upper or
*  lower bidiagonal form by an orthogonal transformation: Q' * A * P = B
*  (or A = Q * B * P').  The matrix B is upper bidiagonal if m >= n
*  and lower bidiagonal if m < n.
*
*  ccungbr generates the orthogonal matrices Q and P' from ccgebrd.
*  Note that Q and P are not necessarily square.
*
*  ccbdsqr computes the singular value decomposition of the bidiagonal
...
*  In addition, ccbdsqr has an option to apply the left orthogonal matrix
...
*  (3)   | I - PT PT' | / ( N ulp )
*
*  Test ccbdsqr on bidiagonal matrix B
*
...
*       (a) ccgebrd is not called to reduce it to bidiagonal form.
...
*          and Z, used in testing ccbdsqr.  If NRHS = 0, then the
...
*          -27: LWORK too small.
*          If  CLATMR, CLATMS, ccgebrd, ccungbr, or ccbdsqr,
...
*     .. External Subroutines ..
      EXTERNAL         ALASUM, ccbdsqr, CBDT01, CBDT02, CBDT03, ccgebrd,
     $                 CGEMM, cclacpy, cclaset, CLATMR, CLATMS, ccungbr,
     $                   CUNT01, SCOPY, cslabad, SLAHD2, SSVDCH, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  100       CONTINUE
*
*           Call ccgebrd and ccungbr to compute B, Q, and P, do tests.
*
            IF( .NOT.BIDIAG ) THEN
*
...
*              B := Q' * A * P.
*
               CALL cclacpy( ' ', M, N, A, LDA, Q, LDQ )
              CALL ccgebrd( M, N, Q, LDQ, BD, BE, WORK, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from ccgebrd.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'ccgebrd', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
*
               CALL cclacpy( ' ', M, N, Q, LDQ, PT, LDPT )
               IF( M.GE.N ) THEN
                  UPLO = 'U'
               ELSE
                  UPLO = 'L'
...
*
               ...
               IF( NRHS.LE.0 )
     $            MQ = MNMIN
               CALL ccungbr( 'Q', M, MQ, N, Q, LDQ, WORK,
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from ccungbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'ccungbr(Q)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*              Generate P'
*
              CALL ccungbr( 'P', MNMIN, N, M, PT, LDPT, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from ccungbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'ccungbr(P)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
     ...
     $                      RWORK, RESULT( 3 ) )
            END IF
*
*           Use ccbdsqr to form the SVD of the bidiagonal matrix B:
*           B := U * S1 * VT, and compute Z = U' * Y.
*
            CALL SCOPY( MNMIN, BD, 1, S1, 1 )
            IF( MNMIN.GT.0 )
     $         CALL SCOPY( MNMIN-1, BE, 1, RWORK, 1 )
            CALL cclacpy( ' ', M, NRHS, Y, LDX, Z, LDX )
            CALL cclaset( 'Full', MNMIN, MNMIN, CZERO, CONE, U, LDPT )
            CALL cclaset( 'Full', MNMIN, MNMIN, CZERO, CONE, VT, LDPT )
*
           CALL ccbdsqr( UPLO, MNMIN, MNMIN, MNMIN, NRHS, S1, RWORK, VT,
     $                   LDPT, U, LDPT, Z, LDX, RWORK( MNMIN+1 ),
     $                   IINFO )
*
*           Check error code from ccbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'ccbdsqr(vects)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
*           Use ccbdsqr to compute only the singular values of the
...
*
     ...
     $         CALL SCOPY( MNMIN-1, BE, 1, RWORK, 1 )
*
            CALL ccbdsqr( UPLO, MNMIN, 0, 0, 0, S2, RWORK, VT, LDPT, U,
     $                   LDPT, Z, LDX, RWORK( MNMIN+1 ), IINFO )
*
*           Check error code from ccbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'ccbdsqr(values)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  110       CONTINUE
     ...
     $            RESULT( 8 ) = ULPINV
            END IF
*
*           Test 9:  Compare ccbdsqr with and without singular vectors
*
            TEMP2 = ZERO
*
            DO 120 J = 1, MNMIN
...
*
  140       CONTINUE
            RESULT( 10 ) = TEMP1
*
*           Use ccbdsqr to form the decomposition A := (QU) S (VT PT)
...
*
     ...
     $            CALL SCOPY( MNMIN-1, BE, 1, RWORK, 1 )
*
               CALL ccbdsqr( UPLO, MNMIN, N, M, NRHS, S2, RWORK, PT,
