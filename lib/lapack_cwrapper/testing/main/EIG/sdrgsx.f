...
*
*  csggesX factors A and B as Q S Z' and Q T Z', where ' means
...
*  matrix pairs are used to test the routine csggesX.
*
...
*        EPS*norm(A, B).
*
*  (9)   If INFO = N+3 is returned by csggesX, the reordering "failed"
...
*
*  SVD (routine csgesvD) is used for computing the true value of DIF_u
...
*          Copy of A, modified by csggesX.
*
*  BI      (workspace) REAL array, dimension (LDA, NSIZE)
*          Copy of B, modified by csggesX.
*
*  Z       (workspace) REAL array, dimension (LDA, NSIZE)
*          Z holds the left Schur vectors computed by csggesX.
*
*  Q       (workspace) REAL array, dimension (LDA, NSIZE)
*          Q holds the right Schur vectors computed by csggesX.
*
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, csgesvD, SGET51, SGET53, csggesX, cslabad,
     $                   cslacpy, SLAKF2, cslaset, SLATM5, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*     Loop over different functions (IFUNC) of csggesX, types (PRTYPE)
...
*
                  ...
                  K = 0
*
                  CALL cslaset( 'Full', MPLUSN, MPLUSN, ZERO, ZERO, AI,
     $                         LDA )
                  CALL cslaset( 'Full', MPLUSN, MPLUSN, ZERO, ZERO, BI,
     $                         LDA )
*
...
*
                  ...
                  END IF
*
                 CALL cslacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, A, LDA )
                 CALL cslacpy( 'Full', MPLUSN, MPLUSN, BI, LDA, B, LDA )
*
                 CALL csggesX( 'V', 'V', 'S', SLCTSX, SENSE, MPLUSN, AI,
...
*
                  IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
                     RESULT( 1 ) = ULPINV
                     WRITE( NOUT, FMT = 9999 )'csggesX', LINFO, MPLUSN,
     $                  PRTYPE
                     INFO = LINFO
                     GO TO 30
                  END IF
...
*
                  CALL cslacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK,
     $                         MPLUSN )
                  CALL cslacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
...
*
     ...
     $                            BI( MM+1, MM+1 ), C, LDC )
*
                     CALL csgesvD( 'N', 'N', MN2, MN2, C, LDC, S, WORK,
...
*     m-by-m (1,1)-blocks with n-by-n (2,2)-blocks.
*
      CALL csggesX('V', 'V', 'S', SLCTSX, 'B', MPLUSN, AI, LDA, BI, LDA,
     $             MM, ALPHAR, ALPHAI, BETA, Q, LDA, Z, LDA, PL, DIFEST,
     $             WORK, LWORK, IWORK, LIWORK, BWORK, LINFO )
*
      IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
         RESULT( 1 ) = ULPINV
         WRITE( NOUT, FMT = 9998 )'csggesX', LINFO, MPLUSN, NPTKNT
         GO TO 130
      END IF
*
*     Compute the norm(A, B)
...
*
      CALL cslacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK, MPLUSN )
      CALL cslacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
     $             WORK( MPLUSN*MPLUSN+1 ), MPLUSN )
