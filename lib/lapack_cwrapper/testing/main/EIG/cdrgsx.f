...
*
*  ccgges factors A and B as Q*S*Z'  and Q*T*Z' , where ' means conjugate
...
*  matrix pairs are used to test the routine ccggesX.
*
...
*        EPS*norm(A, B).
*
*  (9)   If INFO = N+3 is returned by ccggesX, the reordering "failed"
...
*
*  SVD (routine ccgesvD) is used for computing the true value of DIF_u
...
*          read-in test matrices are used to test csggesX.
*
*  NCMAX   (input) INTEGER
...
*          Copy of A, modified by ccggesX.
*
*  BI      (workspace) COMPLEX array, dimension (LDA, NSIZE)
*          Copy of B, modified by ccggesX.
*
*  Z       (workspace) COMPLEX array, dimension (LDA, NSIZE)
*          Z holds the left Schur vectors computed by ccggesX.
*
*  Q       (workspace) COMPLEX array, dimension (LDA, NSIZE)
*          Q holds the right Schur vectors computed by ccggesX.
*
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, ccgesvD, CGET51, ccggesX, cclacpy, CLAKF2,
     $                   cclaset, CLATM5, cslabad, xerbla
*     ..
*     .. Scalars in Common ..
      LOGICAL            FS
...
*
         MAXWRK = NSIZE*( 1+ILAENV( 1, 'ccgeqrf', ' ', NSIZE, 1, NSIZE,
     $            0 ) )
         MAXWRK = MAX( MAXWRK, NSIZE*( 1+ILAENV( 1, 'ccungqr', ' ',
     $            NSIZE, 1, NSIZE, -1 ) ) )
*
...
*
     ...
     $            ( 1+ILAENV( 1, 'ccgebrd', ' ', NSIZE*NSIZE / 2,
     $            NSIZE*NSIZE / 2, -1, -1 ) ) )
...
*     Loop over different functions (IFUNC) of ccggesX, types (PRTYPE)
...
*
                  ...
                  K = 0
*
                 CALL cclaset( 'Full', MPLUSN, MPLUSN, CZERO, CZERO, AI,
     $                         LDA )
                 CALL cclaset( 'Full', MPLUSN, MPLUSN, CZERO, CZERO, BI,
     $                         LDA )
*
...
*
                  ...
                  END IF
*
                 CALL cclacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, A, LDA )
                 CALL cclacpy( 'Full', MPLUSN, MPLUSN, BI, LDA, B, LDA )
*
                 CALL ccggesX( 'V', 'V', 'S', CLCTSX, SENSE, MPLUSN, AI,
...
*
                  IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
                     RESULT( 1 ) = ULPINV
                     WRITE( NOUT, FMT = 9999 )'ccggesX', LINFO, MPLUSN,
     $                  PRTYPE
                     INFO = LINFO
                     GO TO 30
                  END IF
...
*
                  CALL cclacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK,
     $                         MPLUSN )
                  CALL cclacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
...
*
     ...
     $                            BI( MM+1, MM+1 ), C, LDC )
*
                     CALL ccgesvD( 'N', 'N', MN2, MN2, C, LDC, S, WORK,
...
*     m-by-m (1,1)-blocks with n-by-n (2,2)-blocks.
*
      CALL ccggesX('V', 'V', 'S', CLCTSX, 'B', MPLUSN, AI, LDA, BI, LDA,
     $             MM, ALPHA, BETA, Q, LDA, Z, LDA, PL, DIFEST, WORK,
     $             LWORK, RWORK, IWORK, LIWORK, BWORK, LINFO )
*
      IF( LINFO.NE.0 .AND. LINFO.NE.MPLUSN+2 ) THEN
         RESULT( 1 ) = ULPINV
         WRITE( NOUT, FMT = 9998 )'ccggesX', LINFO, MPLUSN, NPTKNT
         GO TO 130
      END IF
*
*     Compute the norm(A, B)
...
*
      CALL cclacpy( 'Full', MPLUSN, MPLUSN, AI, LDA, WORK, MPLUSN )
      CALL cclacpy( 'Full', MPLUSN, MPLUSN, BI, LDA,
     $             WORK( MPLUSN*MPLUSN+1 ), MPLUSN )
