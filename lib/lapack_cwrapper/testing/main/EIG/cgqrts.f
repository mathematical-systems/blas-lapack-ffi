...
*  =======
*
*  CGQRTS tests ccggqrf, which computes the GQR factorization of an
...
*          by ccggqrf, see ccggqrf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL           CGEMM, cclacpy, cclaset, ccungqr,
     $                   ccungrq, CHERK
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL cclacpy( 'Full', N, M, A, LDA, AF, LDA )
      CALL cclacpy( 'Full', N, P, B, LDB, BF, LDB )
*
...
*
      CALL ccggqrf( N, M, P, AF, LDA, TAUA, BF, LDB, TAUB, WORK,
     $             LWORK, INFO )
*
*     Generate the N-by-N matrix Q
*
      CALL cclaset( 'Full', N, N, CROGUE, CROGUE, Q, LDA )
      CALL cclacpy( 'Lower', N-1, M, AF( 2,1 ), LDA, Q( 2,1 ), LDA )
      CALL ccungqr( N, N, MIN( N, M ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
...
*
     ...
     $      CALL cclacpy( 'Full', N, P-N, BF, LDB, Z( P-N+1, 1 ), LDB )
         IF( N.GT.1 )
     $      CALL cclacpy( 'Lower', N-1, N-1, BF( 2, P-N+1 ), LDB,
     $                    Z( P-N+2, P-N+1 ), LDB )
      ELSE
         IF( P.GT.1)
     $      CALL cclacpy( 'Lower', P-1, P-1, BF( N-P+2, 1 ), LDB,
     $                    Z( 2, 1 ), LDB )
      END IF
      CALL ccungrq( P, P, MIN( N, P ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL cclaset( 'Full', N, M, CZERO, CZERO, R, LDA )
      CALL cclacpy( 'Upper', N, M, AF, LDA, R, LDA )
*
*     Copy T
*
      CALL cclaset( 'Full', N, P, CZERO, CZERO, T, LDB )
      IF( N.LE.P ) THEN
         CALL cclacpy('Upper', N, N, BF( 1, P-N+1 ), LDB, T( 1, P-N+1 ),
     $                LDB )
      ELSE
         CALL cclacpy( 'Full', N-P, P, BF, LDB, T, LDB )
         CALL cclacpy('Upper', P, P, BF( N-P+1, 1 ), LDB, T( N-P+1, 1 ),
     $                LDB )
      END IF
*
*     Compute R - Q'*A
