...
*  =======
*
*     DDRVEV  checks the nonsymmetric eigenvalue problem driver cdgeev.
*
...
*          Another copy of the test matrix A, modified by cdgeev.
*
...
*          but those computed when cdgeev only computes a partial
...
*          -23: NWORK too small.
*          If  DLATMR, SLATMS, SLATME or cdgeev returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL        cdgeev, DGET22, cdlabad, cdlacpy, cdlaset, DLASUM,
...
*
     ...
     $                      ZERO, ANORM, 'NO', A, LDA, IWORK, IINFO )
               IF( N.GE.4 ) THEN
                  CALL cdlaset( 'Full', 2, N, ZERO, ZERO, A, LDA )
                  CALL cdlaset( 'Full', N-3, 1, ZERO, ZERO, A( 3, 1 ),
     $                         LDA )
                  CALL cdlaset( 'Full', N-3, 2, ZERO, ZERO, A( 3, N-1 ),
     $                         LDA )
                  CALL cdlaset( 'Full', 1, N, ZERO, ZERO, A( N, 1 ),
     $                         LDA )
               END IF
*
...
*
               ...
               CALL cdgeev( 'V', 'V', N, H, LDA, WR, WI, VL, LDVL, VR,
     $                     LDVR, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'cdgeev1', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cdlacpy( 'F', N, N, A, LDA, H, LDA )
               CALL cdgeev( 'N', 'N', N, H, LDA, WR1, WI1, DUM, 1, DUM,
     $                     1, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'cdgeev2', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cdlacpy( 'F', N, N, A, LDA, H, LDA )
               CALL cdgeev( 'N', 'V', N, H, LDA, WR1, WI1, DUM, 1, LRE,
     $                     LDLRE, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'cdgeev3', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cdlacpy( 'F', N, N, A, LDA, H, LDA )
               CALL cdgeev( 'V', 'N', N, H, LDA, WR1, WI1, LRE, LDLRE,
     $                     DUM, 1, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'cdgeev4', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
