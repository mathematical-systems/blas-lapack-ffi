...
*  routines.
*                                 H          H        H
*  czgghrd factors A and B as U H V  and U T V , where   means conjugate
*  transpose, H is hessenberg, T is triangular and U and V are unitary.
*
*                                  H          H
*  czhgeqz factors H and T as  Q S Z  and Q P Z , where P and S are upper
...
*      det( m(j) A - B ) = 0
*
*  cztgevc computes the matrix L of left eigenvectors and the matrix R
...
*        cdtgevc and back transforming (JOB='B').
*
...
*          The upper Hessenberg matrix computed from A by czgghrd.
*
*  T       (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by czgghrd.
*
*  S1      (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          The Schur (upper triangular) matrix computed from H by czhgeqz
...
*          The upper triangular matrix computed from T by czhgeqz
*          when Q and Z are also computed.
*
...
*          The (left) unitary matrix computed by czgghrd.
*
*  LDU     (input) INTEGER
...
*          The (right) unitary matrix computed by czgghrd.
*
*  Q       (workspace) COMPLEX*16 array, dimension (LDU, max(NN))
*          The (left) unitary matrix computed by czhgeqz.
*
...
*          The generalized eigenvalues of (A,B) computed by czhgeqz
...
*     .. External Subroutines ..
      EXTERNAL        cdlabad, DLASUM, xerbla, czgeqr2, ZGET51, ZGET52,
     $              czgghrd, czhgeqz, czlacpy, czlarfG, czlaset, ZLATM4,
     $                   cztgevc, czunm2r
*     ..
*     .. Intrinsic Functions ..
...
*
               IF( ABS( KATYPE( JTYPE ) ).EQ.3 ) THEN
                  IN = 2*( ( N-1 ) / 2 ) + 1
                  IF( IN.NE.N )
     $               CALL czlaset( 'Full', N, N, CZERO, CZERO, A, LDA )
               ELSE
                  IN = N
               END IF
...
*
               IF( ABS( KBTYPE( JTYPE ) ).EQ.3 ) THEN
                  IN = 2*( ( N-1 ) / 2 ) + 1
                  IF( IN.NE.N )
     $               CALL czlaset( 'Full', N, N, CZERO, CZERO, B, LDA )
               ELSE
                  IN = N
               END IF
...
*                 diagonal matrix.  (Note that czlarfG makes U(j,j) and
*                 V(j,j) real.)
*
                  DO 50 JC = 1, N - 1
                     DO 40 JR = JC, N
   ...
   40                CONTINUE
                    CALL czlarfG( N+1-JC, U( JC, JC ), U( JC+1, JC ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, DBLE( U( JC, JC ) ) )
                     U( JC, JC ) = CONE
                    CALL czlarfG( N+1-JC, V( JC, JC ), V( JC+1, JC ), 1,
     $                            WORK( N+JC ) )
...
*
   ...
   60                CONTINUE
   70             CONTINUE
                  CALL czunm2r( 'L', 'N', N, N, N-1, U, LDU, WORK, A,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL czunm2r( 'R', 'C', N, N, N-1, V, LDU, WORK( N+1 ),
     $                         A, LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL czunm2r( 'L', 'N', N, N, N-1, U, LDU, WORK, B,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL czunm2r( 'R', 'C', N, N, N-1, V, LDU, WORK( N+1 ),
...
*
  110       CONTINUE
*
*           Call czgeqr2, czunm2r, and czgghrd to compute H, T, U, and V
*
            CALL czlacpy( ' ', N, N, A, LDA, H, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T, LDA )
            NTEST = 1
            RESULT( 1 ) = ULPINV
*
            CALL czgeqr2( N, N, T, LDA, WORK, WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czgeqr2', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
            CALL czunm2r( 'L', 'C', N, N, N, T, LDA, WORK, H, LDA,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czunm2r', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
            CALL czlaset( 'Full', N, N, CZERO, CONE, U, LDU )
            CALL czunm2r( 'R', 'N', N, N, N, T, LDA, WORK, U, LDU,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czunm2r', IINFO, N, JTYPE,
     $            IOLDSD
            ...
            END IF
*
            CALL czgghrd( 'V', 'I', N, 1, N, H, LDA, T, LDA, U, LDU, V,
     $                   LDU, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czgghrd', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
     ...
     $                   RWORK, RESULT( 4 ) )
*
*           Call czhgeqz to compute S1, P1, S2, P2, Q, and Z, do tests.
*
*           Compute T1 and UZ
*
*           Eigenvalues only
*
            CALL czlacpy( ' ', N, N, H, LDA, S2, LDA )
            CALL czlacpy( ' ', N, N, T, LDA, P2, LDA )
            NTEST = 5
            RESULT( 5 ) = ULPINV
*
            CALL czhgeqz( 'E', 'N', 'N', N, 1, N, S2, LDA, P2, LDA,
     $                   ALPHA3, BETA3, Q, LDU, Z, LDU, WORK, LWORK,
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhgeqz(E)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
            ...
            CALL czlacpy( ' ', N, N, T, LDA, P2, LDA )
*
            CALL czhgeqz( 'S', 'N', 'N', N, 1, N, S2, LDA, P2, LDA,
     $                   ALPHA1, BETA1, Q, LDU, Z, LDU, WORK, LWORK,
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhgeqz(S)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
            CALL czlacpy( ' ', N, N, H, LDA, S1, LDA )
            CALL czlacpy( ' ', N, N, T, LDA, P1, LDA )
*
            CALL czhgeqz( 'S', 'I', 'I', N, 1, N, S1, LDA, P1, LDA,
     $                   ALPHA1, BETA1, Q, LDU, Z, LDU, WORK, LWORK,
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhgeqz(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
  ...
  130       CONTINUE
*
            CALL cztgevc( 'L', 'S', LLWORK, N, S1, LDA, P1, LDA, EVECTL,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztgevc(L,S1)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
  ...
  150       CONTINUE
*
            CALL cztgevc( 'L', 'S', LLWORK, N, S1, LDA, P1, LDA,
     $                   EVECTL( 1, I1+1 ), LDU, CDUMMA, LDU, N, IN,
     $                   WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztgevc(L,S2)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   ALPHA1, BETA1, WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRSHN ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cztgevc(HOWMNY=S)',
...
*
            NTEST = 10
            RESULT( 10 ) = ULPINV
            CALL czlacpy( 'F', N, N, Q, LDU, EVECTL, LDU )
            CALL cztgevc( 'L', 'B', LLWORK, N, S1, LDA, P1, LDA, EVECTL,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztgevc(L,B)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   BETA1, WORK, RWORK, DUMMA( 1 ) )
            RESULT( 10 ) = DUMMA( 1 )
            IF( DUMMA( 2 ).GT.THRSHN ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cztgevc(HOWMNY=B)',
     $            DUMMA( 2 ), N, JTYPE, IOLDSD
            END IF
*
...
*
  ...
  170       CONTINUE
*
            CALL cztgevc( 'R', 'S', LLWORK, N, S1, LDA, P1, LDA, CDUMMA,
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztgevc(R,S1)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
     ...
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztgevc(R,S2)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   ALPHA1, BETA1, WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cztgevc(HOWMNY=S)',
     $            DUMMA( 2 ), N, JTYPE, IOLDSD
            END IF
*
...
*
            NTEST = 12
            RESULT( 12 ) = ULPINV
            CALL czlacpy( 'F', N, N, Z, LDU, EVECTR, LDU )
            CALL cztgevc( 'R', 'B', LLWORK, N, S1, LDA, P1, LDA, CDUMMA,
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztgevc(R,B)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   ALPHA1, BETA1, WORK, RWORK, DUMMA( 1 ) )
            RESULT( 12 ) = DUMMA( 1 )
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cztgevc(HOWMNY=B)',
     $            DUMMA( 2 ), N, JTYPE, IOLDSD
            END IF
*
