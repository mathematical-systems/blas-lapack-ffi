...
*
*     czhetrd factors A as  U S U* , where * means conjugate transpose,
*     S is real symmetric tridiagonal, and U is unitary.
*     czhetrd can use either just the lower or just the upper triangle
*     of A; ZCHKST checks both cases.
*     U is represented as a product of Householder
*     transformations, whose vectors are stored in the first
*     n-1 columns of V, and whose scale factors are in TAU.
*
*     czhptrd does the same as czhetrd, except that A and V are stored
*     in "packed" format.
*
*     czungtr constructs the matrix U from the contents of V and TAU.
*
*     czupgtr constructs the matrix U from the contents of VP and TAU.
*
*     czsteqr factors S as  Z D1 Z* , where Z is the unitary
...
*
*     czpteqr factors S as  Z4 D4 Z4* , for a
...
*     accuracy.
*
*     czstein computes Y, the eigenvectors of S, given the
*     eigenvalues.
*
*     czstedc factors S as Z D1 Z* , where Z is the unitary
*     matrix of eigenvectors and D1 is a diagonal matrix with
*     the eigenvalues on the diagonal ('I' option). It may also
*     update an input unitary matrix, usually the output
*     from czhetrd/czungtr or czhptrd/czupgtr ('V' option). It may
*     also just compute eigenvalues ('N' option).
*
*     czstegr factors S as Z D1 Z* , where Z is the unitary
*     matrix of eigenvectors and D1 is a diagonal matrix with
*     the eigenvalues on the diagonal ('I' option).  czstegr
...
*
*  (1)     | A - V S V* | / ( |A| n ulp ) czhetrd( UPLO='U', ... )
*
*  (2)     | I - UV* | / ( n ulp )        czungtr( UPLO='U', ... )
*
*  (3)     | A - V S V* | / ( |A| n ulp ) czhetrd( UPLO='L', ... )
*
*  (4)     | I - UV* | / ( n ulp )        czungtr( UPLO='L', ... )
*
*  (5-8)   Same as 1-4, but for czhptrd and czupgtr.
*
*  (9)     | S - Z D Z* | / ( |S| n ulp ) czsteqr('V',...)
*
*  (10)    | I - ZZ* | / ( n ulp )        czsteqr('V',...)
*
*  (11)    | D1 - D2 | / ( |D1| ulp )        czsteqr('N',...)
*
...
*
*  (14)    | S - Z4 D4 Z4* | / ( |S| n ulp ) czpteqr('V',...)
*
*  (15)    | I - Z4 Z4* | / ( n ulp )        czpteqr('V',...)
*
*  (16)    | D4 - D5 | / ( 100 |D4| ulp )       czpteqr('N',...)
*
...
*                                               cdstebz( 'I', 'E', ...)
*
*  (20)    | S - Y WA1 Y* | / ( |S| n ulp )  cdstebz, czstein
*
*  (21)    | I - Y Y* | / ( n ulp )          cdstebz, czstein
*
*  (22)    | S - Z D Z* | / ( |S| n ulp )    czstedc('I')
*
*  (23)    | I - ZZ* | / ( n ulp )           czstedc('I')
*
*  (24)    | S - Z D Z* | / ( |S| n ulp )    czstedc('V')
*
*  (25)    | I - ZZ* | / ( n ulp )           czstedc('V')
*
*  (26)    | D1 - D2 | / ( |D1| ulp )           czstedc('V') and
*                                               czstedc('N')
*
*  Test 27 is disabled at the moment because czstegr does not
...
*
*  (29)    | S - Z D Z* | / ( |S| n ulp )    czstegr('V', 'I')
*
*  (30)    | I - ZZ* | / ( n ulp )           czstegr('V', 'I')
*
*  (31)    ( max { min | WA2(i)-WA3(j) | } +
*             i     j
*            max { min | WA3(i)-WA2(j) | } ) / ( |D3| ulp )
*             i     j
*          czstegr('N', 'I') vs. ccstegr('V', 'I')
*
*  (32)    | S - Z D Z* | / ( |S| n ulp )    czstegr('V', 'V')
*
*  (33)    | I - ZZ* | / ( n ulp )           czstegr('V', 'V')
*
*  (34)    ( max { min | WA2(i)-WA3(j) | } +
*             i     j
*            max { min | WA3(i)-WA2(j) | } ) / ( |D3| ulp )
*             i     j
*          czstegr('N', 'V') vs. ccstegr('V', 'V')
*
*  (35)    | S - Z D Z* | / ( |S| n ulp )    czstegr('V', 'A')
*
*  (36)    | I - ZZ* | / ( n ulp )           czstegr('V', 'A')
*
*  (37)    ( max { min | WA2(i)-WA3(j) | } +
*             i     j
*            max { min | WA3(i)-WA2(j) | } ) / ( |D3| ulp )
*             i     j
*          czstegr('N', 'A') vs. ccstegr('V', 'A')
*
...
*                             dimension( max(NN) )
*          The diagonal of the tridiagonal matrix computed by czhetrd.
...
*          czhetrd.  On exit, SD and SE contain the tridiagonal form of
*          the matrix in A.
*
*  D1      (workspace/output) DOUBLE PRECISION array of
*                             dimension( max(NN) )
*          The eigenvalues of A, as computed by czsteqr simlutaneously
*          with Z.  On exit, the eigenvalues in D1 correspond with the
*          matrix in A.
*
*  D2      (workspace/output) DOUBLE PRECISION array of
*                             dimension( max(NN) )
*          The eigenvalues of A, as computed by czsteqr if Z is not
...
*          The unitary matrix computed by czhetrd + czungtr.
*
*  LDU     (input) INTEGER
...
*                             dimension( LDU, max(NN) ).
*          The Housholder vectors computed by czhetrd in reducing A to
*          tridiagonal form.  The vectors computed with UPLO='U' are
*          in the upper triangle, and the vectors computed with UPLO='L'
*          are in the lower triangle.  (As described in czhetrd, the
*          sub- and superdiagonal are not set to 1, although the
*          true Householder vector has a 1 in that position.  The
*          routines that use V, such as czungtr, set those entries to
...
*                             dimension( max(NN) )
*          The Householder factors computed by czhetrd in reducing A
*          to tridiagonal form.
*
*  Z       (workspace/output) COMPLEX*16 array of
*                             dimension( LDU, max(NN) ).
*          The unitary matrix of eigenvectors computed by czsteqr,
*          czpteqr, and czstein.
*
...
*          -29: LWORK too small.
*          If  ZLATMR, CLATMS, czhetrd, czungtr, czsteqr, cdsterf,
...
*     .. External Subroutines ..
      EXTERNAL         DCOPY, cdlabad, DLASUM, cdstebz, DSTECH, cdsterf,
     $                 xerbla, ZCOPY, ZHET21, czhetrd, ZHPT21, czhptrd,
     $               czlacpy, czlaset, ZLATMR, ZLATMS, czpteqr, czstedc,
     $               czstegr, czstein, czsteqr, ZSTT21, ZSTT22, czungtr,
     $                   czupgtr
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   10 CONTINUE
*
      NBLOCK = ILAENV( 1, 'czhetrd', 'L', NMAX, -1, -1, -1 )
...
*
   70       CONTINUE
*
            CALL czlaset( 'Full', LDA, N, CZERO, CZERO, A, LDA )
            IINFO = 0
            IF( JTYPE.LE.15 ) THEN
               COND = ULPINV
            ELSE
...
*
  100       CONTINUE
*
*           Call czhetrd and czungtr to compute S and U from
*           upper triangle.
*
            CALL czlacpy( 'U', N, N, A, LDA, V, LDU )
*
            NTEST = 1
            CALL czhetrd( 'U', N, V, LDU, SD, SE, TAU, WORK, LWORK,
     $                   IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhetrd(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
            CALL czlacpy( 'U', N, N, V, LDU, U, LDU )
*
            NTEST = 2
            CALL czungtr( 'U', N, U, LDU, TAU, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czungtr(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            CALL czlacpy( 'L', N, N, A, LDA, V, LDU )
*
            NTEST = 3
            CALL czhetrd( 'L', N, V, LDU, SD, SE, TAU, WORK, LWORK,
     $                   IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhetrd(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
            CALL czlacpy( 'L', N, N, V, LDU, U, LDU )
*
            NTEST = 4
            CALL czungtr( 'L', N, U, LDU, TAU, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czungtr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  120       CONTINUE
*
*           Call czhptrd and czupgtr to compute S and U from AP
*
            CALL ZCOPY( NAP, AP, 1, VP, 1 )
*
            NTEST = 5
            CALL czhptrd( 'U', N, VP, SD, SE, TAU, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhptrd(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 6
            CALL czupgtr( 'U', N, VP, TAU, U, LDU, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czupgtr(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 7
            CALL czhptrd( 'L', N, VP, SD, SE, TAU, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhptrd(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 8
            CALL czupgtr( 'L', N, VP, TAU, U, LDU, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czupgtr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   WORK, RWORK, RESULT( 8 ) )
*
*           Call czsteqr to compute D1, D2, and Z, do tests.
*
*           Compute D1 and Z
*
     ...
     $         CALL DCOPY( N-1, SE, 1, RWORK, 1 )
            CALL czlaset( 'Full', N, N, CZERO, CONE, Z, LDU )
*
            NTEST = 9
            CALL czsteqr( 'V', N, D1, RWORK, Z, LDU, RWORK( N+1 ),
     $                   IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czsteqr(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 11
            CALL czsteqr( 'N', N, D2, RWORK, WORK, LDU, RWORK( N+1 ),
     $                   IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czsteqr(N)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  170       CONTINUE
            RESULT( 13 ) = TEMP1
*
*           For positive definite matrices ( JTYPE.GT.15 ) call czpteqr
...
*
               NTEST = 14
               CALL czpteqr( 'V', N, D4, RWORK, Z, LDU, RWORK( N+1 ),
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czpteqr(V)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = 16
               CALL czpteqr( 'N', N, D5, RWORK, Z, LDU, RWORK( N+1 ),
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czpteqr(N)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
*           Call czstein to compute eigenvectors corresponding to
...
*
     ...
     $            JTYPE, IOLDSD
            ...
            END IF
*
           CALL czstein( N, SD, SE, M, WA1, IWORK( 1 ), IWORK( N+1 ), Z,
     $                   LDU, RWORK, IWORK( 2*N+1 ), IWORK( 3*N+1 ),
     $                   IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czstein', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 20 ) )
*
*           Call czstedc(I) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            INDE = 1
...
*
            NTEST = 22
           CALL czstedc( 'I', N, D1, RWORK( INDE ), Z, LDU, WORK, LWEDC,
     $                   RWORK( INDRWK ), LRWEDC, IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czstedc(I)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 22 ) )
*
*           Call czstedc(V) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL DCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 24
           CALL czstedc( 'V', N, D1, RWORK( INDE ), Z, LDU, WORK, LWEDC,
     $                   RWORK( INDRWK ), LRWEDC, IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czstedc(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 24 ) )
*
*           Call czstedc(N) to compute D2, do tests.
*
*           Compute D2
*
            CALL DCOPY( N, SD, 1, D2, 1 )
...
*
            NTEST = 26
           CALL czstedc( 'N', N, D2, RWORK( INDE ), Z, LDU, WORK, LWEDC,
     $                   RWORK( INDRWK ), LRWEDC, IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czstedc(N)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
*           Only test czstegr if IEEE compliant
*
            IF( ILAENV( 10, 'czstegr', 'VA', 1, 0, 0, 0 ).EQ.1 .AND.
     $          ILAENV( 11, 'czstegr', 'VA', 1, 0, 0, 0 ).EQ.1 ) THEN
*
*           Call czstegr, do test 27 (relative eigenvalue accuracy)
*
*              If S is positive definite and diagonally dominant,
*              ask for all eigenvalues with high relative accuracy.
*
               ...
               IF( JTYPE.EQ.21 .AND. CREL ) THEN
                  NTEST = 27
                  ABSTOL = UNFL + UNFL
                  CALL czstegr( 'V', 'A', N, SD, SE, VL, VU, IL, IU,
     $                         ABSTOL, M, WR, Z, LDU, IWORK( 1 ), RWORK,
     $                         LRWORK, IWORK( 2*N+1 ), LWORK-2*N,
     $                         IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czstegr(V,A,rel)',
...
*
                  IF( CRANGE ) THEN
                     NTEST = 28
                     ABSTOL = UNFL + UNFL
                     CALL czstegr( 'V', 'I', N, SD, SE, VL, VU, IL, IU,
     $                            ABSTOL, M, WR, Z, LDU, IWORK( 1 ),
     $                            RWORK, LRWORK, IWORK( 2*N+1 ),
     $                            LWORK-2*N, IINFO )
*
                     IF( IINFO.NE.0 ) THEN
                        WRITE( NOUNIT, FMT = 9999 )'czstegr(V,I,rel)',
     $                     IINFO, N, JTYPE, IOLDSD
                        INFO = ABS( IINFO )
                        IF( IINFO.LT.0 ) THEN
                           RETURN
...
*
               ...
               END IF
*
*           Call czstegr(V,I) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
               CALL DCOPY( N, SD, 1, D5, 1 )
...
*
               IF( CRANGE ) THEN
                  ...
                  IF( IU.LT.IL ) THEN
                     ...
                     IL = ITEMP
                  END IF
                  CALL czstegr( 'V', 'I', N, D5, RWORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D1, Z, LDU, IWORK( 1 ),
     $                         RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czstegr(V,I)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
*
*           Call czstegr to compute D2, do tests.
*
*           Compute D2
*
                  CALL DCOPY( N, SD, 1, D5, 1 )
...
*
                  NTEST = 31
                  CALL czstegr( 'N', 'I', N, D5, RWORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D2, Z, LDU, IWORK( 1 ),
     $                         RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czstegr(N,I)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
*
*           Call czstegr(V,V) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
                  CALL DCOPY( N, SD, 1, D5, 1 )
...
*
     ...
     $                       ULP*ANORM, TWO*RTUNFL )
                  ...
                  END IF
*
                  CALL czstegr( 'V', 'V', N, D5, RWORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D1, Z, LDU, IWORK( 1 ),
     $                         RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czstegr(V,V)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  NTEST = 34
                  CALL czstegr( 'N', 'V', N, D5, RWORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D2, Z, LDU, IWORK( 1 ),
     $                         RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czstegr(N,V)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
*
*           Call czstegr(V,A) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
               CALL DCOPY( N, SD, 1, D5, 1 )
...
*
               NTEST = 35
*
               CALL czstegr( 'V', 'A', N, D5, RWORK, VL, VU, IL, IU,
     $                      ABSTOL, M, D1, Z, LDU, IWORK( 1 ),
     $                      RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ),
     $                      LIWORK-2*N, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czstegr(V,A)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = 37
               CALL czstegr( 'N', 'A', N, D5, RWORK, VL, VU, IL, IU,
     $                      ABSTOL, M, D2, Z, LDU, IWORK( 1 ),
     $                      RWORK( N+1 ), LRWORK-N, IWORK( 2*N+1 ),
     $                      LIWORK-2*N, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czstegr(N,A)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
 9993 FORMAT( / ' Tests performed:   ',
     ...
     $      / ' czhetrd, UPLO=''U'':', / '  1= | A - V S V', A1,
     $      ' | / ( |A| n ulp )     ', '  2= | I - U V', A1,
     $      ' | / ( n ulp )', / ' czhetrd, UPLO=''L'':',
     $      / '  3= | A - V S V', A1, ' | / ( |A| n ulp )     ',
     $      '  4= | I - U V', A1, ' | / ( n ulp )' )
 9992 FORMAT( ' czhptrd, UPLO=''U'':', / '  5= | A - V S V', A1,
     $      ' | / ( |A| n ulp )     ', '  6= | I - U V', A1,
     $      ' | / ( n ulp )', / ' czhptrd, UPLO=''L'':',
 ...
 9990 FORMAT( ' 22= | S - Z D Z', A1,
     $      ' | / ( |S| n ulp ) for czstedc(I)', / ' 23= | I - Z Z', A1,
     $      ' | / ( n ulp )       for czstedc(I)', / ' 24= | S - Z D Z',
     $      A1, ' | / ( |S| n ulp ) for czstedc(V)', / ' 25= | I - Z Z',
     $      A1, ' | / ( n ulp )       for czstedc(V)',
     $     / ' 26= | D1(czstedc(V)) - D2(ccstedc(N)) | / ( |D1| ulp )' )
