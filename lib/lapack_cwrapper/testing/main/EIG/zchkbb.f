...
*
*  czgbbrd factors a general band matrix A as  Q B P* , where * means
*  conjugate transpose, B is upper bidiagonal, and Q and P are unitary;
*  czgbbrd can also overwrite a given matrix C with Q* C .
*
...
*          Used to hold the unitary matrix Q computed by czgbbrd.
*
*  LDQ     (input) INTEGER
...
*          Used to hold the unitary matrix P computed by czgbbrd.
*
*  LDP     (input) INTEGER
...
*          Used to hold the matrix C updated by czgbbrd.
*
*  LDC     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL         DLAHD2, DLASUM, xerbla, ZBDT01, ZBDT02, czgbbrd,
     $                   czlacpy, czlaset, ZLATMR, ZLATMS, ZUNT01
*     ..
*     .. Intrinsic Functions ..
...
*
               ...
               CALL czlaset( 'Full', LDAB, N, CZERO, CZERO, AB, LDAB )
               IINFO = 0
               COND = ULPINV
*
...
*              Copy C
*
               CALL czlacpy( 'Full', M, NRHS, C, LDC, CC, LDC )
*
*              Call czgbbrd to compute B, Q and P, and to update C.
*
               CALL czgbbrd( 'B', M, N, NRHS, KL, KU, AB, LDAB, BD, BE,
     $                      Q, LDQ, P, LDP, CC, LDC, WORK, RWORK,
     $                      IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czgbbrd', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
