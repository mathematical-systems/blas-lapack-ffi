...
*  routines.
*                                T          T        T
*  cdgegs factors A and B as Q S Z  and Q T Z , where   means
...
*      det( m(j) A - B ) = 0
*
*  cdgegv computes the generalized eigenvalues (alpha(1),beta(1)), ...,
...
*          The Schur form matrix computed from A by cdgegs.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) DOUBLE PRECISION array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by cdgegs.
*
*  S2      (workspace) DOUBLE PRECISION array, dimension (LDA, max(NN))
*          The matrix computed from A by cdgegv.  This will be the
...
*          The matrix computed from B by cdgegv.  This will be the
...
*          The (left) orthogonal matrix computed by cdgegs.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by cdgegs.
*
...
*
*          The generalized eigenvalues of (A,B) computed by cdgegs.
...
*
*          The generalized eigenvalues of (A,B) computed by cdgegv.
...
*          the matrices in A and B.  (See cdtgevc for the format.)
*
...
*          cdhgeqz, and NB is the greatest of the blocksizes for
*          cdgeqrf, cdormqr, and cdorgqr.  (The blocksizes and the
...
*     .. External Subroutines ..
      EXTERNAL           ALASVM, cdgegs, cdgegv, DGET51, DGET52, DGET53,
     $              cdlabad, cdlacpy, cdlarfG, cdlaset, DLATM4, cdorm2r,
     $                   xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
      NB = MAX( 1, ILAENV( 1, 'cdgeqrf', ' ', NMAX, NMAX, -1, -1 ),
     $     ILAENV( 1, 'cdormqr', 'LT', NMAX, NMAX, NMAX, -1 ),
     $     ILAENV( 1, 'cdorgqr', ' ', NMAX, NMAX, NMAX, -1 ) )
      NBZ = ILAENV( 1, 'cdhgeqz', 'SII', NMAX, 1, NMAX, 0 )
      NS = ILAENV( 4, 'cdhgeqz', 'SII', NMAX, 1, NMAX, 0 )
      I1 = NBZ + NS
...
*
   ...
   40                CONTINUE
                    CALL cdlarfG( N+1-JC, Q( JC, JC ), Q( JC+1, JC ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, Q( JC, JC ) )
                     Q( JC, JC ) = ONE
                    CALL cdlarfG( N+1-JC, Z( JC, JC ), Z( JC+1, JC ), 1,
     $                            WORK( N+JC ) )
...
*
   ...
   60                CONTINUE
   70             CONTINUE
                  CALL cdorm2r( 'L', 'N', N, N, N-1, Q, LDQ, WORK, A,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL cdorm2r( 'R', 'T', N, N, N-1, Z, LDQ, WORK( N+1 ),
     $                         A, LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL cdorm2r( 'L', 'N', N, N, N-1, Q, LDQ, WORK, B,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL cdorm2r( 'R', 'T', N, N, N-1, Z, LDQ, WORK( N+1 ),
...
*
  110       CONTINUE
*
*           Call cdgegs to compute H, T, Q, Z, alpha, and beta.
*
            CALL cdlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cdlacpy( ' ', N, N, B, LDA, T, LDA )
            NTEST = 1
            RESULT( 1 ) = ULPINV
*
            CALL cdgegs( 'V', 'V', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, Z, LDQ, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cdgegs', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 140
            END IF
...
*
  ...
  120       CONTINUE
            RESULT( 5 ) = TEMP1
*
*           Call cdgegv to compute S2, T2, VL, and VR, do tests.
*
*           Eigenvalues and Eigenvectors
*
            CALL cdlacpy( ' ', N, N, A, LDA, S2, LDA )
            CALL cdlacpy( ' ', N, N, B, LDA, T2, LDA )
            NTEST = 6
            RESULT( 6 ) = ULPINV
*
            CALL cdgegv( 'V', 'V', N, S2, LDA, T2, LDA, ALPHR2, ALPHI2,
     $                  BETA2, VL, LDQ, VR, LDQ, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cdgegv', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 140
            END IF
...
*
     ...
     $                   ALPHI2, BETA2, WORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRSHN ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cdgegv', DUMMA( 2 ),
     $            N, JTYPE, IOLDSD
            END IF
*
     ...
     $                   ALPHI2, BETA2, WORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cdgegv', DUMMA( 2 ),
     $            N, JTYPE, IOLDSD
            END IF
*
