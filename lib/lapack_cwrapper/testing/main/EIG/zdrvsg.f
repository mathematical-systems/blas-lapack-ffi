...
*       drivers.
*
*               czhegv computes all eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite generalized
*               eigenproblem.
*
*               czhegvD computes all eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite generalized
*               eigenproblem using a divide and conquer algorithm.
*
*               czhegvX computes selected eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite generalized
*               eigenproblem.
*
*               czhpgv computes all eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite generalized
*               eigenproblem in packed storage.
*
*               czhpgvD computes all eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite generalized
*               eigenproblem in packed storage using a divide and
*               conquer algorithm.
*
*               czhpgvX computes selected eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite generalized
*               eigenproblem in packed storage.
*
*               czhbgv computes all eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite banded
*               generalized eigenproblem.
*
*               czhbgvD computes all eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite banded
*               generalized eigenproblem using a divide and conquer
*               algorithm.
*
*               czhbgvX computes selected eigenvalues and, optionally,
*               eigenvectors of a complex Hermitian-definite banded
*               generalized eigenproblem.
*
...
*
*       (1) czhegv with ITYPE = 1 and UPLO ='U':
*
*               | A Z - B Z D | / ( |A| |Z| n ulp )
*
*       (2) as (1) but calling czhpgv
*       (3) as (1) but calling czhbgv
*       (4) as (1) but with UPLO = 'L'
*       (5) as (4) but calling czhpgv
*       (6) as (4) but calling czhbgv
*
*       (7) czhegv with ITYPE = 2 and UPLO ='U':
*
*               | A B Z - Z D | / ( |A| |Z| n ulp )
*
*       (8) as (7) but calling czhpgv
*       (9) as (7) but with UPLO = 'L'
*       (10) as (9) but calling czhpgv
*
*       (11) czhegv with ITYPE = 3 and UPLO ='U':
*
*               | B A Z - Z D | / ( |A| |Z| n ulp )
*
*       (12) as (11) but calling czhpgv
*       (13) as (11) but with UPLO = 'L'
*       (14) as (13) but calling czhpgv
*
*       czhegvD, czhpgvD and czhbgvD performed the same 14 tests.
*
*       czhegvX, czhpgvX and czhbgvX performed the above 14 tests with
...
*          -23: LRWORK too small.
*          -25: LIWORK too small.
*          If  ZLATMR, CLATMS, czhegv, czhpgv, czhbgv, cchegvD, cchpgvD,
*              czhpgvD, czhegvX, cchpgvX, czhbgvX returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL        cdlabad, DLAFTS, DLASUM, xerbla, czhbgv, czhbgvD,
     $               czhbgvX, czhegv, czhegvD, czhegvX, czhpgv, czhpgvD,
     $                 czhpgvX, czlacpy, czlaset, ZLATMR, ZLATMS, ZSGT01
*     ..
*     .. Intrinsic Functions ..
...
*
            ...
            END IF
*
*           3) Call czhegv, czhpgv, czhbgv, cchegvD, cchpgvD, cchbgvD,
*              czhegvX, czhpgvX and czhbgvX, do tests.
*
...
*
                  NTEST = NTEST + 1
*
                  CALL czlacpy( ' ', N, N, A, LDA, Z, LDZ )
                  CALL czlacpy( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL czhegv( IBTYPE, 'V', UPLO, N, Z, LDZ, BB, LDB, D,
     $                        WORK, NWORK, RWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhegv(V,' // UPLO //
...
*
                  ...
                  CALL czlacpy( UPLO, N, N, B, LDB, BB, LDB )
*
                 CALL czhegvD( IBTYPE, 'V', UPLO, N, Z, LDZ, BB, LDB, D,
     $                         WORK, NWORK, RWORK, LRWORK, IWORK,
     $                         LIWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhegvD(V,' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  NTEST = NTEST + 1
*
                  CALL czlacpy( ' ', N, N, A, LDA, AB, LDA )
                  CALL czlacpy( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL czhegvX( IBTYPE, 'V', 'A', UPLO, N, AB, LDA, BB,
     $                         LDB, VL, VU, IL, IU, ABSTOL, M, D, Z,
     $                         LDZ, WORK, NWORK, RWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhegvX(V,A' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  VL = ZERO
                  VU = ANORM
                  CALL czhegvX( IBTYPE, 'V', 'V', UPLO, N, AB, LDA, BB,
     $                         LDB, VL, VU, IL, IU, ABSTOL, M, D, Z,
     $                         LDZ, WORK, NWORK, RWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhegvX(V,V,' //
...
*
                  ...
                  CALL czlacpy( UPLO, N, N, B, LDB, BB, LDB )
*
                  CALL czhegvX( IBTYPE, 'V', 'I', UPLO, N, AB, LDA, BB,
     $                         LDB, VL, VU, IL, IU, ABSTOL, M, D, Z,
     $                         LDZ, WORK, NWORK, RWORK, IWORK( N+1 ),
     $                         IWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhegvX(V,I,' //
     $                  UPLO // ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
  ...
  140                CONTINUE
                  END IF
*
                  CALL czhpgv( IBTYPE, 'V', UPLO, N, AP, BP, D, Z, LDZ,
     $                        WORK, RWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhpgv(V,' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
  ...
  180                CONTINUE
                  END IF
*
                  CALL czhpgvD( IBTYPE, 'V', UPLO, N, AP, BP, D, Z, LDZ,
     $                         WORK, NWORK, RWORK, LRWORK, IWORK,
     $                         LIWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhpgvD(V,' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
  ...
  220                CONTINUE
                  END IF
*
                  CALL czhpgvX( IBTYPE, 'V', 'A', UPLO, N, AP, BP, VL,
     $                         VU, IL, IU, ABSTOL, M, D, Z, LDZ, WORK,
     $                         RWORK, IWORK( N+1 ), IWORK, INFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhpgvX(V,A' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  VL = ZERO
                  VU = ANORM
                  CALL czhpgvX( IBTYPE, 'V', 'V', UPLO, N, AP, BP, VL,
     $                         VU, IL, IU, ABSTOL, M, D, Z, LDZ, WORK,
     $                         RWORK, IWORK( N+1 ), IWORK, INFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhpgvX(V,V' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
  ...
  300                CONTINUE
                  END IF
*
                  CALL czhpgvX( IBTYPE, 'V', 'I', UPLO, N, AP, BP, VL,
     $                         VU, IL, IU, ABSTOL, M, D, Z, LDZ, WORK,
     $                         RWORK, IWORK( N+1 ), IWORK, INFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'czhpgvX(V,I' // UPLO //
     $                  ')', IINFO, N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
  ...
  370                   CONTINUE
                     END IF
*
                    CALL czhbgv( 'V', UPLO, N, KA, KB, AB, LDA, BB, LDB,
     $                           D, Z, LDZ, WORK, RWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
                        WRITE( NOUNIT, FMT = 9999 )'czhbgv(V,' //
     $                     UPLO // ')', IINFO, N, JTYPE, IOLDSD
                        INFO = ABS( IINFO )
                        IF( IINFO.LT.0 ) THEN
                           RETURN
...
*
  ...
  430                   CONTINUE
                     END IF
*
                     CALL czhbgvD( 'V', UPLO, N, KA, KB, AB, LDA, BB,
     $                            LDB, D, Z, LDZ, WORK, NWORK, RWORK,
     $                            LRWORK, IWORK, LIWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
                        WRITE( NOUNIT, FMT = 9999 )'czhbgvD(V,' //
     $                     UPLO // ')', IINFO, N, JTYPE, IOLDSD
                        INFO = ABS( IINFO )
                        IF( IINFO.LT.0 ) THEN
                           RETURN
...
*
                     VL = ZERO
                     VU = ANORM
                     CALL czhbgvX( 'V', 'V', UPLO, N, KA, KB, AB, LDA,
     $                            BB, LDB, BP, MAX( 1, N ), VL, VU, IL,
     $                            IU, ABSTOL, M, D, Z, LDZ, WORK, RWORK,
     $                            IWORK( N+1 ), IWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
                        WRITE( NOUNIT, FMT = 9999 )'czhbgvX(V,V' //
     $                     UPLO // ')', IINFO, N, JTYPE, IOLDSD
                        INFO = ABS( IINFO )
                        IF( IINFO.LT.0 ) THEN
                           RETURN
...
*
  ...
  610                   CONTINUE
                     END IF
*
                     CALL czhbgvX( 'V', 'I', UPLO, N, KA, KB, AB, LDA,
     $                            BB, LDB, BP, MAX( 1, N ), VL, VU, IL,
     $                            IU, ABSTOL, M, D, Z, LDZ, WORK, RWORK,
     $                            IWORK( N+1 ), IWORK, IINFO )
                     IF( IINFO.NE.0 ) THEN
                        WRITE( NOUNIT, FMT = 9999 )'czhbgvX(V,I' //
     $                     UPLO // ')', IINFO, N, JTYPE, IOLDSD
                        INFO = ABS( IINFO )
                        IF( IINFO.LT.0 ) THEN
                           RETURN
