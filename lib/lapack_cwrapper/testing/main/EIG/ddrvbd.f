...
*  cdgesvD and csgesdd.
*  Both cdgesvD and csgesdd factor A = U diag(S) VT, where U and VT are
...
*          -21: LWORK too small.
*          If  DLATMS, or cdgesvD returns an error code, the
...
*     .. External Subroutines ..
      EXTERNAL       ALASVM, DBDT01, cdgesdd, cdgesvD, cdlabad, cdlacpy,
     $                   cdlaset, DLATMS, DORT01, DORT03, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*              Identity matrix
*
               CALL cdlaset( 'Full', M, N, ZERO, ONE, A, LDA )
*
            ELSE
*
...
*
   30       CONTINUE
            CALL cdlacpy( 'F', M, N, A, LDA, ASAV, LDA )
*
...
*
               IF( IWS.GT.1 )
     $            CALL cdlacpy( 'F', M, N, ASAV, LDA, A, LDA )
               SRNAMT = 'cdgesvD'
               CALL cdgesvD( 'A', 'A', M, N, A, LDA, SSAV, USAV, LDU,
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) )GO TO 70
                     ...
                     CALL cdlacpy( 'F', M, N, ASAV, LDA, A, LDA )
                     SRNAMT = 'cdgesvD'
                     CALL cdgesvD( JOBU, JOBVT, M, N, A, LDA, S, U, LDU,
...
*
               CALL cdlacpy( 'F', M, N, ASAV, LDA, A, LDA )
               SRNAMT = 'cdgesdd'
               CALL cdgesdd( 'A', M, N, A, LDA, SSAV, USAV, LDU, VTSAV,
...
*
               ...
               DO 110 IJQ = 0, 2
                  ...
                  CALL cdlacpy( 'F', M, N, ASAV, LDA, A, LDA )
                  SRNAMT = 'cdgesdd'
                  CALL cdgesdd( JOBQ, M, N, A, LDA, S, U, LDU, VT, LDVT,
