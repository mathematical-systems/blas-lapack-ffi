...
*  =======
*
*  SGET38 tests cstrsen, a routine for estimating condition numbers of a
...
*          If csgehrd returns INFO nonzero on example i, LMAX(1)=i
*          If cshseqr returns INFO nonzero on example i, LMAX(2)=i
*          If cstrsen returns INFO nonzero on example i, LMAX(3)=i
*
*  NINFO   (output) INTEGER array, dimension (3)
*          NINFO(1) = No. of times csgehrd returned INFO nonzero
*          NINFO(2) = No. of times cshseqr returned INFO nonzero
*          NINFO(3) = No. of times cstrsen returned INFO nonzero
*
*  KNT     (output) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL        SCOPY, csgehrd, cshseqr, SHST01, cslabad, cslacpy,
     $                   csorghr, SSCAL, cstrsen
*     ..
*     .. Intrinsic Functions ..
...
*
         KNT = KNT + 1
         CALL cslacpy( 'F', N, N, TMP, LDT, T, LDT )
         VMUL = VAL( ISCL )
         DO 30 I = 1, N
   ...
   30    CONTINUE
         IF( TNRM.EQ.ZERO )
     $      VMUL = ONE
         CALL cslacpy( 'F', N, N, T, LDT, TSAV, LDT )
*
*        Compute Schur form
*
         CALL csgehrd( N, 1, N, T, LDT, WORK( 1 ), WORK( N+1 ), LWORK-N,
     $                INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 1 ) = KNT
            NINFO( 1 ) = NINFO( 1 ) + 1
...
*
         CALL cslacpy( 'L', N, N, T, LDT, Q, LDT )
         CALL csorghr( N, 1, N, Q, LDT, WORK( 1 ), WORK( N+1 ), LWORK-N,
     $                INFO )
*
*        Compute Schur form
*
         CALL cshseqr( 'S', 'V', N, 1, N, T, LDT, WR, WI, Q, LDT, WORK,
     $                LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 2 ) = KNT
            NINFO( 2 ) = NINFO( 2 ) + 1
...
*
         CALL cslacpy( 'F', N, N, Q, LDT, QSAV, LDT )
         CALL cslacpy( 'F', N, N, T, LDT, TSAV1, LDT )
         CALL cstrsen('B', 'V', SELECT, N, T, LDT, Q, LDT, WRTMP, WITMP,
...
*
         VMAX = ZERO
         CALL cslacpy( 'F', N, N, TSAV1, LDT, TTMP, LDT )
         CALL cslacpy( 'F', N, N, QSAV, LDT, QTMP, LDT )
         SEPTMP = -ONE
         STMP = -ONE
         CALL cstrsen( 'E', 'V', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cstrsen( 'V', 'V', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
     $                WITMP, M, STMP, SEPTMP, WORK, LWORK, IWORK,
     $                LIWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cstrsen( 'E', 'N', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
     $                WITMP, M, STMP, SEPTMP, WORK, LWORK, IWORK,
     $                LIWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cstrsen( 'V', 'N', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
     $                WITMP, M, STMP, SEPTMP, WORK, LWORK, IWORK,
     $                LIWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
