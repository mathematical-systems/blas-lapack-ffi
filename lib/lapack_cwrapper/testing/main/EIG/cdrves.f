...
*          Another copy of the test matrix A, modified by ccgees.
*
*  HT      (workspace) COMPLEX array, dimension (LDA, max(NN))
*          Yet another copy of the test matrix A, modified by ccgees.
*
...
*          but those computed when ccgees only computes a partial
...
*          -18: NWORK too small.
*          If  CLATMR, CLATMS, CLATME or ccgees returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL          ccgees, CHST01, cclacpy, CLATME, CLATMR, CLATMS,
...
*
                 ...
                 CALL ccgees( 'V', SORT, CSLECT, N, H, LDA, SDIM, W, VS,
     $                        LDVS, WORK, NNWORK, RWORK, BWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     RESULT( 1+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'ccgees1', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 190
                  END IF
...
*
                  ...
                  CALL ccgees( 'N', SORT, CSLECT, N, HT, LDA, SDIM, WT,
     $                        VS, LDVS, WORK, NNWORK, RWORK, BWORK,
     $                        IINFO )
                  IF( IINFO.NE.0 ) THEN
                     RESULT( 5+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'ccgees2', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 190
                  END IF
