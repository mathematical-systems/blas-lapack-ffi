...
*  =======
*
*     ZDRVEV  checks the nonsymmetric eigenvalue problem driver czgeev.
*
...
*          Another copy of the test matrix A, modified by czgeev.
*
...
*          but those computed when czgeev only computes a partial
...
*          -21: NWORK too small.
*          If  ZLATMR, CLATMS, CLATME or czgeev returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL        cdlabad, DLASUM, xerbla, czgeev, ZGET22, czlacpy,
     $                   czlaset, ZLATME, ZLATMR, ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                      ZERO, ANORM, 'NO', A, LDA, IWORK, IINFO )
               IF( N.GE.4 ) THEN
                  CALL czlaset( 'Full', 2, N, CZERO, CZERO, A, LDA )
                  CALL czlaset( 'Full', N-3, 1, CZERO, CZERO, A( 3, 1 ),
     $                         LDA )
                  CALL czlaset( 'Full', N-3, 2, CZERO, CZERO,
     $                         A( 3, N-1 ), LDA )
                  CALL czlaset( 'Full', 1, N, CZERO, CZERO, A( N, 1 ),
     $                         LDA )
               END IF
*
...
*
               ...
               CALL czgeev( 'V', 'V', N, H, LDA, W, VL, LDVL, VR, LDVR,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'czgeev1', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL czlacpy( 'F', N, N, A, LDA, H, LDA )
               CALL czgeev( 'N', 'N', N, H, LDA, W1, DUM, 1, DUM, 1,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'czgeev2', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL czlacpy( 'F', N, N, A, LDA, H, LDA )
               CALL czgeev( 'N', 'V', N, H, LDA, W1, DUM, 1, LRE, LDLRE,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'czgeev3', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL czlacpy( 'F', N, N, A, LDA, H, LDA )
               CALL czgeev( 'V', 'N', N, H, LDA, W1, LRE, LDLRE, DUM, 1,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'czgeev4', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
