...
*  =======
*
*  CGRQTS tests ccggrqf, which computes the GRQ factorization of an
...
*          by ccggrqf, see ccggrqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL           CGEMM, ccggrqf, cclacpy, cclaset, ccungqr,
     $                   ccungrq, CHERK
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL cclacpy( 'Full', M, N, A, LDA, AF, LDA )
      CALL cclacpy( 'Full', P, N, B, LDB, BF, LDB )
*
...
*
      CALL ccggrqf( M, P, N, AF, LDA, TAUA, BF, LDB, TAUB, WORK,
     $             LWORK, INFO )
*
*     Generate the N-by-N matrix Q
*
      CALL cclaset( 'Full', N, N, CROGUE, CROGUE, Q, LDA )
      IF( M.LE.N ) THEN
         IF( M.GT.0 .AND. M.LT.N )
     $      CALL cclacpy( 'Full', M, N-M, AF, LDA, Q( N-M+1, 1 ), LDA )
         IF( M.GT.1 )
     $      CALL cclacpy( 'Lower', M-1, M-1, AF( 2, N-M+1 ), LDA,
     $                   Q( N-M+2, N-M+1 ), LDA )
      ELSE
         IF( N.GT.1 )
     $      CALL cclacpy( 'Lower', N-1, N-1, AF( M-N+2, 1 ), LDA,
     $                   Q( 2, 1 ), LDA )
      END IF
      CALL ccungrq( N, N, MIN( M, N ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL cclaset( 'Full', P, P, CROGUE, CROGUE, Z, LDB )
      IF( P.GT.1 )
     $   CALL cclacpy( 'Lower', P-1, N, BF( 2,1 ), LDB, Z( 2,1 ), LDB )
      CALL ccungqr( P, P, MIN( P,N ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL cclaset( 'Full', M, N, CZERO, CZERO, R, LDA )
      IF( M.LE.N )THEN
         CALL cclacpy('Upper', M, M, AF( 1, N-M+1 ), LDA, R( 1, N-M+1 ),
     $                LDA )
      ELSE
         CALL cclacpy('Full', M-N, N, AF, LDA, R, LDA )
         CALL cclacpy('Upper', N, N, AF( M-N+1, 1 ), LDA, R( M-N+1, 1 ),
     $                LDA )
      END IF
*
*     Copy T
*
      CALL cclaset( 'Full', P, N, CZERO, CZERO, T, LDB )
      CALL cclacpy( 'Upper', P, N, BF, LDB, T, LDB )
*
*     Compute R - A*Q'
*
...
*     Compute I - Q*Q'
*
      CALL cclaset( 'Full', N, N, CZERO, CONE, R, LDA )
...
*     Compute I - Z'*Z
*
      CALL cclaset( 'Full', P, P, CZERO, CONE, T, LDB )
