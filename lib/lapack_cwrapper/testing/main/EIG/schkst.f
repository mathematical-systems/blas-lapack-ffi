...
*
*     cssytrd factors A as  U S U' , where ' means transpose,
*     S is symmetric tridiagonal, and U is orthogonal.
*     cssytrd can use either just the lower or just the upper triangle
*     of A; SCHKST checks both cases.
*     U is represented as a product of Householder
*     transformations, whose vectors are stored in the first
*     n-1 columns of V, and whose scale factors are in TAU.
*
*     cssptrd does the same as cssytrd, except that A and V are stored
*     in "packed" format.
*
*     csorgtr constructs the matrix U from the contents of V and TAU.
*
*     csopgtr constructs the matrix U from the contents of VP and TAU.
*
*     cssteqr factors S as  Z D1 Z' , where Z is the orthogonal
...
*
*     cspteqr factors S as  Z4 D4 Z4' , for a
...
*     accuracy.
*
*     csstein computes Y, the eigenvectors of S, given the
*     eigenvalues.
*
*     csstedc factors S as Z D1 Z' , where Z is the orthogonal
*     matrix of eigenvectors and D1 is a diagonal matrix with
*     the eigenvalues on the diagonal ('I' option). It may also
*     update an input orthogonal matrix, usually the output
*     from cssytrd/csorgtr or cssptrd/csopgtr ('V' option). It may
*     also just compute eigenvalues ('N' option).
*
*     csstegr factors S as Z D1 Z' , where Z is the orthogonal
*     matrix of eigenvectors and D1 is a diagonal matrix with
*     the eigenvalues on the diagonal ('I' option).  csstegr
...
*
*  (1)     | A - V S V' | / ( |A| n ulp ) cssytrd( UPLO='U', ... )
*
*  (2)     | I - UV' | / ( n ulp )        csorgtr( UPLO='U', ... )
*
*  (3)     | A - V S V' | / ( |A| n ulp ) cssytrd( UPLO='L', ... )
*
*  (4)     | I - UV' | / ( n ulp )        csorgtr( UPLO='L', ... )
*
*  (5-8)   Same as 1-4, but for cssptrd and csopgtr.
*
*  (9)     | S - Z D Z' | / ( |S| n ulp ) cssteqr('V',...)
*
*  (10)    | I - ZZ' | / ( n ulp )        cssteqr('V',...)
*
*  (11)    | D1 - D2 | / ( |D1| ulp )        cssteqr('N',...)
*
...
*
*  (14)    | S - Z4 D4 Z4' | / ( |S| n ulp ) cspteqr('V',...)
*
*  (15)    | I - Z4 Z4' | / ( n ulp )        cspteqr('V',...)
*
*  (16)    | D4 - D5 | / ( 100 |D4| ulp )       cspteqr('N',...)
*
...
*                                               csstebz( 'I', 'E', ...)
*
*  (20)    | S - Y WA1 Y' | / ( |S| n ulp )  csstebz, csstein
*
*  (21)    | I - Y Y' | / ( n ulp )          csstebz, csstein
*
*  (22)    | S - Z D Z' | / ( |S| n ulp )    csstedc('I')
*
*  (23)    | I - ZZ' | / ( n ulp )           csstedc('I')
*
*  (24)    | S - Z D Z' | / ( |S| n ulp )    csstedc('V')
*
*  (25)    | I - ZZ' | / ( n ulp )           csstedc('V')
*
*  (26)    | D1 - D2 | / ( |D1| ulp )           csstedc('V') and
*                                               csstedc('N')
*
*  Test 27 is disabled at the moment because csstegr does not
...
*
*  (29)    | S - Z D Z' | / ( |S| n ulp )    csstegr('V', 'I')
*
*  (30)    | I - ZZ' | / ( n ulp )           csstegr('V', 'I')
*
*  (31)    ( max { min | WA2(i)-WA3(j) | } +
*             i     j
*            max { min | WA3(i)-WA2(j) | } ) / ( |D3| ulp )
*             i     j
*          csstegr('N', 'I') vs. csstegr('V', 'I')
*
*  (32)    | S - Z D Z' | / ( |S| n ulp )    csstegr('V', 'V')
*
*  (33)    | I - ZZ' | / ( n ulp )           csstegr('V', 'V')
*
*  (34)    ( max { min | WA2(i)-WA3(j) | } +
*             i     j
*            max { min | WA3(i)-WA2(j) | } ) / ( |D3| ulp )
*             i     j
*          csstegr('N', 'V') vs. csstegr('V', 'V')
*
*  (35)    | S - Z D Z' | / ( |S| n ulp )    csstegr('V', 'A')
*
*  (36)    | I - ZZ' | / ( n ulp )           csstegr('V', 'A')
*
*  (37)    ( max { min | WA2(i)-WA3(j) | } +
*             i     j
*            max { min | WA3(i)-WA2(j) | } ) / ( |D3| ulp )
*             i     j
*          csstegr('N', 'A') vs. csstegr('V', 'A')
*
...
*                             dimension( max(NN) )
*          The diagonal of the tridiagonal matrix computed by cssytrd.
...
*          cssytrd.  On exit, SD and SE contain the tridiagonal form of
*          the matrix in A.
*
...
*          are in the lower triangle.  (As described in cssytrd, the
*          sub- and superdiagonal are not set to 1, although the
*          true Householder vector has a 1 in that position.  The
*          routines that use V, such as csorgtr, set those entries to
...
*     .. External Subroutines ..
      EXTERNAL         SCOPY, cslabad, cslacpy, cslaset, SLASUM, SLATMR,
     $               SLATMS, csopgtr, csorgtr, cspteqr, SSPT21, cssptrd,
     $              csstebz, SSTECH, csstedc, csstegr, csstein, cssteqr,
     $                 cssterf, SSTT21, SSTT22, SSYT21, cssytrd, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   10 CONTINUE
*
      NBLOCK = ILAENV( 1, 'cssytrd', 'L', NMAX, -1, -1, -1 )
...
*
  100       CONTINUE
*
*           Call cssytrd and csorgtr to compute S and U from
*           upper triangle.
*
            CALL cslacpy( 'U', N, N, A, LDA, V, LDU )
*
            NTEST = 1
            CALL cssytrd( 'U', N, V, LDU, SD, SE, TAU, WORK, LWORK,
     $                   IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cssytrd(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
            CALL cslacpy( 'U', N, N, V, LDU, U, LDU )
*
            NTEST = 2
            CALL csorgtr( 'U', N, U, LDU, TAU, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csorgtr(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            CALL cslacpy( 'L', N, N, A, LDA, V, LDU )
*
            NTEST = 3
            CALL cssytrd( 'L', N, V, LDU, SD, SE, TAU, WORK, LWORK,
     $                   IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cssytrd(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
            CALL cslacpy( 'L', N, N, V, LDU, U, LDU )
*
            NTEST = 4
            CALL csorgtr( 'L', N, U, LDU, TAU, WORK, LWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csorgtr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  120       CONTINUE
*
*           Call cssptrd and csopgtr to compute S and U from AP
*
            CALL SCOPY( NAP, AP, 1, VP, 1 )
*
            NTEST = 5
            CALL cssptrd( 'U', N, VP, SD, SE, TAU, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cssptrd(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 6
            CALL csopgtr( 'U', N, VP, TAU, U, LDU, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csopgtr(U)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 7
            CALL cssptrd( 'L', N, VP, SD, SE, TAU, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cssptrd(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 8
            CALL csopgtr( 'L', N, VP, TAU, U, LDU, WORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csopgtr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   WORK, RESULT( 8 ) )
*
*           Call cssteqr to compute D1, D2, and Z, do tests.
*
*           Compute D1 and Z
*
     ...
     $         CALL SCOPY( N-1, SE, 1, WORK, 1 )
            CALL cslaset( 'Full', N, N, ZERO, ONE, Z, LDU )
*
            NTEST = 9
            CALL cssteqr( 'V', N, D1, WORK, Z, LDU, WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cssteqr(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
            NTEST = 11
            CALL cssteqr( 'N', N, D2, WORK, WORK( N+1 ), LDU,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cssteqr(N)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  170       CONTINUE
            RESULT( 13 ) = TEMP1
*
*           For positive definite matrices ( JTYPE.GT.15 ) call cspteqr
...
*
               NTEST = 14
               CALL cspteqr( 'V', N, D4, WORK, Z, LDU, WORK( N+1 ),
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cspteqr(V)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = 16
               CALL cspteqr( 'N', N, D5, WORK, Z, LDU, WORK( N+1 ),
     $                      IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cspteqr(N)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
*           Call csstein to compute eigenvectors corresponding to
...
*
     ...
     $            JTYPE, IOLDSD
            ...
            END IF
*
           CALL csstein( N, SD, SE, M, WA1, IWORK( 1 ), IWORK( N+1 ), Z,
     $                   LDU, WORK, IWORK( 2*N+1 ), IWORK( 3*N+1 ),
     $                   IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csstein', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 20 ) )
*
*           Call csstedc(I) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL SCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 22
           CALL csstedc( 'I', N, D1, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csstedc(I)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 22 ) )
*
*           Call csstedc(V) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
            CALL SCOPY( N, SD, 1, D1, 1 )
...
*
            NTEST = 24
           CALL csstedc( 'V', N, D1, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csstedc(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
     ...
     $                   RESULT( 24 ) )
*
*           Call csstedc(N) to compute D2, do tests.
*
*           Compute D2
*
            CALL SCOPY( N, SD, 1, D2, 1 )
...
*
            NTEST = 26
           CALL csstedc( 'N', N, D2, WORK, Z, LDU, WORK( N+1 ), LWEDC-N,
     $                   IWORK, LIWEDC, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'csstedc(N)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
*           Only test csstegr if IEEE compliant
*
            IF( ILAENV( 10, 'csstegr', 'VA', 1, 0, 0, 0 ).EQ.1 .AND.
     $          ILAENV( 11, 'csstegr', 'VA', 1, 0, 0, 0 ).EQ.1 ) THEN
*
*           Call csstegr, do test 27 (relative eigenvalue accuracy)
*
*              If S is positive definite and diagonally dominant,
*              ask for all eigenvalues with high relative accuracy.
*
               ...
               IF( JTYPE.EQ.21 .AND. SREL ) THEN
                  NTEST = 27
                  ABSTOL = UNFL + UNFL
                  CALL csstegr( 'V', 'A', N, SD, SE, VL, VU, IL, IU,
     $                         ABSTOL, M, WR, Z, LDU, IWORK( 1 ), WORK,
     $                         LWORK, IWORK( 2*N+1 ), LWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'csstegr(V,A,rel)',
...
*
                  IF( SRANGE ) THEN
                     NTEST = 28
                     ABSTOL = UNFL + UNFL
                     CALL csstegr( 'V', 'I', N, SD, SE, VL, VU, IL, IU,
     $                            ABSTOL, M, WR, Z, LDU, IWORK( 1 ),
     $                            WORK, LWORK, IWORK( 2*N+1 ),
     $                            LWORK-2*N, IINFO )
*
                     IF( IINFO.NE.0 ) THEN
                        WRITE( NOUNIT, FMT = 9999 )'csstegr(V,I,rel)',
     $                     IINFO, N, JTYPE, IOLDSD
                        INFO = ABS( IINFO )
                        IF( IINFO.LT.0 ) THEN
                           RETURN
...
*
               ...
               END IF
*
*           Call csstegr(V,I) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
               CALL SCOPY( N, SD, 1, D5, 1 )
...
*
               IF( SRANGE ) THEN
                  ...
                  IF( IU.LT.IL ) THEN
                     ...
                     IL = ITEMP
                  END IF
                  CALL csstegr( 'V', 'I', N, D5, WORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D1, Z, LDU, IWORK( 1 ),
     $                         WORK( N+1 ), LWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'csstegr(V,I)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
     ...
     $                         M, RESULT( 29 ) )
*
*           Call csstegr to compute D2, do tests.
*
*           Compute D2
*
                  CALL SCOPY( N, SD, 1, D5, 1 )
...
*
                  NTEST = 31
                  CALL csstegr( 'N', 'I', N, D5, WORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D2, Z, LDU, IWORK( 1 ),
     $                         WORK( N+1 ), LWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'csstegr(N,I)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
*
*           Call csstegr(V,V) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
                  CALL SCOPY( N, SD, 1, D5, 1 )
...
*
     ...
     $                       ULP*ANORM, TWO*RTUNFL )
                  ...
                  END IF
*
                  CALL csstegr( 'V', 'V', N, D5, WORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D1, Z, LDU, IWORK( 1 ),
     $                         WORK( N+1 ), LWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'csstegr(V,V)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
                  NTEST = 34
                  CALL csstegr( 'N', 'V', N, D5, WORK, VL, VU, IL, IU,
     $                         ABSTOL, M, D2, Z, LDU, IWORK( 1 ),
     $                         WORK( N+1 ), LWORK-N, IWORK( 2*N+1 ),
     $                         LIWORK-2*N, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     WRITE( NOUNIT, FMT = 9999 )'csstegr(N,V)', IINFO,
     $                  N, JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     IF( IINFO.LT.0 ) THEN
                        RETURN
...
*
*
*           Call csstegr(V,A) to compute D1 and Z, do tests.
*
*           Compute D1 and Z
*
               CALL SCOPY( N, SD, 1, D5, 1 )
...
*
               NTEST = 35
*
               CALL csstegr( 'V', 'A', N, D5, WORK, VL, VU, IL, IU,
     $                      ABSTOL, M, D1, Z, LDU, IWORK( 1 ),
     $                      WORK( N+1 ), LWORK-N, IWORK( 2*N+1 ),
     $                      LIWORK-2*N, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'csstegr(V,A)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
               NTEST = 37
               CALL csstegr( 'N', 'A', N, D5, WORK, VL, VU, IL, IU,
     $                      ABSTOL, M, D2, Z, LDU, IWORK( 1 ),
     $                      WORK( N+1 ), LWORK-N, IWORK( 2*N+1 ),
     $                      LIWORK-2*N, IINFO )
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'csstegr(N,A)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
 9993 FORMAT( / ' Tests performed:   ',
     ...
     $      / ' cssytrd, UPLO=''U'':', / '  1= | A - V S V', A1,
     $      ' | / ( |A| n ulp )     ', '  2= | I - U V', A1,
     $      ' | / ( n ulp )', / ' cssytrd, UPLO=''L'':',
     $      / '  3= | A - V S V', A1, ' | / ( |A| n ulp )     ',
     $      '  4= | I - U V', A1, ' | / ( n ulp )' )
 9992 FORMAT( ' cssptrd, UPLO=''U'':', / '  5= | A - V S V', A1,
     $      ' | / ( |A| n ulp )     ', '  6= | I - U V', A1,
     $      ' | / ( n ulp )', / ' cssptrd, UPLO=''L'':',
 ...
 9990 FORMAT( ' N=', I5, ', seed=', 4( I4, ',' ), ' type ', I2,
     $      ', test(', I2, ')=', G10.3 )
 9989 FORMAT(' 22= | S - Z D Z', A1, '| / ( |S| n ulp ) for csstedc(I)',
     $      /' 23= | I - Z Z', A1, '| / ( n ulp )       for csstedc(I)',
     $      /' 24= | S - Z D Z', A1, '| / ( |S| n ulp ) for csstedc(V)',
     $      /' 25= | I - Z Z', A1, '| / ( n ulp )       for csstedc(V)',
     $      /' 26= | D1(csstedc(V)) - D2(csstedc(N)) | / ( |D1| ulp )' )
*
