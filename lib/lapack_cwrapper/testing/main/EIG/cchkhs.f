...
*
*             ccgehrd factors A as  U H U' , where ' means conjugate
*             transpose, H is hessenberg, and U is unitary.
*
*             ccunghr generates the unitary matrix U.
*
*             ccunmhr multiplies a matrix by the unitary matrix U.
*
*             cchseqr factors H as  Z T Z' , where Z is unitary and T
*             is upper triangular.  It also computes the eigenvalues,
*             w(1), ..., w(n); we define a diagonal matrix W whose
*             (diagonal) entries are the eigenvalues.
*
*             cctrevc computes the left eigenvector matrix L and the
...
*             upper triangular.
*
*             cchsein computes the left eigenvector matrix Y and the
...
*           The upper hessenberg matrix computed by ccgehrd.  On exit,
*           H contains the Hessenberg form of the matrix in A.
*           Modified.
*
*  T1     - COMPLEX array, dimension (LDA,max(NN))
*           The Schur (="quasi-triangular") matrix computed by cchseqr
*           if Z is computed.  On exit, T1 contains the Schur form of
*           the matrix in A.
*           Modified.
*
*  T2     - COMPLEX array, dimension (LDA,max(NN))
*           The Schur matrix computed by cchseqr when Z is not computed.
...
*           The unitary matrix computed by ccgehrd.
*           Modified.
*
*  Z      - COMPLEX array, dimension (LDU,max(NN))
*           The unitary matrix computed by cchseqr.
*           Modified.
*
...
*           Details of the unitary matrix computed by ccgehrd.
*           Modified.
*
*  TAU    - COMPLEX array, dimension (max(NN))
*           Further details of the unitary matrix computed by ccgehrd.
*           Modified.
*
...
*           If 1, then cchseqr could not find all the shifts.
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, ccgehrd, CGEMM, CGET10, CGET22, cchsein,
     $                cchseqr, CHST01, cclacpy, cclaset, CLATME, CLATMR,
     $               CLATMS, cctrevc, ccunghr, ccunmhr, cslabad, SLAFTS,
     $                   SLASUM, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  100       CONTINUE
*
*           Call ccgehrd to compute H and U, do tests.
*
...
*
            ...
            IHI = N
*
            CALL ccgehrd( N, ILO, IHI, H, LDA, WORK, WORK( N+1 ),
     $                   NWORK-N, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'ccgehrd', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
*
  ...
  120       CONTINUE
            ...
            CALL ccunghr( N, ILO, IHI, U, LDU, WORK, WORK( N+1 ),
     $                   NWORK-N, IINFO )
            NTEST = 2
*
...
*
*           Call cchseqr to compute T1, T2 and Z, do tests.
*
*           Eigenvalues only (W3)
*
            CALL cclacpy( ' ', N, N, H, LDA, T2, LDA )
            NTEST = 3
            RESULT( 3 ) = ULPINV
*
            CALL cchseqr( 'E', 'N', N, ILO, IHI, T2, LDA, W3, UZ, LDU,
     $                   WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchseqr(E)', IINFO, N, JTYPE,
     $            IOLDSD
...
*
            CALL cclacpy( ' ', N, N, H, LDA, T2, LDA )
*
            CALL cchseqr( 'S', 'N', N, ILO, IHI, T2, LDA, W1, UZ, LDU,
     $                   WORK, NWORK, IINFO )
            IF( IINFO.NE.0 .AND. IINFO.LE.N+2 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchseqr(S)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
            CALL cclacpy( ' ', N, N, H, LDA, T1, LDA )
            CALL cclacpy( ' ', N, N, U, LDU, UZ, LDU )
*
            CALL cchseqr( 'S', 'V', N, ILO, IHI, T1, LDA, W1, UZ, LDU,
     $                   WORK, NWORK, IINFO )
            IF( IINFO.NE.0 .AND. IINFO.LE.N+2 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchseqr(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
  ...
  140       CONTINUE
            DO 150 J = 1, N, 2
               SELECT( J ) = .TRUE.
  150       CONTINUE
            CALL cctrevc( 'Right', 'All', SELECT, N, T1, LDA, CDUMMA,
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctrevc(R,A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
     ...
     $                   WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cctrevc',
...
*
            CALL cctrevc( 'Right', 'Some', SELECT, N, T1, LDA, CDUMMA,
     $                   LDU, EVECTL, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctrevc(R,S)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
*
  ...
  180       CONTINUE
            IF( .NOT.MATCH )
     $         WRITE( NOUNIT, FMT = 9997 )'Right', 'cctrevc', N, JTYPE,
     $         IOLDSD
*
...
*
            NTEST = 10
            RESULT( 10 ) = ULPINV
           CALL cctrevc( 'Left', 'All', SELECT, N, T1, LDA, EVECTL, LDU,
     $                   CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctrevc(L,A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
     ...
     $                   WORK, RWORK, DUMMA( 3 ) )
            ...
            IF( DUMMA( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cctrevc', DUMMA( 4 ),
     $            N, JTYPE, IOLDSD
            END IF
*
...
*
            CALL cctrevc( 'Left', 'Some', SELECT, N, T1, LDA, EVECTR,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctrevc(L,S)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
*
  ...
  210       CONTINUE
            IF( .NOT.MATCH )
     $         WRITE( NOUNIT, FMT = 9997 )'Left', 'cctrevc', N, JTYPE,
     $         IOLDSD
*
*           Call cchsein for Right eigenvectors of H, do test 11
*
            NTEST = 11
            RESULT( 11 ) = ULPINV
            DO 220 J = 1, N
  ...
  220       CONTINUE
*
           CALL cchsein( 'Right', 'Qr', 'Ninitv', SELECT, N, H, LDA, W3,
     $                   CDUMMA, LDU, EVECTX, LDU, N1, IN, WORK, RWORK,
     $                   IWORK, IWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchsein(R)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 240
...
*
     ...
     $            RESULT( 11 ) = DUMMA( 1 )*ANINV
               IF( DUMMA( 2 ).GT.THRESH ) THEN
                  WRITE( NOUNIT, FMT = 9998 )'Right', 'cchsein',
     $               DUMMA( 2 ), N, JTYPE, IOLDSD
               END IF
            END IF
*
*           Call cchsein for Left eigenvectors of H, do test 12
*
            NTEST = 12
            RESULT( 12 ) = ULPINV
            DO 230 J = 1, N
  ...
  230       CONTINUE
*
            CALL cchsein( 'Left', 'Qr', 'Ninitv', SELECT, N, H, LDA, W3,
     $                   EVECTY, LDU, CDUMMA, LDU, N1, IN, WORK, RWORK,
     $                   IWORK, IWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchsein(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 240
...
*
     ...
     $            RESULT( 12 ) = DUMMA( 3 )*ANINV
               IF( DUMMA( 4 ).GT.THRESH ) THEN
                  WRITE( NOUNIT, FMT = 9998 )'Left', 'cchsein',
     $               DUMMA( 4 ), N, JTYPE, IOLDSD
               END IF
            END IF
*
*           Call ccunmhr for Right eigenvectors of A, do test 13
*
            NTEST = 13
            RESULT( 13 ) = ULPINV
*
            CALL ccunmhr( 'Left', 'No transpose', N, N, ILO, IHI, UU,
     $                   LDU, TAU, EVECTX, LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'ccunmhr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 240
...
*
     ...
     $            RESULT( 13 ) = DUMMA( 1 )*ANINV
            END IF
*
*           Call ccunmhr for Left eigenvectors of A, do test 14
*
            NTEST = 14
            RESULT( 14 ) = ULPINV
*
