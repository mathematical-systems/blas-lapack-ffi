...
*
*  cdgebrd reduces a real general m by n matrix A to upper or lower
*  bidiagonal form B by an orthogonal transformation:  Q' * A * P = B
*  (or A = Q * B * P').  The matrix B is upper bidiagonal if m >= n
*  and lower bidiagonal if m < n.
*
*  cdorgbr generates the orthogonal matrices Q and P' from cdgebrd.
*  Note that Q and P are not necessarily square.
*
*  cdbdsqr computes the singular value decomposition of the bidiagonal
...
*In addition, cdbdsqr has an option to apply the left orthogonal matrix
*  U to a matrix X, useful in least squares applications.
*
*  cdbdsdc computes the singular value decomposition of the bidiagonal
...
*  (3)   | I - PT PT' | / ( N ulp )
*
*  Test cdbdsqr on bidiagonal matrix B
*
...
*
*  Test cdbdsdc on bidiagonal matrix B
*
...
*       (a) cdgebrd is not called to reduce it to bidiagonal form.
...
*          and Z, used in testing cdbdsqr.  If NRHS = 0, then the
...
*          -27: LWORK too small.
*          If  DLATMR, SLATMS, cdgebrd, cdorgbr, or cdbdsqr,
...
*     .. External Subroutines ..
      EXTERNAL         ALASUM, cdbdsdc, cdbdsqr, DBDT01, DBDT02, DBDT03,
     $                  DCOPY, cdgebrd, DGEMM, cdlabad, cdlacpy, DLAHD2,
     $                 cdlaset, DLATMR, DLATMS, cdorgbr, DORT01, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  100       CONTINUE
*
*           Call cdgebrd and cdorgbr to compute B, Q, and P, do tests.
*
            IF( .NOT.BIDIAG ) THEN
*
...
*              B := Q' * A * P.
*
               CALL cdlacpy( ' ', M, N, A, LDA, Q, LDQ )
              CALL cdgebrd( M, N, Q, LDQ, BD, BE, WORK, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from cdgebrd.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'cdgebrd', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
*
               CALL cdlacpy( ' ', M, N, Q, LDQ, PT, LDPT )
               IF( M.GE.N ) THEN
                  UPLO = 'U'
               ELSE
                  UPLO = 'L'
...
*
               ...
               IF( NRHS.LE.0 )
     $            MQ = MNMIN
               CALL cdorgbr( 'Q', M, MQ, N, Q, LDQ, WORK,
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from cdorgbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'cdorgbr(Q)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*              Generate P'
*
              CALL cdorgbr( 'P', MNMIN, N, M, PT, LDPT, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from cdorgbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'cdorgbr(P)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
     ...
     $                      RESULT( 3 ) )
            END IF
*
*           Use cdbdsqr to form the SVD of the bidiagonal matrix B:
*           B := U * S1 * VT, and compute Z = U' * Y.
*
            CALL DCOPY( MNMIN, BD, 1, S1, 1 )
            IF( MNMIN.GT.0 )
     $         CALL DCOPY( MNMIN-1, BE, 1, WORK, 1 )
            CALL cdlacpy( ' ', M, NRHS, Y, LDX, Z, LDX )
            CALL cdlaset( 'Full', MNMIN, MNMIN, ZERO, ONE, U, LDPT )
            CALL cdlaset( 'Full', MNMIN, MNMIN, ZERO, ONE, VT, LDPT )
*
            CALL cdbdsqr( UPLO, MNMIN, MNMIN, MNMIN, NRHS, S1, WORK, VT,
     $                   LDPT, U, LDPT, Z, LDX, WORK( MNMIN+1 ), IINFO )
*
*           Check error code from cdbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'cdbdsqr(vects)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
*           Use cdbdsqr to compute only the singular values of the
...
*
     ...
     $         CALL DCOPY( MNMIN-1, BE, 1, WORK, 1 )
*
            CALL cdbdsqr( UPLO, MNMIN, 0, 0, 0, S2, WORK, VT, LDPT, U,
     $                   LDPT, Z, LDX, WORK( MNMIN+1 ), IINFO )
*
*           Check error code from cdbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'cdbdsqr(values)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  110       CONTINUE
     ...
     $            RESULT( 8 ) = ULPINV
            END IF
*
*           Test 9:  Compare cdbdsqr with and without singular vectors
*
            TEMP2 = ZERO
*
            DO 120 J = 1, MNMIN
...
*
  140       CONTINUE
            RESULT( 10 ) = TEMP1
*
*           Use cdbdsqr to form the decomposition A := (QU) S (VT PT)
...
*
     ...
     $            CALL DCOPY( MNMIN-1, BE, 1, WORK, 1 )
*
              CALL cdbdsqr( UPLO, MNMIN, N, M, NRHS, S2, WORK, PT, LDPT,
...
*
     ...
     $                      RESULT( 14 ) )
            END IF
*
*           Use cdbdsdc to form the SVD of the bidiagonal matrix B:
*           B := U * S1 * VT
*
            CALL DCOPY( MNMIN, BD, 1, S1, 1 )
            IF( MNMIN.GT.0 )
     $         CALL DCOPY( MNMIN-1, BE, 1, WORK, 1 )
            ...
            CALL cdlaset( 'Full', MNMIN, MNMIN, ZERO, ONE, VT, LDPT )
*
            CALL cdbdsdc( UPLO, 'I', MNMIN, S1, WORK, U, LDPT, VT, LDPT,
     $                   DUM, IDUM, WORK( MNMIN+1 ), IWORK, IINFO )
*
*           Check error code from cdbdsdc.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'cdbdsdc(vects)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
*           Use cdbdsdc to compute only the singular values of the
...
*
     ...
     $         CALL DCOPY( MNMIN-1, BE, 1, WORK, 1 )
*
            CALL cdbdsdc( UPLO, 'N', MNMIN, S2, WORK, DUM, 1, DUM, 1,
     $                   DUM, IDUM, WORK( MNMIN+1 ), IWORK, IINFO )
*
*           Check error code from cdbdsdc.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'cdbdsdc(values)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  150       CONTINUE
     ...
     $            RESULT( 18 ) = ULPINV
            END IF
*
*           Test 19:  Compare cdbdsqr with and without singular vectors
*
            TEMP2 = ZERO
*
            DO 160 J = 1, MNMIN
