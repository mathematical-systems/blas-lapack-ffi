...
*          Another copy of the test matrix A, modified by csgees.
*
*  HT      (workspace) REAL array, dimension (LDA, max(NN))
*          Yet another copy of the test matrix A, modified by csgees.
*
...
*          but those computed when csgees only computes a partial
...
*          -20: NWORK too small.
*          If  SLATMR, SLATMS, SLATME or csgees returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL         csgees, SHST01, cslabad, cslacpy, SLASUM, SLATME,
...
*
                  ...
                  CALL csgees( 'V', SORT, SSLECT, N, H, LDA, SDIM, WR,
     $                        WI, VS, LDVS, WORK, NNWORK, BWORK, IINFO )
                  IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                     RESULT( 1+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'csgees1', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 220
                  END IF
...
*
                  ...
                  CALL csgees( 'N', SORT, SSLECT, N, HT, LDA, SDIM, WRT,
     $                        WIT, VS, LDVS, WORK, NNWORK, BWORK,
     $                        IINFO )
                  IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                     RESULT( 5+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'csgees2', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 220
                  END IF
