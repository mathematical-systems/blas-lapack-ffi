...
*  =======
*
*  ZDRVBD checks the singular value decomposition (SVD) driver czgesvD
*  and czgesdd.
*  czgesvD and ccgesdd factors A = U diag(S) VT, where U and VT are
...
*          -21: LWORK too small.
*          If  ZLATMS, or czgesvD returns an error code, the
...
*     .. External Subroutines ..
      EXTERNAL       ALASVM, xerbla, ZBDT01, czgesdd, czgesvD, czlacpy,
     $                   czlaset, ZLATMS, ZUNT01, ZUNT03
*     ..
*     .. Intrinsic Functions ..
...
*              Identity matrix
*
               CALL czlaset( 'Full', M, N, CZERO, CONE, A, LDA )
               DO 40 I = 1, MIN( M, N )
                  S( I ) = ONE
   40          CONTINUE
*
...
*
   50       CONTINUE
            CALL czlacpy( 'F', M, N, A, LDA, ASAV, LDA )
*
...
*
               IF( IWSPC.GT.1 )
     $            CALL czlacpy( 'F', M, N, ASAV, LDA, A, LDA )
               CALL czgesvD( 'A', 'A', M, N, A, LDA, SSAV, USAV, LDU,
...
*
     ...
     $                   ( IJU.EQ.1 .AND. IJVT.EQ.1 ) )GO TO 90
                     ...
                     JOBVT = CJOB( IJVT+1 )
                     CALL czlacpy( 'F', M, N, ASAV, LDA, A, LDA )
                     CALL czgesvD( JOBU, JOBVT, M, N, A, LDA, S, U, LDU,
...
*
               CALL czlacpy( 'F', M, N, ASAV, LDA, A, LDA )
               CALL czgesdd( 'A', M, N, A, LDA, SSAV, USAV, LDU, VTSAV,
...
*
               ...
               DO 130 IJQ = 0, 2
                  JOBQ = CJOB( IJQ+1 )
                  CALL czlacpy( 'F', M, N, ASAV, LDA, A, LDA )
                  CALL czgesdd( JOBQ, M, N, A, LDA, S, U, LDU, VT, LDVT,
