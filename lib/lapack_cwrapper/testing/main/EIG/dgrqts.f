...
*  =======
*
*  DGRQTS tests cdggrqf, which computes the GRQ factorization of an
...
*          by cdggrqf, see csggrqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL       DGEMM, cdggrqf, cdlacpy, cdlaset, cdorgqr, cdorgrq,
     $                   DSYRK
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL cdlacpy( 'Full', M, N, A, LDA, AF, LDA )
      CALL cdlacpy( 'Full', P, N, B, LDB, BF, LDB )
*
...
*
      CALL cdggrqf( M, P, N, AF, LDA, TAUA, BF, LDB, TAUB, WORK, LWORK,
     $             INFO )
*
...
*
     ...
     $      CALL cdlacpy( 'Full', M, N-M, AF, LDA, Q( N-M+1, 1 ), LDA )
         IF( M.GT.1 )
     $      CALL cdlacpy( 'Lower', M-1, M-1, AF( 2, N-M+1 ), LDA,
     $                   Q( N-M+2, N-M+1 ), LDA )
      ELSE
         IF( N.GT.1 )
     $      CALL cdlacpy( 'Lower', N-1, N-1, AF( M-N+2, 1 ), LDA,
     $                   Q( 2, 1 ), LDA )
      END IF
      CALL cdorgrq( N, N, MIN( M, N ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL cdlaset( 'Full', P, P, ROGUE, ROGUE, Z, LDB )
      IF( P.GT.1 )
     $  CALL cdlacpy( 'Lower', P-1, N, BF( 2, 1 ), LDB, Z( 2, 1 ), LDB )
      CALL cdorgqr( P, P, MIN( P, N ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL cdlaset( 'Full', M, N, ZERO, ZERO, R, LDA )
      IF( M.LE.N ) THEN
        CALL cdlacpy( 'Upper', M, M, AF( 1, N-M+1 ), LDA, R( 1, N-M+1 ),
     $                LDA )
      ELSE
         CALL cdlacpy( 'Full', M-N, N, AF, LDA, R, LDA )
        CALL cdlacpy( 'Upper', N, N, AF( M-N+1, 1 ), LDA, R( M-N+1, 1 ),
     $                LDA )
      END IF
*
*     Copy T
*
      CALL cdlaset( 'Full', P, N, ZERO, ZERO, T, LDB )
      CALL cdlacpy( 'Upper', P, N, BF, LDB, T, LDB )
*
*     Compute R - A*Q'
*
