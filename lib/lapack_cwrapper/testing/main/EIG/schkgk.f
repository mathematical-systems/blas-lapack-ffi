...
*  =======
*
*  SCHKGK tests csggbak, a routine for backward balancing  of
*  a matrix pair (A, B).
*
*  Arguments
*  =========
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, csggbak, csggbal, cslacpy
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
*     ..
...
*
      CALL cslacpy( 'FULL', N, N, A, LDA, AF, LDA )
      CALL cslacpy( 'FULL', N, N, B, LDB, BF, LDB )
*
      CALL csggbal( 'B', N, A, LDA, B, LDB, ILO, IHI, LSCALE, RSCALE,
     $             WORK, INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 1 ) = KNT
      END IF
*
      CALL cslacpy( 'FULL', N, M, VL, LDVL, VLF, LDVL )
      CALL cslacpy( 'FULL', N, M, VR, LDVR, VRF, LDVR )
*
      CALL csggbak( 'B', 'L', N, ILO, IHI, LSCALE, RSCALE, M, VL, LDVL,
     $             INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 2 ) = KNT
      END IF
*
      CALL csggbak( 'B', 'R', N, ILO, IHI, LSCALE, RSCALE, M, VR, LDVR,
     $             INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 3 ) = KNT
...
*
      WRITE( NOUT, FMT = 9999 )
 9999 FORMAT( 1X, '.. test output of csggbak .. ' )
*
      WRITE( NOUT, FMT = 9998 )RMAX
 9998 FORMAT( ' value of largest test error                  =', E12.3 )
      WRITE( NOUT, FMT = 9997 )LMAX( 1 )
 9997 FORMAT( ' example number where csggbal info is not 0    =', I4 )
      WRITE( NOUT, FMT = 9996 )LMAX( 2 )
 9996 FORMAT( ' example number where csggbak(L) info is not 0 =', I4 )
      WRITE( NOUT, FMT = 9995 )LMAX( 3 )
 9995 FORMAT( ' example number where csggbak(R) info is not 0 =', I4 )
