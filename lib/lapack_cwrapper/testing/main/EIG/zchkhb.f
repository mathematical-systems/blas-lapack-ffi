...
*
*  czhbtrd factors a Hermitian band matrix A as  U S U* , where * means
*  conjugate transpose, S is symmetric tridiagonal, and U is unitary.
*  czhbtrd can use either just the lower or just the upper triangle
...
*
*  (1)     | A - V S V* | / ( |A| n ulp )  computed by czhbtrd with
*                                          UPLO='U'
*
*  (2)     | I - UU* | / ( n ulp )
*
*  (3)     | A - V S V* | / ( |A| n ulp )  computed by czhbtrd with
*                                          UPLO='L'
*
*  (4)     | I - UU* | / ( n ulp )
*
...
*          Used to hold the unitary matrix computed by czhbtrd.
*
*  LDU     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL       DLASUM, xerbla, ZHBT21, czhbtrd, czlacpy, czlaset,
     $                   ZLATMR, ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*
  100          CONTINUE
*
*              Call czhbtrd to compute S and U from upper triangle.
*
               CALL czlacpy( ' ', K+1, N, A, LDA, WORK, LDA )
*
               NTEST = 1
               CALL czhbtrd( 'V', 'U', N, K, WORK, LDA, SD, SE, U, LDU,
     $                      WORK( LDA*N+1 ), IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czhbtrd(U)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  140          CONTINUE
*
*              Call czhbtrd to compute S and U from lower triangle
*
               CALL czlacpy( ' ', K+1, N, A, LDA, WORK, LDA )
*
               NTEST = 3
               CALL czhbtrd( 'V', 'L', N, K, WORK, LDA, SD, SE, U, LDU,
     $                      WORK( LDA*N+1 ), IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'czhbtrd(L)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
