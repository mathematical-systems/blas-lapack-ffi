...
*          The Schur form matrix computed from A by ccgges.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) COMPLEX array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by ccgges.
*
*  Q       (workspace) COMPLEX array, dimension (LDQ, max(NN))
*          The (left) orthogonal matrix computed by ccgges.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by ccgges.
*
*  ALPHA   (workspace) COMPLEX array, dimension (max(NN))
*  BETA    (workspace) COMPLEX array, dimension (max(NN))
*          The generalized eigenvalues of (A,B) computed by ccgges.
...
*     .. External Subroutines ..
      EXTERNAL         ALASVM, CGET51, CGET54, ccgges, cclacpy, cclarfG,
     $                   cclaset, CLATM4, ccunm2r, cslabad, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
            DO 150 ISORT = 0, 1
               ...
               END IF
*
*              Call ccgges to compute H, T, Q, Z, alpha, and beta.
*
               CALL cclacpy( 'Full', N, N, A, LDA, S, LDA )
               CALL cclacpy( 'Full', N, N, B, LDA, T, LDA )
               NTEST = 1 + RSUB + ISORT
               RESULT( 1+RSUB+ISORT ) = ULPINV
               CALL ccgges( 'V', 'V', SORT, CLCTES, N, S, LDA, T, LDA,
     $                     SDIM, ALPHA, BETA, Q, LDQ, Z, LDQ, WORK,
     $                     LWORK, RWORK, BWORK, IINFO )
               IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                  RESULT( 1+RSUB+ISORT ) = ULPINV
                  WRITE( NOUNIT, FMT = 9999 )'ccgges', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 160
               END IF
