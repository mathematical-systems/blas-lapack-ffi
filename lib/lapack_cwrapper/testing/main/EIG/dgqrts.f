...
*  =======
*
*  DGQRTS tests cdggqrf, which computes the GQR factorization of an
...
*          by cdggqrf, see csggqrf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL       DGEMM, cdggqrf, cdlacpy, cdlaset, cdorgqr, cdorgrq,
     $                   DSYRK
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL cdlacpy( 'Full', N, M, A, LDA, AF, LDA )
      CALL cdlacpy( 'Full', N, P, B, LDB, BF, LDB )
*
...
*
      CALL cdggqrf( N, M, P, AF, LDA, TAUA, BF, LDB, TAUB, WORK, LWORK,
     $             INFO )
*
*     Generate the N-by-N matrix Q
*
      CALL cdlaset( 'Full', N, N, ROGUE, ROGUE, Q, LDA )
      CALL cdlacpy( 'Lower', N-1, M, AF( 2, 1 ), LDA, Q( 2, 1 ), LDA )
      CALL cdorgqr( N, N, MIN( N, M ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL cdlaset( 'Full', P, P, ROGUE, ROGUE, Z, LDB )
      IF( N.LE.P ) THEN
         IF( N.GT.0 .AND. N.LT.P )
     $      CALL cdlacpy( 'Full', N, P-N, BF, LDB, Z( P-N+1, 1 ), LDB )
         IF( N.GT.1 )
     $      CALL cdlacpy( 'Lower', N-1, N-1, BF( 2, P-N+1 ), LDB,
     $                   Z( P-N+2, P-N+1 ), LDB )
      ELSE
         IF( P.GT.1 )
     $      CALL cdlacpy( 'Lower', P-1, P-1, BF( N-P+2, 1 ), LDB,
     $                   Z( 2, 1 ), LDB )
      END IF
      CALL cdorgrq( P, P, MIN( N, P ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL cdlaset( 'Full', N, M, ZERO, ZERO, R, LDA )
      CALL cdlacpy( 'Upper', N, M, AF, LDA, R, LDA )
*
*     Copy T
*
      CALL cdlaset( 'Full', N, P, ZERO, ZERO, T, LDB )
      IF( N.LE.P ) THEN
        CALL cdlacpy( 'Upper', N, N, BF( 1, P-N+1 ), LDB, T( 1, P-N+1 ),
     $                LDB )
      ELSE
         CALL cdlacpy( 'Full', N-P, P, BF, LDB, T, LDB )
        CALL cdlacpy( 'Upper', P, P, BF( N-P+1, 1 ), LDB, T( N-P+1, 1 ),
     $                LDB )
      END IF
*
*     Compute R - Q'*A
...
*     Compute I - Q'*Q
*
      CALL cdlaset( 'Full', N, N, ZERO, ONE, R, LDA )
...
*     Compute I - Z'*Z
*
      CALL cdlaset( 'Full', P, P, ZERO, ONE, T, LDB )
