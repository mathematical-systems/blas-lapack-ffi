...
*
*             czgehrd factors A as  U H U' , where ' means conjugate
*             transpose, H is hessenberg, and U is unitary.
*
*             czunghr generates the unitary matrix U.
*
*             czunmhr multiplies a matrix by the unitary matrix U.
*
*             czhseqr factors H as  Z T Z' , where Z is unitary and T
*             is upper triangular.  It also computes the eigenvalues,
*             w(1), ..., w(n); we define a diagonal matrix W whose
*             (diagonal) entries are the eigenvalues.
*
*             cztrevc computes the left eigenvector matrix L and the
...
*             upper triangular.
*
*             czhsein computes the left eigenvector matrix Y and the
...
*           The upper hessenberg matrix computed by czgehrd.  On exit,
*           H contains the Hessenberg form of the matrix in A.
*           Modified.
*
*  T1     - COMPLEX*16 array, dimension (LDA,max(NN))
*           The Schur (="quasi-triangular") matrix computed by czhseqr
*           if Z is computed.  On exit, T1 contains the Schur form of
*           the matrix in A.
*           Modified.
*
*  T2     - COMPLEX*16 array, dimension (LDA,max(NN))
*           The Schur matrix computed by czhseqr when Z is not computed.
...
*           The unitary matrix computed by czgehrd.
*           Modified.
*
*  Z      - COMPLEX*16 array, dimension (LDU,max(NN))
*           The unitary matrix computed by czhseqr.
*           Modified.
*
...
*           Details of the unitary matrix computed by czgehrd.
*           Modified.
*
*  TAU    - COMPLEX*16 array, dimension (max(NN))
*           Further details of the unitary matrix computed by czgehrd.
*           Modified.
*
...
*           If 1, then czhseqr could not find all the shifts.
...
*     .. External Subroutines ..
      EXTERNAL         cdlabad, DLAFTS, DLASUM, xerbla, ZCOPY, czgehrd,
     $                  ZGEMM, ZGET10, ZGET22, czhsein, czhseqr, ZHST01,
     $                czlacpy, czlaset, ZLATME, ZLATMR, ZLATMS, cztrevc,
     $                   czunghr, czunmhr
*     ..
*     .. Intrinsic Functions ..
...
*
  100       CONTINUE
*
*           Call czgehrd to compute H and U, do tests.
*
...
*
            ...
            IHI = N
*
            CALL czgehrd( N, ILO, IHI, H, LDA, WORK, WORK( N+1 ),
     $                   NWORK-N, IINFO )
*
            IF( IINFO.NE.0 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'czgehrd', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
*
  ...
  120       CONTINUE
            ...
            CALL czunghr( N, ILO, IHI, U, LDU, WORK, WORK( N+1 ),
     $                   NWORK-N, IINFO )
            NTEST = 2
*
...
*
*           Call czhseqr to compute T1, T2 and Z, do tests.
*
*           Eigenvalues only (W3)
*
            CALL czlacpy( ' ', N, N, H, LDA, T2, LDA )
            NTEST = 3
            RESULT( 3 ) = ULPINV
*
            CALL czhseqr( 'E', 'N', N, ILO, IHI, T2, LDA, W3, UZ, LDU,
     $                   WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhseqr(E)', IINFO, N, JTYPE,
     $            IOLDSD
...
*
            CALL czlacpy( ' ', N, N, H, LDA, T2, LDA )
*
            CALL czhseqr( 'S', 'N', N, ILO, IHI, T2, LDA, W1, UZ, LDU,
     $                   WORK, NWORK, IINFO )
            IF( IINFO.NE.0 .AND. IINFO.LE.N+2 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhseqr(S)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
            CALL czlacpy( ' ', N, N, H, LDA, T1, LDA )
            CALL czlacpy( ' ', N, N, U, LDU, UZ, LDU )
*
            CALL czhseqr( 'S', 'V', N, ILO, IHI, T1, LDA, W1, UZ, LDU,
     $                   WORK, NWORK, IINFO )
            IF( IINFO.NE.0 .AND. IINFO.LE.N+2 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhseqr(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
  ...
  140       CONTINUE
            DO 150 J = 1, N, 2
               SELECT( J ) = .TRUE.
  150       CONTINUE
            CALL cztrevc( 'Right', 'All', SELECT, N, T1, LDA, CDUMMA,
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztrevc(R,A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
     ...
     $                   WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cztrevc',
...
*
            CALL cztrevc( 'Right', 'Some', SELECT, N, T1, LDA, CDUMMA,
     $                   LDU, EVECTL, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztrevc(R,S)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
*
  ...
  180       CONTINUE
            IF( .NOT.MATCH )
     $         WRITE( NOUNIT, FMT = 9997 )'Right', 'cztrevc', N, JTYPE,
     $         IOLDSD
*
...
*
            NTEST = 10
            RESULT( 10 ) = ULPINV
           CALL cztrevc( 'Left', 'All', SELECT, N, T1, LDA, EVECTL, LDU,
     $                   CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztrevc(L,A)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
...
*
     ...
     $                   WORK, RWORK, DUMMA( 3 ) )
            ...
            IF( DUMMA( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cztrevc', DUMMA( 4 ),
     $            N, JTYPE, IOLDSD
            END IF
*
...
*
            CALL cztrevc( 'Left', 'Some', SELECT, N, T1, LDA, EVECTR,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cztrevc(L,S)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 240
            END IF
*
  ...
  210       CONTINUE
            IF( .NOT.MATCH )
     $         WRITE( NOUNIT, FMT = 9997 )'Left', 'cztrevc', N, JTYPE,
     $         IOLDSD
*
*           Call czhsein for Right eigenvectors of H, do test 11
*
            NTEST = 11
            RESULT( 11 ) = ULPINV
            DO 220 J = 1, N
  ...
  220       CONTINUE
*
           CALL czhsein( 'Right', 'Qr', 'Ninitv', SELECT, N, H, LDA, W3,
     $                   CDUMMA, LDU, EVECTX, LDU, N1, IN, WORK, RWORK,
     $                   IWORK, IWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhsein(R)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 240
...
*
     ...
     $            RESULT( 11 ) = DUMMA( 1 )*ANINV
               IF( DUMMA( 2 ).GT.THRESH ) THEN
                  WRITE( NOUNIT, FMT = 9998 )'Right', 'czhsein',
     $               DUMMA( 2 ), N, JTYPE, IOLDSD
               END IF
            END IF
*
*           Call czhsein for Left eigenvectors of H, do test 12
*
            NTEST = 12
            RESULT( 12 ) = ULPINV
            DO 230 J = 1, N
  ...
  230       CONTINUE
*
            CALL czhsein( 'Left', 'Qr', 'Ninitv', SELECT, N, H, LDA, W3,
     $                   EVECTY, LDU, CDUMMA, LDU, N1, IN, WORK, RWORK,
     $                   IWORK, IWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czhsein(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 240
...
*
     ...
     $            RESULT( 12 ) = DUMMA( 3 )*ANINV
               IF( DUMMA( 4 ).GT.THRESH ) THEN
                  WRITE( NOUNIT, FMT = 9998 )'Left', 'czhsein',
     $               DUMMA( 4 ), N, JTYPE, IOLDSD
               END IF
            END IF
*
*           Call czunmhr for Right eigenvectors of A, do test 13
*
            NTEST = 13
            RESULT( 13 ) = ULPINV
*
            CALL czunmhr( 'Left', 'No transpose', N, N, ILO, IHI, UU,
     $                   LDU, TAU, EVECTX, LDU, WORK, NWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'czunmhr(L)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 )
     $            GO TO 240
...
*
     ...
     $            RESULT( 13 ) = DUMMA( 1 )*ANINV
            END IF
*
*           Call czunmhr for Left eigenvectors of A, do test 14
*
            NTEST = 14
            RESULT( 14 ) = ULPINV
*
