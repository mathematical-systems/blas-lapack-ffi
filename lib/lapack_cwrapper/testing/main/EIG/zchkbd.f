...
*
*  czgebrd reduces a complex general m by n matrix A to real upper or
*  lower bidiagonal form by an orthogonal transformation: Q' * A * P = B
*  (or A = Q * B * P').  The matrix B is upper bidiagonal if m >= n
*  and lower bidiagonal if m < n.
*
*  czungbr generates the orthogonal matrices Q and P' from czgebrd.
*  Note that Q and P are not necessarily square.
*
*  czbdsqr computes the singular value decomposition of the bidiagonal
...
*  In addition, czbdsqr has an option to apply the left orthogonal matrix
...
*  (3)   | I - PT PT' | / ( N ulp )
*
*  Test czbdsqr on bidiagonal matrix B
*
...
*       (a) czgebrd is not called to reduce it to bidiagonal form.
...
*          and Z, used in testing czbdsqr.  If NRHS = 0, then the
...
*          -27: LWORK too small.
*          If  ZLATMR, CLATMS, czgebrd, czungbr, or czbdsqr,
...
*     .. External Subroutines ..
      EXTERNAL          ALASUM, DCOPY, cdlabad, DLAHD2, DSVDCH, xerbla,
     $                  czbdsqr, ZBDT01, ZBDT02, ZBDT03, czgebrd, ZGEMM,
     $                 czlacpy, czlaset, ZLATMR, ZLATMS, czungbr, ZUNT01
*     ..
*     .. Intrinsic Functions ..
...
*
  100       CONTINUE
*
*           Call czgebrd and czungbr to compute B, Q, and P, do tests.
*
            IF( .NOT.BIDIAG ) THEN
*
...
*              B := Q' * A * P.
*
               CALL czlacpy( ' ', M, N, A, LDA, Q, LDQ )
              CALL czgebrd( M, N, Q, LDQ, BD, BE, WORK, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from czgebrd.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'czgebrd', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
*
               CALL czlacpy( ' ', M, N, Q, LDQ, PT, LDPT )
               IF( M.GE.N ) THEN
                  UPLO = 'U'
               ELSE
                  UPLO = 'L'
...
*
               ...
               IF( NRHS.LE.0 )
     $            MQ = MNMIN
               CALL czungbr( 'Q', M, MQ, N, Q, LDQ, WORK,
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from czungbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'czungbr(Q)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*              Generate P'
*
              CALL czungbr( 'P', MNMIN, N, M, PT, LDPT, WORK( MNMIN+1 ),
     $                      WORK( 2*MNMIN+1 ), LWORK-2*MNMIN, IINFO )
*
*              Check error code from czungbr.
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUT, FMT = 9998 )'czungbr(P)', IINFO, M, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  RETURN
               END IF
...
*
     ...
     $                      RWORK, RESULT( 3 ) )
            END IF
*
*           Use czbdsqr to form the SVD of the bidiagonal matrix B:
*           B := U * S1 * VT, and compute Z = U' * Y.
*
            CALL DCOPY( MNMIN, BD, 1, S1, 1 )
            IF( MNMIN.GT.0 )
     $         CALL DCOPY( MNMIN-1, BE, 1, RWORK, 1 )
            CALL czlacpy( ' ', M, NRHS, Y, LDX, Z, LDX )
            CALL czlaset( 'Full', MNMIN, MNMIN, CZERO, CONE, U, LDPT )
            CALL czlaset( 'Full', MNMIN, MNMIN, CZERO, CONE, VT, LDPT )
*
           CALL czbdsqr( UPLO, MNMIN, MNMIN, MNMIN, NRHS, S1, RWORK, VT,
     $                   LDPT, U, LDPT, Z, LDX, RWORK( MNMIN+1 ),
     $                   IINFO )
*
*           Check error code from czbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'czbdsqr(vects)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
            ...
            END IF
*
*           Use czbdsqr to compute only the singular values of the
...
*
     ...
     $         CALL DCOPY( MNMIN-1, BE, 1, RWORK, 1 )
*
            CALL czbdsqr( UPLO, MNMIN, 0, 0, 0, S2, RWORK, VT, LDPT, U,
     $                   LDPT, Z, LDX, RWORK( MNMIN+1 ), IINFO )
*
*           Check error code from czbdsqr.
*
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUT, FMT = 9998 )'czbdsqr(values)', IINFO, M, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               IF( IINFO.LT.0 ) THEN
                  RETURN
...
*
  ...
  110       CONTINUE
     ...
     $            RESULT( 8 ) = ULPINV
            END IF
*
*           Test 9:  Compare czbdsqr with and without singular vectors
*
            TEMP2 = ZERO
*
            DO 120 J = 1, MNMIN
...
*
  140       CONTINUE
            RESULT( 10 ) = TEMP1
*
*           Use czbdsqr to form the decomposition A := (QU) S (VT PT)
...
*
     ...
     $            CALL DCOPY( MNMIN-1, BE, 1, RWORK, 1 )
*
               CALL czbdsqr( UPLO, MNMIN, N, M, NRHS, S2, RWORK, PT,
