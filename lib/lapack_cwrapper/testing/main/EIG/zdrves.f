...
*          Another copy of the test matrix A, modified by czgees.
*
*  HT      (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          Yet another copy of the test matrix A, modified by czgees.
*
...
*          but those computed when czgees only computes a partial
...
*          -18: NWORK too small.
*          If  ZLATMR, CLATMS, CLATME or czgees returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL        cdlabad, DLASUM, xerbla, czgees, ZHST01, czlacpy,
...
*
                 ...
                 CALL czgees( 'V', SORT, ZSLECT, N, H, LDA, SDIM, W, VS,
     $                        LDVS, WORK, NNWORK, RWORK, BWORK, IINFO )
                  IF( IINFO.NE.0 ) THEN
                     RESULT( 1+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'czgees1', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 190
                  END IF
...
*
                  ...
                  CALL czgees( 'N', SORT, ZSLECT, N, HT, LDA, SDIM, WT,
     $                        VS, LDVS, WORK, NNWORK, RWORK, BWORK,
     $                        IINFO )
                  IF( IINFO.NE.0 ) THEN
                     RESULT( 5+RSUB ) = ULPINV
                     WRITE( NOUNIT, FMT = 9992 )'czgees2', IINFO, N,
     $                  JTYPE, IOLDSD
                     INFO = ABS( IINFO )
                     GO TO 190
                  END IF
