...
*
*  csgbbrd factors a general band matrix A as  Q B P* , where * means
*  transpose, B is upper bidiagonal, and Q and P are orthogonal;
*  csgbbrd can also overwrite a given matrix C with Q* C .
*
...
*          Used to hold the orthogonal matrix Q computed by csgbbrd.
*
*  LDQ     (input) INTEGER
...
*          Used to hold the orthogonal matrix P computed by csgbbrd.
*
*  LDP     (input) INTEGER
...
*          Used to hold the matrix C updated by csgbbrd.
*
*  LDC     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL        SBDT01, SBDT02, csgbbrd, cslacpy, SLAHD2, cslaset,
...
*
               ...
               CALL cslaset( 'Full', LDAB, N, ZERO, ZERO, AB, LDAB )
               IINFO = 0
               COND = ULPINV
*
...
*              Copy C
*
               CALL cslacpy( 'Full', M, NRHS, C, LDC, CC, LDC )
*
*              Call csgbbrd to compute B, Q and P, and to update C.
*
               CALL csgbbrd( 'B', M, N, NRHS, KL, KU, AB, LDAB, BD, BE,
     $                      Q, LDQ, P, LDP, CC, LDC, WORK, IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'csgbbrd', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
