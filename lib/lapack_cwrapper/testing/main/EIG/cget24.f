...
*        computed by ccgeesX and RCDEIN (the precomputed true value)
...
*        number computed by ccgeesX and RCDVIN (the precomputed true
...
*          Another copy of the test matrix A, modified by ccgeesX.
*
*  HT      (workspace) COMPLEX array, dimension (LDA, N)
*          Yet another copy of the test matrix A, modified by ccgeesX.
*
...
*          but those computed when ccgeesX only computes a partial
...
*
*  LWORK   (input) INTEGER
*          The number of entries in WORK to be passed to ccgeesX. This
...
*          If >0, ccgeesX returned an error code, the absolute
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, ccgeesX, CGEMM, cclacpy, CUNT01, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL cclacpy( 'F', N, N, A, LDA, H, LDA )
         CALL ccgeesX( 'V', SORT, CSLECT, 'N', N, H, LDA, SDIM, W, VS,
     $                LDVS, RCONDE, RCONDV, WORK, LWORK, RWORK, BWORK,
     $                IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 1+RSUB ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX1', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX1', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            RETURN
...
*
         CALL cclacpy( ' ', N, N, A, LDA, VS1, LDVS )
*
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'N', SORT, CSLECT, 'N', N, HT, LDA, SDIM, WT, VS,
     $                LDVS, RCONDE, RCONDV, WORK, LWORK, RWORK, BWORK,
     $                IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 5+RSUB ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX2', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX2', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         ...
         RESULT( 15 ) = ZERO
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'V', SORT, CSLECT, 'B', N, HT, LDA, SDIM1, WT,
     ...
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            ...
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX3', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX3', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'N', SORT, CSLECT, 'B', N, HT, LDA, SDIM1, WT,
     ...
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            ...
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX4', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX4', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'V', SORT, CSLECT, 'E', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 14 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX5', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX5', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'N', SORT, CSLECT, 'E', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 14 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX6', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX6', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'V', SORT, CSLECT, 'V', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX7', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX7', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX( 'N', SORT, CSLECT, 'V', N, HT, LDA, SDIM1, WT,
     $                VS1, LDVS, RCNDE1, RCNDV1, WORK, LWORK, RWORK,
     $                BWORK, IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 15 ) = ULPINV
            IF( JTYPE.NE.22 ) THEN
               WRITE( NOUNIT, FMT = 9998 )'ccgeesX8', IINFO, N, JTYPE,
     $            ISEED
            ELSE
               WRITE( NOUNIT, FMT = 9999 )'ccgeesX8', IINFO, N,
     $            ISEED( 1 )
            END IF
            INFO = ABS( IINFO )
            GO TO 220
...
*
         CALL cclacpy( 'F', N, N, A, LDA, HT, LDA )
         CALL ccgeesX('N', 'S', CSLECT, 'B', N, HT, LDA, SDIM1, WT, VS1,
     $                LDVS, RCONDE, RCONDV, WORK, LWORK, RWORK, BWORK,
     $                IINFO )
         IF( IINFO.NE.0 ) THEN
            RESULT( 16 ) = ULPINV
            RESULT( 17 ) = ULPINV
            WRITE( NOUNIT, FMT = 9999 )'ccgeesX9', IINFO, N, ISEED( 1 )
            INFO = ABS( IINFO )
            GO TO 270
         END IF
*
