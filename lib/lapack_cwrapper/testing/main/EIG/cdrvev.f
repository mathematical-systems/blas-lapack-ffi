...
*  =======
*
*     CDRVEV  checks the nonsymmetric eigenvalue problem driver ccgeev.
*
...
*          Another copy of the test matrix A, modified by ccgeev.
*
...
*          but those computed when ccgeev only computes a partial
...
*          -21: NWORK too small.
*          If  CLATMR, CLATMS, CLATME or ccgeev returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL          ccgeev, CGET22, cclacpy, CLATME, CLATMR, CLATMS,
     $                   cclaset, cslabad, SLASUM, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                      ZERO, ANORM, 'NO', A, LDA, IWORK, IINFO )
               IF( N.GE.4 ) THEN
                  CALL cclaset( 'Full', 2, N, CZERO, CZERO, A, LDA )
                  CALL cclaset( 'Full', N-3, 1, CZERO, CZERO, A( 3, 1 ),
     $                         LDA )
                  CALL cclaset( 'Full', N-3, 2, CZERO, CZERO,
     $                         A( 3, N-1 ), LDA )
                  CALL cclaset( 'Full', 1, N, CZERO, CZERO, A( N, 1 ),
     $                         LDA )
               END IF
*
...
*
               ...
               CALL ccgeev( 'V', 'V', N, H, LDA, W, VL, LDVL, VR, LDVR,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'ccgeev1', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cclacpy( 'F', N, N, A, LDA, H, LDA )
               CALL ccgeev( 'N', 'N', N, H, LDA, W1, DUM, 1, DUM, 1,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'ccgeev2', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cclacpy( 'F', N, N, A, LDA, H, LDA )
               CALL ccgeev( 'N', 'V', N, H, LDA, W1, DUM, 1, LRE, LDLRE,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'ccgeev3', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cclacpy( 'F', N, N, A, LDA, H, LDA )
               CALL ccgeev( 'V', 'N', N, H, LDA, W1, LRE, LDLRE, DUM, 1,
     $                     WORK, NNWORK, RWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'ccgeev4', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
