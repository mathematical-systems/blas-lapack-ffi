...
*  =======
*
*  DGET38 tests cdtrsen, a routine for estimating condition numbers of a
...
*          If cdgehrd returns INFO nonzero on example i, LMAX(1)=i
*          If cdhseqr returns INFO nonzero on example i, LMAX(2)=i
*          If cdtrsen returns INFO nonzero on example i, LMAX(3)=i
*
*  NINFO   (output) INTEGER array, dimension (3)
*          NINFO(1) = No. of times cdgehrd returned INFO nonzero
*          NINFO(2) = No. of times cdhseqr returned INFO nonzero
*          NINFO(3) = No. of times cdtrsen returned INFO nonzero
*
*  KNT     (output) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL        DCOPY, cdgehrd, cdhseqr, DHST01, cdlabad, cdlacpy,
     $                   cdorghr, DSCAL, cdtrsen
*     ..
*     .. Intrinsic Functions ..
...
*
         KNT = KNT + 1
         CALL cdlacpy( 'F', N, N, TMP, LDT, T, LDT )
         VMUL = VAL( ISCL )
         DO 30 I = 1, N
   ...
   30    CONTINUE
         IF( TNRM.EQ.ZERO )
     $      VMUL = ONE
         CALL cdlacpy( 'F', N, N, T, LDT, TSAV, LDT )
*
*        Compute Schur form
*
         CALL cdgehrd( N, 1, N, T, LDT, WORK( 1 ), WORK( N+1 ), LWORK-N,
     $                INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 1 ) = KNT
            NINFO( 1 ) = NINFO( 1 ) + 1
...
*
         CALL cdlacpy( 'L', N, N, T, LDT, Q, LDT )
         CALL cdorghr( N, 1, N, Q, LDT, WORK( 1 ), WORK( N+1 ), LWORK-N,
     $                INFO )
*
*        Compute Schur form
*
         CALL cdhseqr( 'S', 'V', N, 1, N, T, LDT, WR, WI, Q, LDT, WORK,
     $                LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 2 ) = KNT
            NINFO( 2 ) = NINFO( 2 ) + 1
...
*
         CALL cdlacpy( 'F', N, N, Q, LDT, QSAV, LDT )
         CALL cdlacpy( 'F', N, N, T, LDT, TSAV1, LDT )
         CALL cdtrsen('B', 'V', SELECT, N, T, LDT, Q, LDT, WRTMP, WITMP,
...
*
         VMAX = ZERO
         CALL cdlacpy( 'F', N, N, TSAV1, LDT, TTMP, LDT )
         CALL cdlacpy( 'F', N, N, QSAV, LDT, QTMP, LDT )
         SEPTMP = -ONE
         STMP = -ONE
         CALL cdtrsen( 'E', 'V', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cdtrsen( 'V', 'V', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
     $                WITMP, M, STMP, SEPTMP, WORK, LWORK, IWORK,
     $                LIWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cdtrsen( 'E', 'N', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
     $                WITMP, M, STMP, SEPTMP, WORK, LWORK, IWORK,
     $                LIWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cdtrsen( 'V', 'N', SELECT, N, TTMP, LDT, QTMP, LDT, WRTMP,
     $                WITMP, M, STMP, SEPTMP, WORK, LWORK, IWORK,
     $                LIWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
