...
*
*  cdsbtrd factors a symmetric band matrix A as  U S U' , where ' means
*  transpose, S is symmetric tridiagonal, and U is orthogonal.
*  cdsbtrd can use either just the lower or just the upper triangle
...
*
*  (1)     | A - V S V' | / ( |A| n ulp )  computed by cdsbtrd with
*                                          UPLO='U'
*
*  (2)     | I - UU' | / ( n ulp )
*
*  (3)     | A - V S V' | / ( |A| n ulp )  computed by cdsbtrd with
*                                          UPLO='L'
*
*  (4)     | I - UU' | / ( n ulp )
*
...
*          Used to hold the orthogonal matrix computed by cdsbtrd.
*
*  LDU     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL         cdlacpy, cdlaset, DLASUM, DLATMR, DLATMS, DSBT21,
     $                   cdsbtrd, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  100          CONTINUE
*
*              Call cdsbtrd to compute S and U from upper triangle.
*
               CALL cdlacpy( ' ', K+1, N, A, LDA, WORK, LDA )
*
               NTEST = 1
               CALL cdsbtrd( 'V', 'U', N, K, WORK, LDA, SD, SE, U, LDU,
     $                      WORK( LDA*N+1 ), IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cdsbtrd(U)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  140          CONTINUE
*
*              Call cdsbtrd to compute S and U from lower triangle
*
               CALL cdlacpy( ' ', K+1, N, A, LDA, WORK, LDA )
*
               NTEST = 3
               CALL cdsbtrd( 'V', 'L', N, K, WORK, LDA, SD, SE, U, LDU,
     $                      WORK( LDA*N+1 ), IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cdsbtrd(L)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
