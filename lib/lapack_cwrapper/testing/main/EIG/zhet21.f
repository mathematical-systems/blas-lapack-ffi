...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, ZHER, ZHER2, czlacpy, czlarfY, czlaset,
     $                   czunm2l, czunm2r
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL czlaset( 'Full', N, N, CZERO, CZERO, WORK, N )
         CALL czlacpy( CUPLO, N, N, A, LDA, WORK, N )
*
         DO 10 J = 1, N
...
*
               VSAVE = V( J+1, J )
               V( J+1, J ) = ONE
               CALL czlarfY( 'L', N-J, V( J+1, J ), 1, TAU( J ),
...
*
               VSAVE = V( J, J+1 )
               V( J, J+1 ) = ONE
               CALL czlarfY( 'U', J, V( 1, J+1 ), 1, TAU( J ), WORK, N,
     $                      WORK( N**2+1 ) )
               V( J, J+1 ) = VSAVE
               WORK( ( N+1 )*J+1 ) = D( J+1 )
   60       CONTINUE
...
*
         IF( N.LT.2 )
     $      RETURN
         CALL czlacpy( ' ', N, N, U, LDU, WORK, N )
         IF( LOWER ) THEN
            CALL czunm2r( 'R', 'C', N, N-1, N-1, V( 2, 1 ), LDV, TAU,
     $                   WORK( N+1 ), N, WORK( N**2+1 ), IINFO )
         ELSE
            CALL czunm2l( 'R', 'C', N, N-1, N-1, V( 1, 2 ), LDV, TAU,
