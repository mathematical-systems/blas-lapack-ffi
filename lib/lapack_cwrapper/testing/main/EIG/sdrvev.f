...
*  =======
*
*     SDRVEV  checks the nonsymmetric eigenvalue problem driver csgeev.
*
...
*          Another copy of the test matrix A, modified by csgeev.
*
...
*          but those computed when csgeev only computes a partial
...
*          -23: NWORK too small.
*          If  SLATMR, SLATMS, SLATME or csgeev returns an error code,
...
*     .. External Subroutines ..
      EXTERNAL         csgeev, SGET22, cslabad, cslacpy, SLASUM, SLATME,
     $                   SLATMR, SLATMS, cslaset, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                      ZERO, ANORM, 'NO', A, LDA, IWORK, IINFO )
               IF( N.GE.4 ) THEN
                  CALL cslaset( 'Full', 2, N, ZERO, ZERO, A, LDA )
                  CALL cslaset( 'Full', N-3, 1, ZERO, ZERO, A( 3, 1 ),
     $                         LDA )
                  CALL cslaset( 'Full', N-3, 2, ZERO, ZERO, A( 3, N-1 ),
     $                         LDA )
                  CALL cslaset( 'Full', 1, N, ZERO, ZERO, A( N, 1 ),
     $                         LDA )
               END IF
*
...
*
               ...
               CALL csgeev( 'V', 'V', N, H, LDA, WR, WI, VL, LDVL, VR,
     $                     LDVR, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'csgeev1', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cslacpy( 'F', N, N, A, LDA, H, LDA )
               CALL csgeev( 'N', 'N', N, H, LDA, WR1, WI1, DUM, 1, DUM,
     $                     1, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'csgeev2', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cslacpy( 'F', N, N, A, LDA, H, LDA )
               CALL csgeev( 'N', 'V', N, H, LDA, WR1, WI1, DUM, 1, LRE,
     $                     LDLRE, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'csgeev3', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
...
*
               CALL cslacpy( 'F', N, N, A, LDA, H, LDA )
               CALL csgeev( 'V', 'N', N, H, LDA, WR1, WI1, LRE, LDLRE,
     $                     DUM, 1, WORK, NNWORK, IINFO )
               IF( IINFO.NE.0 ) THEN
                  RESULT( 1 ) = ULPINV
                  WRITE( NOUNIT, FMT = 9993 )'csgeev4', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 220
               END IF
