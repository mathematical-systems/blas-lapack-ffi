...
*  routine czggev.
*
*  czggev computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*          The Schur form matrix computed from A by czggev.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by czggev.
*
*  Q      (workspace) COMPLEX*16 array, dimension (LDQ, max(NN))
*          The (left) eigenvectors matrix computed by czggev.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by czggev.
*
...
*          The generalized eigenvalues of (A,B) computed by czggev.
...
*          eigenvalues of A and B, but those computed when czggev only
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, cdlabad, xerbla, ZGET52, czggev, czlacpy,
     $                   czlarfG, czlaset, ZLATM4, czunm2r
*     ..
*     .. Intrinsic Functions ..
...
*
  ...
  110       CONTINUE
*
*           Call czggev to compute eigenvalues and eigenvectors.
*
            CALL czlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL czggev( 'V', 'V', N, S, LDA, T, LDA, ALPHA, BETA, Q,
     $                  LDQ, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'czggev1', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
     ...
     $                   WORK, RWORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'czggev1',
...
*
     ...
     $                   BETA, WORK, RWORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'czggev1',
...
*
            CALL czlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL czggev( 'N', 'N', N, S, LDA, T, LDA, ALPHA1, BETA1, Q,
     $                  LDQ, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'czggev2', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL czlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL czggev( 'V', 'N', N, S, LDA, T, LDA, ALPHA1, BETA1, QE,
     $                  LDQE, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'czggev3', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL czlacpy( ' ', N, N, A, LDA, S, LDA )
            CALL czlacpy( ' ', N, N, B, LDA, T, LDA )
            CALL czggev( 'N', 'V', N, S, LDA, T, LDA, ALPHA1, BETA1, Q,
     $                  LDQ, QE, LDQE, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'czggev4', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
