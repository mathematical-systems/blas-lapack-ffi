...
*           LRIGHT are .TRUE. must be at least zero; if not, cxerbla
*           will be called.
*           Not modified.
*
*  C, S   - COMPLEX
...
*           output of ccrotG or to most versions of ccrot, both C and S
...
*
      IF( NL.LT.NT ) THEN
         CALL cxerbla( 'CLAROT', 4 )
         RETURN
      END IF
      IF( LDA.LE.0 .OR. ( .NOT.LROWS .AND. LDA.LT.NL-NT ) ) THEN
         CALL cxerbla( 'CLAROT', 8 )
         RETURN
      END IF
*
*     Rotate
*
*     ccrot( NL-NT, A(IX),IINC, A(IY),IINC, C, S ) with complex C, S
*
      DO 10 J = 0, NL - NT - 1
   ...
   10 CONTINUE
*
*     ccrot( NT, XT,1, YT,1, C, S ) with complex C, S
*
      DO 20 J = 1, NT
         TEMPX = C*XT( J ) + S*YT( J )
