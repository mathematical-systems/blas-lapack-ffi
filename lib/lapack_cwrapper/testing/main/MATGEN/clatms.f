...
*     .. External Subroutines ..
      EXTERNAL         CLAGGE, CLAGHE, CLAGSY, CLAROT, cclartg, cclaset,
     $                   SLATM1, SSCAL, cxerbla
*     ..
*     .. Intrinsic Functions ..
...
*
                     ...
                     DO 50 JCH = JR - JKL, 1, -JKL - JKU
                        IF( IR.LT.M ) THEN
                           CALL cclartg( A( IR+1-ISKEW*( IC+1 )+IOFFST,
     ...
     $                               ILDA, CTEMP, EXTRA )
                        IF( ILTEMP ) THEN
                          CALL cclartg( A( IROW+1-ISKEW*( IC+1 )+IOFFST,
...
*
     ...
     $                               ILDA, CTEMP, EXTRA )
                        IF( ILTEMP ) THEN
                          CALL cclartg( A( IR+1-ISKEW*( ICOL+1 )+IOFFST,
...
*
                     ...
                     DO 110 JCH = JC + JKL, IENDCH, JKL + JKU
                        ILEXTR = IC.GT.0
                        IF( ILEXTR ) THEN
                           CALL cclartg( A( JCH-ISKEW*IC+IOFFST, IC ),
     $                                  EXTRA, REALC, S, DUMMY )
                           DUMMY = CLARND( 5, ISEED )
                           C = REALC*DUMMY
                           S = S*DUMMY
     ...
     $                               ILDA, EXTRA, CTEMP )
                        IF( ILTEMP ) THEN
                           CALL cclartg( A( JCH-ISKEW*ICOL+IOFFST,
...
*
                     ...
                     DO 140 JCH = JR + JKU, IENDCH, JKL + JKU
                        ILEXTR = IR.GT.0
                        IF( ILEXTR ) THEN
                           CALL cclartg( A( IR-ISKEW*JCH+IOFFST, JCH ),
     $                                  EXTRA, REALC, S, DUMMY )
                           DUMMY = CLARND( 5, ISEED )
                           C = REALC*DUMMY
                           S = S*DUMMY
     ...
     $                               JCH ), ILDA, EXTRA, CTEMP )
                        IF( ILTEMP ) THEN
                          CALL cclartg( A( IROW-ISKEW*JCH+IOFFST, JCH ),
     $                                  CTEMP, REALC, S, DUMMY )
                           DUMMY = CLARND( 5, ISEED )
                           C = REALC*DUMMY
                           S = S*DUMMY
...
*
                     ICOL = JC
                     DO 180 JCH = JC - K, 1, -K
                        CALL cclartg( A( JCH+1-ISKEW*( ICOL+1 )+IOFFG,
...
*
                     ...
                     DO 270 JCH = JC + K, N - 1, K
                        CALL cclartg( A( JCH-ISKEW*ICOL+IOFFG, ICOL ),
     $                               EXTRA, REALC, S, DUMMY )
                        DUMMY = CLARND( 5, ISEED )
                        C = REALC*DUMMY
                        S = S*DUMMY
