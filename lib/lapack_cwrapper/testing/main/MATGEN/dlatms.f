...
*     .. External Subroutines ..
      EXTERNAL          DCOPY, DLAGGE, DLAGSY, DLAROT, cdlartg, cdlaset,
     $                   DLATM1, DSCAL, cxerbla
*     ..
*     .. Intrinsic Functions ..
...
*
                     ...
                     DO 30 JCH = JR - JKL, 1, -JKL - JKU
                        IF( IR.LT.M ) THEN
                           CALL cdlartg( A( IR+1-ISKEW*( IC+1 )+IOFFST,
     ...
     $                               ILDA, TEMP, EXTRA )
                        IF( ILTEMP ) THEN
                          CALL cdlartg( A( IROW+1-ISKEW*( IC+1 )+IOFFST,
     $                                  IC+1 ), TEMP, C, S, DUMMY )
...
*
     ...
     $                               ILDA, TEMP, EXTRA )
                        IF( ILTEMP ) THEN
                          CALL cdlartg( A( IR+1-ISKEW*( ICOL+1 )+IOFFST,
...
*
                     ...
                     DO 90 JCH = JC + JKL, IENDCH, JKL + JKU
                        ILEXTR = IC.GT.0
                        IF( ILEXTR ) THEN
                           CALL cdlartg( A( JCH-ISKEW*IC+IOFFST, IC ),
     $                                  EXTRA, C, S, DUMMY )
                        END IF
                        IC = MAX( 1, IC )
     ...
     $                               ILDA, EXTRA, TEMP )
                        IF( ILTEMP ) THEN
                           CALL cdlartg( A( JCH-ISKEW*ICOL+IOFFST,
     $                                  ICOL ), TEMP, C, S, DUMMY )
...
*
                     ...
                     DO 120 JCH = JR + JKU, IENDCH, JKL + JKU
                        ILEXTR = IR.GT.0
                        IF( ILEXTR ) THEN
                           CALL cdlartg( A( IR-ISKEW*JCH+IOFFST, JCH ),
     $                                  EXTRA, C, S, DUMMY )
                        END IF
                        IR = MAX( 1, IR )
     ...
     $                               JCH ), ILDA, EXTRA, TEMP )
                        IF( ILTEMP ) THEN
                          CALL cdlartg( A( IROW-ISKEW*JCH+IOFFST, JCH ),
     $                                  TEMP, C, S, DUMMY )
                           IL = MIN( IENDCH, JCH+JKL+JKU ) + 2 - JCH
                           EXTRA = ZERO
...
*
                     ICOL = JC
                     DO 150 JCH = JC - K, 1, -K
                        CALL cdlartg( A( JCH+1-ISKEW*( ICOL+1 )+IOFFG,
...
*
                     ...
                     DO 220 JCH = JC + K, N - 1, K
                        CALL cdlartg( A( JCH-ISKEW*ICOL+IOFFG, ICOL ),
     $                               EXTRA, C, S, DUMMY )
