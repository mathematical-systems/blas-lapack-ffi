...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SGEMV, SGER, cslarfG, SLARGE, cslarnv,
     $                   SLATM1, cslaset, SSCAL, cxerbla
*     ..
*     .. Intrinsic Functions ..
      ...
      IF( IUPPER.NE.0 ) THEN
         DO 70 JC = 2, N
            ...
            ELSE
               JR = JC - 1
            END IF
            CALL cslarnv( IDIST, ISEED, JR, A( 1, JC ) )
   70    CONTINUE
      END IF
*
...
*
            ...
            XNORMS = WORK( 1 )
            CALL cslarfG( IROWS, XNORMS, WORK( 2 ), 1, TAU )
            WORK( 1 ) = ONE
*
...
*
            A( JCR, IC ) = XNORMS
           CALL cslaset( 'Full', IROWS-1, 1, ZERO, ZERO, A( JCR+1, IC ),
     $                   LDA )
   90    CONTINUE
...
*
            ...
            XNORMS = WORK( 1 )
            CALL cslarfG( ICOLS, XNORMS, WORK( 2 ), 1, TAU )
            WORK( 1 ) = ONE
*
...
*
            A( IR, JCR ) = XNORMS
           CALL cslaset( 'Full', 1, ICOLS-1, ZERO, ZERO, A( IR, JCR+1 ),
     $                   LDA )
  100    CONTINUE
      END IF
*
