...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CGEMV, CGERC, cclacgv, cclarfG, CLARGE,
     $                  cclarnv, CLATM1, cclaset, CSCAL, CSSCAL, SLATM1,
     $                   cxerbla
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( IUPPER.NE.0 ) THEN
         DO 40 JC = 2, N
            CALL cclarnv( IDIST, ISEED, JC-1, A( 1, JC ) )
   40    CONTINUE
      END IF
*
...
*
            ...
            XNORMS = WORK( 1 )
            CALL cclarfG( IROWS, XNORMS, WORK( 2 ), 1, TAU )
            TAU = CONJG( TAU )
            WORK( 1 ) = CONE
...
*
            A( JCR, IC ) = XNORMS
            CALL cclaset( 'Full', IROWS-1, 1, CZERO, CZERO,
     $                   A( JCR+1, IC ), LDA )
*
...
*
            ...
            XNORMS = WORK( 1 )
            CALL cclarfG( ICOLS, XNORMS, WORK( 2 ), 1, TAU )
            TAU = CONJG( TAU )
            WORK( 1 ) = CONE
            CALL cclacgv( ICOLS-1, WORK( 2 ), 1 )
            ALPHA = CLARND( 5, ISEED )
*
...
*
            A( IR, JCR ) = XNORMS
            CALL cclaset( 'Full', 1, ICOLS-1, CZERO, CZERO,
     $                   A( IR, JCR+1 ), LDA )
*
