...
*  =======
*
*  CSLASD2 merges the two sets of singular values together into a single
...
*
*  CSLASD2 is called from CSLASD1.
*
*  Arguments
*  =========
*
...
*         will be used by CSLASD3 in a matrix multiply (SGEMM) to solve
...
*         which will be used by CSLASD3 in a matrix multiply (SGEMM) to
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, CSLACPY, CSLAMRG, CSLASET, SROT, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
...
*
   ...
   60 CONTINUE
*
      CALL CSLAMRG( NL, NR, DSIGMA( 2 ), 1, 1, IDX( 2 ) )
*
      DO 70 I = 2, N
         IDXI = 1 + IDX( I )
         D( I ) = DSIGMA( IDXI )
...
*     last row of VT.
*
      CALL CSLASET( 'A', N, 1, ZERO, ZERO, U2, LDU2 )
      U2( NLP1, 1 ) = ONE
      IF( M.GT.N ) THEN
         DO 170 I = 1, NLP1
            VT( M, I ) = -S*VT( NLP1, I )
...
*
      IF( N.GT.K ) THEN
         ...
         CALL CSLACPY( 'A', N, N-K, U2( 1, K+1 ), LDU2, U( 1, K+1 ),
     $                LDU )
         CALL CSLACPY( 'A', N-K, M, VT2( K+1, 1 ), LDVT2, VT( K+1, 1 ),
     $                LDVT )
      END IF
*
*     Copy CTOT into COLTYP for referencing in CSLASD3.
*
      DO 190 J = 1, 4
         COLTYP( J ) = CTOT( J )
  190 CONTINUE
