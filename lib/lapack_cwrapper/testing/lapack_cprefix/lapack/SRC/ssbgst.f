...
*  =======
*
*  CSSBGST reduces a real symmetric-definite banded generalized
*  eigenproblem  A*x = lambda*B*x  to standard form  C*y = lambda*y,
*  such that C has the same bandwidth as A.
*
*  B must have been previously factorized as S**T*S by CSPBSTF, using a
...
*          B, as returned by CSPBSTF, stored in the first KB+1 rows of
*          the array.
*
*  LDBB    (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL           SGER, CSLAR2V, CSLARGV, CSLARTG, CSLARTV, 
     $  CSLASET, 
     $                   SROT, SSCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( WANTX )
     $   CALL CSLASET( 'Full', N, N, ZERO, ONE, X, LDX )
*
*     Set M to the splitting point m. It must be the same value as is
*     used in CSPBSTF. The chosen value allows the arrays WORK and RWORK
...
*
                  CALL CSLARTG( AB( K+1, I-K+KA ), RA1,
...
*
            IF( NRT.GT.0 )
     $         CALL CSLARGV( NRT, AB( 1, J2T ), INCA, WORK( J2T-M ), 
     $  KA1, 
     $                      WORK( N+J2T-M ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 100 L = 1, KA - 1
                  CALL CSLARTV( NR, AB( KA1-L, J2 ), INCA,
...
*              blocks
*
               CALL CSLAR2V( NR, AB( KA1, J2 ), AB( KA1, J2+1 ),
...
*
            DO 110 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( L, J2+KA1-L ), INCA,
...
*
            DO 140 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( L, J2-L+1 ), INCA,
...
*
               CALL CSLARGV( NR, AB( 1, J2 ), INCA, WORK( J2 ), KA1,
     $                      WORK( N+J2 ), KA1 )
*
...
*
                  CALL CSLARTG( AB( KA1-K, I ), RA1, WORK( N+I-K+KA-M 
     $  ), 
     $                         WORK( I-K+KA-M ), RA )
*
...
*
            IF( NRT.GT.0 )
     $         CALL CSLARGV( NRT, AB( KA1, J2T-KA ), INCA, WORK( J2T-M 
     $  ), 
...
*
               DO 330 L = 1, KA - 1
                  CALL CSLARTV( NR, AB( L+1, J2-L ), INCA,
...
*              blocks
*
               CALL CSLAR2V( NR, AB( 1, J2 ), AB( 1, J2+1 ), AB( 2, J2 
     $  ), 
...
*
            DO 340 L = KA - 1, KB - K + 1, -1
               NRT = ( N-J2+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( KA1-L+1, J2 ), INCA,
...
*
            DO 370 L = KB - K, 1, -1
               NRT = ( N-J2+KA+L ) / KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( KA1-L+1, J2-KA ), INCA,
...
*              which have been created outside the band
*
               CALL CSLARGV( NR, AB( KA1, J2-KA ), INCA, WORK( J2 ), 
     $  KA1, 
     $                      WORK( N+J2 ), KA1 )
*
...
*                 generate rotation to annihilate a(i+k-ka-1,i)
*
                  CALL CSLARTG( AB( K+1, I ), RA1, WORK( N+I+K-KA ),
     $                         WORK( I+K-KA ), RA )
*
*                 create nonzero element a(i+k-ka-1,i+k) outside the
...
*
            IF( NRT.GT.0 )
     $         CALL CSLARGV( NRT, AB( 1, J1+KA ), INCA, WORK( J1 ), 
     $  KA1, 
     $                      WORK( N+J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
...
*
               DO 580 L = 1, KA - 1
                  CALL CSLARTV( NR, AB( KA1-L, J1+L ), INCA,
...
*              blocks
*
               CALL CSLAR2V( NR, AB( KA1, J1 ), AB( KA1, J1-1 ),
...
*
            DO 590 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( L, J1T ), INCA,
...
*
            DO 620 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( L, J1T+KA ), INCA,
...
*              which have been created outside the band
*
               CALL CSLARGV( NR, AB( 1, J1+KA ), INCA, WORK( M-KB+J1 ),
...
*                 generate rotation to annihilate a(i,i+k-ka-1)
*
                  CALL CSLARTG( AB( KA1-K, I+K-KA ), RA1,
...
*
            IF( NRT.GT.0 )
     $         CALL CSLARGV( NRT, AB( KA1, J1 ), INCA, WORK( J1 ), KA1,
     $                      WORK( N+J1 ), KA1 )
            IF( NR.GT.0 ) THEN
*
*              apply rotations in 1st set from the right
*
               DO 810 L = 1, KA - 1
                  CALL CSLARTV( NR, AB( L+1, J1 ), INCA, AB( L+2, J1-1 
     $  ), 
...
*              blocks
*
               CALL CSLAR2V( NR, AB( 1, J1 ), AB( 1, J1-1 ),
...
*
            DO 820 L = KA - 1, KB - K + 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( KA1-L+1, J1T-KA1+L ), INCA,
...
*
            DO 850 L = KB - K, 1, -1
               ...
               J1T = J2 - ( NRT-1 )*KA1
               IF( NRT.GT.0 )
     $            CALL CSLARTV( NRT, AB( KA1-L+1, J1T+L-1 ), INCA,
...
*              which have been created outside the band
*
               CALL CSLARGV( NR, AB( KA1, J1 ), INCA, WORK( M-KB+J1 ),
     $                      KA1, WORK( N+M-KB+J1 ), KA1 )
*
*              apply rotations in 2nd set from the right
*
