...
*  =======
*
*  CCLAHQR is an auxiliary routine called by CCHSEQR to update the
*  eigenvalues and Schur decomposition already computed by CCHSEQR, by
...
*          CCLAHQR works primarily with the Hessenberg submatrix in rows
...
*          matrix Z of transformations accumulated by CCHSEQR, and on
...
*  INFO    (output) INTEGER
*          = 0: successful exit
*          > 0: if INFO = i, CCLAHQR failed to compute all the
...
*           submatrix.
*
*           V(2) is always real before the call to CCLARFG, and hence
*           after the call T2 ( = T1*V(2) ) is also real.
*
            IF( K.GT.M )
     $         CALL CCOPY( 2, H( K, K-1 ), 1, V, 1 )
            CALL CCLARFG( 2, V( 1 ), V( 2 ), 1, T1 )
            IF( K.GT.M ) THEN
               H( K, K-1 ) = V( 1 )
               H( K+1, K-1 ) = ZERO
            END IF
