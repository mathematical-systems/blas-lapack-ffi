...
*  =======
*
*  CCLALS0 applies back the multiplying factors of either the left or the
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CCLACPY, CCLASCL, CCSROT, CSSCAL, 
     $  SGEMV, 
     $                   XERBLA
*     ..
*     .. External Functions ..
...
*
         DO 10 I = 1, GIVPTR
            CALL CCSROT( NRHS, B( GIVCOL( I, 2 ), 1 ), LDB,
...
*
   ...
   80          CONTINUE
     ...
     $                           RWORK( JCOL+K+NRHS ) )
   90          CONTINUE
               CALL CCLASCL( 'G', 0, 0, TEMP, ONE, 1, NRHS, B( J, 1 ),
     $                      LDB, INFO )
  100       CONTINUE
         END IF
*
...
*
         IF( K.LT.MAX( M, N ) )
     $      CALL CCLACPY( 'A', N-K, NRHS, BX( K+1, 1 ), LDBX,
     $                   B( K+1, 1 ), LDB )
      ELSE
*
     ...
     $                         ( DSIGJ+POLES( J, 1 ) ) / DIFR( J, 2 )
               ...
               DO 110 I = 1, J - 1
                  IF( Z( J ).EQ.ZERO ) THEN
                     RWORK( I ) = ZERO
                  ELSE
                     RWORK( I ) = Z( J ) / ( SLAMC3( DSIGJ, -POLES( 
     $  I+1, 
     $                            2 ) )-DIFR( I, 1 ) ) /
...
*
         IF( SQRE.EQ.1 ) THEN
            ...
            CALL CCSROT( NRHS, BX( 1, 1 ), LDBX, BX( M, 1 ), LDBX, C, S 
     $  ) 
         END IF
         IF( K.LT.MAX( M, N ) )
     $      CALL CCLACPY( 'A', N-K, NRHS, B( K+1, 1 ), LDB,
     $                   BX( K+1, 1 ), LDBX )
*
