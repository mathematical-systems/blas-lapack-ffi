...
*  =======
*
*  CZGTRFS improves the computed solution to a system of linear
...
*          LU factorization of A as computed by CZGTTRF.
*
...
*          On entry, the solution matrix X, as computed by CZGTTRS.
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZAXPY, ZCOPY, CZGTTRS, CZLACN2, 
     $  CZLAGTM 
*     ..
*     .. Intrinsic Functions ..
...
*
         ...
         CALL CZLAGTM( TRANS, N, 1, -ONE, DL, D, DU, X( 1, J ), LDX, 
     $  ONE, 
     $                WORK, N )
*
...
*
            CALL CZGTTRS( TRANS, N, 1, DLF, DF, DUF, DU2, IPIV, WORK, 
     $  N, 
     $                   INFO )
...
*
*        Use CZLACN2 to estimate the infinity-norm of the matrix
*           inv(op(A)) * diag(W),
...
*
         KASE = 0
   70    CONTINUE
         CALL CZLACN2( N, WORK( N+1 ), WORK, FERR( J ), KASE, ISAVE )
         IF( KASE.NE.0 ) THEN
            IF( KASE.EQ.1 ) THEN
*
*              Multiply by diag(W)*inv(op(A)**H).
*
               CALL CZGTTRS( TRANST, N, 1, DLF, DF, DUF, DU2, IPIV, 
     $  WORK, 
     $                      N, INFO )
               DO 80 I = 1, N
...
*
               DO 90 I = 1, N
                  WORK( I ) = RWORK( I )*WORK( I )
   90          CONTINUE
               CALL CZGTTRS( TRANSN, N, 1, DLF, DF, DUF, DU2, IPIV, 
     $  WORK, 
     $                      N, INFO )
            END IF
            GO TO 70
