...
*  =======
*
*  CZTGSY2 solves the generalized Sylvester equation
*
*              A * R - L * B = scale *   C               (1)
*              D * R - L * E = scale * F
*
...
*  = sigma_min(Z) using reverse communicaton with CZLACON.
*
*  CZTGSY2 also (IJOB >= 1) contributes to the computation in CZTGSYL
...
*          NOTE: RDSUM only makes sense when CZTGSY2 is called by
*          CZTGSYL.
*
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZAXPY, CZGESC2, CZGETC2, CZLATDF, 
     $  ZSCAL 
*     ..
*     .. Intrinsic Functions ..
...
*              Solve Z * x = RHS
*
               CALL CZGETC2( LDZ, Z, LDZ, IPIV, JPIV, IERR )
               IF( IERR.GT.0 )
     $            INFO = IERR
               IF( IJOB.EQ.0 ) THEN
                  CALL CZGESC2( LDZ, Z, LDZ, RHS, IPIV, JPIV, SCALOC )
   ...
   10                CONTINUE
                  ...
                  END IF
               ELSE
                  CALL CZLATDF( IJOB, LDZ, Z, LDZ, RHS, RDSUM, RDSCAL,
     $                         IPIV, JPIV )
               END IF
*
