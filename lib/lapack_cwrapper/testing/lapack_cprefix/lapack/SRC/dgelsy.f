...
*  =======
*
*  CCDGELSY computes the minimum-norm solution to a real linear least
*  squares problem:
*      minimize || A * X - B ||
...
*          by ILAENV for the routines CDGEQP3, CDTZRZF, CSTZRQF, CDORMQR,
*          and CDORMRZ.
*
...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, CDGEQP3, CDLABAD, CDLAIC1, CDLASCL, 
     $  CDLASET, 
     $                   CDORMQR, CDORMRZ, DTRSM, CDTZRZF, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INFO.EQ.0 ) THEN
         IF( MN.EQ.0 .OR. NRHS.EQ.0 ) THEN
            ...
            LWKOPT = 1
         ELSE
            NB1 = ILAENV( 1, 'CDGEQRF', ' ', M, N, -1, -1 )
            NB2 = ILAENV( 1, 'CDGERQF', ' ', M, N, -1, -1 )
            NB3 = ILAENV( 1, 'CDORMQR', ' ', M, N, NRHS, -1 )
            NB4 = ILAENV( 1, 'CDORMRQ', ' ', M, N, NRHS, -1 )
...
*        A * P = Q * R
*
      CALL CDGEQP3( M, N, A, LDA, JPVT, WORK( 1 ), WORK( MN+1 ),
     $             LWORK-MN, INFO )
      WSIZE = MN + WORK( MN+1 )
*
...
*
   10 CONTINUE
      IF( RANK.LT.MN ) THEN
         I = RANK + 1
         CALL CDLAIC1( IMIN, RANK, WORK( ISMIN ), SMIN, A( 1, I ),
     $                A( I, I ), SMINPR, S1, C1 )
         CALL CDLAIC1( IMAX, RANK, WORK( ISMAX ), SMAX, A( 1, I ),
     $                A( I, I ), SMAXPR, S2, C2 )
*
...
*
      IF( RANK.LT.N )
     $   CALL CDTZRZF( RANK, N, A, LDA, WORK( MN+1 ), WORK( 2*MN+1 ),
     $                LWORK-2*MN, INFO )
*
*     workspace: 2*MN.
...
*
      CALL CDORMQR( 'Left', 'Transpose', M, NRHS, MN, A, LDA, WORK( 1 
     $  ), 
...
*
      IF( RANK.LT.N ) THEN
         CALL CDORMRZ( 'Left', 'Transpose', N, NRHS, RANK, N-RANK, A,
...
*
      IF( IASCL.EQ.1 ) THEN
         ...
         CALL CDLASCL( 'U', 0, 0, SMLNUM, ANRM, RANK, RANK, A, LDA,
     $                INFO )
      ELSE IF( IASCL.EQ.2 ) THEN
         CALL CDLASCL( 'G', 0, 0, ANRM, BIGNUM, N, NRHS, B, LDB, INFO )
         CALL CDLASCL( 'U', 0, 0, BIGNUM, ANRM, RANK, RANK, A, LDA,
     $                INFO )
      END IF
      IF( IBSCL.EQ.1 ) THEN
