...
*  =======
*
*  CDTGEVC computes some or all of the right and/or left eigenvectors of
...
*     A = Q*S*Z**T,  B = Q*P*Z**T
*
*  as computed by CDGGHRD + CDHGEQZ.
*
...
*          factorization, as computed by CDHGEQZ.
*
*  LDS     (input) INTEGER
...
*          of left Schur vectors returned by CDHGEQZ).
...
*          of right Schur vectors returned by CDHGEQZ).
*
...
*     .. Local Scalars ..
     ...
     $                   CIM2B, CIMAGA, CIMAGB, CRE2A, CRE2B, REALA,
     $                   REALB, DMIN, SAFMIN, SALFAR, SBETA, SCALE,
...
*     .. External Subroutines ..
      EXTERNAL           DGEMV, CDLABAD, CDLACPY, CDLAG2, CDLALN2, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*              Complex eigenvalue
*
               CALL CDLAG2( S( JE, JE ), LDS, P( JE, JE ), LDP,
...
*
               CALL CDLALN2( .TRUE., NA, NW, DMIN, ACOEF, S( J, J ), 
     $  LDS, 
...
*
     ...
     $                        WORK( ( JW+4 )*N+1 ), 1 )
  170          CONTINUE
               CALL CDLACPY( ' ', N, NW, WORK( 4*N+1 ), N, VL( 1, JE ),
     $                      LDVL )
               IBEG = 1
            ELSE
               CALL CDLACPY( ' ', N, NW, WORK( 2*N+1 ), N, VL( 1, IEIG 
     $  ), 
     $                      LDVL )
               IBEG = JE
            END IF
...
*              Complex eigenvalue
*
               CALL CDLAG2( S( JE-1, JE-1 ), LDS, P( JE-1, JE-1 ), LDP,
     $                     SAFMIN*SAFETY, ACOEF, TEMP, BCOEFR, TEMP2,
     $                     BCOEFI )
               IF( BCOEFI.EQ.ZERO ) THEN
                  INFO = JE - 1
...
*
               REALA = ACOEF*WORK( 2*N+JE-1 )
               CIMAGA = ACOEF*WORK( 3*N+JE-1 )
               REALB = BCOEFR*WORK( 2*N+JE-1 ) -
...
*
               CALL CDLALN2( .FALSE., NA, NW, DMIN, ACOEF, S( J, J ),
...
*
                  DO 360 JA = 1, NA
                     IF( ILCPLX ) THEN
                        REALA = ACOEF*WORK( 2*N+J+JA-1 )
                        CIMAGA = ACOEF*WORK( 3*N+J+JA-1 )
                        REALB = BCOEFR*WORK( 2*N+J+JA-1 ) -
