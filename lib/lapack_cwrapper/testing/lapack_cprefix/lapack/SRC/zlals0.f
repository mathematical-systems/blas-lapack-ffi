...
*  =======
*
*  CZLALS0 applies back the multiplying factors of either the left or the
...
*     .. External Subroutines ..
      EXTERNAL           DGEMV, XERBLA, ZCOPY, CZDROT, ZDSCAL, 
     $  CZLACPY, 
     $                   CZLASCL
*     ..
*     .. External Functions ..
...
*
         DO 10 I = 1, GIVPTR
            CALL CZDROT( NRHS, B( GIVCOL( I, 2 ), 1 ), LDB,
...
*
   ...
   80          CONTINUE
     ...
     $                           RWORK( JCOL+K+NRHS ) )
   90          CONTINUE
               CALL CZLASCL( 'G', 0, 0, TEMP, ONE, 1, NRHS, B( J, 1 ),
     $                      LDB, INFO )
  100       CONTINUE
         END IF
*
...
*
         IF( K.LT.MAX( M, N ) )
     $      CALL CZLACPY( 'A', N-K, NRHS, BX( K+1, 1 ), LDBX,
     $                   B( K+1, 1 ), LDB )
      ELSE
*
     ...
     $                         ( DSIGJ+POLES( J, 1 ) ) / DIFR( J, 2 )
               ...
               DO 110 I = 1, J - 1
                  IF( Z( J ).EQ.ZERO ) THEN
                     RWORK( I ) = ZERO
                  ELSE
                     RWORK( I ) = Z( J ) / ( DLAMC3( DSIGJ, -POLES( 
     $  I+1, 
     $                            2 ) )-DIFR( I, 1 ) ) /
...
*
         IF( SQRE.EQ.1 ) THEN
            ...
            CALL CZDROT( NRHS, BX( 1, 1 ), LDBX, BX( M, 1 ), LDBX, C, S 
     $  ) 
         END IF
         IF( K.LT.MAX( M, N ) )
     $      CALL CZLACPY( 'A', N-K, NRHS, B( K+1, 1 ), LDB, BX( K+1, 1 
     $  ), 
     $                   LDBX )
*
