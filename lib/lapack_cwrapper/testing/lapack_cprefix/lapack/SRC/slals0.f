...
*  =======
*
*  CSLALS0 applies back the multiplying factors of either the left or the
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, SGEMV, CSLACPY, CSLASCL, SROT, SSCAL,
     $                   XERBLA
*     ..
*     .. External Functions ..
      REAL               SLAMC3, SNRM2
...
*
   ...
   40          CONTINUE
     ...
     $                     B( J, 1 ), LDB )
               CALL CSLASCL( 'G', 0, 0, TEMP, ONE, 1, NRHS, B( J, 1 ),
     $                      LDB, INFO )
   50       CONTINUE
         END IF
*
...
*
         IF( K.LT.MAX( M, N ) )
     $      CALL CSLACPY( 'A', N-K, NRHS, BX( K+1, 1 ), LDBX,
     $                   B( K+1, 1 ), LDB )
      ELSE
*
...
*
         ...
         END IF
         IF( K.LT.MAX( M, N ) )
     $      CALL CSLACPY( 'A', N-K, NRHS, B( K+1, 1 ), LDB, BX( K+1, 1 
     $  ), 
     $                   LDBX )
*
