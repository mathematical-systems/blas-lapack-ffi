...
*  =======
*
*  CCDGELSD computes the minimum-norm solution to a real linear least
*  squares problem:
...
*     .. External Subroutines ..
      EXTERNAL           CDGEBRD, CDGELQF, CDGEQRF, CDLABAD, CDLACPY, 
     $  CDLALSD, 
     $                   CDLASCL, CDLASET, CDORMBR, CDORMLQ, CDORMQR, 
     $  XERBLA 
*     ..
*     .. External Functions ..
      INTEGER            ILAENV
...
*
      ...
      MINMN = MIN( M, N )
      MAXMN = MAX( M, N )
      MNTHR = ILAENV( 6, 'CCDGELSD', ' ', M, N, NRHS, -1 )
      LQUERY = ( LWORK.EQ.-1 )
      IF( M.LT.0 ) THEN
         INFO = -1
      ELSE IF( N.LT.0 ) THEN
      ...
      END IF
*
      SMLSIZ = ILAENV( 9, 'CCDGELSD', ' ', 0, 0, 0, 0 )
*
*     Compute workspace.
...
*
            MM = N
            MAXWRK = MAX( MAXWRK, N+N*ILAENV( 1, 'CDGEQRF', ' ', M, N,
     $               -1, -1 ) )
            MAXWRK = MAX( MAXWRK, N+NRHS*
     $               ILAENV( 1, 'CDORMQR', 'LT', M, NRHS, N, -1 ) )
         END IF
         IF( M.GE.N ) THEN
*
...
*
     ...
     $               ILAENV( 1, 'CDGEBRD', ' ', MM, N, -1, -1 ) )
            MAXWRK = MAX( MAXWRK, 3*N+NRHS*
     $               ILAENV( 1, 'CDORMBR', 'QLT', MM, NRHS, N, -1 ) )
            MAXWRK = MAX( MAXWRK, 3*N+( N-1 )*
     $               ILAENV( 1, 'CDORMBR', 'PLN', N, NRHS, N, -1 ) )
...
*              than rows.
*
               MAXWRK = M + M*ILAENV( 1, 'CDGELQF', ' ', M, N, -1, -1 
     $  ) 
               MAXWRK = MAX( MAXWRK, M*M+4*M+2*M*
     $                  ILAENV( 1, 'CDGEBRD', ' ', M, M, -1, -1 ) )
               MAXWRK = MAX( MAXWRK, M*M+4*M+NRHS*
     $                  ILAENV( 1, 'CDORMBR', 'QLT', M, NRHS, M, -1 ) 
     $  ) 
               MAXWRK = MAX( MAXWRK, M*M+4*M+( M-1 )*
     $                  ILAENV( 1, 'CDORMBR', 'PLN', M, NRHS, M, -1 ) 
     $  ) 
               IF( NRHS.GT.1 ) THEN
     ...
     $                  ILAENV( 1, 'CDORMLQ', 'LT', N, NRHS, M, -1 ) )
               MAXWRK = MAX( MAXWRK, M*M+4*M+WLALSD )
            ELSE
*
*              Path 2 - remaining underdetermined cases.
*
               MAXWRK = 3*M + ( N+M )*ILAENV( 1, 'CDGEBRD', ' ', M, N,
     $                  -1, -1 )
               MAXWRK = MAX( MAXWRK, 3*M+NRHS*
     $                  ILAENV( 1, 'CDORMBR', 'QLT', M, NRHS, N, -1 ) 
     $  ) 
               MAXWRK = MAX( MAXWRK, 3*M+M*
     $                  ILAENV( 1, 'CDORMBR', 'PLN', N, NRHS, M, -1 ) 
     $  ) 
...
*
         CALL CDLALSD( 'U', SMLSIZ, N, NRHS, S, WORK( IE ), B, LDB,
...
*
         CALL CDORMBR( 'P', 'L', 'N', N, NRHS, N, A, LDA, WORK( ITAUP 
     $  ), 
     $                B, LDB, WORK( NWORK ), LWORK-NWORK+1, INFO )
*
...
*        Solve the bidiagonal least squares problem.
*
         CALL CDLALSD( 'U', SMLSIZ, M, NRHS, S, WORK( IE ), B, LDB,
     $                RCOND, RANK, WORK( NWORK ), IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            GO TO 10
         END IF
...
*
         CALL CDORMBR( 'P', 'L', 'N', M, NRHS, M, WORK( IL ), LDWORK,
...
*        Solve the bidiagonal least squares problem.
*
         CALL CDLALSD( 'L', SMLSIZ, M, NRHS, S, WORK( IE ), B, LDB,
     $                RCOND, RANK, WORK( NWORK ), IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            GO TO 10
         END IF
...
*
         CALL CDORMBR( 'P', 'L', 'N', N, NRHS, M, A, LDA, WORK( ITAUP 
     $  ), 
     $                B, LDB, WORK( NWORK ), LWORK-NWORK+1, INFO )
*
      END IF
