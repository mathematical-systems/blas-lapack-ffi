...
*  =======
*
*  CZLAED7 computes the updated eigensystem of a diagonal
...
*        equation via the routine CDLAED4 (as called by CSLAED3).
...
*     .. External Subroutines ..
      EXTERNAL           CDLAED9, CDLAEDA, CDLAMRG, XERBLA, CZLACRM, 
     $  CZLAED8 
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*     used by a particular array in CDLAED2 and CSLAED3.
*
      IZ = 1
      IDLMDA = IZ + N
      IW = IDLMDA + N
...
*
      CALL CZLAED8( K, N, QSIZ, Q, LDQ, D, RHO, CUTPNT, RWORK( IZ ),
...
*
      IF( K.NE.0 ) THEN
         CALL CDLAED9( K, 1, K, N, D, RWORK( IQ ), K, RHO,
     $                RWORK( IDLMDA ), RWORK( IW ),
     $                QSTORE( QPTR( CURR ) ), K, INFO )
         CALL CZLACRM( QSIZ, K, WORK, QSIZ, QSTORE( QPTR( CURR ) ), K, 
     $  Q, 
     $                LDQ, RWORK( IQ ) )
