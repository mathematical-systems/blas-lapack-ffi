...
*  =======
*
*  CSGBBRD reduces a real general m-by-n band matrix A to upper
...
*     .. External Subroutines ..
      EXTERNAL           CSLARGV, CSLARTG, CSLARTV, CSLASET, SROT, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*
      IF( WANTQ )
     $   CALL CSLASET( 'Full', M, M, ZERO, ONE, Q, LDQ )
      IF( WANTPT )
     $   CALL CSLASET( 'Full', N, N, ZERO, ONE, PT, LDPT )
*
...
*
               IF( NR.GT.0 )
     $            CALL CSLARGV( NR, AB( KLU1, J1-KLM-1 ), INCA,
...
*
               DO 10 L = 1, KB
                  ...
                  END IF
                  IF( NRT.GT.0 )
     $               CALL CSLARTV( NRT, AB( KLU1-L, J1-KLM+L-1 ), INCA,
...
*
                     CALL CSLARTG( AB( KU+ML-1, I ), AB( KU+ML, I ),
...
*
               IF( NR.GT.0 )
     $            CALL CSLARGV( NR, AB( 1, J1+KUN-1 ), INCA,
...
*
               DO 50 L = 1, KB
                  ...
                  END IF
                  IF( NRT.GT.0 )
     $               CALL CSLARTV( NRT, AB( L+1, J1+KUN-1 ), INCA,
     $                            AB( L, J1+KUN ), INCA,
...
*
                     CALL CSLARTG( AB( KU-MU+3, I+MU-2 ),
     $                            AB( KU-MU+2, I+MU-1 ),
...
*
         DO 100 I = 1, MIN( M-1, N )
            CALL CSLARTG( AB( 1, I ), AB( 2, I ), RC, RS, RA )
            D( I ) = RA
            IF( I.LT.N ) THEN
               E( I ) = RS*AB( 1, I+1 )
...
*
            RB = AB( KU, M+1 )
            DO 110 I = M, 1, -1
               CALL CSLARTG( AB( KU+1, I ), RB, RC, RS, RA )
               D( I ) = RA
               IF( I.GT.1 ) THEN
                  RB = -RS*AB( KU, I )
                  E( I-1 ) = RC*AB( KU, I )
