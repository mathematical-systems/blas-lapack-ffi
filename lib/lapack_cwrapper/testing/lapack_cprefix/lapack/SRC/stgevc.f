...
*  =======
*
*  CSTGEVC computes some or all of the right and/or left eigenvectors of
...
*     A = Q*S*Z**T,  B = Q*P*Z**T
*
*  as computed by CSGGHRD + CSHGEQZ.
*
...
*          factorization, as computed by CSHGEQZ.
*
*  LDS     (input) INTEGER
...
*          of left Schur vectors returned by CSHGEQZ).
...
*          of right Schur vectors returned by CSHGEQZ).
*
...
*     .. External Subroutines ..
      EXTERNAL           SGEMV, CSLABAD, CSLACPY, CSLAG2, CSLALN2, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*              Complex eigenvalue
*
               CALL CSLAG2( S( JE, JE ), LDS, P( JE, JE ), LDP,
...
*
               CALL CSLALN2( .TRUE., NA, NW, DMIN, ACOEF, S( J, J ), 
     $  LDS, 
...
*
     ...
     $                        WORK( ( JW+4 )*N+1 ), 1 )
  170          CONTINUE
               CALL CSLACPY( ' ', N, NW, WORK( 4*N+1 ), N, VL( 1, JE ),
     $                      LDVL )
               IBEG = 1
            ELSE
               CALL CSLACPY( ' ', N, NW, WORK( 2*N+1 ), N, VL( 1, IEIG 
     $  ), 
     $                      LDVL )
               IBEG = JE
            END IF
...
*              Complex eigenvalue
*
               CALL CSLAG2( S( JE-1, JE-1 ), LDS, P( JE-1, JE-1 ), LDP,
     $                     SAFMIN*SAFETY, ACOEF, TEMP, BCOEFR, TEMP2,
     $                     BCOEFI )
               IF( BCOEFI.EQ.ZERO ) THEN
                  INFO = JE - 1
...
*
               CALL CSLALN2( .FALSE., NA, NW, DMIN, ACOEF, S( J, J ),
