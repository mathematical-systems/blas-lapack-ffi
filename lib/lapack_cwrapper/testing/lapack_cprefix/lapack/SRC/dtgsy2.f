...
*     .. Scalar Arguments ..
      CHARACTER          TRANS
      INTEGER            IJOB, INFO, LDA, LDB, LDC, LDD, LDE, LDF, M, 
     $  N, 
     $                   PQ
...
*  =======
*
*  CDTGSY2 solves the generalized Sylvester equation:
*
*              A * R - L * B = scale * C                (1)
*              D * R - L * E = scale * F,
*
...
*  sigma_min(Z) using reverse communicaton with CDLACON.
*
*  CDTGSY2 also (IJOB >= 1) contributes to the computation in CSTGSYL
*  of an upper bound on the separation between to matrix pairs. Then
*  the input (A, D), (B, E) are sub-pencils of the matrix pair in
*  CDTGSYL. See CSTGSYL for details.
*
*  Arguments
*  =========
*
...
*               norm-based estimate of the separation between two matrix
*               pairs is computed. (CDGECON on sub-systems is used.)
...
*          the Dif-estimate under computation by CDTGSYL, where the
*          scaling factor RDSCAL (see below) has been factored out.
*          On exit, the corresponding sum of squares updated with the
*          contributions from the current sub-system.
*          If TRANS = 'T' RDSUM is not touched.
*          NOTE: RDSUM only makes sense when CDTGSY2 is called by CSTGSYL.
*
...
*          NOTE: RDSCAL only makes sense when CDTGSY2 is called by
*                CDTGSYL.
*
...
*
*  =====================================================================
*  Replaced various illegal calls to DCOPY by calls to CDLASET.
...
*     .. External Subroutines ..
      EXTERNAL           DAXPY, DCOPY, DGEMM, DGEMV, DGER, CDGESC2,
     $                   CDGETC2, CDLASET, CDLATDF, DSCAL, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*                 Solve Z * x = RHS
*
                  CALL CDGETC2( ZDIM, Z, LDZ, IPIV, JPIV, IERR )
                  IF( IERR.GT.0 )
     $               INFO = IERR
*
                  IF( IJOB.EQ.0 ) THEN
                     CALL CDGESC2( ZDIM, Z, LDZ, RHS, IPIV, JPIV,
     $                            SCALOC )
   ...
   50                   CONTINUE
                     ...
                     END IF
                  ELSE
                     CALL CDLATDF( IJOB, ZDIM, Z, LDZ, RHS, RDSUM,
     $                            RDSCAL, IPIV, JPIV )
                  END IF
*
...
*
                  CALL CDLASET( 'F', LDZ, LDZ, ZERO, ZERO, Z, LDZ )
*
                  Z( 1, 1 ) = A( IS, IS )
                  Z( 2, 1 ) = A( ISP1, IS )
                  Z( 5, 1 ) = D( IS, IS )
...
*
     ...
     $               INFO = IERR
*
                  CALL CDGESC2( ZDIM, Z, LDZ, RHS, IPIV, JPIV, SCALOC )
                  IF( SCALOC.NE.ONE ) THEN
                     DO 130 K = 1, N
                        CALL DSCAL( M, SCALOC, C( 1, K ), 1 )
                        CALL DSCAL( M, SCALOC, F( 1, K ), 1 )
...
*
     ...
     $                           C( IS, JS ), LDC, B( 1, JS ), LDB, 
     $  ONE, 
     $                           F( IS, 1 ), LDF )
                     CALL DGEMM( 'N', 'T', MB, JS-1, NB, ONE,
     $                           F( IS, JS ), LDF, E( 1, JS ), LDE, 
     $  ONE, 
     $                           F( IS, 1 ), LDF )
                  END IF
                  IF( I.LT.P ) THEN
