...
*  =======
*
*  CZHEGS2 reduces a complex Hermitian-definite generalized
...
*
*  B must have been previously factorized as U'*U or L*L' by CZPOTRF.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZAXPY, ZDSCAL, ZHER2, CZLACGV, ZTRMV,
     $                   ZTRSV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
...
*
               ...
               IF( K.LT.N ) THEN
                  ...
                  CT = -HALF*AKK
                  CALL CZLACGV( N-K, A( K, K+1 ), LDA )
                  CALL CZLACGV( N-K, B( K, K+1 ), LDB )
...
*
               AKK = A( K, K )
               BKK = B( K, K )
               CALL CZLACGV( K-1, A( K, 1 ), LDA )
               CALL ZTRMV( UPLO, 'Conjugate transpose', 'Non-unit', 
     $  K-1, 
     $                     B, LDB, A( K, 1 ), LDA )
               CT = HALF*AKK
               CALL CZLACGV( K-1, B( K, 1 ), LDB )
