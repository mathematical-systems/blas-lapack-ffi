...
*  =======
*
*  CZSPTRI computes the inverse of a complex symmetric indefinite matrix
*  A in packed storage using the factorization A = U*D*U**T or
*  A = L*D*L**T computed by CZSPTRF.
*
*  Arguments
*  =========
*
...
*          used to obtain the factor U or L as computed by CZSPTRF,
...
*     .. External Subroutines ..
      EXTERNAL           XERBLA, ZCOPY, CZSPMV, ZSWAP
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS
*     ..
...
*
            IF( K.GT.1 ) THEN
               ...
               CALL CZSPMV( UPLO, K-1, -ONE, AP, WORK, 1, ZERO, AP( KC 
     $  ), 
     $                     1 )
               AP( KC+K-1 ) = AP( KC+K-1 ) -
...
*
     ...
     $                            1 )
               ...
               CALL CZSPMV( UPLO, K-1, -ONE, AP, WORK, 1, ZERO,
     $                     AP( KCNEXT ), 1 )
...
*
            IF( K.LT.N ) THEN
               ...
               CALL CZSPMV( UPLO, N-K, -ONE, AP( KC+N-K+1 ), WORK, 1,
     $                     ZERO, AP( KC+1 ), 1 )
...
*
            IF( K.LT.N ) THEN
               CALL ZCOPY( N-K, AP( KC+1 ), 1, WORK, 1 )
               CALL CZSPMV( UPLO, N-K, -ONE, AP( KC+( N-K+1 ) ), WORK, 
     $  1, 
     $                     ZERO, AP( KC+1 ), 1 )
               AP( KC ) = AP( KC ) - ZDOTU( N-K, WORK, 1, AP( KC+1 ),
     $                    1 )
