...
*  =======
*
*  CCTGSY2 solves the generalized Sylvester equation
*
*              A * R - L * B = scale *   C               (1)
*              D * R - L * E = scale * F
*
...
*  = sigma_min(Z) using reverse communicaton with CCLACON.
*
*  CCTGSY2 also (IJOB >= 1) contributes to the computation in CCTGSYL
...
*          NOTE: RDSUM only makes sense when CCTGSY2 is called by
*          CCTGSYL.
*
...
*     .. External Subroutines ..
      EXTERNAL           CAXPY, CCGESC2, CCGETC2, CSCAL, CCLATDF, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*              Solve Z * x = RHS
*
               CALL CCGETC2( LDZ, Z, LDZ, IPIV, JPIV, IERR )
               IF( IERR.GT.0 )
     $            INFO = IERR
               IF( IJOB.EQ.0 ) THEN
                  CALL CCGESC2( LDZ, Z, LDZ, RHS, IPIV, JPIV, SCALOC )
   ...
   10                CONTINUE
                  ...
                  END IF
               ELSE
                  CALL CCLATDF( IJOB, LDZ, Z, LDZ, RHS, RDSUM, RDSCAL,
     $                         IPIV, JPIV )
               END IF
*
