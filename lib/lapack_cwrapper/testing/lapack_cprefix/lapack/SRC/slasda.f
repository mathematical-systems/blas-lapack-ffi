...
*  =======
*
*  Using a divide and conquer approach, CSLASDA computes the singular
...
*  compact form.
*
*  A related subroutine, CSLASD0, computes the singular values and
...
*         the right singular vector matrix. See CSLASD8 for details.
*
*  Z      (output) REAL array,
...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, CSLASD6, CSLASDQ, CSLASDT, CSLASET, 
     $  XERBLA 
*     ..
...
*
      ...
      END IF
      IF( INFO.NE.0 ) THEN
         CALL XERBLA( 'CSLASDA', -INFO )
         RETURN
      END IF
*
      M = N + SQRE
*
*     If the input matrix is too small, call CSLASDQ to find the SVD.
*
      IF( N.LE.SMLSIZ ) THEN
         IF( ICOMPQ.EQ.0 ) THEN
            CALL CSLASDQ( 'U', SQRE, N, 0, 0, 0, D, E, VT, LDU, U, LDU,
     $                   U, LDU, WORK, INFO )
         ELSE
            CALL CSLASDQ( 'U', SQRE, N, M, N, 0, D, E, VT, LDU, U, LDU,
     $                   U, LDU, WORK, INFO )
         END IF
         RETURN
      END IF
...
*
      CALL CSLASDT( N, NLVL, ND, IWORK( INODE ), IWORK( NDIML ),
     $             IWORK( NDIMR ), SMLSIZ )
*
*     for the nodes on bottom level of the tree, solve
*     their subproblems by CSLASDQ.
*
      NDB1 = ( ND+1 ) / 2
      DO 30 I = NDB1, ND
*
...
*
         ...
         SQREI = 1
         IF( ICOMPQ.EQ.0 ) THEN
            CALL CSLASET( 'A', NLP1, NLP1, ZERO, ONE, WORK( NWORK1 ),
     $                   SMLSZP )
            CALL CSLASDQ( 'U', SQREI, NL, NLP1, NRU, NCC, D( NLF ),
     ...
     $                   WORK( NWORK2 ), INFO )
         ...
         ELSE
            CALL CSLASET( 'A', NL, NL, ZERO, ONE, U( NLF, 1 ), LDU )
            CALL CSLASET( 'A', NLP1, NLP1, ZERO, ONE, VT( NLF, 1 ), LDU 
     $  ) 
            CALL CSLASDQ( 'U', SQREI, NL, NLP1, NL, NCC, D( NLF ),
   ...
   10    CONTINUE
         ...
         NRP1 = NR + SQREI
         IF( ICOMPQ.EQ.0 ) THEN
            CALL CSLASET( 'A', NRP1, NRP1, ZERO, ONE, WORK( NWORK1 ),
     $                   SMLSZP )
            CALL CSLASDQ( 'U', SQREI, NR, NRP1, NRU, NCC, D( NRF ),
     ...
     $                   WORK( NWORK2 ), INFO )
         ...
         ELSE
            CALL CSLASET( 'A', NR, NR, ZERO, ONE, U( NRF, 1 ), LDU )
            CALL CSLASET( 'A', NRP1, NRP1, ZERO, ONE, VT( NRF, 1 ), LDU 
     $  ) 
            CALL CSLASDQ( 'U', SQREI, NR, NRP1, NR, NCC, D( NRF ),
...
*
         ...
         DO 40 I = LF, LL
            ...
            BETA = E( IC )
            IF( ICOMPQ.EQ.0 ) THEN
               CALL CSLASD6( ICOMPQ, NL, NR, SQREI, D( NLF ),
