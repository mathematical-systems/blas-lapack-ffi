...
*  =======
*
*  CZSTEIN computes the eigenvectors of a real symmetric tridiagonal
...
*  array, which may be passed to CZUNMTR or CZUPMTR for back
...
*          W from CDSTEBZ with ORDER = 'B' is expected here. )
*
...
*          ( The output array ISPLIT from CDSTEBZ is expected here. )
*
...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, CDLAGTF, CDLAGTS, CDLARNV, DSCAL, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
*     Initialize seed for random number generator CDLARNV.
*
      DO 40 I = 1, 4
         ISEED( I ) = 1
   40 CONTINUE
...
*
            CALL CDLARNV( 2, ISEED, BLKSIZ, WORK( INDRV1+1 ) )
*
...
*
            TOL = ZERO
            CALL CDLAGTF( BLKSIZ, WORK( INDRV4+1 ), XJ, WORK( INDRV2+2 
     $  ), 
...
*
            CALL CDLAGTS( -1, BLKSIZ, WORK( INDRV4+1 ), WORK( INDRV2+2 
     $  ), 
