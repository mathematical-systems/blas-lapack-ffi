...
*  =======
*
*  CCGBBRD reduces a complex general m-by-n band matrix A to real upper
...
*     .. External Subroutines ..
      EXTERNAL           CCLARGV, CCLARTG, CCLARTV, CCLASET, CCROT, 
     $  CSCAL, 
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( WANTQ )
     $   CALL CCLASET( 'Full', M, M, CZERO, CONE, Q, LDQ )
      IF( WANTPT )
     $   CALL CCLASET( 'Full', N, N, CZERO, CONE, PT, LDPT )
*
...
*
               IF( NR.GT.0 )
     $            CALL CCLARGV( NR, AB( KLU1, J1-KLM-1 ), INCA,
...
*
               DO 10 L = 1, KB
                  ...
                  END IF
                  IF( NRT.GT.0 )
     $               CALL CCLARTV( NRT, AB( KLU1-L, J1-KLM+L-1 ), INCA,
...
*
                     CALL CCLARTG( AB( KU+ML-1, I ), AB( KU+ML, I ),
     $                            RWORK( I+ML-1 ), WORK( I+ML-1 ), RA )
                     AB( KU+ML-1, I ) = RA
                     IF( I.LT.N )
     $                  CALL CCROT( MIN( KU+ML-2, N-I ),
...
*
                  DO 20 J = J1, J2, KB1
                     CALL CCROT( M, Q( 1, J-1 ), 1, Q( 1, J ), 1,
...
*
                  DO 30 J = J1, J2, KB1
                     CALL CCROT( NCC, C( J-1, 1 ), LDC, C( J, 1 ), LDC,
     $                          RWORK( J ), WORK( J ) )
   30             CONTINUE
               END IF
*
...
*
               IF( NR.GT.0 )
     $            CALL CCLARGV( NR, AB( 1, J1+KUN-1 ), INCA,
...
*
               DO 50 L = 1, KB
                  ...
                  END IF
                  IF( NRT.GT.0 )
     $               CALL CCLARTV( NRT, AB( L+1, J1+KUN-1 ), INCA,
     $                            AB( L, J1+KUN ), INCA,
...
*
                     CALL CCLARTG( AB( KU-MU+3, I+MU-2 ),
     $                            AB( KU-MU+2, I+MU-1 ),
     $                            RWORK( I+MU-1 ), WORK( I+MU-1 ), RA )
                     AB( KU-MU+3, I+MU-2 ) = RA
                     CALL CCROT( MIN( KL+MU-2, M-I ),
     $                          AB( KU-MU+4, I+MU-2 ), 1,
     $                          AB( KU-MU+3, I+MU-1 ), 1,
...
*
                  DO 60 J = J1, J2, KB1
                     CALL CCROT( N, PT( J+KUN-1, 1 ), LDPT,
...
*
         DO 100 I = 1, MIN( M-1, N )
            CALL CCLARTG( AB( 1, I ), AB( 2, I ), RC, RS, RA )
            AB( 1, I ) = RA
            IF( I.LT.N ) THEN
               AB( 2, I ) = RS*AB( 1, I+1 )
            ...
            END IF
            IF( WANTQ )
     $         CALL CCROT( M, Q( 1, I ), 1, Q( 1, I+1 ), 1, RC,
     $                    CONJG( RS ) )
            IF( WANTC )
     $         CALL CCROT( NCC, C( I, 1 ), LDC, C( I+1, 1 ), LDC, RC,
     $                    RS )
  100    CONTINUE
      ELSE
*
...
*
            RB = AB( KU, M+1 )
            DO 110 I = M, 1, -1
               CALL CCLARTG( AB( KU+1, I ), RB, RC, RS, RA )
               AB( KU+1, I ) = RA
               IF( I.GT.1 ) THEN
                  RB = -CONJG( RS )*AB( KU, I )
                  AB( KU, I ) = RC*AB( KU, I )
               END IF
               IF( WANTPT )
     $            CALL CCROT( N, PT( I, 1 ), LDPT, PT( M+1, 1 ), LDPT,
     $                       RC, CONJG( RS ) )
  110       CONTINUE
         END IF
      END IF
