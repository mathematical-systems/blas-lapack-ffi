...
*  =======
*
*  CSLAGS2 computes 2-by-2 orthogonal matrices U, V and Q, such
*  that if ( UPPER ) then
*
*            U'*A*Q = U'*( A1 A2 )*Q = ( x  0  )
*                        ( 0  A3 )     ( x  x  )
...
*     .. Local Scalars ..
     ...
     $                   SNR, UA11R, UA22R, VB11R, VB22R, B, C, R, 
     $  UA11, 
     $                   UA12, UA21, UA22, VB11, VB12, VB21, VB22
*     ..
*     .. External Subroutines ..
      EXTERNAL           CSLARTG, CSLASV2
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS
*     ..
...
*
         CALL CSLASV2( A, B, D, S1, S2, SNR, CSR, SNL, CSL )
*
...
*
     ...
     $             ( ABS( VB11R )+ABS( VB12 ) ) ) THEN
                  CALL CSLARTG( -UA11R, UA12, CSQ, SNQ, R )
               ELSE
                  CALL CSLARTG( -VB11R, VB12, CSQ, SNQ, R )
               END IF
            ELSE
               CALL CSLARTG( -VB11R, VB12, CSQ, SNQ, R )
            END IF
...
*
     ...
     $             ( ABS( VB21 )+ABS( VB22 ) ) ) THEN
                  CALL CSLARTG( -UA21, UA22, CSQ, SNQ, R )
               ELSE
                  CALL CSLARTG( -VB21, VB22, CSQ, SNQ, R )
               END IF
            ELSE
               CALL CSLARTG( -VB21, VB22, CSQ, SNQ, R )
            END IF
...
*
         CALL CSLASV2( A, C, D, S1, S2, SNR, CSR, SNL, CSL )
*
...
*
     ...
     $             ( ABS( VB21 )+ABS( VB22R ) ) ) THEN
                  CALL CSLARTG( UA22R, UA21, CSQ, SNQ, R )
               ELSE
                  CALL CSLARTG( VB22R, VB21, CSQ, SNQ, R )
               END IF
            ELSE
               CALL CSLARTG( VB22R, VB21, CSQ, SNQ, R )
            END IF
...
*
     ...
     $             ( ABS( VB11 )+ABS( VB12 ) ) ) THEN
                  CALL CSLARTG( UA12, UA11, CSQ, SNQ, R )
               ELSE
                  CALL CSLARTG( VB12, VB11, CSQ, SNQ, R )
               END IF
            ELSE
               CALL CSLARTG( VB12, VB11, CSQ, SNQ, R )
            END IF
