...
*  =======
*
*  CZLAQPS computes a step of QR factorization with column pivoting
...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, ZGEMV, CZLARFG, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( RK.LT.M ) THEN
            CALL CZLARFG( M-RK+1, A( RK, K ), A( RK+1, K ), 1, TAU( K ) 
     $  ) 
         ELSE
            CALL CZLARFG( 1, A( RK, K ), A( RK, K ), 1, TAU( K ) )
         END IF
*
         AKK = A( RK, K )
         A( RK, K ) = CONE
