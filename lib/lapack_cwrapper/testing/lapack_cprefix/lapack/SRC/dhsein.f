...
*  =======
*
*  CDHSEIN uses inverse iteration to find specified right and/or left
...
*          = 'Q': the eigenvalues were found using CDHSEQR; thus, if
*                 H has zero subdiagonal elements, and so is
*                 block-triangular, then the j-th eigenvalue can be
*                 assumed to be an eigenvalue of the block containing
*                 the j-th row/column.  This property allows CDHSEIN to
*                 perform inverse iteration on just one diagonal block.
*          = 'N': no assumptions are made on the correspondence
*                 between eigenvalues and diagonal blocks.  In this
*                 case, CDHSEIN must always perform inverse iteration
...
*
               CALL CDLAEIN( .FALSE., NOINIT, N-KL+1, H( KL, KL ), LDH,
     $                      WKR, WKI, VL( KL, KSR ), VL( KL, KSI ),
     $                      WORK, LDWORK, WORK( N*N+N+1 ), EPS3, 
     $  SMLNUM, 
     $                      BIGNUM, IINFO )
               IF( IINFO.GT.0 ) THEN
                  IF( PAIR ) THEN
...
*
               CALL CDLAEIN( .TRUE., NOINIT, KR, H, LDH, WKR, WKI,
