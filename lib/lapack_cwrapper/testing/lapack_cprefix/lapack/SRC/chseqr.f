...
*  =======
*
*  CCHSEQR computes the eigenvalues of a complex upper Hessenberg
...
*          set by a previous call to CCGEBAL, and then passed to CCGEHRD
*          when the matrix output by CCGEBAL is reduced to Hessenberg
...
*          Normally Q is the unitary matrix generated by CCUNGHR after
*          the call to CCGEHRD which formed the Hessenberg matrix H.
*
*  LDZ     (input) INTEGER
...
*          > 0:  if INFO = i, CCHSEQR failed to compute all the
...
*     .. External Functions ..
      ...
      EXTERNAL           LSAME, ICAMAX, ILAENV, CLANHS, SLAMCH, 
     $  SLAPY2 
*     ..
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CGEMV, CCLACPY, CCLAHQR, CCLARFG, 
     $  CCLARFX, 
     $                   CCLASET, CSCAL, CSSCAL, CSLABAD, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INITZ )
     $   CALL CCLASET( 'Full', N, N, ZERO, ONE, Z, LDZ )
*
*     Store the eigenvalues isolated by CCGEBAL.
*
      DO 10 I = 1, ILO - 1
         W( I ) = H( I, I )
   10 CONTINUE
...
*
      NS = ILAENV( 4, 'CCHSEQR', JOB // COMPZ, N, ILO, IHI, -1 )
      MAXB = ILAENV( 8, 'CCHSEQR', JOB // COMPZ, N, ILO, IHI, -1 )
      IF( NS.LE.1 .OR. NS.GT.NH .OR. MAXB.GE.NH ) THEN
*
*        Use the standard double-shift algorithm
*
         CALL CCLAHQR( WANTT, WANTZ, N, ILO, IHI, H, LDH, W, ILO, IHI, 
     $  Z, 
     $                LDZ, INFO )
         RETURN
      END IF
...
*
            CALL CCLACPY( 'Full', NS, NS, H( I-NS+1, I-NS+1 ), LDH, S,
     $                   LDS )
            CALL CCLAHQR( .FALSE., .FALSE., NS, 1, NS, S, LDS,
     $                   W( I-NS+1 ), 1, NS, Z, LDZ, IERR )
            IF( IERR.GT.0 ) THEN
*
*              If CCLAHQR failed to compute all NS eigenvalues, use the
...
*           in columns K to I2.
*
            CALL CCLARFX( 'Left', NR, I2-K+1, V, CONJG( TAU ), H( K, K 
     $  ), 
     $                   LDH, WORK )
*
...
*
            CALL CCLARFX( 'Right', MIN( K+NR, I )-I1+1, NR, V, TAU,
     $                   H( I1, K ), LDH, WORK )
*
            IF( WANTZ ) THEN
*
...
*
               CALL CCLARFX( 'Right', NH, NR, V, TAU, Z( ILO, K ), 
     $  LDZ, 
     $                      WORK )
            END IF
  150    CONTINUE
...
*
      CALL CCLAHQR( WANTT, WANTZ, N, L, I, H, LDH, W, ILO, IHI, Z, LDZ,
     $             INFO )
      IF( INFO.GT.0 )
     $   RETURN
*
