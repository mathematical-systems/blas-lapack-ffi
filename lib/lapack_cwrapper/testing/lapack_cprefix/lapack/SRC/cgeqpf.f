...
*  =======
*
*  This routine is deprecated and has been replaced by routine CCGEQP3.
*
*  CCGEQPF computes a QR factorization with column pivoting of a
...
*     .. External Subroutines ..
      EXTERNAL           CCGEQR2, CCLARF, CCLARFG, CSWAP, CCUNM2R, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( ITEMP.GT.0 ) THEN
         MA = MIN( ITEMP, M )
         CALL CCGEQR2( M, MA, A, LDA, TAU, WORK, INFO )
         IF( MA.LT.N ) THEN
            CALL CCUNM2R( 'Left', 'Conjugate transpose', M, N-MA, MA, 
     $  A, 
...
*
            AII = A( I, I )
            CALL CCLARFG( M-I+1, AII, A( MIN( I+1, M ), I ), 1,
     $                   TAU( I ) )
            A( I, I ) = AII
*
            IF( I.LT.N ) THEN
