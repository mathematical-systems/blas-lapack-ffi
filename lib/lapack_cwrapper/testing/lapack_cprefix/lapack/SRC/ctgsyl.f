...
*  =======
*
*  CCTGSYL solves the generalized Sylvester equation:
*
*              A * R - L * B = scale * C            (1)
*              D * R - L * E = scale * F
*
...
*  and (B,E), using CCLACON.
*
*  If IJOB >= 1, CCTGSYL computes a Frobenius norm-based estimate of
...
*              (CCGECON on sub-systems is used).
...
*
*  =====================================================================
*  Replaced various illegal calls to CCOPY by calls to CCLASET.
...
*     .. External Subroutines ..
      EXTERNAL           CGEMM, CCLACPY, CCLASET, CSCAL, CCTGSY2, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
      MB = ILAENV( 2, 'CCTGSYL', TRANS, M, N, -1, -1 )
      NB = ILAENV( 5, 'CCTGSYL', TRANS, M, N, -1, -1 )
*
      ISOLVE = 1
      IFUNC = 0
      IF( NOTRAN ) THEN
         IF( IJOB.GE.3 ) THEN
            IFUNC = IJOB - 2
            CALL CCLASET( 'F', M, N, CZERO, CZERO, C, LDC )
            CALL CCLASET( 'F', M, N, CZERO, CZERO, F, LDF )
...
*
            ...
            DSUM = ONE
            PQ = M*N
            CALL CCTGSY2( TRANS, IFUNC, M, N, A, LDA, B, LDB, C, LDC, 
     $  D, 
     ...
     $                   INFO )
            ...
            IF( ISOLVE.EQ.2 .AND. IROUND.EQ.1 ) THEN
               ...
               END IF
               SCALE2 = SCALE
               CALL CCLACPY( 'F', M, N, C, LDC, WORK, M )
               CALL CCLACPY( 'F', M, N, F, LDF, WORK( M*N+1 ), M )
               CALL CCLASET( 'F', M, N, CZERO, CZERO, C, LDC )
               CALL CCLASET( 'F', M, N, CZERO, CZERO, F, LDF )
            ELSE IF( ISOLVE.EQ.2 .AND. IROUND.EQ.2 ) THEN
               CALL CCLACPY( 'F', M, N, WORK, M, C, LDC )
               CALL CCLACPY( 'F', M, N, WORK( M*N+1 ), M, F, LDF )
               SCALE = SCALE2
            END IF
   30    CONTINUE
*
...
*
            ...
            DO 130 J = P + 2, Q
               ...
               DO 120 I = P, 1, -1
                  ...
                  IE = IWORK( I+1 ) - 1
                  MB = IE - IS + 1
                  CALL CCTGSY2( TRANS, IFUNC, MB, NB, A( IS, IS ), LDA,
