...
*  =======
*
*  CDGGGLM solves a general Gauss-Markov linear model (GLM) problem:
*
...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, DGEMV, CDGGQRF, CDORMQR, CDORMRQ, 
     $  CDTRTRS, 
     $                   XERBLA
*     ..
*     .. External Functions ..
...
*
      IF( INFO.EQ.0) THEN
         ...
         ELSE
            ...
            NB2 = ILAENV( 1, 'CDGERQF', ' ', N, M, -1, -1 )
            NB3 = ILAENV( 1, 'CDORMQR', ' ', N, M, P, -1 )
            NB4 = ILAENV( 1, 'CDORMRQ', ' ', N, M, P, -1 )
...
*     orthogonal.
*
      CALL CDGGQRF( N, M, P, A, LDA, WORK, B, LDB, WORK( M+1 ),
...
*                                             ( d2 ) N-M
*
      CALL CDORMQR( 'Left', 'Transpose', N, 1, M, A, LDA, WORK, D,
...
*
      IF( N.GT.M ) THEN
         CALL CDTRTRS( 'Upper', 'No transpose', 'Non unit', N-M, 1,
...
*
      IF( M.GT.0 ) THEN
         CALL CDTRTRS( 'Upper', 'No Transpose', 'Non unit', M, 1, A, 
     $  LDA, 
     $                D, M, INFO )
*
         IF( INFO.GT.0 ) THEN
...
*
      CALL CDORMRQ( 'Left', 'Transpose', P, 1, NP,
