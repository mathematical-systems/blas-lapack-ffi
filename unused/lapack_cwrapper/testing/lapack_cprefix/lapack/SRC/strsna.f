...
*  =======
*
*  CSTRSNA estimates reciprocal condition numbers for specified
...
*     .. Local Scalars ..
     ...
     $                   MU, PROD, PROD1, PROD2, RNRM, SCALE, SMLNUM, 
     $  SN 
*     ..
*     .. Local Arrays ..
      INTEGER            ISAVE( 3 )
...
*     .. External Subroutines ..
      EXTERNAL           CSLABAD, CSLACN2, CSLACPY, CSLAQTR, CSTREXC, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
            CALL CSLACPY( 'Full', N, N, T, LDT, WORK, LDWORK )
            IFST = K
            ILST = 1
            CALL CSTREXC( 'No Q', N, WORK, LDWORK, DUMMY, 1, IFST, 
     $  ILST, 
     $                   WORK( 1, N+1 ), IERR )
*
...
*
               ...
               KASE = 0
   50          CONTINUE
               CALL CSLACN2( NN, WORK( 1, N+2 ), WORK( 1, N+4 ), IWORK,
     $                      EST, KASE, ISAVE )
               IF( KASE.NE.0 ) THEN
                  IF( KASE.EQ.1 ) THEN
                     IF( N2.EQ.1 ) THEN
...
*
                        CALL CSLAQTR( .TRUE., .TRUE., N-1, WORK( 2, 2 
     $  ), 
...
*
                        CALL CSLAQTR( .TRUE., .FALSE., N-1, WORK( 2, 2 
     $  ), 
...
*                       Real eigenvalue: solve C*x = scale*c.
*
                        CALL CSLAQTR( .FALSE., .TRUE., N-1, WORK( 2, 2 
     $  ), 
     $                               LDWORK, DUMMY, DUMM, SCALE,
     $                               WORK( 1, N+4 ), WORK( 1, N+6 ),
     $                               IERR )
