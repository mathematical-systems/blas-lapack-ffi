...
*  =======
*
*  CCTGSJA computes the generalized singular value decomposition (GSVD)
*  of two complex upper triangular (or trapezoidal) matrices A and B.
*
*  On entry, it is assumed that matrices A and B have the following
*  forms, which may be obtained by the preprocessing subroutine CCGGSVP
...
*          of A and B, whose GSVD is going to be computed by CCTGSJA.
*          See Further details.
*
...
*          the unitary matrix returned by CCGGSVP).
*          On exit,
...
*  ===============
*
*  CCTGSJA essentially uses a variant of Kogbetliantz algorithm to reduce
...
*     .. Local Scalars ..
*
      LOGICAL            INITQ, INITU, INITV, UPPER, WANTQ, WANTU, 
     $  WANTV 
      INTEGER            I, J, KCYCLE
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CCLAGS2, CCLAPLL, CCLASET, CCROT, 
     $  CSSCAL, 
     $                   CSLARTG, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      IF( INITU )
     $   CALL CCLASET( 'Full', M, M, CZERO, CONE, U, LDU )
      IF( INITV )
     $   CALL CCLASET( 'Full', P, P, CZERO, CONE, V, LDV )
      IF( INITQ )
...
*
     ...
     $               A2 = A( K+J, N-L+I )
               ...
               END IF
*
               CALL CCLAGS2( UPPER, A1, A2, A3, B1, B2, B3, CSU, SNU,
     $                      CSV, SNV, CSQ, SNQ )
*
*              Update (K+I)-th and (K+J)-th rows of matrix A: U'*A
*
               IF( K+J.LE.M )
     $            CALL CCROT( L, A( K+J, N-L+1 ), LDA, A( K+I, N-L+1 ),
     $                       LDA, CSU, CONJG( SNU ) )
*
*              Update I-th and J-th rows of matrix B: V'*B
*
               CALL CCROT( L, B( J, N-L+1 ), LDB, B( I, N-L+1 ), LDB,
     $                    CSV, CONJG( SNV ) )
*
*              Update (N-L+I)-th and (N-L+J)-th columns of matrices
*              A and B: A*Q and B*Q
*
               CALL CCROT( MIN( K+L, M ), A( 1, N-L+J ), 1,
     $                    A( 1, N-L+I ), 1, CSQ, SNQ )
*
               CALL CCROT( L, B( 1, N-L+J ), 1, B( 1, N-L+I ), 1, CSQ,
     $                    SNQ )
*
               IF( UPPER ) THEN
                  IF( K+I.LE.M )
...
*
     ...
     $            CALL CCROT( M, U( 1, K+J ), 1, U( 1, K+I ), 1, CSU,
     $                       SNU )
*
               IF( WANTV )
     $            CALL CCROT( P, V( 1, J ), 1, V( 1, I ), 1, CSV, SNV )
*
               IF( WANTQ )
     $            CALL CCROT( N, Q( 1, N-L+J ), 1, Q( 1, N-L+I ), 1, 
     $  CSQ, 
     $                       SNQ )
*
   10       CONTINUE
...
*
            ...
            DO 30 I = 1, MIN( L, M-K )
               ...
               CALL CCLAPLL( L-I+1, WORK, 1, WORK( L+1 ), 1, SSMIN )
...
*
     ...
     $            CALL CSSCAL( P, -ONE, V( 1, I ), 1 )
            END IF
*
            CALL CSLARTG( ABS( GAMMA ), ONE, BETA( K+I ), ALPHA( K+I ),
     $                   RWK )
*
