...
*  =======
*
*  CCLAQPS computes a step of QR factorization with column pivoting
...
*     .. External Subroutines ..
      EXTERNAL           CGEMM, CGEMV, CCLARFG, CSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( RK.LT.M ) THEN
            CALL CCLARFG( M-RK+1, A( RK, K ), A( RK+1, K ), 1, TAU( K ) 
     $  ) 
         ELSE
            CALL CCLARFG( 1, A( RK, K ), A( RK, K ), 1, TAU( K ) )
         END IF
*
         AKK = A( RK, K )
         A( RK, K ) = CONE
