...
*  =======
*
*  CDTGSYL solves the generalized Sylvester equation:
*
*              A * R - L * B = scale * C                 (1)
*              D * R - L * E = scale * F
*
...
*
*  If TRANS = 'T', CDTGSYL solves the transposed system Z'*y = scale*b,
...
*  and (B,E), using CDLACON.
*
*  If IJOB >= 1, CDTGSYL computes a Frobenius norm-based estimate
...
*               ( CDGECON on sub-systems is used ).
...
*     .. External Subroutines ..
      EXTERNAL           DGEMM, CDLACPY, CDLASET, DSCAL, CDTGSY2, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
      MB = ILAENV( 2, 'CDTGSYL', TRANS, M, N, -1, -1 )
      NB = ILAENV( 5, 'CDTGSYL', TRANS, M, N, -1, -1 )
*
      ISOLVE = 1
      IFUNC = 0
      IF( NOTRAN ) THEN
         IF( IJOB.GE.3 ) THEN
            IFUNC = IJOB - 2
            CALL CDLASET( 'F', M, N, ZERO, ZERO, C, LDC )
            CALL CDLASET( 'F', M, N, ZERO, ZERO, F, LDF )
...
*
            ...
            DSUM = ONE
            PQ = 0
            CALL CDTGSY2( TRANS, IFUNC, M, N, A, LDA, B, LDB, C, LDC, 
     $  D, 
...
*
            IF( ISOLVE.EQ.2 .AND. IROUND.EQ.1 ) THEN
               ...
               END IF
               SCALE2 = SCALE
               CALL CDLACPY( 'F', M, N, C, LDC, WORK, M )
               CALL CDLACPY( 'F', M, N, F, LDF, WORK( M*N+1 ), M )
               CALL CDLASET( 'F', M, N, ZERO, ZERO, C, LDC )
               CALL CDLASET( 'F', M, N, ZERO, ZERO, F, LDF )
            ELSE IF( ISOLVE.EQ.2 .AND. IROUND.EQ.2 ) THEN
               CALL CDLACPY( 'F', M, N, WORK, M, C, LDC )
               CALL CDLACPY( 'F', M, N, WORK( M*N+1 ), M, F, LDF )
               SCALE = SCALE2
            END IF
   30    CONTINUE
*
...
*
            ...
            DO 130 J = P + 2, Q
               ...
               DO 120 I = P, 1, -1
                  ...
                  MB = IE - IS + 1
                  PPQQ = 0
                  CALL CDTGSY2( TRANS, IFUNC, MB, NB, A( IS, IS ), LDA,
