...
*  =======
*
* CCSTEGR computes selected eigenvalues and, optionally, eigenvectors
...
*
*  Note 1 : Currently CCSTEGR is only set up to find ALL the n
*  eigenvalues and eigenvectors of T in O(n^2) time
*  Note 2 : Currently the routine CCSTEIN is called when an appropriate
*  sigma_i cannot be chosen in step (c) above. CCSTEIN invokes modified
*  Gram-Schmidt when eigenvalues are close.
*  Note 3 : CCSTEGR works only on machines which follow ieee-754
*  floating-point standard in their handling of infinities and NaNs.
*  Normal execution of CCSTEGR may create NaNs and infinities and hence
...
*     .. Local Scalars ..
     ...
     $                   LWMIN, NSPLIT
      REAL               BIGNUM, EPS, RMAX, RMIN, SAFMIN, SCALE, 
     $  SMLNUM, 
     $                   THRESH, TMP, TNRM, TOL
*     ..
*     .. External Functions ..
...
*     .. External Subroutines ..
      EXTERNAL           CCLARRV, CCLASET, CSWAP, CSLARRE, SSCAL, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IINDWK = 2*N + 1
*
      CALL CCLASET( 'Full', N, N, CZERO, CZERO, Z, LDZ )
*
...
*
      THRESH = EPS*TNRM
      CALL CSLARRE( N, D, E, THRESH, NSPLIT, IWORK( IINSPL ), M, W,
...
*
   ...
   20    CONTINUE
*
         CALL CCLARRV( N, D, E, IWORK( IINSPL ), M, W, IWORK( IINDBL ),
