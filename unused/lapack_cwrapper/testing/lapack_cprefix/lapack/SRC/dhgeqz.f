...
*  =======
*
*  CDHGEQZ computes the eigenvalues of a real matrix pair (H,T),
...
*  If Q1 and Z1 are the orthogonal matrices from CDGGHRD that reduced
...
*     .. External Subroutines ..
      EXTERNAL           CDLAG2, CDLARFG, CDLARTG, CDLASET, CDLASV2, 
     $  DROT, 
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
               IF( ILAZRO .OR. ILAZR2 ) THEN
                  DO 40 JCH = J, ILAST - 1
                     TEMP = H( JCH, JCH )
                     CALL CDLARTG( TEMP, H( JCH+1, JCH ), C, S,
     $                            H( JCH, JCH ) )
                     H( JCH+1, JCH ) = ZERO
...
*
                  DO 50 JCH = J, ILAST - 1
                     TEMP = T( JCH, JCH+1 )
                     CALL CDLARTG( TEMP, T( JCH+1, JCH+1 ), C, S,
     $                            T( JCH, JCH+1 ) )
                     T( JCH+1, JCH+1 ) = ZERO
                     IF( JCH.LT.ILASTM-1 )
     ...
     $                             C, S )
                     TEMP = H( JCH+1, JCH )
                     CALL CDLARTG( TEMP, H( JCH+1, JCH-1 ), C, S,
     $                            H( JCH+1, JCH ) )
                     H( JCH+1, JCH-1 ) = ZERO
...
*
   70    CONTINUE
         TEMP = H( ILAST, ILAST )
         CALL CDLARTG( TEMP, H( ILAST, ILAST-1 ), C, S,
     $                H( ILAST, ILAST ) )
         H( ILAST, ILAST-1 ) = ZERO
...
*           returned by CDLAG2 is the Wilkinson shift (AEP p.512),
*
            CALL CDLAG2( H( ILAST-1, ILAST-1 ), LDH,
...
*
         ...
         CALL CDLARTG( TEMP, TEMP2, C, S, TEMPR )
*
*        Sweep
*
         DO 190 J = ISTART, ILAST - 1
            IF( J.GT.ISTART ) THEN
               TEMP = H( J, J-1 )
               CALL CDLARTG( TEMP, H( J+1, J-1 ), C, S, H( J, J-1 ) )
               H( J+1, J-1 ) = ZERO
            END IF
*
            DO 140 JC = J, ILASTM
...
*
            TEMP = T( J+1, J+1 )
            CALL CDLARTG( TEMP, T( J+1, J ), C, S, T( J+1, J+1 ) )
            T( J+1, J ) = ZERO
*
...
*                       (  0  B22 )
*
            CALL CDLASV2( T( ILAST-1, ILAST-1 ), T( ILAST-1, ILAST ),
...
*
               IF( J.GT.ISTART ) THEN
                  ...
                  V( 3 ) = H( J+2, J-1 )
*
                  CALL CDLARFG( 3, H( J, J-1 ), V( 2 ), 1, TAU )
                  V( 1 ) = ONE
                  H( J+1, J-1 ) = ZERO
                  H( J+2, J-1 ) = ZERO
               END IF
