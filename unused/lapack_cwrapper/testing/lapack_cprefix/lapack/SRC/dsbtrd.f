...
*  =======
*
*  CDSBTRD reduces a real symmetric band matrix A to symmetric
...
*     .. External Subroutines ..
      EXTERNAL           CDLAR2V, CDLARGV, CDLARTG, CDLARTV, CDLASET, 
     $  DROT, 
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
                     CALL CDLARGV( NR, AB( 1, J1-1 ), INCA, WORK( J1 ),
     $                            KD1, D( J1 ), KD1 )
*
...
*                    CDLARTV or DROT is used
*
                     IF( NR.GE.2*KD-1 ) THEN
                        DO 10 L = 1, KD - 1
                           CALL CDLARTV( NR, AB( L+1, J1-1 ), INCA,
     $                                  AB( L, J1 ), INCA, D( J1 ),
     $                                  WORK( J1 ), KD1 )
   10                   CONTINUE
*
...
*
                  IF( NR.GT.0 )
     $               CALL CDLAR2V( NR, AB( KD1, J1-1 ), AB( KD1, J1 ),
     $                            AB( KD, J1 ), INCA, D( J1 ),
     $                            WORK( J1 ), KD1 )
*
...
*                    elements which have been created outside the band
*
                     CALL CDLARGV( NR, AB( KD1, J1-KD1 ), INCA,
...
*
                     IF( NR.GT.2*KD-1 ) THEN
                        DO 130 L = 1, KD - 1
                           CALL CDLARTV( NR, AB( KD1-L, J1-KD1+L ), 
     $  INCA, 
...
*
                  IF( NR.GT.0 )
     $               CALL CDLAR2V( NR, AB( 1, J1-1 ), AB( 1, J1 ),
     $                            AB( 2, J1-1 ), INCA, D( J1 ),
     $                            WORK( J1 ), KD1 )
*
                  ...
                  IF( NR.GT.0 ) THEN
                     IF( NR.GT.2*KD-1 ) THEN
                        DO 150 L = 1, KD - 1
                           ...
                           END IF
                           IF( NRT.GT.0 )
     $                        CALL CDLARTV( NRT, AB( L+2, J1-1 ), INCA,
     $                                     AB( L+1, J1 ), INCA, D( J1 
     $  ), 
     $                                     WORK( J1 ), KD1 )
  150                   CONTINUE
