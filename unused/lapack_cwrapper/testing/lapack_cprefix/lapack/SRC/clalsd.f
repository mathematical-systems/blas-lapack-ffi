...
*  =======
*
*  CCLALSD uses the singular value decomposition of A to solve the least
...
*     .. Local Scalars ..
     ...
     $                   JREAL, JROW, K, NLVL, NM1, NRWORK, NSIZE, 
     $  NSUB, 
...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, CCLACPY, CCLALSA, CCLASCL, CCLASET, 
     $  CCSROT, 
     $                   SGEMM, CSLARTG, CSLASCL, CSLASDA, CSLASDQ, 
     $  CSLASET, 
     $                   CSLASRT, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      ELSE IF( N.EQ.1 ) THEN
         IF( D( 1 ).EQ.ZERO ) THEN
            CALL CCLASET( 'A', 1, NRHS, CZERO, CZERO, B, LDB )
         ELSE
            RANK = 1
            CALL CCLASCL( 'G', 0, 0, D( 1 ), ONE, 1, NRHS, B, LDB, INFO 
     $  ) 
            D( 1 ) = ABS( D( 1 ) )
         END IF
         RETURN
...
*
      IF( UPLO.EQ.'L' ) THEN
         DO 10 I = 1, N - 1
            CALL CSLARTG( D( I ), E( I ), CS, SN, R )
            D( I ) = R
            E( I ) = SN*D( I+1 )
            D( I+1 ) = CS*D( I+1 )
            IF( NRHS.EQ.1 ) THEN
               CALL CCSROT( 1, B( I, 1 ), 1, B( I+1, 1 ), 1, CS, SN )
            ELSE
               RWORK( I*2-1 ) = CS
               RWORK( I*2 ) = SN
            END IF
         ...
         IF( NRHS.GT.1 ) THEN
            DO 30 I = 1, NRHS
               DO 20 J = 1, N - 1
                  CS = RWORK( J*2-1 )
                  SN = RWORK( J*2 )
                  CALL CCSROT( 1, B( J, I ), 1, B( J+1, I ), 1, CS, SN 
     $  ) 
   20          CONTINUE
   30       CONTINUE
         END IF
...
*
      ...
      IF( ORGNRM.EQ.ZERO ) THEN
         CALL CCLASET( 'A', N, NRHS, CZERO, CZERO, B, LDB )
         RETURN
      END IF
*
      CALL CSLASCL( 'G', 0, 0, ORGNRM, ONE, N, 1, D, N, INFO )
      CALL CSLASCL( 'G', 0, 0, ORGNRM, ONE, NM1, 1, E, NM1, INFO )
*
...
*
      IF( N.LE.SMLSIZ ) THEN
         ...
         IRWIB = IRWRB + N*NRHS
         IRWB = IRWIB + N*NRHS
         CALL CSLASET( 'A', N, N, ZERO, ONE, RWORK( IRWU ), N )
         CALL CSLASET( 'A', N, N, ZERO, ONE, RWORK( IRWVT ), N )
         CALL CSLASDQ( 'U', 0, N, N, N, 0, D, E, RWORK( IRWVT ), N,
     ...
     $                RWORK( IRWWRK ), INFO )
         ...
         END IF
*
*        In the real version, B is passed to CSLASDQ and multiplied
...
*
         ...
         DO 100 I = 1, N
            IF( D( I ).LE.TOL ) THEN
               CALL CCLASET( 'A', 1, NRHS, CZERO, CZERO, B( I, 1 ), LDB 
     $  ) 
            ELSE
               CALL CCLASCL( 'G', 0, 0, D( I ), ONE, 1, NRHS, B( I, 1 
     $  ), 
     $                      LDB, INFO )
               RANK = RANK + 1
            END IF
...
*        Unscale.
*
         CALL CSLASCL( 'G', 0, 0, ONE, ORGNRM, N, 1, D, N, INFO )
         CALL CSLASRT( 'D', N, D, INFO )
         CALL CCLASCL( 'G', 0, 0, ORGNRM, ONE, N, NRHS, B, LDB, INFO )
*
         RETURN
      END IF
*
...
*
      NLVL = INT( LOG( REAL( N ) / REAL( SMLSIZ+1 ) ) / LOG( TWO ) ) + 
     $  1 
*
      SMLSZP = SMLSIZ + 1
*
...
*
*              This is a small subproblem and is solved by CSLASDQ.
*
               CALL CSLASET( 'A', NSIZE, NSIZE, ZERO, ONE,
     $                      RWORK( VT+ST1 ), N )
               CALL CSLASET( 'A', NSIZE, NSIZE, ZERO, ONE,
     $                      RWORK( U+ST1 ), N )
               CALL CSLASDQ( 'U', 0, NSIZE, NSIZE, NSIZE, 0, D( ST ),
...
*
  ...
  230          CONTINUE
*
               CALL CCLACPY( 'A', NSIZE, NRHS, B( ST, 1 ), LDB,
     $                      WORK( BX+ST1 ), N )
            ELSE
*
*              A large problem. Solve it using divide and conquer.
*
               CALL CSLASDA( ICMPQ1, SMLSIZ, NSIZE, SQRE, D( ST ),
...
*
         IF( ABS( D( I ) ).LE.TOL ) THEN
            CALL CCLASET( 'A', 1, NRHS, CZERO, CZERO, WORK( BX+I-1 ), N 
     $  ) 
         ELSE
            RANK = RANK + 1
            CALL CCLASCL( 'G', 0, 0, D( I ), ONE, 1, NRHS,
     $                   WORK( BX+I-1 ), N, INFO )
         END IF
         D( I ) = ABS( D( I ) )
  250 CONTINUE
...
*
  ...
  310       CONTINUE
         ELSE
            CALL CCLALSA( ICMPQ2, SMLSIZ, NSIZE, NRHS, WORK( BXST ), N,
