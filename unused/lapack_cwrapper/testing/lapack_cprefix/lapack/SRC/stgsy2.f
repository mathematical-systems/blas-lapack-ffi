...
*  =======
*
*  CSTGSY2 solves the generalized Sylvester equation:
*
*              A * R - L * B = scale * C                (1)
*              D * R - L * E = scale * F,
*
...
*  sigma_min(Z) using reverse communicaton with CSLACON.
*
*  CSTGSY2 also (IJOB >= 1) contributes to the computation in CSTGSYL
*  of an upper bound on the separation between to matrix pairs. Then
*  the input (A, D), (B, E) are sub-pencils of the matrix pair in
*  CSTGSYL. See CSTGSYL for details.
*
*  Arguments
*  =========
*
...
*               norm-based estimate of the separation between two matrix
*               pairs is computed. (CSGECON on sub-systems is used.)
...
*          the Dif-estimate under computation by CSTGSYL, where the
*          scaling factor RDSCAL (see below) has been factored out.
*          On exit, the corresponding sum of squares updated with the
*          contributions from the current sub-system.
*          If TRANS = 'T' RDSUM is not touched.
*          NOTE: RDSUM only makes sense when CSTGSY2 is called by CSTGSYL.
*
...
*          NOTE: RDSCAL only makes sense when CSTGSY2 is called by
*                CSTGSYL.
*
...
*
*  =====================================================================
*  Replaced various illegal calls to SCOPY by calls to CSLASET.
...
*     .. External Subroutines ..
     ...
     $                   CSGETC2, SSCAL, CSLASET, CSLATDF, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
*     ..
...
*                 Solve Z * x = RHS
*
                  CALL CSGETC2( ZDIM, Z, LDZ, IPIV, JPIV, IERR )
                  IF( IERR.GT.0 )
     $               INFO = IERR
*
                  IF( IJOB.EQ.0 ) THEN
   ...
   50                   CONTINUE
                     ...
                     END IF
                  ELSE
                     CALL CSLATDF( IJOB, ZDIM, Z, LDZ, RHS, RDSUM,
     $                            RDSCAL, IPIV, JPIV )
                  END IF
*
...
*
                  CALL CSLASET( 'F', LDZ, LDZ, ZERO, ZERO, Z, LDZ )
*
                  Z( 1, 1 ) = A( IS, IS )
                  Z( 2, 1 ) = A( ISP1, IS )
                  Z( 5, 1 ) = D( IS, IS )
