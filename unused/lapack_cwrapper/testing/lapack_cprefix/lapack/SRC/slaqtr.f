...
*  =======
*
*  CSLAQTR solves the real quasi-triangular system
*
...
*                  a small number in CSLALN2 to keep nonsingularity.
...
*     .. External Subroutines ..
      EXTERNAL           SAXPY, CSLADIV, CSLALN2, SSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
*     ..
...
*
                  D( 1, 1 ) = X( J1 )
                  D( 2, 1 ) = X( J2 )
                  CALL CSLALN2( .FALSE., 2, 1, SMIN, ONE, T( J1, J1 ),
...
*
     ...
     $                        1 )
*
                  CALL CSLALN2( .TRUE., 2, 1, SMIN, ONE, T( J1, J1 ),
     $                         LDT, ONE, ONE, D, 2, ZERO, ZERO, V, 2,
     $                         SCALOC, XNORM, IERR )
                  IF( IERR.NE.0 )
     $               INFO = 2
...
*
                  IF( TJJ.LT.ONE ) THEN
                     ...
                     END IF
                  END IF
                  CALL CSLADIV( X( J1 ), X( N+J1 ), TMP, Z, SR, SI )
                  X( J1 ) = SR
                  X( N+J1 ) = SI
                  XJ = ABS( X( J1 ) ) + ABS( X( N+J1 ) )
*
...
*
                  ...
                  D( 1, 2 ) = X( N+J1 )
                  D( 2, 2 ) = X( N+J2 )
                  CALL CSLALN2( .FALSE., 2, 2, SMINW, ONE, T( J1, J1 ),
...
*
                  CALL CSLALN2( .TRUE., 2, 2, SMINW, ONE, T( J1, J1 ),
     $                         LDT, ONE, ONE, D, 2, ZERO, W, V, 2,
     $                         SCALOC, XNORM, IERR )
                  IF( IERR.NE.0 )
     $               INFO = 2
