...
*  =======
*
*  CSTGSYL solves the generalized Sylvester equation:
*
*              A * R - L * B = scale * C                 (1)
*              D * R - L * E = scale * F
*
...
*
*  If TRANS = 'T', CSTGSYL solves the transposed system Z'*y = scale*b,
...
*  and (B,E), using CSLACON.
*
*  If IJOB >= 1, CSTGSYL computes a Frobenius norm-based estimate
...
*               ( CSGECON on sub-systems is used ).
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, CSLACPY, CSLASET, SSCAL, CSTGSY2, 
     $  XERBLA 
*     ..
*     .. Intrinsic Functions ..
...
*
      MB = ILAENV( 2, 'CSTGSYL', TRANS, M, N, -1, -1 )
      NB = ILAENV( 5, 'CSTGSYL', TRANS, M, N, -1, -1 )
*
      ISOLVE = 1
      IFUNC = 0
      IF( NOTRAN ) THEN
         IF( IJOB.GE.3 ) THEN
            IFUNC = IJOB - 2
            CALL CSLASET( 'F', M, N, ZERO, ZERO, C, LDC )
            CALL CSLASET( 'F', M, N, ZERO, ZERO, F, LDF )
...
*
            ...
            DSUM = ONE
            PQ = 0
            CALL CSTGSY2( TRANS, IFUNC, M, N, A, LDA, B, LDB, C, LDC, 
     $  D, 
...
*
            IF( ISOLVE.EQ.2 .AND. IROUND.EQ.1 ) THEN
               ...
               END IF
               SCALE2 = SCALE
               CALL CSLACPY( 'F', M, N, C, LDC, WORK, M )
               CALL CSLACPY( 'F', M, N, F, LDF, WORK( M*N+1 ), M )
               CALL CSLASET( 'F', M, N, ZERO, ZERO, C, LDC )
               CALL CSLASET( 'F', M, N, ZERO, ZERO, F, LDF )
            ELSE IF( ISOLVE.EQ.2 .AND. IROUND.EQ.2 ) THEN
               CALL CSLACPY( 'F', M, N, WORK, M, C, LDC )
               CALL CSLACPY( 'F', M, N, WORK( M*N+1 ), M, F, LDF )
               SCALE = SCALE2
            END IF
   30    CONTINUE
*
...
*
            ...
            DO 130 J = P + 2, Q
               ...
               DO 120 I = P, 1, -1
                  ...
                  MB = IE - IS + 1
                  PPQQ = 0
                  CALL CSTGSY2( TRANS, IFUNC, MB, NB, A( IS, IS ), LDA,
