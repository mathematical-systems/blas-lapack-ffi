...
*  =======
*
*  CCHEGS2 reduces a complex Hermitian-definite generalized
...
*
*  B must have been previously factorized as U'*U or L*L' by CCPOTRF.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           CAXPY, CHER2, CCLACGV, CSSCAL, CTRMV, CTRSV,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX
...
*
               ...
               IF( K.LT.N ) THEN
                  ...
                  CT = -HALF*AKK
                  CALL CCLACGV( N-K, A( K, K+1 ), LDA )
                  CALL CCLACGV( N-K, B( K, K+1 ), LDB )
...
*
               AKK = A( K, K )
               BKK = B( K, K )
               CALL CCLACGV( K-1, A( K, 1 ), LDA )
               CALL CTRMV( UPLO, 'Conjugate transpose', 'Non-unit', 
     $  K-1, 
     $                     B, LDB, A( K, 1 ), LDA )
               CT = HALF*AKK
               CALL CCLACGV( K-1, B( K, 1 ), LDB )
