...
*  =======
*
*  CSLAGV2 computes the Generalized Schur factorization of a real 2-by-2
...
*     .. External Subroutines ..
      EXTERNAL           CSLAG2, CSLARTG, CSLASV2, SROT
*     ..
*     .. External Functions ..
      REAL               SLAMCH, SLAPY2
...
*
      ELSE IF( ABS( B( 1, 1 ) ).LE.ULP ) THEN
         CALL CSLARTG( A( 1, 1 ), A( 2, 1 ), CSL, SNL, R )
         CSR = ONE
         SNR = ZERO
...
*
      ELSE IF( ABS( B( 2, 2 ) ).LE.ULP ) THEN
         CALL CSLARTG( A( 2, 2 ), A( 2, 1 ), CSR, SNR, T )
         SNR = -SNR
...
*
         CALL CSLAG2( A, LDA, B, LDB, SAFMIN, SCALE1, SCALE2, WR1, WR2,
     $               WI )
*
         IF( WI.EQ.ZERO ) THEN
*
...
*              (sA - wB)
*
               CALL CSLARTG( H3, SCALE1*A( 2, 1 ), CSR, SNR, T )
*
            END IF
*
            SNR = -SNR
...
*
               CALL CSLARTG( B( 1, 1 ), B( 2, 1 ), CSL, SNL, R )
*
            ELSE
*
...
*
            CALL CSLASV2( B( 1, 1 ), B( 1, 2 ), B( 2, 2 ), R, T, SNR,
     $                   CSR, SNL, CSL )
*
*           Form (A,B) := Q(A,B)Z' where Q is left rotation matrix and
*           Z is right rotation matrix computed from CSLASV2
*
            CALL SROT( 2, A( 1, 1 ), LDA, A( 2, 1 ), LDA, CSL, SNL )
            CALL SROT( 2, B( 1, 1 ), LDB, B( 2, 1 ), LDB, CSL, SNL )
            CALL SROT( 2, A( 1, 1 ), 1, A( 1, 2 ), 1, CSR, SNR )
