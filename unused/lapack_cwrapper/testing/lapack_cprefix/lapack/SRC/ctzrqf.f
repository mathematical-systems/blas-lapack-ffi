...
*  =======
*
*  This routine is deprecated and has been replaced by routine CCTZRZF.
*
*  CCTZRQF reduces the M-by-N ( M<=N ) complex upper trapezoidal matrix A
...
*     .. External Subroutines ..
      EXTERNAL           CAXPY, CCOPY, CGEMV, CGERC, CCLACGV, CCLARFG,
     $                   XERBLA
*     ..
...
*
            A( K, K ) = CONJG( A( K, K ) )
            CALL CCLACGV( N-M, A( K, M1 ), LDA )
            ALPHA = A( K, K )
            CALL CCLARFG( N-M+1, ALPHA, A( K, M1 ), LDA, TAU( K ) )
            A( K, K ) = ALPHA
            TAU( K ) = CONJG( TAU( K ) )
*
