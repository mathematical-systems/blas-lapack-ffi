...
*  =======
*
*  ZGET38 tests cztrsen, a routine for estimating condition numbers of a
...
*          If czgehrd returns INFO nonzero on example i, LMAX(1)=i
*          If czhseqr returns INFO nonzero on example i, LMAX(2)=i
*          If cztrsen returns INFO nonzero on example i, LMAX(3)=i
*
*  NINFO   (output) INTEGER array, dimension (3)
*          NINFO(1) = No. of times czgehrd returned INFO nonzero
*          NINFO(2) = No. of times czhseqr returned INFO nonzero
*          NINFO(3) = No. of times cztrsen returned INFO nonzero
*
*  KNT     (output) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL       cdlabad, ZDSCAL, czgehrd, czhseqr, ZHST01, czlacpy,
     $                   cztrsen, czunghr
*     ..
*     .. Intrinsic Functions ..
...
*
         KNT = KNT + 1
         CALL czlacpy( 'F', N, N, TMP, LDT, T, LDT )
         VMUL = VAL( ISCL )
         DO 30 I = 1, N
   ...
   30    CONTINUE
         IF( TNRM.EQ.ZERO )
     $      VMUL = ONE
         CALL czlacpy( 'F', N, N, T, LDT, TSAV, LDT )
*
*        Compute Schur form
*
         CALL czgehrd( N, 1, N, T, LDT, WORK( 1 ), WORK( N+1 ), LWORK-N,
     $                INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 1 ) = KNT
            NINFO( 1 ) = NINFO( 1 ) + 1
...
*
         CALL czlacpy( 'L', N, N, T, LDT, Q, LDT )
         CALL czunghr( N, 1, N, Q, LDT, WORK( 1 ), WORK( N+1 ), LWORK-N,
     $                INFO )
*
*        Compute Schur form
*
   ...
   40       CONTINUE
   50    CONTINUE
        CALL czhseqr( 'S', 'V', N, 1, N, T, LDT, W, Q, LDT, WORK, LWORK,
     $                INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 2 ) = KNT
            NINFO( 2 ) = NINFO( 2 ) + 1
...
*
         CALL czlacpy( 'F', N, N, Q, LDT, QSAV, LDT )
         CALL czlacpy( 'F', N, N, T, LDT, TSAV1, LDT )
         CALL cztrsen( 'B', 'V', SELECT, N, T, LDT, Q, LDT, WTMP, M, S,
     $                SEP, WORK, LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
         VMAX = ZERO
         CALL czlacpy( 'F', N, N, TSAV1, LDT, TTMP, LDT )
         CALL czlacpy( 'F', N, N, QSAV, LDT, QTMP, LDT )
         SEPTMP = -ONE
         STMP = -ONE
         CALL cztrsen( 'E', 'V', SELECT, N, TTMP, LDT, QTMP, LDT, WTMP,
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cztrsen( 'V', 'V', SELECT, N, TTMP, LDT, QTMP, LDT, WTMP,
     $                M, STMP, SEPTMP, WORK, LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cztrsen( 'E', 'N', SELECT, N, TTMP, LDT, QTMP, LDT, WTMP,
     $                M, STMP, SEPTMP, WORK, LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
         ...
         SEPTMP = -ONE
         STMP = -ONE
         CALL cztrsen( 'V', 'N', SELECT, N, TTMP, LDT, QTMP, LDT, WTMP,
     $                M, STMP, SEPTMP, WORK, LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
