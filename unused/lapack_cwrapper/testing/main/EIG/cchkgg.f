...
*  routines.
*                                 H          H        H
*  ccgghrd factors A and B as U H V  and U T V , where   means conjugate
*  transpose, H is hessenberg, T is triangular and U and V are unitary.
*
*                                  H          H
*  cchgeqz factors H and T as  Q S Z  and Q P Z , where P and S are upper
...
*      det( m(j) A - B ) = 0
*
*  cctgevc computes the matrix L of left eigenvectors and the matrix R
...
*        cstgevc and back transforming (JOB='B').
*
...
*          The upper Hessenberg matrix computed from A by ccgghrd.
*
*  T       (workspace) COMPLEX array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by ccgghrd.
*
*  S1      (workspace) COMPLEX array, dimension (LDA, max(NN))
*          The Schur (upper triangular) matrix computed from H by cchgeqz
...
*          The upper triangular matrix computed from T by cchgeqz
*          when Q and Z are also computed.
*
...
*          The (left) unitary matrix computed by ccgghrd.
*
*  LDU     (input) INTEGER
...
*          The (right) unitary matrix computed by ccgghrd.
*
*  Q       (workspace) COMPLEX array, dimension (LDU, max(NN))
*          The (left) unitary matrix computed by cchgeqz.
*
...
*          The generalized eigenvalues of (A,B) computed by cchgeqz
...
*     .. External Subroutines ..
      EXTERNAL       ccgeqr2, CGET51, CGET52, ccgghrd, cchgeqz, cclacpy,
     $              cclarfG, cclaset, CLATM4, cctgevc, ccunm2r, cslabad,
     $                   SLASUM, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*                 diagonal matrix.  (Note that cclarfG makes U(j,j) and
*                 V(j,j) real.)
*
                  DO 50 JC = 1, N - 1
                     DO 40 JR = JC, N
   ...
   40                CONTINUE
                    CALL cclarfG( N+1-JC, U( JC, JC ), U( JC+1, JC ), 1,
     $                            WORK( JC ) )
                     WORK( 2*N+JC ) = SIGN( ONE, REAL( U( JC, JC ) ) )
                     U( JC, JC ) = CONE
                    CALL cclarfG( N+1-JC, V( JC, JC ), V( JC+1, JC ), 1,
     $                            WORK( N+JC ) )
...
*
   ...
   60                CONTINUE
   70             CONTINUE
                  CALL ccunm2r( 'L', 'N', N, N, N-1, U, LDU, WORK, A,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL ccunm2r( 'R', 'C', N, N, N-1, V, LDU, WORK( N+1 ),
     $                         A, LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                  CALL ccunm2r( 'L', 'N', N, N, N-1, U, LDU, WORK, B,
     $                         LDA, WORK( 2*N+1 ), IINFO )
                  IF( IINFO.NE.0 )
     $               GO TO 100
                 CALL ccunm2r( 'R', 'C', N, N, N-1, V, LDU, WORK( N+1 ),
...
*
  110       CONTINUE
*
*           Call ccgeqr2, ccunm2r, and ccgghrd to compute H, T, U, and V
*
            CALL cclacpy( ' ', N, N, A, LDA, H, LDA )
            CALL cclacpy( ' ', N, N, B, LDA, T, LDA )
            NTEST = 1
            RESULT( 1 ) = ULPINV
*
            CALL ccgeqr2( N, N, T, LDA, WORK, WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'ccgeqr2', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
            CALL ccunm2r( 'L', 'C', N, N, N, T, LDA, WORK, H, LDA,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'ccunm2r', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
            CALL cclaset( 'Full', N, N, CZERO, CONE, U, LDU )
            CALL ccunm2r( 'R', 'N', N, N, N, T, LDA, WORK, U, LDU,
     $                   WORK( N+1 ), IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'ccunm2r', IINFO, N, JTYPE,
     $            IOLDSD
            ...
            END IF
*
            CALL ccgghrd( 'V', 'I', N, 1, N, H, LDA, T, LDA, U, LDU, V,
     $                   LDU, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'ccgghrd', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
     ...
     $                   RWORK, RESULT( 4 ) )
*
*           Call cchgeqz to compute S1, P1, S2, P2, Q, and Z, do tests.
*
*           Compute T1 and UZ
*
*           Eigenvalues only
*
            CALL cclacpy( ' ', N, N, H, LDA, S2, LDA )
            CALL cclacpy( ' ', N, N, T, LDA, P2, LDA )
            NTEST = 5
            RESULT( 5 ) = ULPINV
*
            CALL cchgeqz( 'E', 'N', 'N', N, 1, N, S2, LDA, P2, LDA,
     $                   ALPHA3, BETA3, Q, LDU, Z, LDU, WORK, LWORK,
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchgeqz(E)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
            ...
            CALL cclacpy( ' ', N, N, T, LDA, P2, LDA )
*
            CALL cchgeqz( 'S', 'N', 'N', N, 1, N, S2, LDA, P2, LDA,
     $                   ALPHA1, BETA1, Q, LDU, Z, LDU, WORK, LWORK,
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchgeqz(S)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
            CALL cclacpy( ' ', N, N, H, LDA, S1, LDA )
            CALL cclacpy( ' ', N, N, T, LDA, P1, LDA )
*
            CALL cchgeqz( 'S', 'I', 'I', N, 1, N, S1, LDA, P1, LDA,
     $                   ALPHA1, BETA1, Q, LDU, Z, LDU, WORK, LWORK,
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cchgeqz(V)', IINFO, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
  ...
  130       CONTINUE
*
            CALL cctgevc( 'L', 'S', LLWORK, N, S1, LDA, P1, LDA, EVECTL,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctgevc(L,S1)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
  ...
  150       CONTINUE
*
            CALL cctgevc( 'L', 'S', LLWORK, N, S1, LDA, P1, LDA,
     $                   EVECTL( 1, I1+1 ), LDU, CDUMMA, LDU, N, IN,
     $                   WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctgevc(L,S2)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   ALPHA1, BETA1, WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRSHN ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cctgevc(HOWMNY=S)',
...
*
            NTEST = 10
            RESULT( 10 ) = ULPINV
            CALL cclacpy( 'F', N, N, Q, LDU, EVECTL, LDU )
            CALL cctgevc( 'L', 'B', LLWORK, N, S1, LDA, P1, LDA, EVECTL,
     $                   LDU, CDUMMA, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctgevc(L,B)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   BETA1, WORK, RWORK, DUMMA( 1 ) )
            RESULT( 10 ) = DUMMA( 1 )
            IF( DUMMA( 2 ).GT.THRSHN ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'cctgevc(HOWMNY=B)',
     $            DUMMA( 2 ), N, JTYPE, IOLDSD
            END IF
*
...
*
  ...
  170       CONTINUE
*
            CALL cctgevc( 'R', 'S', LLWORK, N, S1, LDA, P1, LDA, CDUMMA,
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctgevc(R,S1)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
...
*
     ...
     $                   RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctgevc(R,S2)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   ALPHA1, BETA1, WORK, RWORK, DUMMA( 1 ) )
            ...
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cctgevc(HOWMNY=S)',
     $            DUMMA( 2 ), N, JTYPE, IOLDSD
            END IF
*
...
*
            NTEST = 12
            RESULT( 12 ) = ULPINV
            CALL cclacpy( 'F', N, N, Z, LDU, EVECTR, LDU )
            CALL cctgevc( 'R', 'B', LLWORK, N, S1, LDA, P1, LDA, CDUMMA,
     $                   LDU, EVECTR, LDU, N, IN, WORK, RWORK, IINFO )
            IF( IINFO.NE.0 ) THEN
               WRITE( NOUNIT, FMT = 9999 )'cctgevc(R,B)', IINFO, N,
     $            JTYPE, IOLDSD
               INFO = ABS( IINFO )
               GO TO 210
            END IF
*
     ...
     $                   ALPHA1, BETA1, WORK, RWORK, DUMMA( 1 ) )
            RESULT( 12 ) = DUMMA( 1 )
            IF( DUMMA( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'cctgevc(HOWMNY=B)',
     $            DUMMA( 2 ), N, JTYPE, IOLDSD
            END IF
*
