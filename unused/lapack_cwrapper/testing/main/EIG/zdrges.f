...
*          The Schur form matrix computed from A by czgges.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) COMPLEX*16 array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by czgges.
*
*  Q       (workspace) COMPLEX*16 array, dimension (LDQ, max(NN))
*          The (left) orthogonal matrix computed by czgges.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by czgges.
*
*  ALPHA   (workspace) COMPLEX*16 array, dimension (max(NN))
*  BETA    (workspace) COMPLEX*16 array, dimension (max(NN))
*          The generalized eigenvalues of (A,B) computed by czgges.
...
*     .. External Subroutines ..
      EXTERNAL         ALASVM, cdlabad, xerbla, ZGET51, ZGET54, czgges,
     $                   czlacpy, czlarfG, czlaset, ZLATM4, czunm2r
*     ..
*     .. Intrinsic Functions ..
...
*
            DO 150 ISORT = 0, 1
               ...
               END IF
*
*              Call czgges to compute H, T, Q, Z, alpha, and beta.
*
               CALL czlacpy( 'Full', N, N, A, LDA, S, LDA )
               CALL czlacpy( 'Full', N, N, B, LDA, T, LDA )
               NTEST = 1 + RSUB + ISORT
               RESULT( 1+RSUB+ISORT ) = ULPINV
               CALL czgges( 'V', 'V', SORT, ZLCTES, N, S, LDA, T, LDA,
     $                     SDIM, ALPHA, BETA, Q, LDQ, Z, LDQ, WORK,
     $                     LWORK, RWORK, BWORK, IINFO )
               IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                  RESULT( 1+RSUB+ISORT ) = ULPINV
                  WRITE( NOUNIT, FMT = 9999 )'czgges', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 160
               END IF
