...
*  routine ccggev.
*
*  ccggev computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*          The Schur form matrix computed from A by ccggev.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) COMPLEX array, dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by ccggev.
*
*  Q      (workspace) COMPLEX array, dimension (LDQ, max(NN))
*          The (left) eigenvectors matrix computed by ccggev.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by ccggev.
*
...
*          The generalized eigenvalues of (A,B) computed by ccggev.
...
*          eigenvalues of A and B, but those computed when ccggev only
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, CGET52, ccggev, cclacpy, cclarfG, cclaset,
     $                   CLATM4, ccunm2r, cslabad, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  ...
  110       CONTINUE
*
*           Call ccggev to compute eigenvalues and eigenvectors.
*
            CALL cclacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cclacpy( ' ', N, N, B, LDA, T, LDA )
            CALL ccggev( 'V', 'V', N, S, LDA, T, LDA, ALPHA, BETA, Q,
     $                  LDQ, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'ccggev1', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
     ...
     $                   WORK, RWORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'ccggev1',
...
*
     ...
     $                   BETA, WORK, RWORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'ccggev1',
...
*
            CALL cclacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cclacpy( ' ', N, N, B, LDA, T, LDA )
            CALL ccggev( 'N', 'N', N, S, LDA, T, LDA, ALPHA1, BETA1, Q,
     $                  LDQ, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'ccggev2', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL cclacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cclacpy( ' ', N, N, B, LDA, T, LDA )
            CALL ccggev( 'V', 'N', N, S, LDA, T, LDA, ALPHA1, BETA1, QE,
     $                  LDQE, Z, LDQ, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'ccggev3', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL cclacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cclacpy( ' ', N, N, B, LDA, T, LDA )
            CALL ccggev( 'N', 'V', N, S, LDA, T, LDA, ALPHA1, BETA1, Q,
     $                  LDQ, QE, LDQE, WORK, LWORK, RWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'ccggev4', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
