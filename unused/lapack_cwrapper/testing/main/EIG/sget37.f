...
*  =======
*
*  SGET37 tests cstrsna, a routine for estimating condition numbers of
...
*          RMAX(1) = largest ratio comparing different calls to cstrsna
...
*          If cstrsna returns INFO nonzero on example i, LMAX(3)=i
*
...
*          NINFO(3) = No. of times cstrsna returned INFO nonzero
*
*  KNT     (output) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL         SCOPY, csgehrd, cshseqr, cslabad, cslacpy, SSCAL,
     $                   cstrevc, cstrsna
*     ..
*     .. Intrinsic Functions ..
...
*        Compute Schur form
*
         CALL cshseqr( 'S', 'N', N, 1, N, T, LDT, WR, WI, DUM, 1, WORK,
     $                LWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 2 ) = KNT
            NINFO( 2 ) = NINFO( 2 ) + 1
...
*
         CALL cstrevc( 'Both', 'All', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, N, M, WORK, INFO )
*
*        Compute condition numbers
*
         CALL cstrsna( 'Both', 'All', SELECT, N, T, LDT, LE, LDT, RE,
...
*
         ...
         CALL cstrsna( 'Eigcond', 'All', SELECT, N, T, LDT, LE, LDT, RE,
...
*
         CALL SCOPY( N, DUM, 0, STMP, 1 )
         CALL SCOPY( N, DUM, 0, SEPTMP, 1 )
         CALL cstrsna( 'Veccond', 'All', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, STMP, SEPTMP, N, M, WORK, N, IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
  ...
  150    CONTINUE
         CALL SCOPY( N, DUM, 0, STMP, 1 )
         CALL SCOPY( N, DUM, 0, SEPTMP, 1 )
         CALL cstrsna( 'Bothcond', 'Some', SELECT, N, T, LDT, LE, LDT,
...
*
         CALL SCOPY( N, DUM, 0, STMP, 1 )
         CALL SCOPY( N, DUM, 0, SEPTMP, 1 )
         CALL cstrsna('Eigcond', 'Some', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, STMP, SEPTMP, N, M, WORK, N, IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
...
*
         CALL SCOPY( N, DUM, 0, STMP, 1 )
         CALL SCOPY( N, DUM, 0, SEPTMP, 1 )
         CALL cstrsna('Veccond', 'Some', SELECT, N, T, LDT, LE, LDT, RE,
     $                LDT, STMP, SEPTMP, N, M, WORK, N, IWORK, INFO )
         IF( INFO.NE.0 ) THEN
            LMAX( 3 ) = KNT
            NINFO( 3 ) = NINFO( 3 ) + 1
