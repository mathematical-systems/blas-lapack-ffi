...
*     .. External Subroutines ..
      EXTERNAL       DGEMM, cdlacpy, cdlarfY, cdlaset, cdorm2l, cdorm2r,
     $                   DSYR, DSYR2
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL cdlaset( 'Full', N, N, ZERO, ZERO, WORK, N )
         CALL cdlacpy( CUPLO, N, N, A, LDA, WORK, N )
*
         DO 10 J = 1, N
...
*
               VSAVE = V( J+1, J )
               V( J+1, J ) = ONE
               CALL cdlarfY( 'L', N-J, V( J+1, J ), 1, TAU( J ),
...
*
               VSAVE = V( J, J+1 )
               V( J, J+1 ) = ONE
               CALL cdlarfY( 'U', J, V( 1, J+1 ), 1, TAU( J ), WORK, N,
     $                      WORK( N**2+1 ) )
               V( J, J+1 ) = VSAVE
               WORK( ( N+1 )*J+1 ) = D( J+1 )
   60       CONTINUE
...
*
         IF( N.LT.2 )
     $      RETURN
         CALL cdlacpy( ' ', N, N, U, LDU, WORK, N )
         IF( LOWER ) THEN
            CALL cdorm2r( 'R', 'T', N, N-1, N-1, V( 2, 1 ), LDV, TAU,
     $                   WORK( N+1 ), N, WORK( N**2+1 ), IINFO )
         ELSE
            CALL cdorm2l( 'R', 'T', N, N-1, N-1, V( 1, 2 ), LDV, TAU,
