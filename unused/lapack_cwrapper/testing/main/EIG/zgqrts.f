...
*  =======
*
*  ZGQRTS tests czggqrf, which computes the GQR factorization of an
...
*          by czggqrf, see ccggqrf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL         ZGEMM, czggqrf, ZHERK, czlacpy, czlaset, czungqr,
     $                   czungrq
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL czlacpy( 'Full', N, M, A, LDA, AF, LDA )
      CALL czlacpy( 'Full', N, P, B, LDB, BF, LDB )
*
...
*
      CALL czggqrf( N, M, P, AF, LDA, TAUA, BF, LDB, TAUB, WORK, LWORK,
     $             INFO )
*
*     Generate the N-by-N matrix Q
*
      CALL czlaset( 'Full', N, N, CROGUE, CROGUE, Q, LDA )
      CALL czlacpy( 'Lower', N-1, M, AF( 2, 1 ), LDA, Q( 2, 1 ), LDA )
      CALL czungqr( N, N, MIN( N, M ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL czlaset( 'Full', P, P, CROGUE, CROGUE, Z, LDB )
      IF( N.LE.P ) THEN
         IF( N.GT.0 .AND. N.LT.P )
     $      CALL czlacpy( 'Full', N, P-N, BF, LDB, Z( P-N+1, 1 ), LDB )
         IF( N.GT.1 )
     $      CALL czlacpy( 'Lower', N-1, N-1, BF( 2, P-N+1 ), LDB,
     $                   Z( P-N+2, P-N+1 ), LDB )
      ELSE
         IF( P.GT.1 )
     $      CALL czlacpy( 'Lower', P-1, P-1, BF( N-P+2, 1 ), LDB,
     $                   Z( 2, 1 ), LDB )
      END IF
      CALL czungrq( P, P, MIN( N, P ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL czlaset( 'Full', N, M, CZERO, CZERO, R, LDA )
      CALL czlacpy( 'Upper', N, M, AF, LDA, R, LDA )
*
*     Copy T
*
      CALL czlaset( 'Full', N, P, CZERO, CZERO, T, LDB )
      IF( N.LE.P ) THEN
        CALL czlacpy( 'Upper', N, N, BF( 1, P-N+1 ), LDB, T( 1, P-N+1 ),
     $                LDB )
      ELSE
         CALL czlacpy( 'Full', N-P, P, BF, LDB, T, LDB )
        CALL czlacpy( 'Upper', P, P, BF( N-P+1, 1 ), LDB, T( N-P+1, 1 ),
     $                LDB )
      END IF
*
*     Compute R - Q'*A
...
*     Compute I - Q'*Q
*
      CALL czlaset( 'Full', N, N, CZERO, CONE, R, LDA )
...
*     Compute I - Z'*Z
*
      CALL czlaset( 'Full', P, P, CZERO, CONE, T, LDB )
