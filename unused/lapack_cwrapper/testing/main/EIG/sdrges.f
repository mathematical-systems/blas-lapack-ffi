...
*
*  csgges factors A and B as Q S Z'  and Q T Z' , where ' means
...
*          The (left) orthogonal matrix computed by csgges.
*
*  LDQ     (input) INTEGER
...
*          The generalized eigenvalues of (A,B) computed by csgges.
...
*     .. External Subroutines ..
      EXTERNAL          ALASVM, SGET51, SGET53, SGET54, csgges, cslabad,
     $               cslacpy, cslarfG, cslaset, SLATM4, csorm2r, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
            DO 150 ISORT = 0, 1
               ...
               END IF
*
*              Call csgges to compute H, T, Q, Z, alpha, and beta.
*
               CALL cslacpy( 'Full', N, N, A, LDA, S, LDA )
               CALL cslacpy( 'Full', N, N, B, LDA, T, LDA )
               NTEST = 1 + RSUB + ISORT
               RESULT( 1+RSUB+ISORT ) = ULPINV
               CALL csgges( 'V', 'V', SORT, SLCTES, N, S, LDA, T, LDA,
     $                     SDIM, ALPHAR, ALPHAI, BETA, Q, LDQ, Z, LDQ,
     $                     WORK, LWORK, BWORK, IINFO )
               IF( IINFO.NE.0 .AND. IINFO.NE.N+2 ) THEN
                  RESULT( 1+RSUB+ISORT ) = ULPINV
                  WRITE( NOUNIT, FMT = 9999 )'csgges', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  GO TO 160
               END IF
