...
*  =======
*
*  DCHKGK tests cdggbak, a routine for backward balancing  of
*  a matrix pair (A, B).
*
*  Arguments
*  =========
...
*     .. External Subroutines ..
      EXTERNAL           DGEMM, cdggbak, cdggbal, cdlacpy
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          ABS, MAX
*     ..
...
*
      CALL cdlacpy( 'FULL', N, N, A, LDA, AF, LDA )
      CALL cdlacpy( 'FULL', N, N, B, LDB, BF, LDB )
*
      CALL cdggbal( 'B', N, A, LDA, B, LDB, ILO, IHI, LSCALE, RSCALE,
     $             WORK, INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 1 ) = KNT
      END IF
*
      CALL cdlacpy( 'FULL', N, M, VL, LDVL, VLF, LDVL )
      CALL cdlacpy( 'FULL', N, M, VR, LDVR, VRF, LDVR )
*
      CALL cdggbak( 'B', 'L', N, ILO, IHI, LSCALE, RSCALE, M, VL, LDVL,
     $             INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 2 ) = KNT
      END IF
*
      CALL cdggbak( 'B', 'R', N, ILO, IHI, LSCALE, RSCALE, M, VR, LDVR,
     $             INFO )
      IF( INFO.NE.0 ) THEN
         NINFO = NINFO + 1
         LMAX( 3 ) = KNT
...
*
      WRITE( NOUT, FMT = 9999 )
 9999 FORMAT( 1X, '.. test output of cdggbak .. ' )
*
      WRITE( NOUT, FMT = 9998 )RMAX
 9998 FORMAT( ' value of largest test error                  =', D12.3 )
      WRITE( NOUT, FMT = 9997 )LMAX( 1 )
 9997 FORMAT( ' example number where cdggbal info is not 0    =', I4 )
      WRITE( NOUT, FMT = 9996 )LMAX( 2 )
 9996 FORMAT( ' example number where cdggbak(L) info is not 0 =', I4 )
      WRITE( NOUT, FMT = 9995 )LMAX( 3 )
 9995 FORMAT( ' example number where cdggbak(R) info is not 0 =', I4 )
