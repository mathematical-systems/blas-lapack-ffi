...
*
*  cchbtrd factors a Hermitian band matrix A as  U S U* , where * means
*  conjugate transpose, S is symmetric tridiagonal, and U is unitary.
*  cchbtrd can use either just the lower or just the upper triangle
...
*
*  (1)     | A - V S V* | / ( |A| n ulp )  computed by cchbtrd with
*                                          UPLO='U'
*
*  (2)     | I - UU* | / ( n ulp )
*
*  (3)     | A - V S V* | / ( |A| n ulp )  computed by cchbtrd with
*                                          UPLO='L'
*
*  (4)     | I - UU* | / ( n ulp )
*
...
*          Used to hold the unitary matrix computed by cchbtrd.
*
*  LDU     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL           CHBT21, cchbtrd, cclacpy,
     $                   CLATMR, CLATMS, cclaset,
     $                   SLASUM, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
  100          CONTINUE
*
*              Call cchbtrd to compute S and U from upper triangle.
*
               CALL cclacpy( ' ', K+1, N, A, LDA, WORK, LDA )
*
               NTEST = 1
               CALL cchbtrd( 'V', 'U', N, K, WORK, LDA, SD, SE, U, LDU,
     $                      WORK( LDA*N+1 ), IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cchbtrd(U)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
...
*
  ...
  140          CONTINUE
*
*              Call cchbtrd to compute S and U from lower triangle
*
               CALL cclacpy( ' ', K+1, N, A, LDA, WORK, LDA )
*
               NTEST = 3
               CALL cchbtrd( 'V', 'L', N, K, WORK, LDA, SD, SE, U, LDU,
     $                      WORK( LDA*N+1 ), IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cchbtrd(L)', IINFO, N,
     $               JTYPE, IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
