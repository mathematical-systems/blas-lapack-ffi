...
*  =======
*
*  SGQRTS tests csggqrf, which computes the GQR factorization of an
...
*          by csggqrf, see csggqrf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, cslacpy, cslaset, csorgqr,
     $                   csorgrq, SSYRK
*     ..
*     .. Intrinsic Functions ..
...
*
      CALL cslacpy( 'Full', N, M, A, LDA, AF, LDA )
      CALL cslacpy( 'Full', N, P, B, LDB, BF, LDB )
*
...
*
      CALL csggqrf( N, M, P, AF, LDA, TAUA, BF, LDB, TAUB, WORK,
     $             LWORK, INFO )
*
*     Generate the N-by-N matrix Q
*
      CALL cslaset( 'Full', N, N, ROGUE, ROGUE, Q, LDA )
      CALL cslacpy( 'Lower', N-1, M, AF( 2,1 ), LDA, Q( 2,1 ), LDA )
      CALL csorgqr( N, N, MIN( N, M ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL cslaset( 'Full', P, P, ROGUE, ROGUE, Z, LDB )
      IF( N.LE.P ) THEN
         IF( N.GT.0 .AND. N.LT.P )
     $      CALL cslacpy( 'Full', N, P-N, BF, LDB, Z( P-N+1, 1 ), LDB )
         IF( N.GT.1 )
     $      CALL cslacpy( 'Lower', N-1, N-1, BF( 2, P-N+1 ), LDB,
     $                    Z( P-N+2, P-N+1 ), LDB )
      ELSE
         IF( P.GT.1)
     $      CALL cslacpy( 'Lower', P-1, P-1, BF( N-P+2, 1 ), LDB,
     $                    Z( 2, 1 ), LDB )
      END IF
      CALL csorgrq( P, P, MIN( N, P ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL cslaset( 'Full', N, M, ZERO, ZERO, R, LDA )
      CALL cslacpy( 'Upper', N, M, AF, LDA, R, LDA )
*
*     Copy T
*
      CALL cslaset( 'Full', N, P, ZERO, ZERO, T, LDB )
      IF( N.LE.P ) THEN
        CALL cslacpy( 'Upper', N, N, BF( 1, P-N+1 ), LDB, T( 1, P-N+1 ),
     $                LDB )
      ELSE
         CALL cslacpy( 'Full', N-P, P, BF, LDB, T, LDB )
        CALL cslacpy( 'Upper', P, P, BF( N-P+1, 1 ), LDB, T( N-P+1, 1 ),
     $                LDB )
      END IF
*
*     Compute R - Q'*A
...
*     Compute I - Q'*Q
*
      CALL cslaset( 'Full', N, N, ZERO, ONE, R, LDA )
...
*     Compute I - Z'*Z
*
      CALL cslaset( 'Full', P, P, ZERO, ONE, T, LDB )
