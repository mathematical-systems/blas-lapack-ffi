...
*
*  cdgbbrd factors a general band matrix A as  Q B P* , where * means
*  transpose, B is upper bidiagonal, and Q and P are orthogonal;
*  cdgbbrd can also overwrite a given matrix C with Q* C .
*
...
*          Used to hold the orthogonal matrix Q computed by cdgbbrd.
*
*  LDQ     (input) INTEGER
...
*          Used to hold the orthogonal matrix P computed by cdgbbrd.
*
*  LDP     (input) INTEGER
...
*          Used to hold the matrix C updated by cdgbbrd.
*
*  LDC     (input) INTEGER
...
*     .. External Subroutines ..
      EXTERNAL        DBDT01, DBDT02, cdgbbrd, cdlacpy, DLAHD2, cdlaset,
...
*
               ...
               CALL cdlaset( 'Full', LDAB, N, ZERO, ZERO, AB, LDAB )
               IINFO = 0
               COND = ULPINV
*
...
*              Copy C
*
               CALL cdlacpy( 'Full', M, NRHS, C, LDC, CC, LDC )
*
*              Call cdgbbrd to compute B, Q and P, and to update C.
*
               CALL cdgbbrd( 'B', M, N, NRHS, KL, KU, AB, LDAB, BD, BE,
     $                      Q, LDQ, P, LDP, CC, LDC, WORK, IINFO )
*
               IF( IINFO.NE.0 ) THEN
                  WRITE( NOUNIT, FMT = 9999 )'cdgbbrd', IINFO, N, JTYPE,
     $               IOLDSD
                  INFO = ABS( IINFO )
                  IF( IINFO.LT.0 ) THEN
                     RETURN
