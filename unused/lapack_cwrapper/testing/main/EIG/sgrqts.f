...
*  =======
*
*  SGRQTS tests csggrqf, which computes the GRQ factorization of an
...
*          by csggrqf, see csggrqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL           SGEMM, csggrqf, cslacpy, cslaset, csorgqr,
     $                   csorgrq, SSYRK
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      CALL cslacpy( 'Full', P, N, B, LDB, BF, LDB )
*
...
*
      CALL csggrqf( M, P, N, AF, LDA, TAUA, BF, LDB, TAUB, WORK,
     $             LWORK, INFO )
*
...
*
     ...
     $      CALL cslacpy( 'Full', M, N-M, AF, LDA, Q( N-M+1, 1 ), LDA )
         IF( M.GT.1 )
     $      CALL cslacpy( 'Lower', M-1, M-1, AF( 2, N-M+1 ), LDA,
     $                   Q( N-M+2, N-M+1 ), LDA )
      ELSE
         IF( N.GT.1 )
     $      CALL cslacpy( 'Lower', N-1, N-1, AF( M-N+2, 1 ), LDA,
     $                   Q( 2, 1 ), LDA )
      END IF
      CALL csorgrq( N, N, MIN( M, N ), Q, LDA, TAUA, WORK, LWORK, INFO )
*
*     Generate the P-by-P matrix Z
*
      CALL cslaset( 'Full', P, P, ROGUE, ROGUE, Z, LDB )
      IF( P.GT.1 )
     $   CALL cslacpy( 'Lower', P-1, N, BF( 2,1 ), LDB, Z( 2,1 ), LDB )
      CALL csorgqr( P, P, MIN( P,N ), Z, LDB, TAUB, WORK, LWORK, INFO )
*
*     Copy R
*
      CALL cslaset( 'Full', M, N, ZERO, ZERO, R, LDA )
      IF( M.LE.N )THEN
        CALL cslacpy( 'Upper', M, M, AF( 1, N-M+1 ), LDA, R( 1, N-M+1 ),
     $                LDA )
      ELSE
         CALL cslacpy( 'Full', M-N, N, AF, LDA, R, LDA )
        CALL cslacpy( 'Upper', N, N, AF( M-N+1, 1 ), LDA, R( M-N+1, 1 ),
     $                LDA )
      END IF
*
*     Copy T
*
      CALL cslaset( 'Full', P, N, ZERO, ZERO, T, LDB )
      CALL cslacpy( 'Upper', P, N, BF, LDB, T, LDB )
*
*     Compute R - A*Q'
*
