...
*  routine csggev.
*
*  csggev computes for a pair of n-by-n nonsymmetric matrices (A,B) the
...
*                                 dimension (LDA, max(NN))
*          The Schur form matrix computed from A by csgges.  On exit, S
*          contains the Schur form matrix corresponding to the matrix
*          in A.
*
*  T       (workspace) REAL array,
*                                 dimension (LDA, max(NN))
*          The upper triangular matrix computed from B by csgges.
*
*  Q       (workspace) REAL array,
*                                 dimension (LDQ, max(NN))
*          The (left) eigenvectors matrix computed by csggev.
*
*  LDQ     (input) INTEGER
...
*          The (right) orthogonal matrix computed by csgges.
*
...
*          The generalized eigenvalues of (A,B) computed by csggev.
...
*          eigenvalues of A and B, but those computed when csggev only
...
*     .. External Subroutines ..
      EXTERNAL        ALASVM, SGET52, csggev, cslabad, cslacpy, cslarfG,
     $                   cslaset, SLATM4, csorm2r, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
      ...
      IF( INFO.EQ.0 .AND. LWORK.GE.1 ) THEN
        ...
        MAXWRK = 7*NMAX + NMAX*ILAENV( 1, 'csgeqrf', ' ', NMAX, 1, NMAX,
     $            0 )
...
*
  ...
  110       CONTINUE
*
*           Call csggev to compute eigenvalues and eigenvectors.
*
            CALL cslacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cslacpy( ' ', N, N, B, LDA, T, LDA )
            CALL csggev( 'V', 'V', N, S, LDA, T, LDA, ALPHAR, ALPHAI,
     $                  BETA, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'csggev1', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 1 ) )
            IF( RESULT( 2 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Left', 'csggev1',
...
*
     ...
     $                   ALPHAI, BETA, WORK, RESULT( 3 ) )
            IF( RESULT( 4 ).GT.THRESH ) THEN
               WRITE( NOUNIT, FMT = 9998 )'Right', 'csggev1',
...
*
            CALL cslacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cslacpy( ' ', N, N, B, LDA, T, LDA )
            CALL csggev( 'N', 'N', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'csggev2', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL cslacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cslacpy( ' ', N, N, B, LDA, T, LDA )
            CALL csggev( 'V', 'N', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, QE, LDQE, Z, LDQ, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'csggev3', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
...
*
            CALL cslacpy( ' ', N, N, A, LDA, S, LDA )
            CALL cslacpy( ' ', N, N, B, LDA, T, LDA )
            CALL csggev( 'N', 'V', N, S, LDA, T, LDA, ALPHR1, ALPHI1,
     $                  BETA1, Q, LDQ, QE, LDQE, WORK, LWORK, IERR )
            IF( IERR.NE.0 .AND. IERR.NE.N+1 ) THEN
               RESULT( 1 ) = ULPINV
               WRITE( NOUNIT, FMT = 9999 )'csggev4', IERR, N, JTYPE,
     $            IOLDSD
               INFO = ABS( IERR )
               GO TO 190
            END IF
