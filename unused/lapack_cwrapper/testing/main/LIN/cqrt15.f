...
*     .. External Subroutines ..
      EXTERNAL         CGEMM, cclarf, cclarnv, CLAROR, cclascl, cclaset,
     $                   CSSCAL, cslabad, SLAORD, cslascl, xerbla
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL cclarnv( 2, ISEED, M, WORK )
         CALL CSSCAL( M, ONE / SCNRM2( M, WORK, 1 ), WORK, 1 )
         CALL cclaset( 'Full', M, RANK, CZERO, CONE, A, LDA )
         CALL cclarf( 'Left', M, RANK, WORK, 1, CMPLX( TWO ), A, LDA,
     $               WORK( M+1 ) )
*
*        workspace used: m+mn
*
...
*
         CALL cclarnv( 2, ISEED, RANK*NRHS, WORK )
...
*
   ...
   40    CONTINUE
         IF( RANK.LT.N )
     $      CALL cclaset( 'Full', M, N-RANK, CZERO, CZERO,
     $                   A( 1, RANK+1 ), LDA )
...
*
         DO 50 J = 1, MN
            S( J ) = ZERO
   50    CONTINUE
         CALL cclaset( 'Full', M, N, CZERO, CZERO, A, LDA )
         CALL cclaset( 'Full', M, NRHS, CZERO, CZERO, B, LDB )
*
      END IF
*
*     Scale the matrix
...
*              matrix scaled up
*
               CALL cclascl( 'General', 0, 0, NORMA, BIGNUM, M, N, A,
     $                      LDA, INFO )
               CALL cslascl( 'General', 0, 0, NORMA, BIGNUM, MN, 1, S,
     $                      MN, INFO )
               CALL cclascl( 'General', 0, 0, NORMA, BIGNUM, M, NRHS, B,
     $                      LDB, INFO )
            ELSE IF( SCALE.EQ.3 ) THEN
*
*              matrix scaled down
*
               CALL cclascl( 'General', 0, 0, NORMA, SMLNUM, M, N, A,
     $                      LDA, INFO )
               CALL cslascl( 'General', 0, 0, NORMA, SMLNUM, MN, 1, S,
     $                      MN, INFO )
               CALL cclascl( 'General', 0, 0, NORMA, SMLNUM, M, NRHS, B,
     $                      LDB, INFO )
            ELSE
