...
*  =======
*
*  CRQT03 tests ccunmrq, which computes Q*C, Q'*C, C*Q or C*Q'.
*
*  CRQT03 compares the results of a call to ccunmrq with the results of
*  forming Q explicitly by a call to ccungrq and then performing matrix
...
*          returned by ccgerqf. See ccgerqf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL        CGEMM, cclacpy, cclarnv, cclaset, ccungrq, ccunmrq
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
*     ..
...
*
     ...
     $   CALL cclacpy( 'Full', K, N-K, AF( M-K+1, 1 ), LDA,
     $                Q( N-K+1, 1 ), LDA )
      IF( K.GT.1 )
     $   CALL cclacpy( 'Lower', K-1, K-1, AF( M-K+2, N-K+1 ), LDA,
     $                Q( N-K+2, N-K+1 ), LDA )
*
*     Generate the n-by-n matrix Q
*
      SRNAMT = 'ccungrq'
      CALL ccungrq( N, N, K, Q, LDA, TAU( MINMN-K+1 ), WORK, LWORK,
     $             INFO )
*
      DO 30 ISIDE = 1, 2
         IF( ISIDE.EQ.1 ) THEN
...
*           Copy C
*
            CALL cclacpy( 'Full', MC, NC, C, LDA, CC, LDA )
*
*           Apply Q or Q' to C
*
            SRNAMT = 'ccunmrq'
            IF( K.GT.0 )
     $        CALL ccunmrq( SIDE, TRANS, MC, NC, K, AF( M-K+1, 1 ), LDA,
