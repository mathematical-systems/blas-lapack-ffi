...
*  =======
*
*  CDRVPP tests the driver routines ccppsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                cclacpy, CLAIPD, cclaqhp, CLARHS, cclaset, CLATB4,
     $                 CLATMS, ccppequ, ccppsv, ccppsvX, CPPT01, CPPT02,
     $                   CPPT05, ccpptrf, ccpptri
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
      CHARACTER*7        SRNAMT
...
*                       the value returned by ccppsvX (FACT = 'N' reuses
...
*
                           CALL ccppequ( UPLO, N, AFAC, S, SCOND, AMAX,
     $                                  INFO )
...
*
                              CALL cclaqhp( UPLO, N, AFAC, S, SCOND,
     $                                     AMAX, EQUED )
                           END IF
                        END IF
*
...
*                       Factor the matrix A.
*
                        CALL ccpptrf( UPLO, N, AFAC, INFO )
*
*                       Form the inverse of A.
*
                        CALL CCOPY( NPP, AFAC, 1, A, 1 )
                        CALL ccpptri( UPLO, N, A, INFO )
*
...
*
                        SRNAMT = 'ccppsv '
                        CALL ccppsv( UPLO, N, NRHS, AFAC, X, LDA, INFO )
*
*                       Check error code from ccppsv .
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'ccppsv ', INFO, IZERO,
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              WRITE( NOUT, FMT = 9999 )'ccppsv ', UPLO,
     $                           N, IMAT, K, RESULT( K )
                              NFAIL = NFAIL + 1
                           END IF
   60                   CONTINUE
...
*
     ...
     $                  CALL cclaset( 'Full', NPP, 1, CMPLX( ZERO ),
     $                               CMPLX( ZERO ), AFAC, NPP )
...
*                       EQUED='Y'.
*
                       CALL cclaqhp( UPLO, N, A, S, SCOND, AMAX, EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using ccppsvX.
*
                     SRNAMT = 'ccppsvX'
                     CALL ccppsvX( FACT, UPLO, N, NRHS, A, AFAC, EQUED,
     $                            S, B, LDA, X, LDA, RCOND, RWORK,
     $                            RWORK( NRHS+1 ), WORK,
     $                            RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from ccppsvX.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'ccppsvX', INFO, IZERO,
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from ccppsvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'ccppsvX', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'ccppsvX', FACT,
