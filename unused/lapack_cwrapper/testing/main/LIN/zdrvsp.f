...
*  =======
*
*  ZDRVSP tests the driver routines czspsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 ZGET04, czlacpy, ZLARHS, czlaset, ZLATB4, ZLATMS,
     $                  ZLATSP, ZPPT05, czspsv, czspsvX, ZSPT01, ZSPT02,
     $                   czsptrf, czsptri
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL czsptrf( UPLO, N, AFAC, IWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL ZCOPY( NPP, AFAC, 1, AINV, 1 )
                     CALL czsptri( UPLO, N, AINV, IWORK, WORK, INFO )
...
*
*                    Factor the matrix and solve the system using czspsv.
*
                     SRNAMT = 'czspsv '
                     CALL czspsv( UPLO, N, NRHS, AFAC, IWORK, X, LDA,
     $                           INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from czspsv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'czspsv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'czspsv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using czspsvX.
*
                  SRNAMT = 'czspsvX'
                  CALL czspsvX( FACT, UPLO, N, NRHS, A, AFAC, IWORK, B,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from czspsvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'czspsvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from czspsvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'czspsvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
