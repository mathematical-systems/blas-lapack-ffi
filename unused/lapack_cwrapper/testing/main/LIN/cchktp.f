...
*  =======
*
*  CCHKTP tests cctptri, -TRS, -RFS, and -CON, and cclatps
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $               cclacpy, CLARHS, cclatps, CLATTP, cctpcon, cctprfs,
     $                  CTPT01, CTPT02, CTPT03, CTPT05, CTPT06, cctptri,
     $                   cctptrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
     ...
     $            CALL CCOPY( LAP, AP, 1, AINVP, 1 )
               SRNAMT = 'cctptri'
               CALL cctptri( UPLO, DIAG, N, AINVP, INFO )
*
*              Check error code from cctptri.
*
               IF( INFO.NE.0 )
     $           CALL ALAERH( PATH, 'cctptri', INFO, 0, UPLO // DIAG, N,
...
*
                     SRNAMT = 'cctptrs'
                     CALL cctptrs( UPLO, TRANS, DIAG, N, NRHS, AP, X,
     $                            LDA, INFO )
*
*                 Check error code from cctptrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cctptrs', INFO, 0,
...
*
                     SRNAMT = 'cctprfs'
                     CALL cctprfs( UPLO, TRANS, DIAG, N, NRHS, AP, B,
     $                            LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                            WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                 Check error code from cctprfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cctprfs', INFO, 0,
     $                               UPLO // TRANS // DIAG, N, N, -1,
...
*
               DO 50 ITRAN = 1, 2
                  ...
                  END IF
                  SRNAMT = 'cctpcon'
                  CALL cctpcon( NORM, UPLO, DIAG, N, AP, RCOND, WORK,
     $                         RWORK, INFO )
*
*                 Check error code from cctpcon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cctpcon', INFO, 0,
...
*
                  IF( RESULT( 7 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9997 ) 'cctpcon', NORM, UPLO,
   ...
   70    CONTINUE
*
*        Use pathological test matrices to test cclatps.
*
...
*
                  ...
                  CALL cclatps( UPLO, TRANS, DIAG, 'N', N, AP, B, SCALE,
     $                         RWORK, INFO )
*
*                 Check error code from cclatps.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'cclatps', INFO, 0,
...
*
                  ...
                  CALL cclatps( UPLO, TRANS, DIAG, 'Y', N, AP, B( N+1 ),
     $                         SCALE, RWORK, INFO )
*
*                 Check error code from cclatps.
*
...
*
                  IF( RESULT( 8 ).GE.THRESH ) THEN
                     IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                  CALL ALAHD( NOUT, PATH )
                     WRITE( NOUT, FMT = 9996 )'cclatps', UPLO, TRANS,
