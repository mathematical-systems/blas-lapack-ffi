...
*  =======
*
*  SDRVPP tests the driver routines csppsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                   cslacpy, cslaqsp, SLARHS, cslaset, SLATB4, 
     $  SLATMS, 
     $                   csppequ, csppsv, csppsvX, SPPT01, SPPT02, 
     $  SPPT05, 
     $                   cspptrf, cspptri
*     ..
*     .. Scalars in Common ..
...
*                       the value returned by csppsvX (FACT = 'N' reuses
...
*
                           CALL csppequ( UPLO, N, AFAC, S, SCOND, AMAX,
     $                                  INFO )
...
*
                              CALL cslaqsp( UPLO, N, AFAC, S, SCOND,
     $                                     AMAX, EQUED )
                           END IF
                        END IF
*
...
*                       Factor the matrix A.
*
                        CALL cspptrf( UPLO, N, AFAC, INFO )
*
*                       Form the inverse of A.
*
                        CALL SCOPY( NPP, AFAC, 1, A, 1 )
                        CALL cspptri( UPLO, N, A, INFO )
*
...
*
                        SRNAMT = 'csppsv '
                        CALL csppsv( UPLO, N, NRHS, AFAC, X, LDA, INFO 
     $  ) 
*
*                       Check error code from csppsv .
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'csppsv ', INFO, IZERO,
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              WRITE( NOUT, FMT = 9999 )'csppsv ', UPLO,
     $                           N, IMAT, K, RESULT( K )
                              NFAIL = NFAIL + 1
                           END IF
   60                   CONTINUE
...
*
     ...
     $                  CALL cslaset( 'Full', NPP, 1, ZERO, ZERO, AFAC,
     $                               NPP )
...
*                       EQUED='Y'.
*
                        CALL cslaqsp( UPLO, N, A, S, SCOND, AMAX, EQUED 
     $  ) 
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using csppsvX.
*
                     SRNAMT = 'csppsvX'
                     CALL csppsvX( FACT, UPLO, N, NRHS, A, AFAC, EQUED,
     $                            S, B, LDA, X, LDA, RCOND, RWORK,
     $                            RWORK( NRHS+1 ), WORK, IWORK, INFO )
*
*                    Check the error code from csppsvX.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'csppsvX', INFO, IZERO,
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from csppsvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'csppsvX', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'csppsvX', FACT,
