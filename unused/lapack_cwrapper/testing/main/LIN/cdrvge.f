...
*  =======
*
*  CDRVGE tests the driver routines ccgesv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALADHD, ALAERH, ALASVM, CERRVX, ccgeequ, ccgesv,
     $                 ccgesvX, CGET01, CGET02, CGET04, CGET07, ccgetrf,
     $               ccgetri, cclacpy, cclaqge, CLARHS, cclaset, CLATB4,
     $                   CLATMS, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   20             CONTINUE
               ELSE
                  CALL cclaset( 'Full', N, N-IZERO+1, CMPLX( ZERO ),
...
*
            CALL cclacpy( 'Full', N, N, A, LDA, ASAV, LDA )
*
            DO 70 IEQUED = 1, 4
               EQUED = EQUEDS( IEQUED )
               IF( IEQUED.EQ.1 ) THEN
...
*                    the value returned by ccgesvX (FACT = 'N' reuses
*                    the condition number from the previous iteration
*                    with FACT = 'F').
*
                     CALL cclacpy( 'Full', N, N, ASAV, LDA, AFAC, LDA )
                     IF( EQUIL .OR. IEQUED.GT.1 ) THEN
*
*                       Compute row and column scale factors to
*                       equilibrate the matrix A.
*
                        CALL ccgeequ( N, N, AFAC, LDA, S, S( N+1 ),
...
*
                           CALL cclaqge( N, N, AFAC, LDA, S, S( N+1 ),
...
*                    Factor the matrix A.
*
                     CALL ccgetrf( N, N, AFAC, LDA, IWORK, INFO )
*
*                    Form the inverse of A.
*
                     CALL cclacpy( 'Full', N, N, AFAC, LDA, A, LDA )
                     LWORK = NMAX*MAX( 3, NRHS )
                     CALL ccgetri( N, A, LDA, IWORK, WORK, LWORK, INFO )
*
...
*
                     CALL cclacpy( 'Full', N, N, ASAV, LDA, A, LDA )
*
...
*
     ...
     $                            ISEED, INFO )
                     XTYPE = 'C'
                     CALL cclacpy( 'Full', N, NRHS, B, LDA, BSAV, LDA )
*
...
*                       solve the system.
*
                        CALL cclacpy( 'Full', N, N, A, LDA, AFAC, LDA )
                        CALL cclacpy( 'Full', N, NRHS, B, LDA, X, LDA )
*
                        SRNAMT = 'ccgesv '
                        CALL ccgesv( N, NRHS, AFAC, LDA, IWORK, X, LDA,
     $                              INFO )
*
*                       Check error code from ccgesv .
*
                        IF( INFO.NE.IZERO )
     $                     CALL ALAERH( PATH, 'ccgesv ', INFO, IZERO,
     $                                  ' ', N, N, -1, -1, NRHS, IMAT,
     $                                  NFAIL, NERRS, NOUT )
*
...
*
                           CALL cclacpy( 'Full', N, NRHS, B, LDA, WORK,
     $                                  LDA )
...
*
                     IF( .NOT.PREFAC )
     $                  CALL cclaset( 'Full', N, N, CMPLX( ZERO ),
     $                               CMPLX( ZERO ), AFAC, LDA )
                     CALL cclaset( 'Full', N, NRHS, CMPLX( ZERO ),
     $                            CMPLX( ZERO ), X, LDA )
...
*                       EQUED = 'R', 'C', or 'B'.
*
                        CALL cclaqge( N, N, A, LDA, S, S( N+1 ), ROWCND,
     $                               COLCND, AMAX, EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using ccgesvX.
*
                     SRNAMT = 'ccgesvX'
                     CALL ccgesvX( FACT, TRANS, N, NRHS, A, LDA, AFAC,
     $                            LDA, IWORK, EQUED, S, S( N+1 ), B,
     $                            LDA, X, LDA, RCOND, RWORK,
     $                            RWORK( NRHS+1 ), WORK,
     $                            RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from ccgesvX.
*
                     IF( INFO.NE.IZERO )
     $                  CALL ALAERH( PATH, 'ccgesvX', INFO, IZERO,
     $                               FACT // TRANS, N, N, -1, -1, NRHS,
     $                               IMAT, NFAIL, NERRS, NOUT )
*
*                    Compare RWORK(2*NRHS+1) from ccgesvX with the
...
*                       Compute residual of the computed solution.
*
                        CALL cclacpy( 'Full', N, NRHS, BSAV, LDA, WORK,
     $                               LDA )
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from ccgesvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9997 )'ccgesvX',
     $                              FACT, TRANS, N, EQUED, IMAT, K,
     $                              RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9998 )'ccgesvX',
   ...
   40                   CONTINUE
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'ccgesvX', FACT,
     $                           TRANS, N, EQUED, IMAT, 1, RESULT( 1 )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'ccgesvX', FACT,
