...
*     .. Array Arguments ..
      ...
      DOUBLE PRECISION AB(*),AINV(*),B(*),RWORK(*),WORK(*),X(*),XACT(*)
*     ..
*
*  Purpose
*  =======
*
*  DCHKTB tests cdtbtrs, -RFS, and -CON, and cdlatbs.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IDIAG,IK,IMAT,IN,INFO,IRHS,ITRAN,IUPLO,J,K,KD,LDA,LDAB,
     +        N,NERRS,NFAIL,NIMAT,NIMAT2,NK,NRHS,NRUN
...
*     .. External Subroutines ..
     ...
     +         cdlaset,cdlatbs,DLATTB,cdtbcon,cdtbrfs,DTBSV,DTBT02,
     +         DTBT03,DTBT05,DTBT06,cdtbtrs
*     ..
*     .. Scalars in Common ..
      LOGICAL LERR,OK
      CHARACTER*7 SRNAMT
...
*
                              SRNAMT = 'cdtbtrs'
                              CALL cdtbtrs(UPLO,TRANS,DIAG,N,KD,NRHS,AB,
     +                                     LDAB,X,LDA,INFO)
*
*                    Check error code from cdtbtrs.
*
                              IF (INFO.NE.0) CALL ALAERH(PATH,'cdtbtrs',
     +                                            INFO,0,UPLO//TRANS//
     +                                            DIAG,N,N,KD,KD,NRHS,
...
*
                              SRNAMT = 'cdtbrfs'
                              CALL cdtbrfs(UPLO,TRANS,DIAG,N,KD,NRHS,AB,
     +                                     LDAB,B,LDA,X,LDA,RWORK,
     +                                     RWORK(NRHS+1),WORK,IWORK,
     +                                     INFO)
*
*                    Check error code from cdtbrfs.
*
                              IF (INFO.NE.0) CALL ALAERH(PATH,'cdtbrfs',
     +                                            INFO,0,UPLO//TRANS//
     +                                            DIAG,N,N,KD,KD,NRHS,
     +                                            IMAT,NFAIL,NERRS,NOUT)
*
...
*
                      DO 70 ITRAN = 1,2
                          ...
                          END IF
                          SRNAMT = 'cdtbcon'
                          CALL cdtbcon(NORM,UPLO,DIAG,N,KD,AB,LDAB,
     +                                 RCOND,WORK,IWORK,INFO)
*
*                    Check error code from cdtbcon.
*
                          IF (INFO.NE.0) CALL ALAERH(PATH,'cdtbcon',
     +                                        INFO,0,NORM//UPLO//DIAG,N,
     +                                        N,KD,KD,-1,IMAT,NFAIL,
     +                                        NERRS,NOUT)
*
...
*
     ...
     +                            NERRS.EQ.0) CALL ALAHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9998) 'cdtbcon',NORM,UPLO,
   ...
   90         CONTINUE
*
*           Use pathological test matrices to test cdlatbs.
*
...
*
                          ...
                          CALL cdlatbs(UPLO,TRANS,DIAG,'N',N,KD,AB,LDAB,
     +                                 B,SCALE,RWORK,INFO)
*
*                    Check error code from cdlatbs.
*
                          IF (INFO.NE.0) CALL ALAERH(PATH,'cdlatbs',
...
*
                          CALL DCOPY(N,X,1,B,1)
                          CALL cdlatbs(UPLO,TRANS,DIAG,'Y',N,KD,AB,LDAB,
     +                                 B,SCALE,RWORK,INFO)
*
*                    Check error code from cdlatbs.
*
...
*
                          IF (RESULT(7).GE.THRESH) THEN
                              IF (NFAIL.EQ.0 .AND.
     +                            NERRS.EQ.0) CALL ALAHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9997) 'cdlatbs',UPLO,
     +                          TRANS,DIAG,'N',N,KD,IMAT,7,RESULT(7)
                              NFAIL = NFAIL + 1
                          END IF
     ...
     +                            NERRS.EQ.0) CALL ALAHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9997) 'cdlatbs',UPLO,
     +                          TRANS,DIAG,'Y',N,KD,IMAT,8,RESULT(8)
                              NFAIL = NFAIL + 1
                          END IF
                          NRUN = NRUN + 2
  100                 CONTINUE
...
*
 9999 FORMAT (' UPLO=''',A1,''', TRANS=''',A1,
     +       ''',                        DIAG=''',A1,''', N=',I5,
     +       ', KD=',I5,', NRHS=',I5,', type ',I2,', test(',I2,')=',
     +       G12.5)
 9998 FORMAT (1X,A6,'( ''',A1,''', ''',A1,''', ''',A1,''',',I5,',',I5,
     +       ',  ... ), type ',I2,', test(',I2,')=',G12.5)
 9997 FORMAT (1X,A6,'( ''',A1,''', ''',A1,''', ''',A1,''', ''',A1,''',',
