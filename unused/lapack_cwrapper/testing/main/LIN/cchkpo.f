...
*  =======
*
*  CCHKPO tests ccpotrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, CERRPO, CGET04, cclacpy,
     $                 CLAIPD, CLARHS, CLATB4, CLATMS, ccpocon, ccporfs,
     $                 CPOT01, CPOT02, CPOT03, CPOT05, ccpotrf, ccpotri,
     $                   ccpotrs, XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
*                 Check error code from ccpotrf.
*
                  IF( INFO.NE.IZERO ) THEN
                     CALL ALAERH( PATH, 'ccpotrf', INFO, IZERO, UPLO, N,
...
*
                  CALL cclacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                  SRNAMT = 'ccpotri'
                  CALL ccpotri( UPLO, N, AINV, LDA, INFO )
*
*                 Check error code from ccpotri.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccpotri', INFO, 0, UPLO, N, N,
...
*
                     SRNAMT = 'ccpotrs'
                     CALL ccpotrs( UPLO, N, NRHS, AFAC, LDA, X, LDA,
     $                            INFO )
*
*                 Check error code from ccpotrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'ccpotrs', INFO, 0, UPLO, N,
...
*
                     SRNAMT = 'ccporfs'
                     CALL ccporfs( UPLO, N, NRHS, A, LDA, AFAC, LDA, B,
     $                            LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                            WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                 Check error code from ccporfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'ccporfs', INFO, 0, UPLO, N,
     $                               N, -1, -1, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
                  ...
                  SRNAMT = 'ccpocon'
                  CALL ccpocon( UPLO, N, AFAC, LDA, ANORM, RCOND, WORK,
     $                         RWORK, INFO )
*
*                 Check error code from ccpocon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'ccpocon', INFO, 0, UPLO, N, N,
     $                            -1, -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
