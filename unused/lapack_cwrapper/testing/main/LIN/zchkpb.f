...
*  =======
*
*  ZCHKPB tests czpbtrf, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 ZGET04, czlacpy, ZLAIPD, ZLARHS, czlaset, ZLATB4,
     $                 ZLATMS, czpbcon, czpbrfs, ZPBT01, ZPBT02, ZPBT05,
     $                   czpbtrf, czpbtrs, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*                    matrix.
*
                    CALL czlacpy( 'Full', KD+1, N, A, LDAB, AFAC, LDAB )
                     SRNAMT = 'czpbtrf'
                     CALL czpbtrf( UPLO, N, KD, AFAC, LDAB, INFO )
*
*                    Check error code from czpbtrf.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'czpbtrf', INFO, IZERO, UPLO,
...
*                    residual.
*
                     CALL czlacpy( 'Full', KD+1, N, AFAC, LDAB, AINV,
     $                            LDAB )
...
*
     ...
     $                            DCMPLX( ONE ), AINV, LDA )
                     SRNAMT = 'czpbtrs'
                    CALL czpbtrs( UPLO, N, KD, N, AFAC, LDAB, AINV, LDA,
     $                            INFO )
*
...
*
                        SRNAMT = 'czpbtrs'
                        CALL czpbtrs( UPLO, N, KD, NRHS, AFAC, LDAB, X,
     $                               LDA, INFO )
*
*                    Check error code from czpbtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'czpbtrs', INFO, 0, UPLO,
...
*
                        SRNAMT = 'czpbrfs'
                        CALL czpbrfs( UPLO, N, KD, NRHS, A, LDAB, AFAC,
     $                               LDAB, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                    Check error code from czpbrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'czpbrfs', INFO, 0, UPLO,
     $                                  N, N, KD, KD, NRHS, IMAT, NFAIL,
     $                                  NERRS, NOUT )
*
                        CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
                     SRNAMT = 'czpbcon'
                    CALL czpbcon( UPLO, N, KD, AFAC, LDAB, ANORM, RCOND,
     $                            WORK, RWORK, INFO )
*
*                    Check error code from czpbcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'czpbcon', INFO, 0, UPLO, N,
     $                               N, KD, KD, -1, IMAT, NFAIL, NERRS,
     $                               NOUT )
*
