...
*  =======
*
*  ZCHKTZ tests cztzrqf and cztzrzf.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALAHD, ALASUM, DLAORD, ZERRTZ, czgeqr2, czlacpy,
     $                   czlaset, ZLATMS, cztzrqf, cztzrzf
*     ..
*     .. Intrinsic Functions ..
...
*
                  IF( MODE.EQ.0 ) THEN
                     CALL czlaset( 'Full', M, N, DCMPLX( ZERO ),
     $                            DCMPLX( ZERO ), A, LDA )
                     DO 20 I = 1, MNMIN
                        COPYS( I ) = ZERO
   20                CONTINUE
     ...
     $                            LDA, WORK, INFO )
                     CALL czgeqr2( M, N, A, LDA, WORK, WORK( MNMIN+1 ),
     $                            INFO )
                     CALL czlaset( 'Lower', M-1, N, DCMPLX( ZERO ),
     $                            DCMPLX( ZERO ), A( 2 ), LDA )
...
*
                  CALL czlacpy( 'All', M, N, A, LDA, COPYA, LDA )
*
*                 Call cztzrqf to reduce the upper trapezoidal matrix to
*                 upper triangular form.
*
                  SRNAMT = 'cztzrqf'
                  CALL cztzrqf( M, N, A, LDA, TAU, INFO )
*
...
*
                  CALL czlacpy( 'All', M, N, A, LDA, COPYA, LDA )
*
*                 Call cztzrzf to reduce the upper trapezoidal matrix to
*                 upper triangular form.
*
                  SRNAMT = 'cztzrzf'
                  CALL cztzrzf( M, N, A, LDA, TAU, WORK, LWORK, INFO )
*
*                 Compute norm(svd(a) - svd(r))
*
