...
*  =======
*
*  ZCHKPO tests czpotrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 czlacpy, ZLAIPD, ZLARHS, ZLATB4, ZLATMS, czpocon,
     $                 czporfs, ZPOT01, ZPOT02, ZPOT03, ZPOT05, czpotrf,
     $                   czpotri, czpotrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
*                 Check error code from czpotrf.
*
                  IF( INFO.NE.IZERO ) THEN
                     CALL ALAERH( PATH, 'czpotrf', INFO, IZERO, UPLO, N,
...
*
                  CALL czlacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                  SRNAMT = 'czpotri'
                  CALL czpotri( UPLO, N, AINV, LDA, INFO )
*
*                 Check error code from czpotri.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czpotri', INFO, 0, UPLO, N, N,
...
*
                     SRNAMT = 'czpotrs'
                     CALL czpotrs( UPLO, N, NRHS, AFAC, LDA, X, LDA,
     $                            INFO )
*
*                 Check error code from czpotrs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'czpotrs', INFO, 0, UPLO, N,
...
*
                     SRNAMT = 'czporfs'
                     CALL czporfs( UPLO, N, NRHS, A, LDA, AFAC, LDA, B,
     $                            LDA, X, LDA, RWORK, RWORK( NRHS+1 ),
     $                            WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                 Check error code from czporfs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'czporfs', INFO, 0, UPLO, N,
     $                               N, -1, -1, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
                  ...
                  SRNAMT = 'czpocon'
                  CALL czpocon( UPLO, N, AFAC, LDA, ANORM, RCOND, WORK,
     $                         RWORK, INFO )
*
*                 Check error code from czpocon.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czpocon', INFO, 0, UPLO, N, N,
     $                            -1, -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
