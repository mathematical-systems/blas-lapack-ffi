...
*  =======
*
*  DCHKPB tests cdpbtrf, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,I1,I2,IKD,IMAT,IN,INB,INFO,IOFF,IRHS,IUPLO,IW,IZERO,K,
     +        KD,KL,KOFF,KU,LDA,LDAB,MODE,N,NB,NERRS,NFAIL,NIMAT,NKD,
     +        NRHS,NRUN
...
*     .. External Subroutines ..
     ...
     +         cdlaset,DLATB4,DLATMS,cdpbcon,cdpbrfs,DPBT01,DPBT02,
     +         DPBT05,cdpbtrf,cdpbtrs,DSWAP,XLAENV
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX,MIN
*     ..
...
*                    matrix.
*
                          CALL cdlacpy('Full',KD+1,N,A,LDAB,AFAC,LDAB)
                          SRNAMT = 'cdpbtrf'
                          CALL cdpbtrf(UPLO,N,KD,AFAC,LDAB,INFO)
*
*                    Check error code from cdpbtrf.
*
                          IF (INFO.NE.IZERO) THEN
                              CALL ALAERH(PATH,'cdpbtrf',INFO,IZERO,
     +                                    UPLO,N,N,KD,KD,NB,IMAT,NFAIL,
     +                                    NERRS,NOUT)
                              GO TO 50
                          END IF
*
...
*                    residual.
*
                          CALL cdlacpy('Full',KD+1,N,AFAC,LDAB,AINV,
     +                                 LDAB)
                          CALL DPBT01(UPLO,N,KD,A,LDAB,AINV,LDAB,RWORK,
     +                                RESULT(1))
*
...
*
     ...
     +                            NERRS.EQ.0) CALL ALAHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9999) UPLO,N,KD,NB,IMAT,1,
     +                          RESULT(1)
                              NFAIL = NFAIL + 1
                          END IF
                          NRUN = NRUN + 1
...
*
                          CALL cdlaset('Full',N,N,ZERO,ONE,AINV,LDA)
                          SRNAMT = 'cdpbtrs'
                          CALL cdpbtrs(UPLO,N,KD,N,AFAC,LDAB,AINV,LDA,
     +                                 INFO)
*
...
*
                              SRNAMT = 'cdpbtrs'
                              CALL cdpbtrs(UPLO,N,KD,NRHS,AFAC,LDAB,X,
     +                                     LDA,INFO)
*
*                    Check error code from cdpbtrs.
*
                              IF (INFO.NE.0) CALL ALAERH(PATH,'cdpbtrs',
     +                                            INFO,0,UPLO,N,N,KD,KD,
...
*
                              SRNAMT = 'cdpbrfs'
                              CALL cdpbrfs(UPLO,N,KD,NRHS,A,LDAB,AFAC,
     +                                     LDAB,B,LDA,X,LDA,RWORK,
     +                                     RWORK(NRHS+1),WORK,IWORK,
     +                                     INFO)
*
*                    Check error code from cdpbrfs.
*
                              IF (INFO.NE.0) CALL ALAERH(PATH,'cdpbrfs',
     +                                            INFO,0,UPLO,N,N,KD,KD,
     +                                            NRHS,IMAT,NFAIL,NERRS,
     +                                            NOUT)
*
...
*
                          SRNAMT = 'cdpbcon'
                          CALL cdpbcon(UPLO,N,KD,AFAC,LDAB,ANORM,RCOND,
     +                                 WORK,IWORK,INFO)
*
*                    Check error code from cdpbcon.
*
                          IF (INFO.NE.0) CALL ALAERH(PATH,'cdpbcon',
     +                                        INFO,0,UPLO,N,N,KD,KD,-1,
...
*
 9999 FORMAT (' UPLO=''',A1,''', N=',I5,', KD=',I5,', NB=',I4,', type ',
     +       I2,', test ',I2,', ratio= ',G12.5)
 9998 FORMAT (' UPLO=''',A1,''', N=',I5,', KD=',I5,', NRHS=',I3,
     +       ', type ',I2,', test(',I2,') = ',G12.5)
 9997 FORMAT (' UPLO=''',A1,''', N=',I5,', KD=',I5,',',10X,' type ',I2,
     +       ', test(',I2,') = ',G12.5)
      RETURN
*
*     End of DCHKPB
