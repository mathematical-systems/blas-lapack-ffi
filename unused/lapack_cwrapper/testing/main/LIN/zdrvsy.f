...
*  =======
*
*  ZDRVSY tests the driver routines czsysv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 czlacpy, ZLARHS, czlaset, ZLATB4, ZLATMS, ZLATSY,
     $                 ZPOT05, czsysv, czsysvX, ZSYT01, ZSYT02, czsytrf,
     $                   czsytri
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                     ...
                     CALL czsytrf( UPLO, N, AFAC, LDA, IWORK, WORK,
     $                            LWORK, INFO )
*
*                    Compute inv(A) and take its norm.
*
                     CALL czlacpy( UPLO, N, N, AFAC, LDA, AINV, LDA )
                     CALL czsytri( UPLO, N, AINV, LDA, IWORK, WORK,
     $                            INFO )
...
*
*                    Factor the matrix and solve the system using czsysv.
*
                     SRNAMT = 'czsysv '
                     CALL czsysv( UPLO, N, NRHS, AFAC, LDA, IWORK, X,
     $                           LDA, WORK, LWORK, INFO )
*
...
*
  ...
  100                   CONTINUE
                     ...
                     END IF
*
*                    Check error code from czsysv .
*
                     IF( INFO.NE.K ) THEN
                        CALL ALAERH( PATH, 'czsysv ', INFO, K, UPLO, N,
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           WRITE( NOUT, FMT = 9999 )'czsysv ', UPLO, N,
     $                        IMAT, K, RESULT( K )
                           NFAIL = NFAIL + 1
                        END IF
  110                CONTINUE
...
*                 error bounds using czsysvX.
*
                  SRNAMT = 'czsysvX'
                  CALL czsysvX( FACT, UPLO, N, NRHS, A, LDA, AFAC, LDA,
...
*
  ...
  130                CONTINUE
                  ...
                  END IF
*
*                 Check the error code from czsysvX.
*
                  IF( INFO.NE.K ) THEN
                    CALL ALAERH( PATH, 'czsysvX', INFO, K, FACT // UPLO,
...
*
     ...
     $                            RESULT( 4 ) )
                  ...
                  END IF
*
*                 Compare RCOND from czsysvX with the computed value
*                 in RCONDC.
*
...
*
                  DO 140 K = K1, 6
                     IF( RESULT( K ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALADHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 )'czsysvX', FACT, UPLO,
     $                     N, IMAT, K, RESULT( K )
                        NFAIL = NFAIL + 1
                     END IF
  140             CONTINUE
