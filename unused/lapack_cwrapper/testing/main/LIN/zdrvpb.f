...
*  =======
*
*  ZDRVPB tests the driver routines czpbsv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                ZGET04, czlacpy, ZLAIPD, czlaqhb, ZLARHS, czlaset,
     $                 ZLATB4, ZLATMS, czpbequ, czpbsv, czpbsvX, ZPBT01,
     $                   ZPBT02, ZPBT05, czpbtrf, czpbtrs, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
                  CALL czlacpy( 'Full', KD+1, N, A, LDAB, ASAV, LDAB )
*
                  DO 70 IEQUED = 1, 2
                     EQUED = EQUEDS( IEQUED )
                     IF( IEQUED.EQ.1 ) THEN
...
*                          with the value returned by czpbsvX (FACT =
*                          'N' reuses the condition number from the
*                          previous iteration with FACT = 'F').
*
                           CALL czlacpy( 'Full', KD+1, N, ASAV, LDAB,
     $                                  AFAC, LDAB )
...
*
                              CALL czpbequ( UPLO, N, KD, AFAC, LDAB, S,
     $                                     SCOND, AMAX, INFO )
...
*
                                 CALL czlaqhb( UPLO, N, KD, AFAC, LDAB,
     $                                        S, SCOND, AMAX, EQUED )
                              END IF
                           END IF
*
...
*                          Factor the matrix A.
*
                           CALL czpbtrf( UPLO, N, KD, AFAC, LDAB, INFO )
*
...
*
     ...
     $                                  DCMPLX( ONE ), A, LDA )
                           SRNAMT = 'czpbtrs'
                           CALL czpbtrs( UPLO, N, KD, N, AFAC, LDAB, A,
     $                                  LDA, INFO )
*
...
*
                        CALL czlacpy( 'Full', KD+1, N, ASAV, LDAB, A,
     $                               LDAB )
*
...
*
     ...
     $                               LDA, ISEED, INFO )
                        XTYPE = 'C'
                        CALL czlacpy( 'Full', N, NRHS, B, LDA, BSAV,
     $                               LDA )
*
                        IF( NOFACT ) THEN
*
...
*
                           CALL czlacpy( 'Full', KD+1, N, A, LDAB, AFAC,
     $                                  LDAB )
                           CALL czlacpy( 'Full', N, NRHS, B, LDA, X,
     $                                  LDA )
*
                           SRNAMT = 'czpbsv '
                          CALL czpbsv( UPLO, N, KD, NRHS, AFAC, LDAB, X,
     $                                 LDA, INFO )
*
*                          Check error code from czpbsv .
*
                           IF( INFO.NE.IZERO ) THEN
                              CALL ALAERH( PATH, 'czpbsv ', INFO, IZERO,
     $                                     UPLO, N, N, KD, KD, NRHS,
...
*
                        IF( .NOT.PREFAC )
     $                    CALL czlaset( 'Full', KD+1, N, DCMPLX( ZERO ),
...
*                          EQUED='Y'
*
                           CALL czlaqhb( UPLO, N, KD, A, LDAB, S, SCOND,
     $                                  AMAX, EQUED )
                        END IF
*
*                       Solve the system and compute the condition
*                       number and error bounds using czpbsvX.
*
                        SRNAMT = 'czpbsvX'
                        CALL czpbsvX( FACT, UPLO, N, KD, NRHS, A, LDAB,
     $                               AFAC, LDAB, EQUED, S, B, LDA, X,
     $                               LDA, RCOND, RWORK, RWORK( NRHS+1 ),
     $                               WORK, RWORK( 2*NRHS+1 ), INFO )
*
*                       Check the error code from czpbsvX.
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'czpbsvX', INFO, IZERO,
     $                                  FACT // UPLO, N, N, KD, KD,
...
*                          Compute residual of the computed solution.
*
                           CALL czlacpy( 'Full', N, NRHS, BSAV, LDA,
     $                                  WORK, LDA )
...
*
     ...
     $                                  RESULT( 4 ) )
                        ...
                        END IF
*
*                       Compare RCOND from czpbsvX with the computed
*                       value in RCONDC.
*
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9997 )'czpbsvX',
     $                              FACT, UPLO, N, KD, EQUED, IMAT, K,
     $                              RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9998 )'czpbsvX',
