...
*  =======
*
*  CDRVPO tests the driver routines ccposv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALADHD, ALAERH, ALASVM, CERRVX, CGET04, cclacpy,
     $                 CLAIPD, cclaqhe, CLARHS, cclaset, CLATB4, CLATMS,
     $                 ccpoequ, ccposv, ccposvX, CPOT01, CPOT02, CPOT05,
     $                   ccpotrf, ccpotri, XLAENV
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
      CHARACTER*7        SRNAMT
...
*
               CALL cclacpy( UPLO, N, N, A, LDA, ASAV, LDA )
*
               DO 100 IEQUED = 1, 2
                  EQUED = EQUEDS( IEQUED )
                  IF( IEQUED.EQ.1 ) THEN
...
*                       the value returned by ccposvX (FACT = 'N' reuses
*                       the condition number from the previous iteration
*                       with FACT = 'F').
*
                        CALL cclacpy( UPLO, N, N, ASAV, LDA, AFAC, LDA )
                        IF( EQUIL .OR. IEQUED.GT.1 ) THEN
*
*                          Compute row and column scale factors to
*                          equilibrate the matrix A.
*
                           CALL ccpoequ( N, AFAC, LDA, S, SCOND, AMAX,
     $                                  INFO )
...
*
                             CALL cclaqhe( UPLO, N, AFAC, LDA, S, SCOND,
     $                                     AMAX, EQUED )
                           END IF
                        END IF
*
...
*                       Factor the matrix A.
*
                        CALL ccpotrf( UPLO, N, AFAC, LDA, INFO )
*
*                       Form the inverse of A.
*
                        CALL cclacpy( UPLO, N, N, AFAC, LDA, A, LDA )
                        CALL ccpotri( UPLO, N, A, LDA, INFO )
*
...
*
                     CALL cclacpy( UPLO, N, N, ASAV, LDA, A, LDA )
*
...
*
                        CALL cclacpy( UPLO, N, N, A, LDA, AFAC, LDA )
                        CALL cclacpy( 'Full', N, NRHS, B, LDA, X, LDA )
*
                        SRNAMT = 'ccposv '
                        CALL ccposv( UPLO, N, NRHS, AFAC, LDA, X, LDA,
     $                              INFO )
*
*                       Check error code from ccposv .
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'ccposv ', INFO, IZERO,
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              WRITE( NOUT, FMT = 9999 )'ccposv ', UPLO,
     $                           N, IMAT, K, RESULT( K )
                              NFAIL = NFAIL + 1
                           END IF
   60                   CONTINUE
...
*
                     IF( .NOT.PREFAC )
     $                  CALL cclaset( UPLO, N, N, CMPLX( ZERO ),
     $                               CMPLX( ZERO ), AFAC, LDA )
...
*                       EQUED='Y'.
*
                        CALL cclaqhe( UPLO, N, A, LDA, S, SCOND, AMAX,
     $                               EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using ccposvX.
*
                     SRNAMT = 'ccposvX'
                     CALL ccposvX( FACT, UPLO, N, NRHS, A, LDA, AFAC,
     $                            LDA, EQUED, S, B, LDA, X, LDA, RCOND,
     $                            RWORK, RWORK( NRHS+1 ), WORK,
     $                            RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from ccposvX.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'ccposvX', INFO, IZERO,
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from ccposvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'ccposvX', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'ccposvX', FACT,
