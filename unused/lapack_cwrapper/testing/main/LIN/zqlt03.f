...
*  =======
*
*  ZQLT03 tests czunmql, which computes Q*C, Q'*C, C*Q or C*Q'.
*
*  ZQLT03 compares the results of a call to czunmql with the results of
*  forming Q explicitly by a call to czungql and then performing matrix
...
*          returned by czgeqlf. See ccgeqlf for further details.
*
...
*     .. External Subroutines ..
      EXTERNAL        ZGEMM, czlacpy, czlarnv, czlaset, czungql, czunmql
*     ..
*     .. Local Arrays ..
      INTEGER            ISEED( 4 )
*     ..
...
*
     ...
     $   CALL czlacpy( 'Full', M-K, K, AF( 1, N-K+1 ), LDA,
     $                Q( 1, M-K+1 ), LDA )
      IF( K.GT.1 )
     $   CALL czlacpy( 'Upper', K-1, K-1, AF( M-K+1, N-K+2 ), LDA,
     $                Q( M-K+1, M-K+2 ), LDA )
*
*     Generate the m-by-m matrix Q
*
      SRNAMT = 'czungql'
      CALL czungql( M, M, K, Q, LDA, TAU( MINMN-K+1 ), WORK, LWORK,
     $             INFO )
*
      DO 30 ISIDE = 1, 2
         IF( ISIDE.EQ.1 ) THEN
...
*           Copy C
*
            CALL czlacpy( 'Full', MC, NC, C, LDA, CC, LDA )
*
*           Apply Q or Q' to C
*
            SRNAMT = 'czunmql'
            IF( K.GT.0 )
     $        CALL czunmql( SIDE, TRANS, MC, NC, K, AF( 1, N-K+1 ), LDA,
