...
*     .. External Subroutines ..
      EXTERNAL         cdlabad, DLAORD, cdlascl, xerbla, ZDSCAL, ZGEMM,
     $                   czlarf, czlarnv, ZLAROR, czlascl, czlaset
*     ..
*     .. Intrinsic Functions ..
...
*
         CALL czlarnv( 2, ISEED, M, WORK )
         CALL ZDSCAL( M, ONE / DZNRM2( M, WORK, 1 ), WORK, 1 )
         CALL czlaset( 'Full', M, RANK, CZERO, CONE, A, LDA )
         CALL czlarf( 'Left', M, RANK, WORK, 1, DCMPLX( TWO ), A, LDA,
     $               WORK( M+1 ) )
*
*        workspace used: m+mn
*
...
*
         CALL czlarnv( 2, ISEED, RANK*NRHS, WORK )
...
*
   ...
   40    CONTINUE
         IF( RANK.LT.N )
     $      CALL czlaset( 'Full', M, N-RANK, CZERO, CZERO,
     $                   A( 1, RANK+1 ), LDA )
...
*
         DO 50 J = 1, MN
            S( J ) = ZERO
   50    CONTINUE
         CALL czlaset( 'Full', M, N, CZERO, CZERO, A, LDA )
         CALL czlaset( 'Full', M, NRHS, CZERO, CZERO, B, LDB )
*
      END IF
*
*     Scale the matrix
...
*              matrix scaled up
*
               CALL czlascl( 'General', 0, 0, NORMA, BIGNUM, M, N, A,
     $                      LDA, INFO )
               CALL cdlascl( 'General', 0, 0, NORMA, BIGNUM, MN, 1, S,
     $                      MN, INFO )
               CALL czlascl( 'General', 0, 0, NORMA, BIGNUM, M, NRHS, B,
     $                      LDB, INFO )
            ELSE IF( SCALE.EQ.3 ) THEN
*
*              matrix scaled down
*
               CALL czlascl( 'General', 0, 0, NORMA, SMLNUM, M, N, A,
     $                      LDA, INFO )
               CALL cdlascl( 'General', 0, 0, NORMA, SMLNUM, MN, 1, S,
     $                      MN, INFO )
               CALL czlascl( 'General', 0, 0, NORMA, SMLNUM, M, NRHS, B,
     $                      LDB, INFO )
            ELSE
