...
*     .. External Subroutines ..
      EXTERNAL           CCOPY, cclarnv, CLATB4, CLATMS, CSSCAL, CSWAP,
     $                   cslabad, cslarnv
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( KD.EQ.1 ) THEN
            IF( UPPER ) THEN
               ...
               LENJ = ( N-3 ) / 2
               CALL cclarnv( 2, ISEED, LENJ, WORK )
               DO 90 J = 1, LENJ
...
*
         IF( UPPER ) THEN
            DO 120 J = 1, N
               LENJ = MIN( J-1, KD )
               CALL cclarnv( 4, ISEED, LENJ, AB( KD+1-LENJ, J ) )
  ...
  120       CONTINUE
         ELSE
            DO 130 J = 1, N
               LENJ = MIN( N-J, KD )
               IF( LENJ.GT.0 )
     $            CALL cclarnv( 4, ISEED, LENJ, AB( 2, J ) )
...
*
         ...
         IF( UPPER ) THEN
            DO 140 J = 1, N
               LENJ = MIN( J-1, KD )
               IF( LENJ.GT.0 ) THEN
                  CALL cclarnv( 4, ISEED, LENJ, AB( KD+2-LENJ, J ) )
...
*
  ...
  280       CONTINUE
         ELSE
            DO 290 J = 1, N
               LENJ = MIN( N-J+1, KD+1 )
               CALL cclarnv( 4, ISEED, LENJ, AB( 1, J ) )
               IF( J.NE.IY ) THEN
                  AB( 1, J ) = CLARND( 5, ISEED )*TWO
               ELSE
                  AB( 1, J ) = ZERO
...
*        1/3/91:  cclatbs no longer can handle this case
*
         ...
         IF( UPPER ) THEN
            DO 390 J = 1, N
               LENJ = MIN( J, KD+1 )
               CALL cclarnv( 5, ISEED, LENJ, AB( KD+2-LENJ, J ) )
               CALL cslarnv( 1, ISEED, LENJ, RWORK( KD+2-LENJ ) )
  ...
  390       CONTINUE
         ELSE
            DO 410 J = 1, N
               LENJ = MIN( N-J+1, KD+1 )
               CALL cclarnv( 5, ISEED, LENJ, AB( 1, J ) )
               CALL cslarnv( 1, ISEED, LENJ, RWORK )
               DO 400 I = 1, LENJ
                  AB( I, J ) = AB( I, J )*( TLEFT+RWORK( I )*TSCAL )
  400          CONTINUE
  410       CONTINUE
