...
*  =======
*
*  ZCHKGE tests czgetrf, -TRI, -TRS, -RFS, and -CON.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, XLAENV, ZERRGE, czgecon,
     $                  czgerfs, ZGET01, ZGET02, ZGET03, ZGET04, ZGET07,
     $              czgetrf, czgetri, czgetrs, czlacpy, ZLARHS, czlaset,
     $                   ZLATB4, ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   20                CONTINUE
                  ELSE
                     CALL czlaset( 'Full', M, N-IZERO+1, DCMPLX( ZERO ),
...
*
                  CALL czlacpy( 'Full', M, N, A, LDA, AFAC, LDA )
                  SRNAMT = 'czgetrf'
                  CALL czgetrf( M, N, AFAC, LDA, IWORK, INFO )
*
*                 Check error code from czgetrf.
*
                  IF( INFO.NE.IZERO )
     $               CALL ALAERH( PATH, 'czgetrf', INFO, IZERO, ' ', M,
...
*
                  CALL czlacpy( 'Full', M, N, AFAC, LDA, AINV, LDA )
...
*
                  IF( M.EQ.N .AND. INFO.EQ.0 ) THEN
                     CALL czlacpy( 'Full', N, N, AFAC, LDA, AINV, LDA )
                     SRNAMT = 'czgetri'
                     NRHS = NSVAL( 1 )
                     LWORK = NMAX*MAX( 3, NRHS )
                     CALL czgetri( N, AINV, LDA, IWORK, WORK, LWORK,
     $                            INFO )
*
*                    Check error code from czgetri.
*
                     IF( INFO.NE.0 )
     $                 CALL ALAERH( PATH, 'czgetri', INFO, 0, ' ', N, N,
...
*
                        ...
                        SRNAMT = 'czgetrs'
                        CALL czgetrs( TRANS, N, NRHS, AFAC, LDA, IWORK,
     $                               X, LDA, INFO )
*
*                       Check error code from czgetrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'czgetrs', INFO, 0, TRANS,
...
*
                        SRNAMT = 'czgerfs'
                        CALL czgerfs( TRANS, N, NRHS, A, LDA, AFAC, LDA,
     $                               IWORK, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                       Check error code from czgerfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'czgerfs', INFO, 0, TRANS,
     $                                  N, N, -1, -1, NRHS, IMAT, NFAIL,
     $                                  NERRS, NOUT )
*
                        CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
   70             CONTINUE
                  DO 80 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'czgecon'
                     CALL czgecon( NORM, N, AFAC, LDA, ANORM, RCOND,
     $                            WORK, RWORK, INFO )
*
*                       Check error code from czgecon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'czgecon', INFO, 0, NORM, N,
