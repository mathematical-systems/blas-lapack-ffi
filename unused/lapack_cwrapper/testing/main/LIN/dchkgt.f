...
*     .. Array Arguments ..
      ...
      DOUBLE PRECISION A(*),AF(*),B(*),RWORK(*),WORK(*),X(*),XACT(*)
*     ..
*
*  Purpose
*  =======
*
*  DCHKGT tests cdgttrf, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. Local Scalars ..
      ...
      CHARACTER*3 PATH
      INTEGER I,IMAT,IN,INFO,IRHS,ITRAN,IX,IZERO,J,K,KL,KOFF,KU,LDA,M,
     +        MODE,N,NERRS,NFAIL,NIMAT,NRHS,NRUN
...
*     .. External Subroutines ..
      EXTERNAL ALAERH,ALAHD,ALASUM,DCOPY,DERRGE,DGET04,cdgtcon,cdgtrfs,
     +         DGTT01,DGTT02,DGTT05,cdgttrf,cdgttrs,cdlacpy,cdlagtm,
     +         cdlarnv,DLATB4,DLATMS,DSCAL
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX
*     ..
...
*
              ...
              SRNAMT = 'cdgttrf'
              CALL cdgttrf(N,AF,AF(M+1),AF(N+M+1),AF(N+2*M+1),IWORK,
     +                     INFO)
*
*           Check error code from cdgttrf.
*
              IF (INFO.NE.IZERO) CALL ALAERH(PATH,'cdgttrf',INFO,IZERO,
     +                                ' ',N,N,1,1,-1,IMAT,NFAIL,NERRS,
     +                                NOUT)
              TRFCON = INFO .NE. 0
*
...
*                 Use cdgttrs to solve for one column at a time of inv(A)
...
*
   ...
   30                     CONTINUE
                          X(I) = ONE
                          CALL cdgttrs(TRANS,N,1,AF,AF(M+1),AF(N+M+1),
...
*
                  SRNAMT = 'cdgtcon'
                  CALL cdgtcon(NORM,N,AF,AF(M+1),AF(N+M+1),AF(N+2*M+1),
     +                         IWORK,ANORM,RCOND,WORK,IWORK(N+1),INFO)
*
*              Check error code from cdgtcon.
*
                  IF (INFO.NE.0) CALL ALAERH(PATH,'cdgtcon',INFO,0,NORM,
     +                                N,N,-1,-1,-1,IMAT,NFAIL,NERRS,
     +                                NOUT)
*
...
*
                  IX = 1
                  DO 60 J = 1,NRHS
                      CALL cdlarnv(2,ISEED,N,XACT(IX))
                      IX = IX + LDA
   60             CONTINUE
*
                  DO 80 ITRAN = 1,3
...
*
                      CALL cdlagtm(TRANS,N,NRHS,ONE,A,A(M+1),A(N+M+1),
     +                             XACT,LDA,ZERO,B,LDA)
*
*+    TEST 2
...
*
                      ...
                      SRNAMT = 'cdgttrs'
                      CALL cdgttrs(TRANS,N,NRHS,AF,AF(M+1),AF(N+M+1),
     +                             AF(N+2*M+1),IWORK,X,LDA,INFO)
*
*                 Check error code from cdgttrs.
*
                      IF (INFO.NE.0) CALL ALAERH(PATH,'cdgttrs',INFO,0,
     +                                    TRANS,N,N,-1,-1,NRHS,IMAT,
     +                                    NFAIL,NERRS,NOUT)
*
                      CALL cdlacpy('Full',N,NRHS,B,LDA,WORK,LDA)
                      CALL DGTT02(TRANS,N,NRHS,A,A(M+1),A(N+M+1),X,LDA,
...
*
                      SRNAMT = 'cdgtrfs'
                      CALL cdgtrfs(TRANS,N,NRHS,A,A(M+1),A(N+M+1),AF,
     +                             AF(M+1),AF(N+M+1),AF(N+2*M+1),IWORK,
     +                             B,LDA,X,LDA,RWORK,RWORK(NRHS+1),WORK,
     +                             IWORK(N+1),INFO)
*
*                 Check error code from cdgtrfs.
*
                      IF (INFO.NE.0) CALL ALAERH(PATH,'cdgtrfs',INFO,0,
     +                                    TRANS,N,N,-1,-1,NRHS,IMAT,
     +                                    NFAIL,NERRS,NOUT)
*
                      CALL DGET04(N,NRHS,X,LDA,XACT,LDA,RCONDC,
     +                            RESULT(4))
                      CALL DGTT05(TRANS,N,NRHS,A,A(M+1),A(N+M+1),B,LDA,
...
*
     ...
     +                            NERRS.EQ.0) CALL ALAHD(NOUT,PATH)
                              WRITE (NOUT,FMT=9998) TRANS,N,NRHS,IMAT,K,
     +                          RESULT(K)
                              NFAIL = NFAIL + 1
                          END IF
   70                 CONTINUE
...
*
 9999 FORMAT (12X,'N =',I5,',',10X,' type ',I2,', test(',I2,') = ',
     +       G12.5)
 9998 FORMAT (' TRANS=''',A1,''', N =',I5,', NRHS=',I3,', type ',I2,
     +       ', test(',I2,') = ',G12.5)
 9997 FORMAT (' NORM =''',A1,''', N =',I5,',',10X,' type ',I2,', test(',
     +       I2,') = ',G12.5)
      RETURN
*
*     End of DCHKGT
