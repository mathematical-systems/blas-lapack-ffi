...
*  =======
*
*  CCHKEQ tests ccgeequ, ccgbequ, ccpoequ, ccppequ and ccpbequ
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL           ccgbequ, ccgeequ, ccpbequ, ccpoequ, ccppequ
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   40       CONTINUE
*
            CALL ccgeequ( M, N, A, NSZ, R, C, RCOND, CCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 1 ) = ONE
            ELSE
...
*
   ...
   90 CONTINUE
      CALL ccgeequ( NSZ, NSZ, A, NSZ, R, C, RCOND, CCOND, NORM, INFO )
...
*
  ...
  150             CONTINUE
*
                  CALL ccgbequ( M, N, KL, KU, AB, NSZB, R, C, RCOND,
     $                         CCOND, NORM, INFO )
*
                  IF( INFO.NE.0 ) THEN
...
*
  ...
  270    CONTINUE
*
         CALL ccpoequ( N, A, NSZ, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 3 ) = ONE
         ELSE
  ...
  290 CONTINUE
      ...
      CALL ccpoequ( NSZ, A, NSZ, R, RCOND, NORM, INFO )
      IF( INFO.NE.MAX( NSZ-1, 1 ) )
     $   RESLTS( 3 ) = ONE
...
*
  ...
  310    CONTINUE
*
         CALL ccppequ( 'U', N, AP, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 4 ) = ONE
         ELSE
...
*
  ...
  340    CONTINUE
*
         CALL ccppequ( 'L', N, AP, R, RCOND, NORM, INFO )
*
         IF( INFO.NE.0 ) THEN
            RESLTS( 4 ) = ONE
         ELSE
...
*
  360 CONTINUE
      I = ( NSZ*( NSZ+1 ) ) / 2 - 2
      AP( I ) = -CONE
      CALL ccppequ( 'L', NSZ, AP, R, RCOND, NORM, INFO )
      IF( INFO.NE.MAX( NSZ-1, 1 ) )
     $   RESLTS( 4 ) = ONE
...
*
  ...
  390       CONTINUE
*
            CALL ccpbequ( 'U', N, KL, AB, NSZB, R, RCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 5 ) = ONE
            ELSE
...
*
  ...
  430       CONTINUE
*
            CALL ccpbequ( 'L', N, KL, AB, NSZB, R, RCOND, NORM, INFO )
*
            IF( INFO.NE.0 ) THEN
               RESLTS( 5 ) = ONE
            ELSE
 ...
 9998 FORMAT( ' ccgeequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9997 FORMAT( ' ccgbequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9996 FORMAT( ' ccpoequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9995 FORMAT( ' ccppequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
 9994 FORMAT( ' ccpbequ failed test with value ', E10.3, ' exceeding',
     $      ' threshold ', E10.3 )
      RETURN
*
*     End of CCHKEQ
