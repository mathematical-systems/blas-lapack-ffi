...
*  =======
*
*  ZDRVPO tests the driver routines czposv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                czlacpy, ZLAIPD, czlaqhe, ZLARHS, czlaset, ZLATB4,
     $                 ZLATMS, czpoequ, czposv, czposvX, ZPOT01, ZPOT02,
     $                   ZPOT05, czpotrf, czpotri
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
      CHARACTER*7        SRNAMT
...
*
               CALL czlacpy( UPLO, N, N, A, LDA, ASAV, LDA )
*
               DO 100 IEQUED = 1, 2
                  EQUED = EQUEDS( IEQUED )
                  IF( IEQUED.EQ.1 ) THEN
...
*                       the value returned by czposvX (FACT = 'N' reuses
*                       the condition number from the previous iteration
*                       with FACT = 'F').
*
                        CALL czlacpy( UPLO, N, N, ASAV, LDA, AFAC, LDA )
                        IF( EQUIL .OR. IEQUED.GT.1 ) THEN
*
*                          Compute row and column scale factors to
*                          equilibrate the matrix A.
*
                           CALL czpoequ( N, AFAC, LDA, S, SCOND, AMAX,
     $                                  INFO )
...
*
                             CALL czlaqhe( UPLO, N, AFAC, LDA, S, SCOND,
     $                                     AMAX, EQUED )
                           END IF
                        END IF
*
...
*                       Factor the matrix A.
*
                        CALL czpotrf( UPLO, N, AFAC, LDA, INFO )
*
*                       Form the inverse of A.
*
                        CALL czlacpy( UPLO, N, N, AFAC, LDA, A, LDA )
                        CALL czpotri( UPLO, N, A, LDA, INFO )
*
...
*
                     CALL czlacpy( UPLO, N, N, ASAV, LDA, A, LDA )
*
...
*
                        CALL czlacpy( UPLO, N, N, A, LDA, AFAC, LDA )
                        CALL czlacpy( 'Full', N, NRHS, B, LDA, X, LDA )
*
                        SRNAMT = 'czposv '
                        CALL czposv( UPLO, N, NRHS, AFAC, LDA, X, LDA,
     $                              INFO )
*
*                       Check error code from czposv .
*
                        IF( INFO.NE.IZERO ) THEN
                           CALL ALAERH( PATH, 'czposv ', INFO, IZERO,
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              WRITE( NOUT, FMT = 9999 )'czposv ', UPLO,
     $                           N, IMAT, K, RESULT( K )
                              NFAIL = NFAIL + 1
                           END IF
   60                   CONTINUE
...
*
                     IF( .NOT.PREFAC )
     $                  CALL czlaset( UPLO, N, N, DCMPLX( ZERO ),
...
*                       EQUED='Y'.
*
                        CALL czlaqhe( UPLO, N, A, LDA, S, SCOND, AMAX,
     $                               EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using czposvX.
*
                     SRNAMT = 'czposvX'
                     CALL czposvX( FACT, UPLO, N, NRHS, A, LDA, AFAC,
     $                            LDA, EQUED, S, B, LDA, X, LDA, RCOND,
     $                            RWORK, RWORK( NRHS+1 ), WORK,
     $                            RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from czposvX.
*
                     IF( INFO.NE.IZERO ) THEN
                        CALL ALAERH( PATH, 'czposvX', INFO, IZERO,
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from czposvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'czposvX', FACT,
     $                           UPLO, N, EQUED, IMAT, K, RESULT( K )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'czposvX', FACT,
