...
*     .. External Subroutines ..
      EXTERNAL           SCOPY, cslabad, cslarnv, SLATB4, SLATMS, SSCAL,
     $                   SSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
         IF( KD.EQ.1 ) THEN
            IF( UPPER ) THEN
               ...
               LENJ = ( N-3 ) / 2
               CALL cslarnv( 2, ISEED, LENJ, WORK )
               DO 90 J = 1, LENJ
...
*
         IF( UPPER ) THEN
            DO 120 J = 1, N
               LENJ = MIN( J, KD+1 )
               CALL cslarnv( 2, ISEED, LENJ, AB( KD+2-LENJ, J ) )
  ...
  120       CONTINUE
         ELSE
            DO 130 J = 1, N
               LENJ = MIN( N-J+1, KD+1 )
               IF( LENJ.GT.0 )
     $            CALL cslarnv( 2, ISEED, LENJ, AB( 1, J ) )
...
*
         IF( UPPER ) THEN
            DO 370 J = 1, N
               LENJ = MIN( J-1, KD )
               CALL cslarnv( 2, ISEED, LENJ, AB( KD+1-LENJ, J ) )
               AB( KD+1, J ) = REAL( J )
  370       CONTINUE
         ELSE
            DO 380 J = 1, N
               LENJ = MIN( N-J, KD )
               IF( LENJ.GT.0 )
     $            CALL cslarnv( 2, ISEED, LENJ, AB( 2, J ) )
               AB( 1, J ) = REAL( J )
  380       CONTINUE
         END IF
*
