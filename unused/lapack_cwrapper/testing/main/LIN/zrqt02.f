...
*  =======
*
*  ZRQT02 tests czungrq, which generates an m-by-n matrix Q with
...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, ZHERK, czlacpy, czlaset, czungrq
*     ..
*     .. Intrinsic Functions ..
...
*
      SRNAMT = 'czungrq'
      CALL czungrq( M, N, K, Q, LDA, TAU( M-K+1 ), WORK, LWORK, INFO )
*
*     Copy R(m-k+1:m,n-m+1:n)
*
      CALL czlaset( 'Full', K, M, DCMPLX( ZERO ), DCMPLX( ZERO ),
     $             R( M-K+1, N-M+1 ), LDA )
      CALL czlacpy( 'Upper', K, K, AF( M-K+1, N-K+1 ), LDA,
     $             R( M-K+1, N-K+1 ), LDA )
*
