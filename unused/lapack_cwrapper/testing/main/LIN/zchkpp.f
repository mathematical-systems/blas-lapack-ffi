...
*  =======
*
*  ZCHKPP tests czpptrf, -TRI, -TRS, -RFS, and -CON
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $                 czlacpy, ZLAIPD, ZLARHS, ZLATB4, ZLATMS, czppcon,
     $                 czpprfs, ZPPT01, ZPPT02, ZPPT03, ZPPT05, czpptrf,
     $                   czpptri, czpptrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
*              Check error code from czpptrf.
*
               IF( INFO.NE.IZERO ) THEN
                  CALL ALAERH( PATH, 'czpptrf', INFO, IZERO, UPLO, N, N,
...
*
               CALL ZCOPY( NPP, AFAC, 1, AINV, 1 )
               SRNAMT = 'czpptri'
               CALL czpptri( UPLO, N, AINV, INFO )
*
*              Check error code from czpptri.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'czpptri', INFO, 0, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
...
*
                  SRNAMT = 'czpptrs'
                  CALL czpptrs( UPLO, N, NRHS, AFAC, X, LDA, INFO )
*
*              Check error code from czpptrs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czpptrs', INFO, 0, UPLO, N, N,
...
*
                  SRNAMT = 'czpprfs'
                  CALL czpprfs( UPLO, N, NRHS, A, AFAC, B, LDA, X, LDA,
     $                         RWORK, RWORK( NRHS+1 ), WORK,
     $                         RWORK( 2*NRHS+1 ), INFO )
*
*              Check error code from czpprfs.
*
                  IF( INFO.NE.0 )
     $               CALL ALAERH( PATH, 'czpprfs', INFO, 0, UPLO, N, N,
     $                            -1, -1, NRHS, IMAT, NFAIL, NERRS,
     $                            NOUT )
*
                  CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
...
*
               ...
               SRNAMT = 'czppcon'
               CALL czppcon( UPLO, N, AFAC, ANORM, RCOND, WORK, RWORK,
     $                      INFO )
*
*              Check error code from czppcon.
*
               IF( INFO.NE.0 )
     $            CALL ALAERH( PATH, 'czppcon', INFO, 0, UPLO, N, N, -1,
     $                         -1, -1, IMAT, NFAIL, NERRS, NOUT )
*
