...
*  =======
*
*  ZDRVGE tests the driver routines czgesv and -SVX.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
      EXTERNAL          ALADHD, ALAERH, ALASVM, XLAENV, ZERRVX, czgeequ,
     $                  czgesv, czgesvX, ZGET01, ZGET02, ZGET04, ZGET07,
     $              czgetrf, czgetri, czlacpy, czlaqge, ZLARHS, czlaset,
     $                  ZLATB4, ZLATMS
*     ..
*     .. Intrinsic Functions ..
...
*
   ...
   20             CONTINUE
               ELSE
                  CALL czlaset( 'Full', N, N-IZERO+1, DCMPLX( ZERO ),
...
*
            CALL czlacpy( 'Full', N, N, A, LDA, ASAV, LDA )
*
            DO 70 IEQUED = 1, 4
               EQUED = EQUEDS( IEQUED )
               IF( IEQUED.EQ.1 ) THEN
...
*                    the value returned by czgesvX (FACT = 'N' reuses
*                    the condition number from the previous iteration
*                    with FACT = 'F').
*
                     CALL czlacpy( 'Full', N, N, ASAV, LDA, AFAC, LDA )
                     IF( EQUIL .OR. IEQUED.GT.1 ) THEN
*
*                       Compute row and column scale factors to
*                       equilibrate the matrix A.
*
                        CALL czgeequ( N, N, AFAC, LDA, S, S( N+1 ),
...
*
                           CALL czlaqge( N, N, AFAC, LDA, S, S( N+1 ),
...
*                    Factor the matrix A.
*
                     CALL czgetrf( N, N, AFAC, LDA, IWORK, INFO )
*
*                    Form the inverse of A.
*
                     CALL czlacpy( 'Full', N, N, AFAC, LDA, A, LDA )
                     LWORK = NMAX*MAX( 3, NRHS )
                     CALL czgetri( N, A, LDA, IWORK, WORK, LWORK, INFO )
*
...
*
                     CALL czlacpy( 'Full', N, N, ASAV, LDA, A, LDA )
*
...
*
     ...
     $                            ISEED, INFO )
                     XTYPE = 'C'
                     CALL czlacpy( 'Full', N, NRHS, B, LDA, BSAV, LDA )
*
...
*                       solve the system.
*
                        CALL czlacpy( 'Full', N, N, A, LDA, AFAC, LDA )
                        CALL czlacpy( 'Full', N, NRHS, B, LDA, X, LDA )
*
                        SRNAMT = 'czgesv '
                        CALL czgesv( N, NRHS, AFAC, LDA, IWORK, X, LDA,
     $                              INFO )
*
*                       Check error code from czgesv .
*
                        IF( INFO.NE.IZERO )
     $                     CALL ALAERH( PATH, 'czgesv ', INFO, IZERO,
     $                                  ' ', N, N, -1, -1, NRHS, IMAT,
     $                                  NFAIL, NERRS, NOUT )
*
...
*
                           CALL czlacpy( 'Full', N, NRHS, B, LDA, WORK,
     $                                  LDA )
...
*
                     IF( .NOT.PREFAC )
     $                  CALL czlaset( 'Full', N, N, DCMPLX( ZERO ),
     $                               DCMPLX( ZERO ), AFAC, LDA )
                     CALL czlaset( 'Full', N, NRHS, DCMPLX( ZERO ),
     $                            DCMPLX( ZERO ), X, LDA )
...
*                       EQUED = 'R', 'C', or 'B'.
*
                        CALL czlaqge( N, N, A, LDA, S, S( N+1 ), ROWCND,
     $                               COLCND, AMAX, EQUED )
                     END IF
*
*                    Solve the system and compute the condition number
*                    and error bounds using czgesvX.
*
                     SRNAMT = 'czgesvX'
                     CALL czgesvX( FACT, TRANS, N, NRHS, A, LDA, AFAC,
     $                            LDA, IWORK, EQUED, S, S( N+1 ), B,
     $                            LDA, X, LDA, RCOND, RWORK,
     $                            RWORK( NRHS+1 ), WORK,
     $                            RWORK( 2*NRHS+1 ), INFO )
*
*                    Check the error code from czgesvX.
*
                     IF( INFO.NE.IZERO )
     $                  CALL ALAERH( PATH, 'czgesvX', INFO, IZERO,
     $                               FACT // TRANS, N, N, -1, -1, NRHS,
     $                               IMAT, NFAIL, NERRS, NOUT )
*
*                    Compare RWORK(2*NRHS+1) from czgesvX with the
...
*                       Compute residual of the computed solution.
*
                        CALL czlacpy( 'Full', N, NRHS, BSAV, LDA, WORK,
     $                               LDA )
...
*
     ...
     $                               RWORK( NRHS+1 ), RESULT( 4 ) )
                     ...
                     END IF
*
*                    Compare RCOND from czgesvX with the computed value
*                    in RCONDC.
*
...
*
     ...
     $                           CALL ALADHD( NOUT, PATH )
                              IF( PREFAC ) THEN
                                 WRITE( NOUT, FMT = 9997 )'czgesvX',
     $                              FACT, TRANS, N, EQUED, IMAT, K,
     $                              RESULT( K )
                              ELSE
                                 WRITE( NOUT, FMT = 9998 )'czgesvX',
   ...
   40                   CONTINUE
     ...
     $                        CALL ALADHD( NOUT, PATH )
                           IF( PREFAC ) THEN
                              WRITE( NOUT, FMT = 9997 )'czgesvX', FACT,
     $                           TRANS, N, EQUED, IMAT, 1, RESULT( 1 )
                           ELSE
                              WRITE( NOUT, FMT = 9998 )'czgesvX', FACT,
