...
*     .. External Subroutines ..
      EXTERNAL DGBMV,DGEMM,cdlacpy,cdlarnv,DSBMV,DSPMV,DSYMM,DTBMV,
     +         DTPMV,DTRMM,xerbla
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC MAX
...
*
      ...
      ELSE IF (.NOT. (LSAME(XTYPE,'N').OR.LSAME(XTYPE,'C'))) THEN
          INFO = -2
      ELSE IF ((SYM.OR.TRI) .AND. .NOT. (LSAME(UPLO,'U').OR.LSAME(UPLO,
     +         'L'))) THEN
          INFO = -3
...
*
          DO 20 J = 1,NRHS
              CALL DGBMV(TRANS,MB,NX,KL,KU,ONE,A,LDA,X(1,J),1,ZERO,
     +                   B(1,J),1)
   20     CONTINUE
*
...
*
          CALL cdlacpy('Full',N,NRHS,X,LDX,B,LDB)
          IF (KU.EQ.2) THEN
              DIAG = 'U'
          ELSE
              DIAG = 'N'
