...
*  =======
*
*  ZCHKTB tests cztbtrs, -RFS, and -CON, and czlatbs.
*
*  Arguments
*  =========
*
...
*     .. External Subroutines ..
     ...
     $               czlacpy, ZLARHS, czlaset, czlatbs, ZLATTB, cztbcon,
     $                   cztbrfs, ZTBSV, ZTBT02, ZTBT03, ZTBT05, ZTBT06,
     $                   cztbtrs
*     ..
*     .. Scalars in Common ..
      LOGICAL            LERR, OK
...
*
                        SRNAMT = 'cztbtrs'
                       CALL cztbtrs( UPLO, TRANS, DIAG, N, KD, NRHS, AB,
     $                               LDAB, X, LDA, INFO )
*
*                    Check error code from cztbtrs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cztbtrs', INFO, 0,
...
*
                        SRNAMT = 'cztbrfs'
                       CALL cztbrfs( UPLO, TRANS, DIAG, N, KD, NRHS, AB,
     $                               LDAB, B, LDA, X, LDA, RWORK,
     $                               RWORK( NRHS+1 ), WORK,
     $                               RWORK( 2*NRHS+1 ), INFO )
*
*                    Check error code from cztbrfs.
*
                        IF( INFO.NE.0 )
     $                     CALL ALAERH( PATH, 'cztbrfs', INFO, 0,
     $                                  UPLO // TRANS // DIAG, N, N, KD,
     $                                  KD, NRHS, IMAT, NFAIL, NERRS,
     $                                  NOUT )
*
...
*
                  DO 70 ITRAN = 1, 2
                     ...
                     END IF
                     SRNAMT = 'cztbcon'
                     CALL cztbcon( NORM, UPLO, DIAG, N, KD, AB, LDAB,
     $                            RCOND, WORK, RWORK, INFO )
*
*                    Check error code from cztbcon.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'cztbcon', INFO, 0,
...
*
                     IF( RESULT( 6 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9998 ) 'cztbcon', NORM, UPLO,
   ...
   90       CONTINUE
*
*           Use pathological test matrices to test czlatbs.
*
...
*
                     ...
                     CALL czlatbs( UPLO, TRANS, DIAG, 'N', N, KD, AB,
     $                            LDAB, B, SCALE, RWORK, INFO )
*
*                    Check error code from czlatbs.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'czlatbs', INFO, 0,
...
*
                     CALL ZCOPY( N, X, 1, B, 1 )
                     CALL czlatbs( UPLO, TRANS, DIAG, 'Y', N, KD, AB,
     $                            LDAB, B, SCALE, RWORK, INFO )
*
*                    Check error code from czlatbs.
*
...
*
                     IF( RESULT( 7 ).GE.THRESH ) THEN
                        IF( NFAIL.EQ.0 .AND. NERRS.EQ.0 )
     $                     CALL ALAHD( NOUT, PATH )
                        WRITE( NOUT, FMT = 9997 )'czlatbs', UPLO, TRANS,
