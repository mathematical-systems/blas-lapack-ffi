...
*     .. External Subroutines ..
      EXTERNAL           DCOPY, DGEMV, DGER, cdlarfG, DLARGE, cdlarnv,
     $                   cdlaset, DLATM1, DSCAL, cxerbla
*     ..
*     .. Intrinsic Functions ..
      ...
      IF( IUPPER.NE.0 ) THEN
         DO 70 JC = 2, N
            ...
            ELSE
               JR = JC - 1
            END IF
            CALL cdlarnv( IDIST, ISEED, JR, A( 1, JC ) )
   70    CONTINUE
      END IF
*
...
*
            ...
            XNORMS = WORK( 1 )
            CALL cdlarfG( IROWS, XNORMS, WORK( 2 ), 1, TAU )
            WORK( 1 ) = ONE
*
...
*
            A( JCR, IC ) = XNORMS
           CALL cdlaset( 'Full', IROWS-1, 1, ZERO, ZERO, A( JCR+1, IC ),
     $                   LDA )
   90    CONTINUE
...
*
            ...
            XNORMS = WORK( 1 )
            CALL cdlarfG( ICOLS, XNORMS, WORK( 2 ), 1, TAU )
            WORK( 1 ) = ONE
*
...
*
            A( IR, JCR ) = XNORMS
           CALL cdlaset( 'Full', 1, ICOLS-1, ZERO, ZERO, A( IR, JCR+1 ),
     $                   LDA )
  100    CONTINUE
      END IF
*
