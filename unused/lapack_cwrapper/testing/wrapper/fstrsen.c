...
void fstrsen_(char* job, char* compq, long int* select, int* n, float* t, int* ldt, float* q, int* ldq, float* wr, float* wi, int* m, float* s, float* sep, float* work, int* lwork, int* iwork, int* liwork, int* info)
{
	...
	lapack_strsen(jobC, compqC, select, *n, t, *ldt, q, *ldq, wr, wi, m, s, sep, work, *lwork, iwork, *liwork, info);
}
